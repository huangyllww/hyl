...
machineSummary()
{
	...
	let pci_nums=pci_nums+ethernetnums
        if lspci|grep -i -E "3D controller|NVIDIA Corporation">& /dev/null; then
		gpunums=`lspci|grep -i -E "3D controller|NVIDIA Corporation" | sed 's/.*compatible controller:\(.*\)(.*/\1/' |uniq| wc -l`
        else
		gpunums=0
        fi
	let pci_nums=pci_nums+gpunums
	...
	show_item "GPU controller Numbers:" "$gpunums"
		...
		sys_hdd_backup=${sys_hdd[@]}
			...
			let sum_hdds=sum_hdds+hdd_nums
                ...
                if lspci | grep -i 'lsi'|grep -E '3008|3216' >& /dev/null; then
                        hba_nums=`lspci | grep -i 'lsi'|grep -E '3008|3216' | wc -l`
                        for((i=0;i<$hba_nums;i++))
                        do
...
remove_ipmi_driver()
{
        ...
        cd /lib/modules/$kernel/kernel/drivers/char/ipmi/  >/dev/null 2>&1

        rmmod ipmi_devintfi.ko >/dev/null 2>&1
        while [ $? -ne 0 ];
        do
                sleep 2
                rmmod ipmi_devintf.ko >/dev/null 2>&1
                if [[ $? -eq 0 ]] && [[ $count -lt 3 ]]; then
                        break
                fi
                if [ $count -gt 3 ];then
                        echo "Three times failed to uninstall the driver:ipmi_devintf"
                        driverflag=1
                        break
                fi
                let count=count+1
        ...
        count=0
        sleep 2
        rmmod ipmi_msghandler.ko >/dev/null 2>&1
        while [ $? -ne 0 ];
        do
                sleep 2
                rmmod ipmi_msghandler.ko >/dev/null 2>&1
        ...
        cd - >/dev/null
        if [ $driverflag -eq 1 ]; then
                echo "rmmod ipmi dirver fail"
                return 1
        fi
}
install_ipmi_driver()
{
        ...
        insmod ipmi_msghandler.ko >/dev/null 2>&1
        while [ $? -ne 0 ];
        do
                sleep 2
                insmod ipmi_msghandler.ko >/dev/null 2>&1
        ...
        insmod ipmi_si.ko >/dev/null 2>&1
        count=0
        sleep 2
        while [ $? -ne 0 ];
        do
        ...
        insmod ipmi_devintf.ko >/dev/null 2>&1
        count=0
        sleep 2
        while [ $? -ne 0 ];
        do
...
cpuinfo()
{
	...
	for((i=0;i<$cpu_nums;i++))
		...
		do
			cpu_phy_id=`cat /proc/cpuinfo | grep -i 'physical id' |  sed -n "/$i/p" | awk -F':' '{print $NF}' | sed 's/[[:space:]]//g' | uniq`
			cpu_vendor=`cat /proc/cpuinfo | grep -i -E -B10 -A9 "physical id.* $i" | grep -i -E "model name" | awk -F':' '{print $NF}' | uniq | sed 's/[[:space:]]//' | awk '{print $1}' `
...
#   show_produce_message "UPI Message"
   ...
   existFlag=`lspci | grep -i "205b"`
   declare -A upi_array
   if [[ -s tmp ]] && [[ "$existFlag" != "" ]]; then
...
if [ "$?" != 0 ];then
	yum install ndctl
	rpm -ivh --nodeps --force $Cur_Dir/ipmctl/*.rpm
else
...
errinfo(){
	#get os err
:<<!
	show_title "Message Error"
	cat /var/log/messages | grep -i error
	cat /var/log/messages | grep -i fail
	cat /var/log/messages | grep -i fault
	cat /var/log/messages | grep -i warring
	show_title "Mce Error"
	cat /var/log/mcelog | grep -i error
	cat /var/log/mcelog | grep -i fault 
	cat /var/log/mcelog | grep -i warning
	cat /var/log/mcelog | grep -i fail
!
	cat /var/log/mcelog | grep -i ecc 
...
#	   	storcli -pdlist -aALL|grep -i "Firmware state"|awk -F ":" '{print $2}'>3.txt
		...
		printf "%-40s%-20s%-20s%-20s\n" "Inquiry_Data" "link_speed" "Media_Error_Count" "Other_Error_Count"
		#echo "Inquiry Data--link speed--Firmware state--Media Error Count--Other Error Count"
		while [[ $i -le $hdd_num ]];do
        	    inquiry_data=`sed -n "$i p" 1.txt|sed 's/^[[:space:]]*//g' |sed 's/[[:space:]]*$//g' | awk '{print $1,$2}'`
       ...
       #  	   fw_state=`sed -n "$i p" 3.txt|sed 's/^[[:space:]]*//g'`
	            ...
	            printf "%-40s%-20s%-20s%-20s\n" "$inquiry_data" "$link_speed" "$media_err" "$other_err"
		...
		#echo "Loops ---------->"$loop >> $LogAd/ROC_Tem.log
   		...
   		do	
       		    ...
       		    storcli /c"$i"/pall show all|sed '/Controller = 0/d'|sed '/Status = Success/d'|sed '/Description = None/d'|sed '/^ *&/d'
...
raidinfo(){
    ...
    #raid detail message
    storcli /c"$1" show |sed '/Generating detailed summary/d'|sed '/Controller = /d'|sed '/Status = Success/d'|sed '/Description = None/d'|sed '/^ *&/d' >raid_info
:<<!
...
!
    ...
    paste 1temp 2temp >TOPOLOGY.temp
    sed -i '1,3d' TOPOLOGY.temp
    if [ -s TOPOLOGY.temp ]; then
	    show_title "TOPOLOGY :"
	    echo "======="
	    echo "-------------------------------------------------------------------------"
	    cat TOPOLOGY.temp|sed '/^$/d'
	    echo "-------------------------------------------------------------------------"
    fi
    rm *temp -rf
    ##########################VD LIST ########################
    ...
    if [ $vd_nums -gt 0 ]; then
	    ...
	    for((i=0;i<$vd_nums;i++))
    	    do
		storcli /c"$1"/v"$i" show all | sed -n '/DG\/VD/,/Cac=CacheCade/p' | sed '$d' | sed '/^$/d' >> vd.temp.temp
		while read line;
                do
                        echo "$line" |awk  '{ if($NF ~/Name/) {for(i=1;i<=NF;i++) if(i == 8 || i == 5) printf("%s",""); else printf("%-10s",$i);} else {for(i=1;i<=NF;i++){ if(i == 8 || i == 5) printf("%s",""); else if(i == 9)  printf("%s%3s",$i,$((i+1))); else if(i == 10) continue;else printf("%-10s",$i);} }}; END { print ""}'  >>vd.temp
                done < vd.temp.temp
    ...
    if [ ! -f "raid-$1-phyerror.log" ]; then
	echo "RAID $1 initial PHY error status:" > raid-$1-phyerror.log
    	storcli /c"$1"/pall show all|sed '/Controller = /d'|sed '/Status = Success/d'|sed '/Description = None/d'|sed '/^ *&/d' >>raid-$1-phyerror.log
    fi
    phy_title=($(storcli /c$1/pall show all | sed -n '/Phy_No Invld_Dwrd_Cnt/p'))
    storcli /c"$1"/pall show all | sed -n '/Phy_No Invld_Dwrd_Cnt/,$p' | sed -n '/[0-9]/p' > phyerror.txt
    while read line;
    do
	    phylist=($(echo "$line"))
            for((j=1;j<${#phylist[@]};j++))
            do
        	    if [ ${phylist[$j]}  -ge $phy_threshold ]; then
                	    echo "${phy_title[$j]} greater than $phy_threshold"
			    echo "RAID $1 initial PHY error status:"
			    cat raid-$1-phyerror.log
			    echo "RAID $1 current PHY error status:" | tee -a raid-$1-phyerror.log
			    storcli /c"$1"/pall show all|sed '/Controller = /d'|sed '/Status = Success/d'|sed '/Description = None/d'|sed '/^ *&/d' | tee -a raid-$1-phyerror.log
			    touch "raid-phy-error.flag"	    
                    fi
            done
    done < phyerror.txt
    rm *temp -rf
...
get_raid_message()
{
	...
	if [ $flag -eq 1 ];then
	      ...
	      storcli -AdpAllInfo -a$index | sed -n '/HW Configuration/,/ROC temperature/p' | sed '1,2d' | sed '$d'
		
	      show_title "RAID Policy"	      
	      storcli -cfgdsply -a$index |grep Policy
	      ...
	      raidinfo $index
	      #storcli /c0 show|sed '/Generating detailed summary/d'|sed '/Controller = 0/d'|sed '/Status = Success/d'|sed '/Description = None/d'|sed '/^ *&/d' 

              #storcli -AdpAllInfo -a0 | grep -i 'ROC temperature' 
	      #storcli -pdlist -a$index | grep -i 'error' 
   	fi
	if [ $flag -eq 2 ];then
...
raidcard()
{
	local i=0
	local c
	cat pcie_device.txt | grep -E -i "lsi" > raid_controller.txt
...
gpuinfo()
{
    ...
    if [ -s tmp.txt ]; then
        ...
        do
            sed -n "$j"p tmp.txt
            lspci -s $i -vvvv|grep -i LnkCap >/dev/null
            lspci -s $i -vvvv|grep -i LnkSta >/dev/null
            let j=j+1
            echo ""
        done
    else
...
pcieinfo()
{
	...
	local i
        ignore="System|bridge|Performance|PIC|SATA|USB|Unassigned|Communication|Signal|Memory|VGA|SMBus|SPI|Encryption|IOMMU|Non-Essential|NVIDIA Corporation"
...
usb_info()
{
        ...
        lsusb | grep -E -i 'Linux Foundation|American Megatrends'
        show_produce_message "USB Info Message"
}
scelnxmessage()
{
...
function getMEInfo()
{
		local cpu_type=`lscpu | grep -i 'Vendor ID:' | awk -F':' '{print $NF}' | sed 's/[[:space:]]//g'`
        if [[ "$cpu_type" =~ "Intel" ]]; then
			ipmitool -b 6 -t 0x2c mc info	
		fi
...
{
        sleep 30
        ipmitool sdr elist |grep -iE "no reading|'0' degrees"
}
machine_info_check_tool_ver
machineSummary
os_board_info
...
#usb_info
psuinfo
sockets=`lscpu|grep -i 'Socket(s)'|awk -F ':' '{print $2}'`
result=`dmidecode -t system|grep -i "family"|awk -F ':' '{print $2}'`
if [ $sockets -eq 2 -a $result==Rack ];then
    getCoolPolicy
fi
getMEInfo
