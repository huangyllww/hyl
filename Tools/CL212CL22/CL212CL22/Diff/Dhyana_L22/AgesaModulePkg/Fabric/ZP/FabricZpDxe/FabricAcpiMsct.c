...
MSCT_PROX_DOMAIN_INFO         gMsctDomains[MAX_PROXIMITY_DOMAINS];
  ...
  )
{
  ...
  // Initialize gMsctDomains
  for (i = 0; i < MaxProxDomains; i++) {
    gMsctDomains[i].ProxDomain = 0;
    gMsctDomains[i].MaxProcCap = 0;
    gMsctDomains[i].MaxMemCap = 0;
  }
  ...
  if (CalledStatus == EFI_SUCCESS) {
    ...
    for (i = 0; i < MemoryInfoCtr; i++) {
      ...
      for (j = 0; j < NumberOfDomainsInSystem; j++) {
        if (CurrDomain == gMsctDomains[j].ProxDomain) {
          gMsctDomains[j].MaxMemCap += MemoryInfo[i].RegionSize;
          break;
        }
      }
    }
