    ...
    if (FirmwareTableBase->Signature  == FIRMWARE_TABLE_SIGNATURE) {
      ...
      case F18_DN_RAW_ID:
        ...
        if (LogicalId.Revision & HYGON_REV_F18_DN_Ax) {
            *BiosDirBase = FirmwareTableBase->H1BiosDirBase;
            PspChipId = H1_PSP_CHIP_ID;
        } else if (LogicalId.Revision & HYGON_REV_F18_DN_Bx) {
            *BiosDirBase = FirmwareTableBase->H2BiosDirBase;
            PspChipId = H2_PSP_CHIP_ID;
        } else if (LogicalId.Revision & HYGON_REV_F18_DN_Cx) {
          *BiosDirBase = FirmwareTableBase->H3BiosDirBase;
          PspChipId = H3_PSP_CHIP_ID;
        } else if (LogicalId.Revision & HYGON_REV_F18_DN_Dx) {
          *BiosDirBase = FirmwareTableBase->H4BiosDirBase;
          PspChipId = H4_PSP_CHIP_ID;
        }
        else {
          ASSERT(FALSE);
          return FALSE;
