...
/*
	SetMem(Desc, ((USB_DESCRIPTOR_T*)Buffer)->Length, 0);
...
//      dev->configs[Index] = MallocZ(Tmp.TotalLength);
        ...
        //Active Config
        if (Dev->configs[Index]->ConfigurationValue == Dev->dev_info->bConfigNum) {
...
)
{
    ...
    if(Child) {
        ...
        EFI_USB_IO_PROTOCOL *UsbIo;
        USBDEV_T            *Dev ;
        if( EFI_ERROR(gBS->HandleProtocol(Child,& gEfiUsbIoProtocolGuid, (VOID**)&UsbIo)))
        {
            return NULL;
        }
        Dev = UsbIo2Dev(UsbIo);
...
{
                                        //(EIP59272)>
    static NAME_SERVICE_T UsbBusNames;
    EFI_USB_PROTOCOL    *AmiUsbProtocol;
    EFI_STATUS Status;
    ...
    return gBS->InstallMultipleProtocolInterfaces(
                ...
                &gEfiComponentName2ProtocolGuid, InitNamesProtocol(&UsbBusNames,
