...
)
{
    ...
    if (UsbHcClassCode != OHCI_CLASS_CODE) {
        USB_DEBUG(DEBUG_ERROR, DEBUG_LEVEL_3, "OHCI HC Class Code is wrong: %x\n", UsbHcClassCode);
        return USB_ERROR;
    }
    ...
    EfiStatus = AmiValidateMemoryBuffer((VOID*)HcStruc->fpFrameList, 0x100);
    if (EFI_ERROR(EfiStatus)) {
        return USB_ERROR;
    }
#endif
...
)
{
    ...
    UINT8           *MemBlockEnd = (UINT8*)((UINTN)gUsbData->fpMemBlockStart + (gUsbData->MemPages << 12));

    if (((UINTN)DescPtrs < (UINTN)gUsbData->fpMemBlockStart) ||
        (((UINTN)DescPtrs + sizeof(OHCI_DESC_PTRS)) > (UINTN)MemBlockEnd)) {
        return USB_ERROR;
    }

    if (((UINTN)DescPtrs->fpTDRepeat < (UINTN)gUsbData->fpMemBlockStart) ||
        (((UINTN)DescPtrs->fpTDRepeat + sizeof(OHCI_TD)) > (UINTN)MemBlockEnd)) {
        return USB_ERROR;
    }
    ...
    if (((UINTN)DescPtrs->fpEDRepeat < (UINTN)gUsbData->fpMemBlockStart) ||
        (((UINTN)DescPtrs->fpEDRepeat + sizeof(OHCI_ED)) > (UINTN)MemBlockEnd)) {
        return USB_ERROR;
    }
...
)
{
    ...
    if (((UINTN)DescPtrs->fpEDControl < (UINTN)gUsbData->fpMemBlockStart) ||
        (((UINTN)DescPtrs->fpEDControl + sizeof(OHCI_ED)) > (UINTN)MemBlockEnd)) {
        return 0;
    }
    if (((UINTN)DescPtrs->fpTDControlSetup < (UINTN)gUsbData->fpMemBlockStart) ||
        (((UINTN)DescPtrs->fpTDControlSetup + sizeof(OHCI_TD)) > (UINTN)MemBlockEnd)) {
        return 0;
    }
    if (((UINTN)DescPtrs->fpTDControlData < (UINTN)gUsbData->fpMemBlockStart) ||
        (((UINTN)DescPtrs->fpTDControlData + sizeof(OHCI_TD)) > (UINTN)MemBlockEnd)) {
        return 0;
    }
    if (((UINTN)DescPtrs->fpTDControlStatus < (UINTN)gUsbData->fpMemBlockStart) ||
        (((UINTN)DescPtrs->fpTDControlStatus + sizeof(OHCI_TD)) > (UINTN)MemBlockEnd)) {
        return 0;
    }
...
)
{
    ...
    if (((UINTN)DescPtrs->fpEDBulk < (UINTN)gUsbData->fpMemBlockStart) ||
        (((UINTN)DescPtrs->fpEDBulk + sizeof(OHCI_ED)) > (UINTN)MemBlockEnd)) {
        return 0;
    } 

    if (((UINTN)DescPtrs->fpTDBulkData < (UINTN)gUsbData->fpMemBlockStart) ||
        (((UINTN)DescPtrs->fpTDBulkData + sizeof(OHCI_TD)) > (UINTN)MemBlockEnd)) {
        return 0;
    } 
