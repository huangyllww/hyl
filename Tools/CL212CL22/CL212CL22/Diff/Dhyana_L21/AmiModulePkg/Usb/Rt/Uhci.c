...
)
{
    ...
    if (UsbHcClassCode != UHCI_CLASS_CODE) {
        USB_DEBUG(DEBUG_ERROR, DEBUG_LEVEL_3, "UHCI HC Class Code is wrong: %x\n", UsbHcClassCode);
        return USB_ERROR;
    }
    
#if !USB_RT_DXE_DRIVER
    EfiStatus = AmiValidateMemoryBuffer((VOID*)HcStruc->fpFrameList, 0x1000);
    if (EFI_ERROR(EfiStatus)) {
        return USB_ERROR;
    }
#endif
...
)
{
    ...
    if (((UINTN)UhicDescPtrs < (UINTN)gUsbData->fpMemBlockStart) ||
    ...
    if (((UINTN)Qh < (UINTN)gUsbData->fpMemBlockStart) ||
        (((UINTN)Qh + sizeof(UHCI_DESC_PTRS)) > (UINTN)MemBlockEnd)) {
        return USB_ERROR;
    }

    if (((UINTN)Qh->FirstTd < (UINTN)gUsbData->fpMemBlockStart) ||
...
)
{
    ...
    if (((UINTN)DescPtrs->StaticQh < (UINTN)gUsbData->fpMemBlockStart) ||
        (((UINTN)DescPtrs->StaticQh + sizeof(UHCI_QH)) > (UINTN)MemBlockEnd)) {
        return USB_ERROR;
    }

    if (((UINTN)DescPtrs->RootHubQh < (UINTN)gUsbData->fpMemBlockStart) ||
        (((UINTN)DescPtrs->RootHubQh + sizeof(UHCI_QH)) > (UINTN)MemBlockEnd)) {
        return USB_ERROR;
    }

    if (((UINTN)DescPtrs->RepeatQh < (UINTN)gUsbData->fpMemBlockStart) ||
	...
	if (gUsbData->RootHubHcStruc == HcStruc) {
		...
		for (i = 0; i < gUsbData->HcTableCount; i++) {
			...
			if ((gUsbData->HcTable[i]->bHCNumber) && 
                ...
                if (((UINTN)DescPtrs->RootHubQh->FirstTd < (UINTN)gUsbData->fpMemBlockStart) ||
                    (((UINTN)DescPtrs->RootHubQh->FirstTd + sizeof(UHCI_TD)) > (UINTN)MemBlockEnd)) {
                    return USB_ERROR;
                }

                if (((UINTN)DescPtrs->RootHubQh->CurrentTd < (UINTN)gUsbData->fpMemBlockStart) ||
...
)
{
    ...
    while (Td) {
        ...
        Td = (Td->pLinkPtr & UHCI_TERMINATE) ? NULL : (UHCI_TD*)(Td->pLinkPtr & UHCI_POINTER_MASK);
    }
...
)
{
	...
	while (Td) {
		NumTds++;
        Td = (Td->pLinkPtr & UHCI_TERMINATE) ? NULL : (UHCI_TD*)((UINTN)Td->pLinkPtr & UHCI_POINTER_MASK);
	}
