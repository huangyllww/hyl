...
)
{
    ...
    if (!Inq) {
        USB_DEBUG(DEBUG_ERROR, DEBUG_LEVEL_3, "Failed to get the inquiry data\n");
        return NULL;
    }
...
/**
    ...
    @retval Pointer to the identify data
...
)
{
    ...
    if (gCheckUsbApiParameter) {
        Status = AmiValidateMemoryBuffer((VOID*)gUsbData->fpUSBTempBuffer, sizeof(ATA_IDENTIFY_DATA));
        if (EFI_ERROR(Status)) {
            return NULL;
        }
    ...
    MassXactStruc.bCmdSize = sizeof (IDENTIFY_DEVICE_PACKET_CMD);
    MassXactStruc.bXferDir = BIT7;     // IN
    MassXactStruc.fpBuffer = gUsbData->fpUSBTempBuffer;
    MassXactStruc.dLength = sizeof(ATA_IDENTIFY_DATA);
    ...
    if(Data) {
        return (ATA_IDENTIFY_DATA*)gUsbData->fpUSBTempBuffer;
    } else {   
        return NULL;
    }
}
    ...
    if (DevInfo->bSubClass != SUB_CLASS_UFI) {
        for (Count = 0; Count < 30 && VALID_DEVINFO(DevInfo); Count++) {
            ...
            if ((gUsbData->bLastCommandStatus & USB_BULK_TIMEDOUT) && (ReadCapacityTimeout == TRUE)) {
                USB_DEBUG(DEBUG_ERROR, DEBUG_LEVEL_3, "Failed to get the readcapacity data, timeout error\n");
                return USB_ERROR;
            }
        }
    }
...
USBMassInquiryCommand (DEV_INFO* DevInfo)
{
...
#if !USB_RT_DXE_DRIVER
    if (gCheckUsbApiParameter) {
        Status = AmiValidateMemoryBuffer((VOID*)(gUsbData->fpUSBTempBuffer + INQUIRY_TEMP_BUFFER_OFFSET), sizeof(MASS_INQUIRY));
        if (EFI_ERROR(Status)) {
            return NULL;
        }
        gCheckUsbApiParameter = FALSE;
    ...
    CmdBuffer->bOpCode = COMMON_INQUIRY_OPCODE;
    CmdBuffer->bAllocLength = INQUIRY_COMMAND_ALLOCATE_LENGTH;
    ...
    MassXactStruc.fpCmdBuffer = (UINT8*)CmdBuffer;
                                        //(EIP51158+)>
    if (DevInfo->bSubClass == SUB_CLASS_SCSI) {
		MassXactStruc.bCmdSize = INQUIRY_SPC_4_COMMAND_SIZE;	//SPC-4_246	
    } else {
    	MassXactStruc.bCmdSize = sizeof (COMMON_INQ_CMD);
    }
										//<(EIP51158+)
    MassXactStruc.bXferDir = BIT7;     // IN
    MassXactStruc.fpBuffer = gUsbData->fpUSBTempBuffer + INQUIRY_TEMP_BUFFER_OFFSET;
    MassXactStruc.dLength = INQUIRY_TRNASFER_LENGTH;
    ...
    if (Data) {
        return (MASS_INQUIRY*)(gUsbData->fpUSBTempBuffer + INQUIRY_TEMP_BUFFER_OFFSET);
    }
    else {
        return NULL;
    }
