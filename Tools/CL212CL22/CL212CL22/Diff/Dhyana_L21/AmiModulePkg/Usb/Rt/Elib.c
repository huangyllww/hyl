 ...
 )
{
    ...
    if (Counter != 0) {
        ...
        PrevData = IoRead32(PM_BASE_ADDRESS + 8);  //PM1_TMR
        for (Index = 0;  Index < Counter; ) {
            Data32 = IoRead32(PM_BASE_ADDRESS + 8);  //PM1_TMR 
...
)
{
    ...
    if (DevInfo->fpPollEDPtr) {
        if (((UINTN)DevInfo->fpPollEDPtr < (UINTN)gUsbData->fpMemBlockStart) ||
            (((UINTN)DevInfo->fpPollEDPtr + sizeof(MEM_BLK)) > (UINTN)MemBlockEnd)) {
            return EFI_ACCESS_DENIED;
        }
    }

    if (DevInfo->fpPollTDPtr) {
        if (((UINTN)DevInfo->fpPollTDPtr < (UINTN)gUsbData->fpMemBlockStart) ||
            (((UINTN)DevInfo->fpPollTDPtr + sizeof(MEM_BLK)) > (UINTN)MemBlockEnd)) {
            return EFI_ACCESS_DENIED;
        }
    }

    if (DevInfo->fpPollDataBuffer) {
        if (((UINTN)DevInfo->fpPollDataBuffer < (UINTN)gUsbData->fpMemBlockStart) ||
            (((UINTN)DevInfo->fpPollDataBuffer + DevInfo->PollingLength) > (UINTN)MemBlockEnd)) {
            return EFI_ACCESS_DENIED;
        }
    }
#endif
...
)
{
    ...
    if (EFI_ERROR(Status)) {
        USB_DEBUG(DEBUG_INFO, DEBUG_LEVEL_3, "Pci Io Read error: %r at %x\n", Status, Offset);
        Data = 0xFFFFFFFF;
    }
    return Data;
#endif
...
)
{
    ...
    if (EFI_ERROR(Status)) {
        USB_DEBUG(DEBUG_INFO, DEBUG_LEVEL_3, "Pci Io Write32 error: %r at %x\n", Status, Offset);
    }
    return;
#endif
}
...
)
{
    ...
    if (EFI_ERROR(Status)) {
        USB_DEBUG(DEBUG_INFO, DEBUG_LEVEL_3, "Pci Io Write16 error: %r at %x\n", Status, Offset);
    }
    return;
#endif
}
...
)
{
	...
	if (EFI_ERROR(Status)) {
        USB_DEBUG(DEBUG_INFO, DEBUG_LEVEL_3, "Pci Io MMIO Read error: %r at %x\n", Status, Offset);
        Data = 0xFFFFFFFF;
    }
    return Data;
#endif
...
)
{
    ...
    if (EFI_ERROR(Status)) {
        USB_DEBUG(DEBUG_INFO, DEBUG_LEVEL_3, "Pci Io MMIO Write error: %r at %x\n", Status, Offset);
    }
    return;
#endif
}
