...
#else
	UsbFillHcdEntriesDummy(&gUsbData->aHCDriverTable[USB_INDEX_UHCI]);
#endif
#if OHCI_SUPPORT
	OHCI_FillHCDEntries(&gUsbData->aHCDriverTable[USB_INDEX_OHCI]);
#else
	UsbFillHcdEntriesDummy(&gUsbData->aHCDriverTable[USB_INDEX_OHCI]);
#endif
#if EHCI_SUPPORT
	EHCI_FillHCDEntries(&gUsbData->aHCDriverTable[USB_INDEX_EHCI]);
#else
	UsbFillHcdEntriesDummy(&gUsbData->aHCDriverTable[USB_INDEX_EHCI]);
#endif
#if XHCI_SUPPORT
	XHCI_FillHCDEntries(&gUsbData->aHCDriverTable[USB_INDEX_XHCI]);
#else
	UsbFillHcdEntriesDummy(&gUsbData->aHCDriverTable[USB_INDEX_XHCI]);
#endif
}
...
)
{
	...
	if (gUsbData->fpUSBMassConsumeBuffer == NULL) {
        ...
        SetMem(gUsbData->fpUSBMassConsumeBuffer, MAX_CONSUME_BUFFER_SIZE, 0);
    }
	...
	if (gUsbData->fpUSBTempBuffer == NULL) {
        ...
        SetMem(gUsbData->fpUSBTempBuffer, MAX_TEMP_BUFFER_SIZE, 0);
    }
...
)
{
    ...
    if (!gUsbData->kbc_support) {
        Status = pSmst->SmmLocateProtocol(&gEmul6064MsInputProtocolGuid, 
                                    NULL, (VOID**)&gMsInput);
        USB_DEBUG(DEBUG_INFO, DEBUG_LEVEL_3, "Usb Call Back Locate 6064Ms Status = %r\n", Status);
        Status = pSmst->SmmLocateProtocol(&gEmul6064KbdInputProtocolGuid,
                                    NULL, (VOID**)&gKbdInput);
        USB_DEBUG(DEBUG_INFO, DEBUG_LEVEL_3, "Usb Call Back Locate 6064Kbd Status = %r\n", Status);
    ...
    //
   Status = pSmst->SmmLocateProtocol(&gEfiSmmSwDispatch2ProtocolGuid, NULL, (VOID**)&SwSmiDispatch);
   ...
   if (EFI_ERROR (Status)) {
    ...
    //
    gUsbSmmProtocol.GlobalDataValidation.RandomValue = (UINT32)GetPerformanceCounter();
...
)
{
	...
	if (gUsbData->UsbUrp.bFuncNumber != 0) {			// Call from AMIUSB C area
		UsbUrp = &gUsbData->UsbUrp;
	} else {
...
VOID USBAPI_Start (URP_STRUC *fpURP)
{
    ...
    fpURP->bRetValue = USB_StartHostControllers();
...
{
    gCheckUsbApiParameter = FALSE;
    fpURP->bRetValue = USB_StopHostControllers();
...
VOID USBAPI_GetDeviceInfo (URP_STRUC *fpURP)
{
    ...
    //
    if ((!fpURP->ApiData.GetDevInfo.bDevNumber) || 
        (fpURP->ApiData.GetDevInfo.bDevNumber >= MAX_DEVICES)) {
        return;
    }
...
/**
    ...
    @param  Urp   Pointer to Urp structure
**/
VOID
...
VOID USBAPI_HC_Proc(URP_STRUC *fpURP)
{
    ...
    if ((fpURP->bSubFunc >= (sizeof(pHdr->asArray.proc)/sizeof(VOID*))) || (pHdr->asArray.proc[fpURP->bSubFunc] == NULL)) {
...
VOID USBAPI_Core_Proc(URP_STRUC *fpURP)
{
    ...
    if (fpURP->bSubFunc >= COUNTOF(core_proc_table)) {
