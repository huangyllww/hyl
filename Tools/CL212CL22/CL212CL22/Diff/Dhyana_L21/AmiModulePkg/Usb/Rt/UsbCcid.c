...
)
{
    ...
    if (((UINTN)DevInfo->pCCIDDescriptor < (UINTN)gUsbData->fpMemBlockStart) ||
        ((UINTN)DevInfo->pCCIDDescriptor > (UINTN)MemBlockEnd)) {
        Urp->bRetValue = USB_ERROR;
        return;
    }
...
)
{
    ...
    if (((UINTN)fpDevInfo->pCCIDDescriptor < (UINTN)gUsbData->fpMemBlockStart) ||
        ((UINTN)fpDevInfo->pCCIDDescriptor > (UINTN)MemBlockEnd)) {
        return EFI_DEVICE_ERROR;
    }
...
)
{
    ...
    for ( ; dlink; dlink = dlink->pNext) {
        ...
        if ((fpICCDevice->Signature == ICC_DEVICE_SIG) && (fpICCDevice->Slot == Slot)) {
            if (((UINTN)fpICCDevice < (UINTN)gUsbData->fpMemBlockStart) ||
...
)
{
    ...
    if ((ResponseLength != DataLength) && !(CCIDDescriptor->dwFeatures & 0x70000)) {
        ...
        ResponseLength = 2;
        PCToRDRXfrBlock(fpDevInfo, fpICCDevice, 0, Data, 0, ResponseLength);
...
)
{
    ...
    if (fpCCIDDesc->bNumClockSupported) {
        ...
        USBCCIDIssueControlTransfer(fpDevInfo,  
                                    ...
                                    0x0, 0, (UINT8 *)fpDevInfo->ClockFrequencies,
                                    fpCCIDDesc->bNumClockSupported * sizeof(UINT32));
    } else {
