...
)
{
    ...
    for (i = 0; i < gUsbData->HcTableCount; i++) {
        ...
        if ((HcStruc->dHCFlag & (HC_STATE_USED | HC_STATE_INITIALIZED)) 
...
UsbHcStart(HC_STRUC* HcStruc)
{
    ...
    if(gUsbData->UsbUrp.ApiData.HcStartStop.Start != REGISTER_USB_SMI){
...
#if !USB_RT_DXE_DRIVER
    if((gUsbData->RegisterUsbSmiBeforeEndOfDxe && (gUsbData->UsbUrp.ApiData.HcStartStop.Start == REGISTER_USB_SMI))||
            (!gUsbData->RegisterUsbSmiBeforeEndOfDxe && (gUsbData->UsbUrp.ApiData.HcStartStop.Start == START_HOST_CONTROLLER))){
...
#if SHOW_SKIP_PORT_INFORMATION
{
	...
	if(Status != USB_SUCCESS) {
        USB_DEBUG(DEBUG_ERROR, DEBUG_LEVEL_3, "Failed to initialize HC specific data for the device\n");
...
//
    ...
    if (DevDesc == NULL) {
        USB_DEBUG(DEBUG_ERROR, DEBUG_LEVEL_3, "Failed to get the first 8 bytes of the device descriptor.\n");
    ...
    if (Status == USB_ERROR) {
        USB_DEBUG(DEBUG_ERROR, DEBUG_LEVEL_3, "Failed to set device address.\n");
		goto detection_complete;
    }
    ...
    if (DevDesc == NULL) {
        USB_DEBUG(DEBUG_ERROR, DEBUG_LEVEL_3, "Failed to get the completed device descriptor.\n");
		goto detection_complete;
    }
    ...
    if (DevDesc->DescLength < 18) {
        USB_DEBUG(DEBUG_ERROR, DEBUG_LEVEL_3, "The device descriptor is invalid, the length is less than 18.\n");
        goto detection_complete;
    }
    if (DevDesc->NumConfigs == 0) {
        USB_DEBUG(DEBUG_WARN, DEBUG_LEVEL_3, "Number of possible configurations is 0.\n");
        ...
        if (TotalLength > 0xFF) {
            ...
            CnfgDesc = (CNFG_DESC*)USB_GetDescriptor(
                        ...
                        GetCnfgIndex);
								//<(EIP70933+)
            if (CnfgDesc == NULL) {
                USB_DEBUG(DEBUG_ERROR, DEBUG_LEVEL_3, "Failed to get the configuration descriptor, the total length is %x\n", TotalLength);
                break;
            }
        }
...
//
        if (CnfgDesc->bDescType == DESC_TYPE_CONFIG) {
            ...
            if (CnfgDesc->bDescLength == 0) {
                USB_DEBUG(DEBUG_WARN, DEBUG_LEVEL_3, "The length of the configuration descriptor is 0.\n");
...
)
{
    ...
    for (Count = 0; Count < USB_DEVICE_ENUMERATE_RETRY_COUNT; Count++) {
        ...
        if (OEMSkipList(HubAddr,PortNum,HcStruc->wBusDevFuncNum,0,0)) {
            USB_DEBUG(DEBUG_INFO, DEBUG_LEVEL_3, "Match the skip table ; skipping this device.\n");
            return USB_SUCCESS;
        }
    ...
    if (Count == USB_DEVICE_ENUMERATE_RETRY_COUNT) {
        USB_DEBUG(DEBUG_ERROR, DEBUG_LEVEL_3, "Usb Device enumeration is failed.\n");
        USB_DisableHubPort(HcStruc, HubAddr, PortNum);
...
{
    HcdHeader->pfnHCDStart                = UsbHcdStartDummy;
    HcdHeader->pfnHCDStop                 = UsbHcdStopDummy;
    HcdHeader->pfnHCDEnumeratePorts       = UsbHcdEnumeratePortsDummy;
    HcdHeader->pfnHCDDisableInterrupts    = UsbHcdDisableInterruptsDummy;
    HcdHeader->pfnHCDEnableInterrupts     = UsbHcdEnableInterruptsDummy;
    HcdHeader->pfnHCDProcessInterrupt     = UsbHcdProcessInterruptDummy;
    HcdHeader->pfnHCDGetRootHubStatus     = UsbHcdGetRootHubStatusDummy;
    HcdHeader->pfnHCDDisableRootHub       = UsbHcdDisableRootHubDummy;
    HcdHeader->pfnHCDEnableRootHub        = UsbHcdEnableRootHubDummy;
    HcdHeader->pfnHCDControlTransfer      = UsbHcdControlTransferDummy;
    HcdHeader->pfnHCDBulkTransfer         = UsbHcdBulkTransferDummy;
    HcdHeader->pfnHCDIsocTransfer         = UsbHcdIsocTransferDummy;    
    HcdHeader->pfnHCDInterruptTransfer    = UsbHcdInterruptTransferDummy;
    HcdHeader->pfnHCDDeactivatePolling    = UsbHcdDeactivatePollingDummy;
    HcdHeader->pfnHCDActivatePolling      = UsbHcdActivatePollingDummy;
    HcdHeader->pfnHCDDisableKeyRepeat     = UsbHcdDisableKeyRepeatDummy;
    HcdHeader->pfnHCDEnableKeyRepeat      = UsbHcdEnableKeyRepeatDummy;
    HcdHeader->pfnHCDEnableEndpoints      = UsbHcdEnableEndpointsDummy;
    HcdHeader->pfnHCDInitDeviceData       = UsbHcdInitDeviceDataDummy;
    HcdHeader->pfnHCDDeinitDeviceData     = UsbHcdDeinitDeviceDataDummy;
    HcdHeader->pfnHCDResetRootHub         = UsbHcdResetRootHubDummy;
    HcdHeader->pfnHCDClearEndpointState	  = UsbHcdClearEndpointStateDummy;
    HcdHeader->pfnHCDGlobalSuspend        = UsbHcdGlobalSuspendDummy;
    HcdHeader->pfnHCDSmiControl           = UsbHcdSmiControlDummy;
