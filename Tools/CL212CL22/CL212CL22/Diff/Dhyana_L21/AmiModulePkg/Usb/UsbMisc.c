...
)
{
    ...
    if (gAmiUsbController->UsbInvokeApi) {   
	    ...
	    CopyMem(&gUsbData->UsbUrp, Urp, sizeof(URP_STRUC));
        USBGenerateSWSMI(USB_SWSMI);
        CopyMem(Urp, &gUsbData->UsbUrp, sizeof(URP_STRUC));
    }
}
...
)
{
	...
	Status = gBS->AllocatePool (EfiRuntimeServicesData, NewSize, (VOID**)&NewBuffer);
    if (EFI_ERROR(Status)) {
        return Status;
    }
