...
)
{
    ...
    Status = gBS->AllocatePool(EfiRuntimeServicesData, sizeof(USB_DEV_CONFIG_LINK), (VOID**)&DevConfigLink);
    if(EFI_ERROR(Status))   return Status;

    CopyMem(DevConfigLink, DevConfigInfo, sizeof(USB_DEV_CONFIG_INFO));
...
)
{
    ...
    SetupStatus = gRT->GetVariable( L"UsbSupport", &gEfiSetupGuid, NULL,
    ...
    EfiUsbPolicyProtocol.RegisterUsbSmiBeforeEndOfDxe = TRUE;
	...
	Status = gBS->AllocatePool(EfiRuntimeServicesData, gUsbData->MemBlkStsBytes, 
	        (VOID**)&gUsbData->aMemBlkSts);
	ASSERT_EFI_ERROR(Status);
	SetMem(gUsbData->aMemBlkSts, gUsbData->MemBlkStsBytes, 0xFF);
...
)
{
    ...
    //
    Status = gBS->HandleProtocol(Handle, &gNonSmmEmul6064TrapProtocolGuid, (VOID**)&gEmulationTrap);
    ASSERT_EFI_ERROR(Status);
}
...
)
{
    ...
    UpdateHcPciInfo();
    gBS->CloseEvent(Event); // this is one time callback
}
    ...
    IN EFI_DEVICE_PATH_PROTOCOL     *RemainingDevicePath )
{
    ...
    if (!(gUsbData->dUSBStateFlag & USB_FLAG_AFTER_END_OF_DXE)) {
        if(gUsbData->RegisterUsbSmiBeforeEndOfDxe){
            Parameters = &gUsbData->UsbUrp;
            Parameters->ApiData.HcStartStop.HcStruc = HcStruc;
            Parameters->ApiData.HcStartStop.Start = REGISTER_USB_SMI;
	...
	Parameters = &gUsbData->UsbUrp;
    Parameters->bFuncNumber = USB_API_HC_START_STOP;
    Parameters->ApiData.HcStartStop.Start = START_HOST_CONTROLLER;
	Parameters->ApiData.HcStartStop.HcStruc = HcStruc;
...
//    UINT32                  Index;
    ...
    Parameters.bFuncNumber = USB_API_HC_START_STOP;
    Parameters.ApiData.HcStartStop.Start = STOP_HOST_CONTROLLER;
	Parameters.ApiData.HcStartStop.HcStruc = HcStruc;
	InvokeUsbApi(&Parameters);
...
#if USB_RUNTIME_DRIVER_IN_SMM
    gUsbData->UsbUrp.bFuncNumber = USB_API_LOCK_USB_SW_SMI;
    USBGenerateSWSMI(USB_SWSMI);
#endif
...
{
    gUsbData->dUSBStateFlag |= USB_FLAG_AFTER_END_OF_DXE;
}
...
)
{
    ...
    Status = PciIo->Pci.Read(PciIo, EfiPciIoWidthUint16, PCI_VID, 1, &Vid);
    if (EFI_ERROR(Status)) {
        return FALSE;
    }

    Status = PciIo->Pci.Read(PciIo, EfiPciIoWidthUint16, PCI_DID, 1, &Did);
    if (EFI_ERROR(Status)) {
        return FALSE;
    }
...
/**
    ...
    @param  Controller  Host controller handle number
            PciBus      Host controller pci bus number
            PciDev      Host controller pci device number
            PciFunc     Host controller pci function number
            HcType      Host controller type
            Irq         Host controller Irq
            DevicePath  Point to DevicePath protocol 
            PciIo       Point to PciIo protocol
...
)
{
    ...
    PciAddr = (UINT16)((PciBus << 8) | (PciDev << 3) | PciFunc);        //PciAddr   Bit15-Bit8:Bus, Bit7-Bit3:Device,
                                                                        //          Bit2-Bit0:Function
    for (Index = 0; Index < gUsbData->HcTableCount; Index++) {
        ...
        if (!DPCmp(gUsbData->HcTable[Index]->pHCdp, DevicePath)) {
            break;
        }
    }
    ...
    if (Index != gUsbData->HcTableCount) {
        ...
        Status = gBS->AllocatePool(EfiRuntimeServicesData, sizeof(HC_STRUC), (VOID**)&HcStruc);
        if (EFI_ERROR(Status)) {
            return NULL;
        }
	...
	if (aHCSpecificInfo[Index].FrameListSize) {
		...
		if (HcStruc->fpFrameList == NULL) {
            ...
            Status = UsbHcMemoryRecord(HcStruc->Controller,
                                    (EFI_PHYSICAL_ADDRESS)HcStruc->fpFrameList,
                                    EFI_SIZE_TO_PAGES(aHCSpecificInfo[Index].FrameListSize));
            if(EFI_ERROR(Status)){
                return NULL;
            }
		...
		SetMem(HcStruc->fpFrameList, aHCSpecificInfo[Index].FrameListSize, 0);
...
)
{
    ...
    for (Index = DevIndex; Index < MAX_DEVICES; Index++) {
        if (!(gUsbData->aDevInfoTable[Index].Flag & DEV_INFO_DEV_PRESENT)) {
            continue;
        }
        if (gUsbData->aDevInfoTable[Index].bDeviceType == BIOS_DEV_TYPE_STORAGE) {
            break;
        }
    }
    ...
    // Copy device name
    //
    CopyMem(Data, &gUsbData->aDevInfoTable[Index].DevNameString, DataSize);
    ...
    //
    for (Index++; Index < MAX_DEVICES; Index++) {
        if (gUsbData->aDevInfoTable[Index].bDeviceType == BIOS_DEV_TYPE_STORAGE) {
            break;
        }
    }
    if (Index == MAX_DEVICES) {
        ...
        CurrentDevIndex = Index;        // Return next device index
    }
...
//		gUsbData->bLastUSBKeyCode = 0;	//(EIP102150-)
    ...
    SetMem(gUsbData->aKBCCharacterBufferStart, sizeof(gUsbData->aKBCCharacterBufferStart), 0);
	...
	SetMem(gUsbData->aKBCScanCodeBufferStart, sizeof(gUsbData->aKBCScanCodeBufferStart), 0);
	gUsbData->fpKBCScanCodeBufferPtr = gUsbData->aKBCScanCodeBufferStart;
                                        
	SetMem(gUsbData->aKBCDeviceIDBufferStart, sizeof(gUsbData->aKBCDeviceIDBufferStart), 0);               //(EIP102150+)
	SetMem(gUsbData->aKBCShiftKeyStatusBufferStart, sizeof(gUsbData->aKBCShiftKeyStatusBufferStart), 0);   //(EIP102150+)    

	SetMem(gUsbData->aKBInputBuffer, sizeof(gUsbData->aKBInputBuffer), 0);
}
