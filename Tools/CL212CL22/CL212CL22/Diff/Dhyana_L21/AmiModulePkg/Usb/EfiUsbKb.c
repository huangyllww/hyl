...
)
{
    ...
    Status = gBS->AllocatePool(EfiBootServicesData, 
                (MaxKey * sizeof(VOID*)) + (MaxKey * KeySize), (VOID**)&Buffer);
...
)
{
    ...
    SetMem(gUsbData->aKBCCharacterBufferStart, 
    ...
    SetMem(gUsbData->aKBCScanCodeBufferStart, 
...
)
{
    ...
    Status = gBS->AllocatePool(EfiBootServicesData, sizeof(USB_KB_DEV), (VOID**)&UsbKbd);
    if (EFI_ERROR(Status)) {
        return Status;
    }
...
/**
    ...
    @param  This                Pointer to driver binding protocol
            ...
            Children            Pointer to children handle
...
)
{
    ...
    if (IsKeyBufferEmpty(&UsbKbd->EfiKeyBuffer) && 
        ...
        if (UsbKbd->ShiftState != 0) {
            ...
            EfiKey.KeyState.KeyToggleState = 
                UsbKbd->ToggleState | TOGGLE_STATE_VALID;
            Status = EFI_SUCCESS;
        }
    } else {
    ...
    if (!EFI_ERROR(Status)) {
        if ((EfiKey.Key.UnicodeChar == 0) && (EfiKey.Key.ScanCode == SCAN_NULL) &&
            (EfiKey.KeyState.KeyShiftState & ~SHIFT_STATE_VALID)) {
