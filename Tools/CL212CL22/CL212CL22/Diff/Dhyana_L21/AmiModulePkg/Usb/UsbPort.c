...
)
{
	...
	Status = gBS->AllocatePool(
			...
			(VOID**)&gUsbData->gUsbSkipListTable);
    
    if (EFI_ERROR(Status)) {
        return;
    }
    
    SetMem(gUsbData->gUsbSkipListTable, (TotalStruc + 1) * sizeof(USB_SKIP_LIST), 0);
    CopyMem(gUsbData->gUsbSkipListTable, SkipStruc, TotalStruc * sizeof(USB_SKIP_LIST));

#if USB_DEBUG_MESSAGES
{
    UINT8   Index;

    USB_DEBUG(DEBUG_INFO, DEBUG_LEVEL_3, "Usb skip device table list, the total of list is %d\n", TotalStruc); 		
    for (Index = 0; Index < TotalStruc; Index++) {
        USB_DEBUG(DEBUG_INFO, DEBUG_LEVEL_3, "SkipType: %02x, Flag: %02x, BDF: %04x, RootPort: %02x, RoutePath: %08x, BaseClass: %02x\n",
            SkipStruc[Index].bSkipType, SkipStruc[Index].bFlag, SkipStruc[Index].wBDF,
            SkipStruc[Index].bRootPort, SkipStruc[Index].dRoutePath, SkipStruc[Index].bBaseClass);
    }
}
#endif
...
)
{
    ...
    if (EFI_ERROR(Status)) {
        ...
        SetMem(&UsbData->UsbSetupData, sizeof(UsbData->UsbSetupData), 0);
