    ...
    IN EFI_PEI_SERVICES    **PeiServices )
{
    ...
    // {0x3, 0x0, 0x3},{0x15, 0x0, 0x3},{0x43, 0x0, 0x3},{0x55, 0x0, 0x3}
    if(gXhciControllerCount == 4){
        ...
        (*PeiServices)->PciCfg->Read(PeiServices,(*PeiServices)->PciCfg,
            EfiPeiPciCfgWidthUint8, EFI_PEI_PCI_CFG_ADDRESS(0, 0x18 + 1, 0, 0x84), &Bus);
            gXhciControllerPciTable[1].Bus = (Bus + 5);
            if(SystemDieCount > 4){
                (*PeiServices)->PciCfg->Read(PeiServices,(*PeiServices)->PciCfg,
                            EfiPeiPciCfgWidthUint8, EFI_PEI_PCI_CFG_ADDRESS(0, 0x18 + 4, 0, 0x84), &Bus);
                gXhciControllerPciTable[2].Bus = (Bus + 3);
                (*PeiServices)->PciCfg->Read(PeiServices,(*PeiServices)->PciCfg,
                            EfiPeiPciCfgWidthUint8, EFI_PEI_PCI_CFG_ADDRESS(0, 0x18 + 5, 0, 0x84), &Bus);
                gXhciControllerPciTable[3].Bus = (Bus + 5);
            }else{
    ...
    for(;;) {
        ...
        switch (XhciExtCap->CapId) {
            ...
            case XHCI_EXT_CAP_SUPPORTED_PROTOCOL:
                DEBUG((DEBUG_INFO, "XHCI: CAP supported : MajorRev %x MajorRev %x\n", ((XHCI_EXT_PROTOCOL*)XhciExtCap)->MajorRev, ((XHCI_EXT_PROTOCOL*)XhciExtCap)->MajorRev));
