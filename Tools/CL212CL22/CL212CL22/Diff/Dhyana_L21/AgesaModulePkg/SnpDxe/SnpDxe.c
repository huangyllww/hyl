  ...
  )
{
  ...
  if (EFI_SUCCESS == Status) {
    ((PXE_DB_GET_STATUS*)(Private->Db))->TxBuffer[0] = (UINTN) (UINT8 *) Buffer;
  }
  ...
  )
{
  ...
  DMA->HostAddress = (VOID*)DMA->DeviceAddress;
  DMA->Mapping = 0;
...
//PS-67 add for multi xgbe ports
  if (Private->DeviceHandle == NULL) {
    DEBUG((DEBUG_INFO, "PS-67: can not support multi xgbe ports!\n"));
    return EFI_INVALID_PARAMETER;
  }

  //SetMem (Private, sizeof (ENET_DXE_PRIVATE_DATA), 0); //PS-67---set mem in previous step
  //
  //  Set SnpDxe's function pointer
  //
  Private->Signature    = ENET_DRIVER_SIGNATURE;
  //Private->DeviceHandle = NULL;  //PS-67--- for mult xgbe ports
  ...
  DEBUG((DEBUG_INFO, "PS-67: before:\n"));
  DEBUG((DEBUG_INFO, "PS-67: IOMUXx093 = 0x%02x\n", *(volatile UINT8*)0xFED80D93));
  DEBUG((DEBUG_INFO, "PS-67: IOMUXx094 = 0x%02x\n", *(volatile UINT8*)0xFED80D94));
  *(volatile UINT8*)0xFED80D93 = 0x01; //PS-67+++ Set GPIO147 to SPF_SCL
  *(volatile UINT8*)0xFED80D94 = 0x01;  //PS-67+++ Set GPIO148 to SPF_SDA
  gBS->Stall (100);
  DEBUG((DEBUG_INFO, "PS-67: after:\n"));
  DEBUG((DEBUG_INFO, "PS-67: IOMUXx093 = 0x%02x\n", *(volatile UINT8*)0xFED80D93));
  DEBUG((DEBUG_INFO, "PS-67: IOMUXx094 = 0x%02x\n", *(volatile UINT8*)0xFED80D94));
  ...
  XgbePhySetup (Private);
  XgbeInit (Private);
  XgbePlatinit (Private);
  Status = XgbeConfigPhy (Private);
  if (EFI_ERROR (Status)) {
    return EFI_NOT_FOUND;
  }
  ...
  Private->xgmac_pdata.rx_riwt = 0x4b;
        ...
        )
{
  ...
  SetMem (Private, sizeof (ENET_DXE_PRIVATE_DATA), 0);//PS-67+++++for multi xgbe ports
  Private->DeviceHandle = ControllerHandle; //PS-67+++for support multi xgbe ports
