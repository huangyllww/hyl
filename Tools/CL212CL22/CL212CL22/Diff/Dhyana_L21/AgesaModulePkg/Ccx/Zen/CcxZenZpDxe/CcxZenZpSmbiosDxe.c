  ...
  )
{
  ...
  if (NbioSmuServices == NULL) {
    ...
    NbioSmuServices->AmdSmuRegisterRead (NbioSmuServices, (UINT32) DieNumber, 0x5D25C, &CoreDisFuse);
    NbioSmuServices->AmdSmuRegisterRead (NbioSmuServices, (UINT32) DieNumber, 0x5D380, &Fmax);
    Fmax = (Fmax >> 6) & 0xFF; // bits [13:6]
