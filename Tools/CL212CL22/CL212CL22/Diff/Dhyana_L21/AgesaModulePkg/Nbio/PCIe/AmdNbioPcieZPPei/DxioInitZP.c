  ...
  )
{
  ...
  while (PcieWrapper != NULL) {
    ...
    if((CpuGen == Cpu_Gen_Dhyana2) && (GetPcieCorePresetPcd(GnbHandle->InstanceId, PcieWrapper->WrapId) != 0xff)){
  ...
  while (PcieWrapper != NULL) {
    ...
    while (PcieEngine != NULL) {
      if (PcieLibIsEngineAllocated(PcieEngine)) {
          ...
          if((CpuGen == Cpu_Gen_Dhyana2) && (Stepping <= 1)){
              MappingPortID = (PcieEngine->Type.Port.PortId) % 8;
