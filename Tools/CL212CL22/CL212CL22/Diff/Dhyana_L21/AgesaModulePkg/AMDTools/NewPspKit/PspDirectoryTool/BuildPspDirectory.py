...
Version = "5.0.03"
#Version 5.0.01  12/22/2020
  # * Add D2 support

# Version = "4.0.01"
#Version 4.0.00  02/14/2020
  # * Check apcb size

#Version = "4.0.00"
#Version 4.0.00  02/14/2020
  # * Add DN and DP support
...
#                         Type:   DIR, Entry Type    ShortName    Full description  Modifiable 
                          ...
                          0x38: ['PSP' ,'IMAGE_ENTRY','VmGuardData', 'VM Guard Data', 1],
                          0x40: ['PSP' ,'POINT_ENTRY','PspDirLv2', 'Point to PSP level 2 directory', 0],
                          0x49: ['PSP' ,'IMAGE_ENTRY','CsvFirmware', 'Csv Firmware', 0],
                          ...
                          0x19020: ['BIOS','IMAGE_ENTRY','UCodePatchC0','Microcode Patch', 0],
                          0x19021: ['BIOS','IMAGE_ENTRY','UCodePatchC1','Microcode Patch', 0],
                                 }

ProgramTable = {  
  'ZP': {"PSPID": 0xBC090000, "BIOSDIR_OFFSET": 6},
  'RV': {"PSPID": 0xBC0A0000, "BIOSDIR_OFFSET": 7},
  'DN': {"PSPID": 0xBC090000, "BIOSDIR_OFFSET": 6},
  'DP': {"PSPID": 0xBC090200, "BIOSDIR_OFFSET": 7},
  'D2': {"PSPID": 0xBC090201, "BIOSDIR_OFFSET": 8}
}
...
def ParseArg ():
  ...
  DPParser.add_argument ("-p", "--program", help="Specify the program name, Valid choices: DN, DP, D2", required=True)
  ...
  if (args.subcommand == 'dp'):
    if args.program.upper() not in ["ZP", "RV", "DN", "DP", "D2"]:
      log.info ("[Error] Not a valid program string, only DN & DP & D2 are supported")
      FatalErrorExit ()
...
def CheckComboEntries(Entries):
  ...
  for entry in Entries:
    if entry['Id'] == ProgramTable['DP']['PSPID']:
      IsDp = True
      DpEntryIndex = Index
    if entry['Id'] == ProgramTable['D2']['PSPID']:
      IsD2 = True
      D2EntryIndex = Index
    Index += 1
  if (IsDp and IsD2) and (DpEntryIndex < D2EntryIndex):
    Entries[DpEntryIndex], Entries[D2EntryIndex] = Entries[D2EntryIndex], Entries[DpEntryIndex]
...
def GetPspDirectory (BinaryFile):
  ...
  OemSigRaw = FileHandle.read (0x24)
  OemSigArray = struct.unpack ('9L',OemSigRaw)
