  ...
  )
{
  ...
  BackUpCG2 ();
  FixPsp4Ehang ();
  TurnOffUsbPme(0, pFchPolicy);
  
  //Add WA for S4 K&B wakeup which set BIT17(suspend) as 1, when is being into S4 process.
  BaseGetLogicalIdOnExecutingCore (&LogicalId);
  if ((LogicalId.Family == HYGON_FAMILY_18) && (LogicalId.Revision & HYGON_REV_F18_DN_Dx)) {   
    FchSmnRW (0, FCH_TS_USB_GUSB3PIPECTL0, 0x00, 0x01020202, NULL);
    FchSmnRW (0, FCH_TS_USB_GUSB3PIPECTL1, 0x00, 0x01020202, NULL);
    FchSmnRW (0, FCH_TS_USB_GUSB3PIPECTL2, 0x00, 0x01020202, NULL);
    FchSmnRW (0, FCH_TS_USB_GUSB3PIPECTL3, 0x00, 0x01020202, NULL);
  }
