  ...
  )
{
  ...
  if (CoreSelector == ALL_THREADS) {
    ...
    if ((NumberOfSockets == 0) || (NumberOfDiesPerSocket == 0) || (NumberOfComplexesPerDie == 0) || (NumberOfCoresPerComplex == 0) || (NumberOfThreadsPerCore == 0)) {
      gBS->LocateProtocol (&gAmdFabricTopologyServicesProtocolGuid, NULL, &FabricTopology);
      gBS->LocateProtocol (&gAmdCoreTopologyServicesProtocolGuid, NULL, &CoreTopology);
