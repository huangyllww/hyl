  ...
  )
{
  ...
  DEBUG ((EFI_D_ERROR, "ExtendedVerification = %d is not implemented!\n", ExtendedVerification));
  ...
  )
{
  ...
  Private->AnReryCnt = INIT_AN_RETRY_CNT;
  //Program Base address for XPCS and XGBE for each port
  Status = XgbeBaseAddr (Private,ControllerHandle);
  XgbeI2cInit(Private);
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  Private->Db = NULL;
  Status = gBS->AllocatePool (EfiBootServicesData, PRIVATE_DB_SIZE, (VOID*)&(Private->Db));
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "\nCould not allocate DB structures.\n"));
    ASSERT_EFI_ERROR (Status);
    return Status;
  }
  ZeroMem(Private->Db, PRIVATE_DB_SIZE);
  ...
  xgmac_default_config (Private, &Private->xgmac_pdata);
  Private->xgmac_pdata.channel.pdata = &Private->xgmac_pdata;
  Private->xgmac_pdata.channel.dma_regs = Private->xgmac_pdata.xgmac_regs + DMA_CH_BASE; // + (DMA_CH_INC * i)
        ...
        )
{
  ...
  Status = PrivateDataInit (Private, ControllerHandle);
  if (EFI_ERROR (Status)) {
