...
/**
    Hook EXTENDED_SMI for support DWORD SW SMI in HYGON reference code base.
  ...
  )
{
	...
	// 3. steal DispatchHandle from HYGON handle. replace it with ours.
	// 4. when DWORD-SW-SMI come in. we translate it to user callback.
	// 5. restore back HYGON DispatchHandle while unregister.
	//

	Status = gOriginalRegister(This, HygonFchSmmDwordSwSmi, RegisterContext, DispatchHandle);
	...
	// save original handle that HYGON create, Let we have chance to use it when unRetister.
...
/**
    Collect HYGON SW SMI NODE and translate to AMI HANDLER_LINK.
...
**/
VOID
HygonFchSmmSwNodeTranslateToAMI (
  IN EFI_HANDLE  DispatchHandle,
  IN CONST VOID  *Context         OPTIONAL,
  IN OUT VOID    *CommBuffer      OPTIONAL,
  IN OUT UINTN   *CommBufferSize  OPTIONAL
  ...
  )
{
    ...
    case DUMP_REGISTERED_SWSMI:
    	...
    	Status = gOriginalRegister(This, (EFI_SMM_HANDLER_ENTRY_POINT2)HygonFchSmmSwNodeTranslateToAMI, RegisterContext, DispatchHandle);
    	break;
#endif
	case EXTENDED_SMI:
#ifdef EXTENDED_SMI1
...
/**
    ...
    Hook on to HYGON FchSmmDispatcher.
