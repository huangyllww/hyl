    ...
    Method(_CRS, 0, Serialized) {
            ...
            If(CPRB){
        		...
        		CreateWordField(CRS1, ^IOW2._LEN, LEN1)	// Length
                 ...
                 If(LOr(LOr(LEqual(\_SB.IER0,0x1),LEqual(\_SB.IER1, 0x1)),LOr(LEqual(\_SB.IER2,0x1),LEqual(\_SB.IER3, 0x1)))){
                    //SoC legacy UART being decoded, do not reserve IO in PCI RB	                    
                    CreateWordField(CRS1, ^IOBU._MAX, IBX1)	// Max
                    CreateWordField(CRS1, ^IOBU._LEN, IBL1)	// Length
                    Store(0x2E7, IBX1)
                    Store(0x2E8, IBL1)
                    CreateWordField(CRS1, ^IOAU._MIN, IBM2)	// Min
                    CreateWordField(CRS1, ^IOAU._LEN, IBL2)	// Length
                    Store(0x400, IBM2)
                    Store(0x8F8, IBL2)                                        
                  } 
                  //Leave default ASL base at 0xD00 but adjust length based on PCI mod of IOL
                  Subtract(IOL, MIN1, Local1)	//max by pci - 0xD00
                  Store(Local1, LEN1)
                  Store(LEN1,Local0)
		...
		//Update MMIO Above 4G - START

		        CreateQWordField(CRS1, ^MEM8._MIN, MIN8)	// Min

		        CreateQWordField(CRS1, ^MEM8._MAX, MAX8)	// Max

        		CreateQWordField(CRS1, ^MEM8._LEN, LEN8)	// Length
		...
		//Update MMIO Above 4G - END
/*
//THis is older compat code for XP etc that don't support QWORD
