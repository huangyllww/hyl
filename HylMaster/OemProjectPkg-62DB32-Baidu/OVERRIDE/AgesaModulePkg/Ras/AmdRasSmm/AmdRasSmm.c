...
#include <Setup.h> //<Chendl001-20180626 Memory Leaky Bucket Design>

/*----------------------------------------------------------------------------------------
 *                   D E F I N I T I O N S    A N D    M A C R O S
 *----------------------------------------------------------------------------------------
 */
#define FILECODE RAS_AMDRASSMM_AMDRASSMM_FILECODE
 ...
 *
 * Description
 *   Entry point of the AMD RAS SMM driver
...

{
  ...
  if (mAmdRasPolicy->PFEHEnable) {
    ...
    RasThresholdConfig.MemThresholdCount = mAmdRasPolicy->McaMemErrThreshCount;
	RasThresholdConfig.ProThresholdCount = mAmdRasPolicy->McaProErrThreshCount;
	//<Chendl001-20180626 Memory Leaky Bucket Design ->
    RasThresholdConfig.ThresholdIntType = 2;          // SMI

    if (mAmdRasPolicy->McaErrThreshEn) {
	//<Chendl001-20180626 Memory Leaky Bucket Design +>
      DEBUG((EFI_D_ERROR, "[OemDbg]: MCE Error Memory Threshold Enable : Counter = 0x%04x\n", RasThresholdConfig.MemThresholdCount));
	  DEBUG((EFI_D_ERROR, "[OemDbg]: MCE Error Processor Threshold Enable : Counter = 0x%04x\n", RasThresholdConfig.ProThresholdCount));
    }
	//<Chendl001-20180626 Memory Leaky Bucket Design ->
...
//<Hujf1001-20210304 Memory Leaky Bucket Design V2 +>
  ...
  {
    if((AsmReadMsr64 (0xc00020F3) >> 32) !=0 )
    {
