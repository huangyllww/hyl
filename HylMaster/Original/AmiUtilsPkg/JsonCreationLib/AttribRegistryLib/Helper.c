...
void WriteAttributesToFile(ATTRIBUTES 	*In_Attributes)
{
	...
	{		
            ...
            //<Yaosr-20210312 Improve redfish: diplay bios options according to the BIOS setup hierarchy on the BMC web. +>
            //Add MenuName.
            if((NULL != In_Attributes->_MenuName)&&(StrLen(In_Attributes->_MenuName)))
            {
                JsonStr = GetJsonDataStr(TRUE, JSON_DATA,5,BiosSetupDataStr[ATTRIBUTES_INDEX+16],In_Attributes->_MenuName);
                UpdateData(&JsonStr);
            }
            //Add MenuPath.
            if((NULL != In_Attributes->_MenuPath)&&(StrLen(In_Attributes->_MenuPath)))
            {
                JsonStr = GetJsonDataStr(TRUE, JSON_DATA,5,BiosSetupDataStr[ATTRIBUTES_INDEX+17],In_Attributes->_MenuPath);
                UpdateData(&JsonStr);
            }
            //Add DisplayOrder.
            if((NULL != In_Attributes->_DisplayOrder)&&(StrLen(In_Attributes->_DisplayOrder)))
            {
                JsonStr = GetJsonDataStr(TRUE, JSON_DATA_INTEGER,5,BiosSetupDataStr[ATTRIBUTES_INDEX+18],In_Attributes->_DisplayOrder);
                UpdateData(&JsonStr);
            }
            //Add GrayOut
            JsonStr = GetJsonDataStr(TRUE, JSON_DATA_INTEGER,5,BiosSetupDataStr[ATTRIBUTES_INDEX+19],BOOL_STR(In_Attributes->_GrayOut));
            UpdateData(&JsonStr);
            //Add Hidden
            JsonStr = GetJsonDataStr(TRUE, JSON_DATA_INTEGER,5,BiosSetupDataStr[ATTRIBUTES_INDEX+20],BOOL_STR(In_Attributes->_Hidden));
            UpdateData(&JsonStr);
            //<Yaosr-20210312 Improve redfish: diplay bios options according to the BIOS setup hierarchy on the BMC web. ->
