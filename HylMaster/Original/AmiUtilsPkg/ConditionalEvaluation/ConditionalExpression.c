...
EFI_STATUS AddToDependencies (CONTROL_INFO *ControlInfo, UINT8 *CondPtr, UINTN *Offset, DEPENDENCIES  **Dependencies, CHAR16 *AttributeName, MAP_PROPERTY *ConstantProperty, BOOLEAN *IsHide)//<Yaosr-20210312 Improve redfish: diplay bios options according to the BIOS setup hierarchy on the BMC web.>
{
    UINTN               ScopeCount = 0;
    EFI_IFR_OP_HEADER   *opHeader = (EFI_IFR_OP_HEADER *) CondPtr;
    ...
    while (1 == ScopeCount)
    {
        ...
        switch (*(UINT8 *)opHeader)
        {
            case EFI_IFR_EQ_ID_VAL_OP:
                ...
                if (TempDependencies->MapFromValue == 0x11) { //Do not show hidden option(ctrl+F11)
                  *IsHide = TRUE;
                }
                //<Yaosr-20210312 Improve redfish: diplay bios options according to the BIOS setup hierarchy on the BMC web. ->
                break;
...
EFI_STATUS AddControlDependencies (CONTROL_INFO *controlInfo, DEPENDENCIES  **AttribDependencies, CHAR16 *AttributeName, BOOLEAN *ReadOnly, BOOLEAN *IsHide)//<Yaosr-20210312 Improve redfish: Add Hide attribute.>
{
    UINT8           *CondPtr = NULL;
    UINTN           Offset = 0;
    EFI_STATUS      Status = EFI_SUCCESS;
    ...
    while ((UINT8*)CondPtr < (UINT8*)controlInfo->ControlPtr)
    {
        ...
        switch (*CondPtr)
        {
            ...
            case EFI_IFR_GRAY_OUT_IF_OP:
                ...
                {
                    Offset = 0;
                    Status = AddToDependencies (controlInfo, CondPtr, &Offset, &Dependencies, AttributeName, &TempConstantProperty, IsHide);//<Yaosr-20210312 Improve redfish: diplay bios options according to the BIOS setup hierarchy on the BMC web.>
...
EFI_STATUS WriteDependencyToFile (ATTRIBUTES *AttributeList)
{
    ...
    if (NULL == AttributeList->Dependencies)
    {
        ...
        if ((_CONTROL_TYPE_TEXT == AttributeList->_Type) || (_CONTROL_TYPE_MEMO == AttributeList->_Type))
        //<Yaosr-20210312 Improve redfish: diplay bios options according to the BIOS setup hierarchy on the BMC web. ->
        {
            return EFI_UNSUPPORTED;
        }
...
//<Wangbn001-20200106-Fixed memory leak issue ->
            ...
            if (NULL != DependencyAttributes)
            {
                ...
                cJSON_AddBoolToObject (DependencyObject, "MapToValue", (int)1);
                //<Yaosr-20210312 Improve redfish: diplay bios options according to the BIOS setup hierarchy on the BMC web. ->
...
//<Yaosr-20210312 Improve redfish: diplay bios options according to the BIOS setup hierarchy on the BMC web. +>
EFI_STATUS WriteMenusToFile (ATTRIBUTES *AttributeList) {
  EFI_STATUS    Status = EFI_NOT_FOUND;
  ...
  if (NULL == gMenusArray)
    return Status;
  if (AttributeList->_AttributeName && (!StrCmp (AttributeList->_AttributeName, SYSTEM_ACCESS_MAPPINGID))) {
    return EFI_UNSUPPORTED;
  }

  MenusObject = cJSON_CreateObject ();
  String8 = StrDup16to8 (AttributeList->_DisplayName->String);
  cJSON_AddStringToObject (MenusObject, "DisplayName", String8);

  Value = atoi_base(AttributeList->_DisplayOrder, 10);
  cJSON_AddNumberIntToObject (MenusObject, "DisplayOrder", Value);

  cJSON_AddBoolToObject (MenusObject, "GrayOut", (int)0);

  cJSON_AddBoolToObject (MenusObject, "Hidden", AttributeList->_Hidden);

  String8 = StrDup16to8 (AttributeList->_MenuName);
  cJSON_AddStringToObject (MenusObject, "MenuName", String8);

  String8 = StrDup16to8 (AttributeList->_MenuPath);
  cJSON_AddStringToObject (MenusObject, "MenuPath", String8);
  MemFreePointer (&String8);

  cJSON_AddBoolToObject (MenusObject, "ReadOnly", AttributeList->_ReadOnly);
