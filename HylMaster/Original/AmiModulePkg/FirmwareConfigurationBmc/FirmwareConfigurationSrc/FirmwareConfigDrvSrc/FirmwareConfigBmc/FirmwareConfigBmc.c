...
EFI_STATUS GetBiosSdData (CHAR8 **JsonBuffer, UINTN *BufferSize)
{
	...
	Sprintf (GetUrlPath, "GET /redfish/v1/Systems/1/Bios/Settings HTTP/1.1\r\n");
	DEBUG ((DEBUG_ERROR, "\nGetUrlPath :%s",GetUrlPath));
...
EFI_STATUS GetBiosIndexData (CHAR8 **JsonBuffer, UINTN *BufferSize)
{
    ...
    Sprintf(GetUrlPath, "GET /redfish/v1/Systems/1/Bios HTTP/1.1\r\n");
    DEBUG((DEBUG_ERROR, "\nGetUrlPath :%s",GetUrlPath));
    Status = SendBMCGetRequest(GetUrlPath, (UINT32)Strlen (GetUrlPath), (void **)JsonBuffer, BufferSize);//Systems/BIOS/SD/index.json
    DEBUG((DEBUG_ERROR, "\nSendBMCGetRequest Status:%x,:%x,Size:%d",Status,JsonBuffer,BufferSize));
...
EFI_STATUS PostBiosIndexData (CHAR8 *PostData, UINTN PostDataLength)
{
    ...
    Sprintf(RequestStr, 
            "POST /redfish/v1/Systems/1/Bios HTTP/1.1\r\n"
            "Content-length: %d\r\n"
    ...
    if (jsonEntrys != NULL)
    {
      ...
      if (AttrsArray != NULL )
      {
        ...
        for (i = 0; i < ArraySize; i++)
        {
          ...
          if (ArrayMember == NULL) return NULL;
...
UINTN SendPwdGetRequest(CHAR8 	**JsonBufferData) {
	...
	Sprintf(GetUrl,"GET /redfish/v1/Systems/1/Bios/Actions/Bios.ChangePassword/SD HTTP/1.1\r\n");
...
EFI_STATUS GetBiosConfigurationReset (CHAR8 **JsonBuffer, UINTN *BufferSize)
{
    ...
    Sprintf (URL, "GET /redfish/v1/Systems/1/Bios/Actions/Bios.ResetBios HTTP/1.1\r\n");
...
//<kuaibai-20181222-fix hang b2 when ipmi abnormal+>				
        if (EFI_DEVICE_ERROR == Status) { //Eg., if BMC stopped then don't try next communication
          FreePool (Buffer);
          return Status;
        }
//<kuaibai-20181222-fix hang b2 when ipmi abnormal->
...
EFI_STATUS SendPatchRequest (CHAR8 *PostData)
{
    ...
    Sprintf(RequestStr, 
            "PATCH /redfish/v1/Systems/1/Bios HTTP/1.1\r\n"
            "Content-length: %d\r\n"
            "\r\n%s", PostDataLength, Buffer);
