/** @file
  ...
  This is the SEC instance implementation of ReportStatusLib, it is to
  replace the null instance in MdePkg/Library/BaseReportStatusCodeLibNull.
  This library instance is only for SEC boot phase.
  ...
  )
{
  ...
  if (Value == (EFI_SOFTWARE_SEC | EFI_SW_SEC_PC_HANDOFF_TO_NEXT)) {
    SetDebugFlag(PEI_PHASE_FLAG,DEBUG_MODE_PHASE_MASK);
  }
  return EFI_SUCCESS;
}
