...
/* Type checking... */
//# define PREDECLARE_LHASH_OF(type) LHASH_OF(type);	//AptioV Porting

# define LHASH_OF_TEST(type) struct lhash_st_##type

# define DEFINE_LHASH_OF_TEST(type) \
    LHASH_OF_TEST(type) { union lh_##type##_dummy { void* d1; unsigned long d2; int d3; } dummy; }; \
    static ossl_inline LHASH_OF_TEST(type) * \
        lh_##type##_new(unsigned long (*hfn)(const type *), \
                        int (*cfn)(const type *, const type *)) \
    { \
        return (LHASH_OF_TEST(type) *) \
...
//DEFINE_LHASH_OF_TEST(OPENSSL_STRING_TEST);
