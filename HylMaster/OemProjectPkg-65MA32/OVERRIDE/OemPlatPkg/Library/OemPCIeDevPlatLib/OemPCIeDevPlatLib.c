...
EFI_STATUS DynamicGenerationRbtable(VOID *Dev)
{
  ...
  while(Prt->ParentBrg != NULL && Prt->Type!=tPciRootBrg) {
      if(Prt->Type==tPciDevice) {
         ...
         if(Status == EFI_SUCCESS) {
             DEBUG((EFI_D_ERROR,"AddDevInfoInRbtable\n"));
  ...
  DEBUG((EFI_D_ERROR,"DynamicGenerationRbtablecount= %d\n", count));
  return Status;
}
...
EFI_STATUS GetDeviceRootBrg(VOID *Dev,VOID **rBrg)
{
  ...
  *rBrg = Prt;
  ...
  DEBUG((EFI_D_ERROR,"RootPort is DieNum:%02x, DevNum:%02x, FuncNum:%02x\n", DieNum, Prt->Address.Addr.Device, Prt->Address.Addr.Function));
...
EFI_STATUS GetDeviceRootPortData(VOID *Dev,ROOTB_DATA *Rb,UINTN *DevIndex)
{
  ...
  pRb->Socket = pRb->DieNum/4; 
  pRb->Data3  = 0xFF;
  DEBUG((EFI_D_ERROR,"GetDeviceRootPortData"));
  DEBUG((EFI_D_ERROR,"RootPort is DevNum:%02x, FuncNum:%02x\n",Prt->Address.Addr.Device, Prt->Address.Addr.Function));
...
)
{
    ...
    for(Index=0;Index<PciDevListData->PciDevCount;Index++){
     ...
     DEBUG((EFI_D_ERROR, " ***Pcie GetDeviceRootBrgEX***\n"));
      //Prt = (PCI_DEV_INFO *)dev;
      //PreDev = Prt->ParentBrg;
      //Prt_tem = PreDev;
...
//Get PCIE device infomation
   ...
   DEBUG((EFI_D_ERROR, " *** Num %2d PcieDevInfo->PcieLocation is  %s                ***\n", DevIndex, PcieDevInfo->PcieLocation));
...
VOID PrintRbTable(){
    ...
    DEBUG((EFI_D_ERROR,"RbSeg RbBus RbDev RbFunc Sockt RbNum Data3 Index Deven\n"));
    do{
        Prb=&PHead->PciPortInfo;
        DEBUG((EFI_D_ERROR,"%02x    %02x    %02x    %02x    %02x    %02x    %02x    %02x    %02x\n",Prb->RbType,
