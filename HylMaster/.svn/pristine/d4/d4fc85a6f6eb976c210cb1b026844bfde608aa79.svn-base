...
OemIpmiSetDimmInfo(VOID)
{
  ...
  DEBUG((EFI_D_ERROR, "GetMemoryInfo gEfiSmbiosProtocolGuid Status: %r\n",Status));
  ...
  for (i = 0; ; ++i) {
    ...
    DEBUG((EFI_D_ERROR, "SmbiosRecord Status: %r\n",Status));
    //Package buffer init.
    ...
    DEBUG((EFI_D_ERROR, "SmbiosType17Record DeviceLocator Status: %r\n",Status));
    ...
    DEBUG((EFI_D_ERROR, "SmbiosType17Record Manufacturer Status: %r\n",Status));
    ASSERT_EFI_ERROR(Status);
    DEBUG((EFI_D_ERROR, "TempManuf: %s\n",TempManuf));
    UnicodeStrToAsciiStr (TempManuf, OemDimmInfo.DimmManufacture);
    DEBUG((EFI_D_ERROR, "OemDbgDimmInfo.DimmManufacture: %a\n",OemDimmInfo.DimmManufacture));
    ...
    DEBUG((EFI_D_ERROR, "SmbiosType17Record PartNumber Status: %r\n",Status));
    ASSERT_EFI_ERROR(Status); 
    DEBUG((EFI_D_ERROR, "TempPartNum: %s\n",TempPartNum));
    UnicodeStrToAsciiStr (TempPartNum, OemDimmInfo.DimmPartNum);
    DEBUG((EFI_D_ERROR, "OemDbgDimmInfo.DimmPartNum: %a\n",OemDimmInfo.DimmPartNum));
    ...
    DEBUG((EFI_D_ERROR, "SmbiosType17Record SerialNumber Status: %r\n",Status));
    ASSERT_EFI_ERROR(Status); 
    DEBUG((EFI_D_ERROR, "TempSN: %s\n",TempSN));
    UnicodeStrToAsciiStr (TempSN, OemDimmInfo.DimmSN);
    DEBUG((EFI_D_ERROR, "OemDbgDimmInfo.DimmSN: %a\n",OemDimmInfo.DimmSN));
    ...
    DEBUG((EFI_D_ERROR, " DdrVoltage:%d ",OemDimmInfo.DdrVoltage ));
    DEBUG((EFI_D_ERROR, " EccCapbility:%d ", OemDimmInfo.EccCapbility));
    DEBUG((EFI_D_ERROR, " ModuleType:%d ",OemDimmInfo.ModuleType ));
    DEBUG((EFI_D_ERROR, " RankNum:%d ", OemDimmInfo.RankNum));
    DEBUG((EFI_D_ERROR, " DramWidth:%d ", OemDimmInfo.DramWidth));
    DEBUG((EFI_D_ERROR, "\n"));
    //<Malj-20181105 Add Mem Info Send to BMC ->
  ...
  DEBUG((EFI_D_ERROR, " *** Set DIMM Info Over! ***\n"));
  return Status;
}
  ...
  )
{
  ...
  DEBUG((EFI_D_ERROR, "Send Memory info status: %r\n",Status)); 
  return  Status;
}                                                        
