...
// AptioV Porting - Possible NULL pointer dereference >>>
	wsize    = OPENSSL_malloc(totalnum * sizeof (*wsize));
	wNAF_len = OPENSSL_malloc(totalnum * sizeof (*wNAF_len));
	wNAF     = OPENSSL_malloc((totalnum + 1) * sizeof (*wNAF)); /* includes space for pivot */
	val_sub  = OPENSSL_malloc(totalnum * sizeof (*val_sub));
	for (i = 0; i < totalnum; i++)
	{
	  val_sub[i] = OPENSSL_malloc(totalnum * sizeof (*val_sub[i]));
	} 
// AptioV Porting - Possible NULL pointer dereference <<<
     ...
     */
	val = OPENSSL_malloc((num_val + 1) * sizeof (*val));  // AptioV Porting - Possible NULL pointer dereference***
    if (val == NULL) {
        ECerr(EC_F_EC_WNAF_MUL, ERR_R_MALLOC_FAILURE);
        goto err;
    }
