		...
		for(i=0; i < pPageInfo->PageControls.ControlCount; i++)
		{
			...
			//check for warning condition
				if((CheckControlCondition(pControlInfo) == COND_WARN_IF) && (pControlInfo->ControlFlags.ControlEvaluateDefault == 0))
				{
					...
					Status = mPostMgr->DisplayInfoBox(		HiiGetString( 	gHiiHandle, STRING_TOKEN(STR_WARNING)),
																									HiiGetString( pControlInfo->ControlHandle, ((TSE_EFI_IFR_WARNING_IF*)pControlInfo->ControlConditionalPtr)->Warning) ,
				...
				{
					mPostMgr->DisplayMsgBox( HiiGetString( gHiiHandle, STRING_TOKEN(STR_INCONSISTENT_MSG_TITLE)), HiiGetString( pControlInfo->ControlHandle, ((EFI_IFR_INCONSISTENT_IF*)pControlInfo->ControlConditionalPtr)->Error) , MSGBOX_TYPE_OK,NULL);
					return TRUE;
				}
			}
		}
		...
		for(control = 0; control < pageInfo->PageControls.ControlCount; control++)
		{
			...
			if(pControlInfo->ControlType == NO_SUBMIT_IF)			
			{
                if(CheckControlCondition(pControlInfo) == COND_NOSUBMIT){
                    ...
                    Status = gBS->LocateProtocol(&gAmiPostManagerProtocolGuid, NULL,(void**) &mPostMgr);
                    if((Status == EFI_SUCCESS) && (mPostMgr != NULL)){
                        mPostMgr->DisplayMsgBox( HiiGetString( gHiiHandle, STRING_TOKEN(STR_NOSUBMITIF_MSG_TITLE)), HiiGetString( pControlInfo->ControlHandle, ((EFI_IFR_NO_SUBMIT_IF*)pControlInfo->ControlConditionalPtr)->Error) , MSGBOX_TYPE_OK,NULL);
                        return TRUE;
                    }
                }
			}
