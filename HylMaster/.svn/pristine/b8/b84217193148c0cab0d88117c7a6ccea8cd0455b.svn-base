...
SSIF_ALERT_PIN_CHECK* gSsifAlertPinCheckHookList[] = {SSIF_ALERT_PIN_CHECK_HOOK_LIST NULL};
...
)
{
    ...
    for (i = 0; gSsifAlertPinCheckHookList[i];i++) {
        CheckAlertSignal = gSsifAlertPinCheckHookList[i]();
    }
  ...
  IN UINT8                  Command )
{
    ...
    for (Index = 0; Index < TableSize-1; Index++) {
      if (NetFunction == CommandSpecificRetryCountTable[Index].NetFunction &&\
          ...
          SERVER_IPMI_DEBUG((EFI_D_INFO,"Retry Counter value changed\n"));
    ...
    SERVER_IPMI_DEBUG((EFI_D_INFO,"Retry counter value not changed as there is no match\n"));
#if(IPMI_SYSTEM_INTERFACE == KCS_INTERFACE)
  ...
  IN VOID                             *Context )
{
    ...
    //<LiuYB014-20170420 Show ipmi request/response data for debug +>
    {
      UINT8 *DataPoint = CommandData;
      UINT8 *DataEnd   = CommandData + CommandDataSize;

      SERVER_IPMI_DEBUG((EFI_D_INFO, "=====================================================\n"));
      SERVER_IPMI_DEBUG((EFI_D_INFO, "| Req(%02Xh bytes): %02X %02X                    #Net %02Xh, Lun %02Xh\n", CommandDataSize, (NetFunction << 2 | Lun), Command, NetFunction, Lun));
      while (DataPoint < DataEnd) {
        SERVER_IPMI_DEBUG((EFI_D_INFO, "|                 %02X %02X %02X %02X %02X %02X %02X %02X\n", DataPoint[0], DataPoint[1], DataPoint[2], DataPoint[3], DataPoint[4], DataPoint[5], DataPoint[6], DataPoint[7]));
        DataPoint += 8;
      }
    }
    //<LiuYB014-20170420 Show ipmi request/response data for debug ->
    ...
    if ((Status = SendDataToBmcPort (IpmiInstance, Context, IpmiInstance->TempData,
        ...
        IpmiInstance->BmcStatus = EFI_BMC_SOFTFAIL;
        IpmiInstance->SoftErrorCount++;
        SERVER_IPMI_DEBUG((EFI_D_INFO, "| SendDataToBmcPort, Status = %xh, CommandDataSize = %d\n", Status, CommandDataSize));  //<LiuYB014-20170420 Show ipmi request/response data for debug +>
        return Status;
    }
    DataSize = MAX_TEMP_DATA;
    if ((Status = ReceiveBmcDataFromPort (IpmiInstance, Context,
        ...
        IpmiInstance->BmcStatus = EFI_BMC_SOFTFAIL;
        IpmiInstance->SoftErrorCount++;
        SERVER_IPMI_DEBUG((EFI_D_INFO, "| ReceiveBmcDataFromPort, Status = %xh, DataSize = %d\n", Status, DataSize));  //<LiuYB014-20170420 Show ipmi request/response data for debug +>
        return Status;
    }
    ...
    if (ERROR_COMPLETION_CODE(This->CommandCompletionCode)) {
        ...
        SERVER_IPMI_DEBUG((EFI_D_INFO, "| CompletionCode = %xh\n", IpmiInstance->TempData[2]));  //<LiuYB014-20170420 Show ipmi request/response data for debug +>
        return EFI_DEVICE_ERROR;
    }

    if (DataSize < 3) {
        SERVER_IPMI_DEBUG((EFI_D_INFO, "| DataSize %d < 3\n", DataSize));  //<LiuYB014-20170420 Show ipmi request/response data for debug +>
        return EFI_DEVICE_ERROR;
    }

    if ((DataSize - 3) > *ResponseDataSize) {
        *ResponseDataSize = (UINT8) (DataSize - 3);
        SERVER_IPMI_DEBUG((EFI_D_INFO, "| Buffer Small, ResDataSize %d < DataSize %d - 3\n", *ResponseDataSize, DataSize));  //<LiuYB014-20170420 Show ipmi request/response data for debug +>
        return EFI_BUFFER_TOO_SMALL;
    }
    ...
    *ResponseDataSize = (UINT8) (DataSize - 3);
    ...
    //<LiuYB014-20170420 Show ipmi request/response data for debug +>
    {
      UINT8 *DataPoint = ResponseData;
      UINT8 *DataEnd   = ResponseData + *ResponseDataSize;

      SERVER_IPMI_DEBUG((EFI_D_INFO, "| Res(%02Xh bytes): %02X %02X\n", *ResponseDataSize + 1, IpmiInstance->TempData[0], IpmiInstance->TempData[1]));
      while (DataPoint < DataEnd) {
        if (DataPoint == ResponseData) {
          SERVER_IPMI_DEBUG((EFI_D_INFO, "|                 %02X %02X %02X %02X %02X %02X %02X %02X\n", IpmiInstance->TempData[2], DataPoint[0], DataPoint[1], DataPoint[2], DataPoint[3], DataPoint[4], DataPoint[5], DataPoint[6]));
          DataPoint += 7;
        } else {
          SERVER_IPMI_DEBUG((EFI_D_INFO, "|                 %02X %02X %02X %02X %02X %02X %02X %02X\n", DataPoint[0], DataPoint[1], DataPoint[2], DataPoint[3], DataPoint[4], DataPoint[5], DataPoint[6], DataPoint[7]));
          DataPoint += 8;
      ...
      if (*ResponseDataSize == 0) {
        SERVER_IPMI_DEBUG((EFI_D_INFO, "|                 %02X\n", IpmiInstance->TempData[2]));
      }
      SERVER_IPMI_DEBUG((EFI_D_INFO, "=====================================================\n"));
    }
    //<LiuYB014-20170420 Show ipmi request/response data for debug ->
