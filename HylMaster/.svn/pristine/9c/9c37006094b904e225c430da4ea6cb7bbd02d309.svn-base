...
# Version 2.4.0
# Date: 06/23/12
# Add Option "$RunInLine"to allow Running HDTOUT inline of HDT application

# Version 2.5.0
# Date: 10/24/14
# Add secure part support for CZ

# Version 2.5.1
# Date: 12/05/14
# Remove Kysy dependency for unlock the secure part

# Version 2.5.2
# Date: 12/19/14
# Add MEM_PMU filter flag

# Version 2.5.3
# Date: 03/04/16
# Correct the Purple possume connection for secure part
# Workaround read_security_status can't works on HDT NDA version

my $MajorVersion=0x2;
my $MinorVersion=0x53;
...
my $RunInLine = 0;             # 0:Standalone Perl script; 1 Enable HDTOUT Run inline
                               # Note: The command line parameters can't be used;
                               #       And force_reset_to_enable function may not works properly;
                               #       when you select inline mode
...
# Add HDT Perl module to the Perl @INC variable
...
# Try to load HDT module
eval {
  require hdtPerl;    # PLEASE comment out this line when working with SimNow
};
#Error happens during load the HDT module
if ($@) {
  print "Can't load hdtPerl Module, please make sure you have install the HDT with perl extension enabled\n";
        ...
        if ($dump_spd_data) {
            ...
            foreach my $Piece (@Fragments) {
                if ($Piece =~ /^ket\s\d\sChannel\s\d\sDimm\s\d\:\s([\w]+)/) {
                    ...
                    $MemBuffer = read_mem_init(hex($1), 512);
...
sub read_security_status
{
  my $status = 0;
  my $readValue = 0;
  
  ($status, $readValue) = hdtPerl::read_register("P.0/N.0/Die.0/CZ_STAC_Network.0/PSP_Tile.0/PSP_WRG.0/PSP_RRG.0/id:0x0000014D00000006","mJTAG_serial");
  printf "Status: $status, Value read from SS(0x6) : $readValue\n";

  #Check the SS[Secure_State] bit...
  my $sec = ((eval($readValue) & 0x1000) >> 12);
    
  ($status, $readValue) = hdtPerl::read_register("P.0/N.0/Die.0/CZ_STAC_Network.0/PSP_Tile.0/PSP_WRG.0/PSP_RRG.0/id:0x0000014D0000000C","mJTAG_serial");
  printf "Status: $status, Value read from J2PMBX0(0xC) : $readValue\n";

  #Check the Unlock bit#28...
  my $unloc = ((eval($readValue) & 0x10000000) >> 28);
  if ($status == 0) {
    print ("[Warning] Can't access SS register, Force return Secured & Unlocked\n");
    $sec = 1;
    $unloc = 1;
  }
  return ($sec, $unloc);
...
sub connect_to_target {
    if($IsSimNow){
        ...
        if ($RunInLine) {
            hdtPerl::select_emulator($ARGV[0]) or die "\nFAIL TO CONNECT!!!\n";;
            printf ("Connected to HDT device %s\n", $ARGV[0]);
        } else {
        ...
        #Check Secure & Unlock status
        print ("Check Secure & Unlock status\n");
        ($secured, $unlocked) = &read_security_status;
        if ($secured == 0x0) {
            print "Part is Unsecured\n";
        } else {
            print "Part is Secured and Locked, Unlocking the target ...\n";
            hdtPerl::unlock_after_cold_reset("", "");
            #check if successfully unlocked
            ($secured, $unlocked) = &read_security_status;
            if ($unlocked) {
                print ("Target has been unlocked successfully\n");
                #Connect again, or else TAPSDK error will
                # hdtPerl::select_emulator ("protocol=yaap, ip=$ip_addr, username=$default_username, password=$default_password") or die "\nFAIL TO CONNECT!!!\n";
            } else {
                print ("Unlocked failed, please check if your BIOS PSP BL has support the unlock function,
                and also please make sure BIT2 of PSP_ENTRY TYPE=0xb has been set in PspDirectory, Contact BIOS owner for more information\n");
                exit 1;
            }
        }
        ...
        if($val_dr2 != 0x99CC){
            if ($secured == 0) {
                print ("\nHDTOUT is not enabled.\nUnsecured part doesn't support force_reset_to_enable function\n\n");
                print ("Please enable the HDTOUT in BIOS setup->AMD CBS Or\nRebuild a test BIOS with AmdIdsHdtOutSupport always return TRUE\n");
                print ("Contact your BIOS provider for further support.\n");
                exit 1;
            } else {
                my $Answer = "N";
                if(!$force_reset_to_enable){
                    print ("\nHDTOUT is not enabled. Do you want to auto enable it? (Y/N)");
                    $Answer = <STDIN>;
                } else {
                    $Answer = "Y";
                }
                if($Answer =~ /^Y/i){
                    ...
                    print "no need to do the reset on secure part\n";
                    #On secure part, if BIT2 of PSP_ENTRY TYPE=0xb has been set, PSP BL will stop the system before release BIOS 
                    #to waiting infinitely for unlock, so no need to do the reset on secure part
                    ...
                    die if ($val_dr2 != 0x99cc);
                    print ("HDTOUT Enabled\n");
                    hdtPerl::exit_debug(0);
                }
            }
        }
...
sub poll_until_break {
    if($IsSimNow){
        ...
        do {
            ...
            #Checking the unlock status every 1 second
            if ($secured && ($dbrdy_value == 0) && (($count %= 1000) == 0)) {
                #check unlock state on secure part, before running any PDM command
                hdtPerl::enter_debug();
                hdtPerl::dbrdy_status(0, \$dbrdy_value);
                if ($dbrdy_value == 0) {
                    print ("\nSystem has been locked again, maybe due a new power cycle\n");
                    print ("Unlocking the target ...\n");
                    #Connect again, or else TAPSDK error will asserted
                    hdtPerl::unlock_after_cold_reset("", "");
                    ($secured, $unlocked) = &read_security_status;
                    #check if successfully unlocked
                    if ($unlocked) {
...
sub ShowHelp {
    ...
    print   "            MEM_PMU      CPU_TRACE     TOPO_TRACE   FCH_TRACE   IDS_TRACE\n";
    print   "            BVM_TRACE    LIB_PCI_RD    LIB_PCI_WR    TEST_POINT\n";
...
sub TimeAnalyse {
    ...
    my $DescriptionMapping = {
      ...
      0x00010000 =>
      {
        ...
        0x154 => "BeginGnbInitAtRtb",
        0x155 => "EndGnbInitAtRtb",
        0x156 => "BeginGnbLoadScsData",
        0x157 => "EndGnbLoadScsData",
        0x158 => "BeginGnbPcieTraining",
        0x159 => "EndGnbPcieTraining",
        0x15A => "BeginDispatchCpuFeaturesInitRtb",
        0x15B => "EndDispatchCpuFeaturesInitRtb",
        0x15C => "BeginAmdCpuEarly",
        0x15D => "EndAmdCpuEarly",
        0x15E => "BeginAmdGnbMidLate",
        0x15F => "EndAmdGnbMidLate",
      },
    };
