            ...
            for (IndexPciIo = 0; IndexPciIo < HandleCountPciIo; IndexPciIo++) {
                ...
                for (IndexOpenInfo = 0; (IndexOpenInfo < OpenInfoCount)&& (DriveIndex < DRIVEMAXCOUNT); IndexOpenInfo++) {// for 3
                    ...
                    if ((OpenInfo[IndexOpenInfo].Attributes & EFI_OPEN_PROTOCOL_BY_CHILD_CONTROLLER) &&
                        ...
                        // If NameSpace Size is zero, skipping the NameSpace entry 
                        Status = gBS->HandleProtocol(
                                            DeviceHandle,
                        ...
                        InitString(
                         ...
                         for (LinkData = NvmeController->ActiveNameSpaceList.ForwardLink; 
						  ...
						  //InitString(HiiHandle, NVMeSetupStruc[DriveIndex].Size, L"%d.%dGB", NameSpaceSizeInGB, NumTenthsOfGB);
						  InitString(HiiHandle, NVMeSetupStruc[DriveIndex].Size, L"%dGB", NameSpaceSizeInGB);						  
                          //<LiuYB036-20180118 Remove NVMe Tenth of GB info. to sync with BMC web asset info.->
