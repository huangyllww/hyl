...
extern wchar_t *HiiLib_GetHiiString( UINT8 *DataTable, UINT8 *HiiData, UINT16 Token, BOOLEAN MapLang );
...
//  UINT16 Key = 0;
    ...
    PromptStr = HiiLib_GetHiiString( DataTable, HIIDBData, Control->ControlPrompt , FALSE);
...
EFI_STATUS _PopulateGlobalBootOptions()
{
    ...
    //<zanggy-20210924 fix setup006 is null+>
    //gBootOrder = GetVariableByName( L"BootOrder", &Size );
    Status = pRS->GetVariable (L"BootOrder", &gEfiGlobalVariableGuid, &Attributes, (UINTN*)&Size, gBootOrder);
    if( EFI_BUFFER_TOO_SMALL == Status)
    {
        gBootOrder = calloc(sizeof(UINT8), (size_t)Size+1);
        Status = pRS->GetVariable (L"BootOrder", &gEfiGlobalVariableGuid, &Attributes, (UINTN*)&Size, gBootOrder);
    }
    
    if( EFI_ERROR(Status))
        return Status; //<OEMDBG-hujf1001-20201105 Fix system hang when no boot device insert +>
    //<zanggy-20210924 fix setup006 is null->
    ...
    if ( BootOrder != NULL )
    {
        ...
        if ( gBootOptions != NULL )
        {
            ...
            for ( Index = 0; Index < Size; Index += 2 )
            {
                ...
                DEBUG((DEBUG_ERROR, "\n[SK]BootOrder[Index+1]=%x, BootOrder[Index]=%x", BootOrder[Index+1],BootOrder[Index]));
                ...
                else
                {
                    DEBUG((DEBUG_ERROR, "\n[SK]6.6"));
                    swprintf( BootStr, MAX_LENGTH, L"Boot%02x%02x", BootOrder[Index + 1], BootOrder[Index]);
                    BootXXXX = ( UINT8 * )HiiLib_VarGetNvramName( BootStr, &gEfiGlobalVariableGuid, &Attributes, (UINTN*)&BootSize );
                    DEBUG((DEBUG_ERROR, "\n[SK]7.7"));
    ...
    DEBUG((DEBUG_ERROR, "\n[SK]End of _PopulateGlobalBootOptions"));
    return EFI_SUCCESS; //<OEMDBG-hujf1001-20201105 Fix system hang when no boot device insert +>
}
