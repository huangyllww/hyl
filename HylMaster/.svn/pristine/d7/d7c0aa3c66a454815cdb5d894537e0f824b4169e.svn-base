...
// csm_misc_info bits
#define CSM_HDD_ORIGINAL_ORDER_BIT		0x00000001

typedef enum {
	Csm16HddKeepOriginalOrder = 0x80
} CSM16_FEATURE_PRIVATE;
...
EFI_STATUS  GetHddOrderAdjustmentInformation(UINTN*);
EFI_STATUS  Csm16ConfigurationEx(CSM16_CONFIGURATION_ACTION, UINT32, UINT32*);

//---------- SIO related definitions -------------------------
typedef EFI_STATUS (*CSM_DEVICE_IS_CONNECTED_FUNC)(EFI_HANDLE);

EFI_STATUS IsConnectedDummy(EFI_HANDLE);
EFI_STATUS IsConnectedPS2MS(EFI_HANDLE);
EFI_STATUS IsConnectedFloppy(EFI_HANDLE);
EFI_STATUS GetResource(UINT16, UINT8, UINT16*, UINT8*);

typedef enum {
    CsmPS2KeyboardDevice = 0,
    CsmPS2MouseDevice = 1,
    CsmComPortDevice = 2,
    CsmLptPortDevice = 3,
    CsmFddDevice = 4
...
typedef struct _CSM_SIO_DEVICE {
    CSM_SIO_DEVICE_TYPE             Type;
    UINT16                          *PnpIDs;
    UINT8                           SioDevStatusBit;
    CSM_DEVICE_IS_CONNECTED_FUNC    IsConnectedFunction;
} CSM_SIO_DEVICE;
