...
//--------------------------------
    ...
    PCI_TRACE((TRACE_PCI,"ClaimMMIOResources\n"));
    ...
    if(NextRootBrg!=NULL) {
	    ...
	    PCI_TRACE((TRACE_PCI,"setting nar=NULL\n"));
	    nar=NULL;
    }
	...
	if( TmpTop[ResType] != 0 )
	{
		PCI_TRACE((TRACE_PCI,"check TmpTop[%x]=%x\n",ResType,TmpTop[ResType]));
		bot=TmpTop[ResType]+1;
		PCI_TRACE((TRACE_PCI,"bot=%x\n",bot));
	}
//	if( TmpBot[ResType] != 0 ) bot=TmpBot[ResType];
	...
	TmpTop[ResType]=ct;
	TmpBot[ResType]=cb;
	PCI_TRACE((TRACE_PCI,"setting TmpTop[%x]=%x\n",ResType,ct));
	PCI_TRACE((TRACE_PCI,"setting TmpBot[%x]=%x\n",ResType,cb));
...
//--------------------------------
    ...
    // Use IO resources as-is if AllocType is set to EfiGcdAllocateAddress.
   	if(car->AllocType == EfiGcdAllocateAddress) return EFI_SUCCESS;
...
//--------------------
    ...
    if(hb->RootBridgeCount>RootIndex+1)
    {
	    nextrb=hb->RootBridges[RootIndex+1];
	    PCI_TRACE((TRACE_PCI,"nextrb Bus=%x NotPresent=%x\n",nextrb->RbSdlData->Bus,nextrb->NotPresent));
...
VOID EFIAPI RbReadyToBoot(IN EFI_EVENT Event,	IN VOID	*Context){
  ...
  //This may not be necessary- skip
  ...
  for(i=0; i<gHostBrgCount; i++){
    ...
    for(j=0; j<hb->RootBridgeCount; j++){
      ...
      if( rb->NotPresent == TRUE )
      {
        PCI_TRACE((TRACE_PCI,"PciRB: Host[ %d ].Root[ %d ] *NOT PRESENT* \n", i, j));
      } else {
  ...
  //Need to update this code for the limited RB data we want to provide OS. In this case we
  //want to define the data windows available not what is currently allocated.
  ...
  for(i=0; i<gHostBrgCount; i++){
    ...
    for(j=0; j<hb->RootBridgeCount; j++){
      ...
      //4.Update MMIO64 Object			
      Status=UpdateAslNameOfDevice(dsdt, rb->RbAslName, "MABL", rb->AcpiRbRes[raMmio64].Min);
      PCI_TRACE((TRACE_PCI,"        =>'MABL'=0x%lX -> %r; ", rb->AcpiRbRes[raMmio64].Min, Status));
      Status=UpdateAslNameOfDevice(dsdt, rb->RbAslName, "MALL", rb->AcpiRbRes[raMmio64].Len);
      PCI_TRACE((TRACE_PCI,"        =>'MALL'=0x%lX -> %r;\n", rb->AcpiRbRes[raMmio64].Len, Status));

      //4.1 Update PRXM
      if(!rb->NotPresent){
        Status = PciNumaProtocol->GetPciRbNumaNodeDomain(PciNumaProtocol,
            rb->AcpiRbRes[raBus].Min,
            &Socket,
            &Die,
            &Domain);
        ASSERT(!EFI_ERROR(Status));
        Status=UpdateAslNameOfDevice(dsdt, rb->RbAslName, "PRXM", Domain);
        PCI_TRACE((TRACE_PCI,"        =>'PRXM'=0x%lX -> %r;\n", Domain, Status));
      }
  ...
  for(i=0; i<gHostBrgCount; i++){
    ...
    for(j=0; j<hb->RootBridgeCount; j++){
      ...
      //Take code from newer version of file
      //	if(rb->NotPresent) {
        ...
        if(Status==EFI_UNSUPPORTED){
            ...
            if (pMap[i].GcdMemoryType == EfiGcdMemoryTypeNonExistent ){
                Status=dxe->AddMemorySpace(EfiGcdMemoryTypeMemoryMappedIo,
                    pMap[i].BaseAddress,pMap[i].Length, GCD_COMMON_MMIO_CAPS | EFI_MEMORY_WC);
                ASSERT_EFI_ERROR(Status);
                if(EFI_ERROR(Status)) return Status;
            }
            pBS->FreePool(pMap);
