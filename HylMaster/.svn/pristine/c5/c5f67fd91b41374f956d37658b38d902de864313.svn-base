...
#define WAIT_FOR_CPU_COUNT 400
    ...
    #define MP_TRACE PEI_TRACE
    #define MP_TYPE TRACE_ALWAYS,PeiServices
    #define ASSERT_ERROR_CPU(Status) ASSERT_PEI_ERROR(PeiServices, Status)
    #define ASSERT_CPU(Condition) if(!(Condition)) { \
        PEI_TRACE((-1,PeiServices,"ASSERT in %s on %i: %s\n",__FILE__, __LINE__, #Condition));\
        _asm jmp $ \
        }
...
**/
VOID WrapCopyBspMTRRStoAllRunningApMTRRS(UINT8 Cpu, VOID *Context)
{
...
)
{
    ...
    EFI_STATUS              Status;
    UINT8                   ExpectedAPCount = HygonCPUCount() - 1;
...
//    MP_TRACE((MP_TYPE,"StartAllAps> Before StartCpus()\n"));
    ...
    MP_TRACE((MP_TYPE,"StartAllAps: NumCpus = %i.\n", NumCpus));
    MP_TRACE((MP_TYPE,"StartAllAps: HygonCPUCount() = %i.\n", HygonCPUCount()));
...
)
{
    ...
    CPULib_LockByteInc(pRunningAps);        //Increase umber of CPUs running.
