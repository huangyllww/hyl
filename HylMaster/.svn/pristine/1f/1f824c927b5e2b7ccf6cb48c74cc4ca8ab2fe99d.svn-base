...
UINT16 MpsTableAddIoApicEntry(VOID)
{
    ...
    UINT8 i=0;
    AMI_SDL_IO_APIC_INFO   *BrdApicInfo;
    BOOLEAN         Present;  
    UINTN           j; 
...
{
    //-----------------        
    for(i=0; i< gAmiBoardInfo2Protocol->ApicBrdData->IoApicCount; i++){
        BrdApicInfo=&gAmiBoardInfo2Protocol->ApicBrdData->IoApicInfo[i];
        ...
        for(j=0,Present=FALSE; j < CurrentIoApicEntry; j++){
            if(BrdApicInfo->ApicAddress == IoApicEntry[j].IoApicAddress){
                Present = TRUE;
                break;
            } 
        }
        ...
        if(BrdApicInfo->ApicAddress != 0xDEADBEEF)
        	CollectIoApicInfo(NULL, (UINT32) BrdApicInfo->ApicAddress);
