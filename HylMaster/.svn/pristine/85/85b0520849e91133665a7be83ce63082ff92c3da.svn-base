...
**/
//<duanjun 20200701 mantis0039122 fix system hang 0x92 +>
IEVENT                  *PeriodicEvent[1000];
UINT32                  PeriodicEventCount = 0;
//<duanjun 20200701 mantis0039122 fix system hang 0x92 ->
VOID
EFIAPI
CoreCheckTimers (
...
//<duanjun 20200701 mantis0039122 fix system hang 0x92 +>	  
      ...
      PeriodicEvent[PeriodicEventCount] = Event;
      PeriodicEventCount++;
//<duanjun 20200701 mantis0039122 fix system hang 0x92 ->	  
    }
  }
//<duanjun 20200701 mantis0039122 fix system hang 0x92 +>  
  for (i=0;i<PeriodicEventCount;i++)
      CoreInsertEventTimer(PeriodicEvent[i]);
  
  SystemTime = CoreCurrentSystemTime ();
  
  for (Link = mEfiTimerList.ForwardLink; Link != &mEfiTimerList; Link = Link->ForwardLink) {
    Event = CR (Link, IEVENT, Timer.Link, EVENT_SIGNATURE);
    ...
    if (Event->Timer.Period != 0) {
      ...
      //
      if (Event->Timer.TriggerTime <= SystemTime) {
          Event->Timer.TriggerTime = SystemTime + Event->Timer.Period;
      }
    }
  }
//<duanjun 20200701 mantis0039122 fix system hang 0x92 ->  
