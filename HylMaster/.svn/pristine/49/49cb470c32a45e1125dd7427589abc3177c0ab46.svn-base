...
#include <Hob.h>
#include <Library/TimerLib.h>//<huangjin001-20190506 Fix TCM can't be detected on BIOS setup page > 
...
TCM_PC_REGISTERS_PTR      TcmReg = (TCM_PC_REGISTERS_PTR)(UINTN)TCM_MMIO_BASE;
    ...
    IN OUT UINTN        *MASize )
{
    ...
    while ( TRUE )
    {
        Status = (*ps)->FfsFindNextVolume((CONST EFI_PEI_SERVICES **)ps, Instance,  (void **)&pFV );
    ...
    IN EFI_GUID         *Driveguid )
{
    ...
    (*ps)->GetHobList( (CONST EFI_PEI_SERVICES **)ps, (void **)&DrvHob );
    while ( !EFI_ERROR( InternalFindNextHobByType( EFI_HOB_TYPE_GUID_EXTENSION,  (void **)&DrvHob )))
    {
    ...
    IN EFI_BOOT_MODE    BootMode )
{
    ...
    MicroSecondDelay(30);//<huangjin001-20190506 Fix TCM can't be detected on BIOS setup page >   
    ...
    IN EFI_PEI_SERVICES **ps )
{
    ...
    Status =  TcmPpi->TcmLibPassThrough(TcmReg,
                  (UINT8*)&cmd,
                  sizeof (cmd),
                  (UINT8*)&result,
                  sizeof (result));
...
)
{
    ...
    if(ConfigFlags.DisallowTpm == 1)
    {
        ...
        //Check if the TPM 1.2 is present or not
...
// Modified:
//
// Referrals:  TCG EFI Platform Specification Version 1.22 
//
// Notes:
//<AMI_PHDR_END>
//**********************************************************************
    ...
    IN PEI_TCG_PPI      *tcg )
{
    ...
    CHAR8                         Signature[] = "Spec ID Event02";
    UINT32                        n=0;
	...
	// For "TCG EFI Platform Specification Version 1.22", the SpecErrata is 2
    TcgInfo.SpecErrata        = 2;
	// Clearly for notation
