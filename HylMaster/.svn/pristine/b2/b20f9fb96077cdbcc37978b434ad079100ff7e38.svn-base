...
SyncCMOSSetting()
{
  ...
  if((pOemDebugLevelDataBase->DebugModeSupport == 1) && (pOemDebugLevelDataBase->PeiPrintSupport ==1))
    DebugModeMask |=PEI_DEBUG_ENABLED;
  else
    DebugModeMask &=~PEI_DEBUG_ENABLED;

  if((pOemDebugLevelDataBase->DebugModeSupport == 1) && (pOemDebugLevelDataBase->RTSPrintSupport ==1))
    ...
    IN VOID *InvokePpi)
{
  ...
  //Check Debug Support
  if (!pBMCDebugOption->DebugMode_Byte3.Bits.Byte3ValidFlag)
