...
UINT32 Token_SlotBusWidth [NUMBER_OF_SYSTEM_SLOTS] = {
        SlotBusWidth
};
#endif
...
{
    UINT8 CapPtr;
    UINT8 NextCapPtr = 0x34;
    UINT8 CapID;
    UINT8 Count = 0;
    UINT16 SlotStatus;
    BOOLEAN PresenceDetectState = FALSE;
    UINTN PcieBaseAddr;

    PcieBaseAddr = (UINTN) PcdGet64(PcdPciExpressBaseAddress);

    do {
        CapPtr = *(volatile UINT8*) (PcieBaseAddr + (UINTN)(Bus << 20)
                + (UINTN)(Dev << 15) + (UINTN)(Func << 12) + NextCapPtr);
        
        CapID = *(volatile UINT8*) PCIE_CFG_ADDR(Bus, Dev, Func, CapPtr);
        if (CapID == 0x10) {
            SlotStatus = *(volatile UINT16*) (PcieBaseAddr + (UINTN)(Bus << 20)
                    + (UINTN)(Dev << 15) + (UINTN)(Func << 12) + (CapPtr + 0x1A));
            DEBUG((EFI_D_INFO, "Slot SlotStatus = %x\n", SlotStatus));
            PresenceDetectState = SlotStatus & 0x40;
            break;
        } else {
            NextCapPtr = CapPtr + 1;
}
    ...
    DEBUG((EFI_D_INFO, "Slot PresenceDetectState = %x\n", PresenceDetectState));

    return PresenceDetectState;
}
/**
                     ***** PORTING REQUIRED *****
               (if SMBIOS module is part of the project)
...
CreateSlotDataForSMBios (VOID)
{
    ...
    //    SmbiosSlotsVar = NULL;          // Remove this line porting this function
    /*
     ...
     // Return the slot structure inforin a variable named "SmbiosSlotsVar"
     //
     */
    ...
    UINTN PcieBaseAddr;
    BOOLEAN PresenceDetectState = FALSE;
    AMI_SDL_PCI_DEV_INFO    *sdldev, *parent, *parentroot;
    UINT8* point = (UINT8*)&SlotInfo;
    UINT8   ParentBus;
    UINT8   BusWidth;
    
    DEBUG((EFI_D_INFO, "Updating SMBIOS type 9 slot data \n"));
    PcieBaseAddr = (UINTN) PcdGet64(PcdPciExpressBaseAddress);
    Status=AmiSdlInitBoardInfo();
    if(EFI_ERROR(Status)) return Status;
    for(i=0; i<gSdlPciData->PciDevCount; i++){
          sdldev=&gSdlPciData->PciDevices[i];
          if((sdldev->PciDevFlags.Bits.OnBoard == 0) && (sdldev->Slot != 0))
          {                 
                parent=&gSdlPciData->PciDevices[sdldev->ParentIndex];
                parentroot=&gSdlPciData->PciDevices[parent->ParentIndex];
                ParentBus = parentroot->Bus;
                
                VendorIDDeviceID = *(volatile UINT32*) (PcieBaseAddr + \
                                   (UINTN) ((ParentBus)  << 20) + \
                                   (UINTN) ((parent->Device) << 15) + \
                                   (UINTN) ((parent->Function) << 12) + 0 );     
                if (VendorIDDeviceID == 0xFFFFFFFF) 
                {
                    SlotInfo.CurrentUsage[(sdldev->Slot)-1] = SlotUsageAvailable;
                } 
                else if (VendorIDDeviceID != 0x14531D94)
                {
                    SlotInfo.CurrentUsage[(sdldev->Slot)-1] = SlotUsageInUse;
                }
                else if (VendorIDDeviceID == 0x14531D94)
                {
                    PresenceDetectState = DetectHotPlugSlot(parent->Bus, parent->Device, parent->Function);
                    if (PresenceDetectState == FALSE) 
                    {
                        SlotInfo.CurrentUsage[(sdldev->Slot)-1] = SlotUsageAvailable;
                    } 
                    else 
                ...
                SlotInfo.BusNumber[(sdldev->Slot)-1] = ParentBus;
                
                switch (Token_SlotBusWidth[(sdldev->Slot)-1]) {
                case 1:
                    BusWidth = SlotDataBusWidth1X;
                    break;
                case 2:
                    BusWidth = SlotDataBusWidth2X;
                    break;
                case 4:
                    BusWidth = SlotDataBusWidth4X;
                    break;
                case 8:
                    BusWidth = SlotDataBusWidth8X;
                    break;
                case 16:
                    BusWidth = SlotDataBusWidth16X;
                    break;
                case 32:
                    BusWidth = SlotDataBusWidth32X;
                    break;
                default:
                    BusWidth = SlotDataBusWidth16X;
                    break;
                ...
                SlotInfo.SlotDataBusWidth[(sdldev->Slot)-1] = BusWidth;
          }
    } //for
  ...
  )
{
    ...
    UINTN                           Device;
    UINTN                           Function;
    EFI_PCI_IO_PROTOCOL             *RootBridgePciIo;
    EFI_HANDLE                      *HandleBuffer;
    UINTN                           HandleCount;
    UINTN                           HandleIndex;
    EFI_HANDLE                      ReturnHandle;
    PCI_TYPE01                      Type01;
    UINT16                          PcieCapbility;
    
    RootBridgePciIo = NULL;
    HandleBuffer = NULL;
    ReturnHandle = NULL;
    ...
    for (HandleIndex = 0; HandleIndex < HandleCount; HandleIndex++) {
        ...
        Status = RootBridgePciIo->Pci.Read (
                RootBridgePciIo,
                EfiPciIoWidthUint8,
        ...
        if (IS_CLASS1(&Type01, PCI_CLASS_BRIDGE)) {
            Status = RootBridgePciIo->Pci.Read (
                    RootBridgePciIo,
                    EfiPciIoWidthUint16,
                    0x5A,
                    ...
                    );
            {
                ...
                UINTN                           Device2;
                UINTN                           Function2;
                Status = RootBridgePciIo->GetLocation (
                        RootBridgePciIo,
                        &Segment2,
                        &Bus2,
                        &Device2,
            ...
            if (((PcieCapbility & 0xF0) >> 4) == 4
                    && Type01.Bridge.SecondaryBus <= Bus
                    && Type01.Bridge.SubordinateBus >= Bus) {
                ReturnHandle = HandleBuffer[HandleIndex];
                break;
            }
        }
    }
...
//----------------------------------------------------------------------------
    ...
    UINTN                           OnboardDeviceIndex;
    EFI_PCI_IO_PROTOCOL             *PciIo;
    EFI_HANDLE                      *HandleBuffer;
    UINTN                           HandleCount;
    UINTN                           HandleIndex;
    PCI_TYPE_GENERIC                PciGenericHeader;
    UINTN                           Segment;
    UINTN                           Bus;
    UINTN                           Device;
    UINTN                           Function;
    BOOLEAN                         SlotImplemented;
    ...
    for (OnboardDeviceIndex = 0; OnboardDeviceIndex < ONBOARD_DEVICE_EXT_COUNT; OnboardDeviceIndex++) {
        OnBoardDevExtInfo.OnBoardDev[OnboardDeviceIndex] = 0;           // Initialize with disabled
    }
    OnboardDeviceIndex = 0;
    // Onboard VGA
    for (HandleIndex = 0; HandleIndex < HandleCount; HandleIndex++) {
        Status = pBS->HandleProtocol(
                HandleBuffer[HandleIndex],
        ...
        if (!IS_PCI_VGA(&PciGenericHeader.Device)) {
            continue;
        }
        RootBridgeHandle = GetRootBridge (PciIo);
        if (RootBridgeHandle == NULL) {
            Status = EFI_NOT_FOUND;
            goto Error;
        }
        SlotImplemented = IsSlotImplemented (RootBridgeHandle);
        if (SlotImplemented) {
            continue;
        }
        // Onboard VGA found
        ...
        DEBUG((DEBUG_INFO, "Onboard VGA: B/D/F = %02X/%X/%d\n", Bus, Device, Function));
        OnBoardDevExtInfo.OnBoardDev[OnboardDeviceIndex] = 0x80;        // Enabled
        OnBoardDevExtInfo.BusNumber[OnboardDeviceIndex] = (UINT8)Bus;
        OnBoardDevExtInfo.DevFuncNumber[OnboardDeviceIndex] = (UINT8)((Device << 3) | Function);
        break;
    }
    
    // LOM
    OnboardDeviceIndex = 1;
    for (HandleIndex = 0; HandleIndex < HandleCount; HandleIndex++) {
        Status = pBS->HandleProtocol(
                HandleBuffer[HandleIndex],
                &gEfiPciIoProtocolGuid,
        ...
        if (!IS_CLASS2(&PciGenericHeader.Device, PCI_CLASS_NETWORK, PCI_CLASS_NETWORK_ETHERNET)) {
            continue;
        }
        RootBridgeHandle = GetRootBridge (PciIo);
        if (RootBridgeHandle == NULL) {
        ...
        DEBUG((DEBUG_INFO, "LOM: B/D/F = %02X/%X/%d\n", Bus, Device, Function));
        OnBoardDevExtInfo.OnBoardDev[OnboardDeviceIndex] = 0x80;        // Enabled
        OnBoardDevExtInfo.BusNumber[OnboardDeviceIndex] = (UINT8)Bus;
        OnBoardDevExtInfo.DevFuncNumber[OnboardDeviceIndex] = (UINT8)((Device << 3) | Function);
        OnboardDeviceIndex++;
        if (OnboardDeviceIndex == ONBOARD_DEVICE_EXT_COUNT) {
            break;
        }
    }
