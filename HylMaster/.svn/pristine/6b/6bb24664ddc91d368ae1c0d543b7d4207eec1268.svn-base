...
#define FILECODE LIBRARY_NBIOSMUV9LIB_NBIOSMUV9LIB_FILECODE
/*----------------------------------------------------------------------------------------
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "NbioSmuServiceRequestV9 Enter\n");

  DataMask = 0xFFFFFFFF;

  if (RequestId == INVALID_SMU_MSG) {
    IDS_HDT_CONSOLE (GNB_TRACE, "NbioSmuServiceRequestV9 INVALID!!\n");
...
///@todo - implement S3 save for resume
  if (AccessFlags) {
    ...
    DataMask = 0xFFFFFFFF;
    AmdS3SaveScriptPciPoll (AccessWidth32, NbioPciAddress.AddressValue | 0xBC, &RxSmuMessageResponseS3, &DataMask, 0xffffffff);
  }
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "NbioSmuServiceRequestV9 Exit RequestId = 0x%x, Response = 0x%x\n", RequestId, RxSmuMessageResponse);
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (MAIN_FLOW, "NbioSmuFirmwareTestV9 Enter\n");

  NbioSmuServiceCommonInitArgumentsV9 (SmuArg);
  SmuArg[0] = 0xAA55AA55;
  RequestId = SMC_MSG_TestMessage;

  NbioSmuServiceRequestV9 (NbioGetHostPciAddress (NbioHandle), RequestId, SmuArg, 0);

  IDS_HDT_CONSOLE (MAIN_FLOW, "  Service Test Return %x\n", SmuArg[0]);
  IDS_HDT_CONSOLE (MAIN_FLOW, "NbioSmuFirmwareTestV9 Exit\n");
  ...
  )
{
  ...
  *DxioMBoxArg = DxioMBoxRequest;
  RetVal = NbioSmuServiceRequestV9 (
