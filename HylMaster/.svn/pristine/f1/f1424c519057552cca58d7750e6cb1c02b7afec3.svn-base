...
#if (((!defined(AFRI_PASS_TO_AFRI_PROTOCOL) && SMM_SUPPORT) || (AFRI_PASS_TO_SMM_COMMUNICATION && SMM_SUPPORT)) && !defined(_DMIEDIT_PROTOCOL_))
{
    ...
    if (AmiValidateMemoryBuffer((VOID*)p, sizeof(SET_SMBIOS_STRUCTURE)) == EFI_SUCCESS) {
        ...
        BufferSize = 0x10;
        if (AmiValidateMemoryBuffer((VOID*)p->Buffer32BitAddr, BufferSize) == EFI_SUCCESS) {
            if (((SET_SMBIOS_STRUCTURE_DATA*)p->Buffer32BitAddr)->Command > 6) {
                return DMI_BAD_PARAMETER;
            }

            // Adjust buffersize based on "Command" type
            if (((SET_SMBIOS_STRUCTURE_DATA*)p->Buffer32BitAddr)->Command == 3 ||
                ((SET_SMBIOS_STRUCTURE_DATA*)p->Buffer32BitAddr)->Command > 4) {
                BufferSize += ((SET_SMBIOS_STRUCTURE_DATA*)p->Buffer32BitAddr)->DataLength;
            }
