  ...
  )
{
  ...
  // AMI PORTING START.
  //
  if (NetworkStackSetupData.Ipv4Http == 0) 
  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    ...
    //<Kangmm-20220403 Remove http boot configuration form +>
    //
  ...
  )
{
 ...
 // AMI PORTING START.
 //
  if (NetworkStackSetupData.Ipv6Http == 0) 
	  return EFI_UNSUPPORTED;
 //
 // AMI PORTING END.
 //
...
EFI_STATUS   Status;
  ...
  Status= SystemTable->RuntimeServices->GetVariable(L"NetworkStackVar", 
