...
//AptioV Porting - for build error 1.1.0 >>>
/*
 * Span the complement of string s2.
 */
size_t
int_strcspn(const char *s1, const char *s2)
{
	const char *p, *spanp;
	char c, sc;
...
int
int_strspn(const char *p, const char *s)
{
	int i, j;
...
int PEM_get_EVP_CIPHER_INFO(char *header, EVP_CIPHER_INFO *cipher)
{
    ...
    header += int_strspn(header, " \t");	//AptioV Porting - for build error 1.1.0
    ...
    if (strncmp(header, ENCRYPTED, sizeof(ENCRYPTED)-1) != 0 ||
    	int_strspn(header+sizeof(ENCRYPTED)-1, " \t\r\n") == 0) {	//AptioV Porting - for build error 1.1.0
        PEMerr(PEM_F_PEM_GET_EVP_CIPHER_INFO, PEM_R_NOT_ENCRYPTED);
        return 0;
    }
    header += sizeof(ENCRYPTED)-1;
    header += int_strspn(header, " \t\r");	//AptioV Porting - for build error 1.1.0
    if (*header++ != '\n') {
     ...
     */
    dekinfostart = header;
    header += int_strcspn(header, " \t,");	//AptioV Porting - for build error 1.1.0
    c = *header;
    *header = '\0';
