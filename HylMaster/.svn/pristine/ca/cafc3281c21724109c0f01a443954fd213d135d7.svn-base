...
#include "minisetup.h"
//<huangjin-20190723-Keep the value of BoardId when load default + >
#include <Setup.h>
#include "variable.h"
//<huangjin-20190723-Keep the value of BoardId when load default - >
...
extern UINTN gSaveShutdownMsgBox;
extern UINTN gShutdownMsgBox;
#endif
		...
		for ( page = 0; page < gPages->PageCount; page++ )
		{
			...
			for ( control= 0; control < pageInfo->PageControls.ControlCount; control++ )
			{
						...
						)
				{
					...
					if ( //( controlInfo->ControlVariable == VARIABLE_ID_LANGUAGE) || //<Malj-20191216 Bios Setup Load default Language setting>
                    ...
                    //<Malj-20191216 Bios Setup Load default Language setting +>
                    else if ( controlInfo->ControlVariable == VARIABLE_ID_LANGUAGE)
                    {
                        if(NULL != gApp)
                            gApp->CompleteRedraw = TRUE;
                        VarSetValue(VARIABLE_ID_LANGUAGE, 0 , defaults[VARIABLE_ID_LANGUAGE].Size, defaults[VARIABLE_ID_LANGUAGE].Buffer);
                    }
					//<Malj-20191216 Bios Setup Load default Language setting ->
					else
					{
...
BOOLEAN ResetSys( VOID )
{
	if ( CallbackShowMessageBox( (UINTN)gResetMsgBox, MSGBOX_TYPE_YESNO ) == MSGBOX_YES )
	{
		...
		// AMI EIP571350 Fix issue: After language change, discard and reset system language will be changed
		VarSetNvram ( VARIABLE_ID_LANGUAGE, gPrevLanguage,gPrevLangSize );
		return TRUE;
	}
...
**/
BOOLEAN SaveAndShutdown( VOID )
{
    if ( _SaveValues( (UINTN)gSaveShutdownMsgBox ) )
    {
        gVariableChanged = TRUE;
        gResetRequired = TRUE;
...
**/
BOOLEAN ShutdownSys( VOID )
{
    if ( CallbackShowMessageBox( (UINTN)gShutdownMsgBox, MSGBOX_TYPE_YESNO ) == MSGBOX_YES )
    {
        gResetRequired = TRUE; // force a reset 
        gVariableChanged = FALSE;
        gShutdownRequired = TRUE;
