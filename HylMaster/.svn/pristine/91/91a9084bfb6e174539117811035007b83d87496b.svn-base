...
)
{
  ...
  DEBUG((EFI_D_ERROR, "EqTable_PCIE: {Flag, Type, CpuGen, Die, VidDid, SubSysVidDid, StartLane, EndLane, PerPort, CfgValue[16]}\n"));
  DEBUG((EFI_D_ERROR, "EqTable_USB:  {Flag, Type, CpuGen, Die, UsbPort, UsbCfgValue}\n"));
  for (i=0; (EqCfgTable->EqCfgList[i].Flag & EQ_TABLE_END) != EQ_TABLE_END; i++) {
    if ((EqCfgTable->EqCfgList[i].Type & EQ_TYPE_MASK) == pcie_eq_type) {
    ...
    if ((EqCfgTable->EqCfgList[i].Type & EQ_TYPE_MASK) == usb_eq_type) {
      DEBUG((EFI_D_ERROR, "USB_EQ { 0x%x, ", EqCfgTable->EqCfgList[i].Flag));
      DEBUG((EFI_D_ERROR, "0x%x, ", EqCfgTable->EqCfgList[i].Type));
      DEBUG((EFI_D_ERROR, "0x%x, ", EqCfgTable->EqCfgList[i].CpuGen)); 
      DEBUG((EFI_D_ERROR, "0x%x, ", EqCfgTable->EqCfgList[i].Die)); 
      DEBUG((EFI_D_ERROR, "0x%x, ", EqCfgTable->EqCfgList[i].UsbPort)); 
      DEBUG((EFI_D_ERROR, "0x%x, ", EqCfgTable->EqCfgList[i].UsbCfgValue)); 
      DEBUG((EFI_D_ERROR, "}\n"));
    }
  }
}
