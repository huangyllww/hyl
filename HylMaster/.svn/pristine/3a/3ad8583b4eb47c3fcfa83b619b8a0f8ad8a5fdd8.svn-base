...
#include <Protocol/PciIo.h>//<ChengXW001-20180317 If Network is UEFI, Network Card boot option name is same in Setup>
...
//<ASUS-Frank101-20170914 Support new F12 function to boot selected PXE device+>
VOID SaveOriginLegacyDevOrder(){
    UINTN DevOrderSize;
    LEGACY_DEVICE_ORDER *DevOrder;

    BuildLegacyDevOrderBuffer(&DevOrder, &DevOrderSize);
    pRS->SetVariable(
        L"OriginLegacyDevOrder", &LegacyDevOrderGuid,
...
{
    EFI_STATUS Status;
    URI_DEVICE_PATH *UriDp;
    EFI_DEVICE_PATH_PROTOCOL *DevDp;

    if(    OptionDevicePath->Type != MESSAGING_DEVICE_PATH
        || OptionDevicePath->SubType != MSG_URI_DP
    ) return FALSE; //boot option is not URI boot option

    Status = pBS->HandleProtocol(Device->DeviceHandle, &gEfiDevicePathProtocolGuid, (VOID**)&DevDp);
    if(EFI_ERROR(Status)) return FALSE;

    UriDp = (URI_DEVICE_PATH*)GetLastDevicePathNodeOfType(DevDp, MESSAGING_DEVICE_PATH);
    if (   UriDp == NULL 
        || UriDp->Header.SubType!=MSG_URI_DP 
        || NODE_LENGTH(&UriDp->Header) != NODE_LENGTH(OptionDevicePath)
        || MemCmp( UriDp->Uri, OptionDevicePath+1, NODE_LENGTH(OptionDevicePath) - sizeof(EFI_DEVICE_PATH_PROTOCOL) )
    ) return FALSE;
...
EFI_HANDLE GetPhysicalNetworkCardHandle(EFI_HANDLE Handle, UINT8 *IpType, BOOLEAN *Uri){
    ...
    if (EFI_ERROR(Status)) return Handle;
    //zanggy-20210724-Fix Usb Lan boot options don't show dails in setup+>
    //Sync from Rome CRB20
/*
...
UINTN ConstructBootOptionNameByHandle(
    ...
    //<ChengXW001-20180317 If Network is UEFI, Network Card boot option name is same in Setup +>
    ...
    NumberOfCharacters = Swprintf_s(Name, NameSize, L"B%02xD%02xF%02x %s%s%s", BusNum, DevNum, FunNum, Prefix2, Prefix, ControllerName);
...
UINTN ConstructAtaBootOptionNameBySerialNumber(
    ...
    SerialNumber[20] = 0;
    NumberOfCharacters = Swprintf_s(Name, NameSize, L"%a", SerialNumber);
    return NumberOfCharacters;
}
...
UINTN ConstructNvmeBootOptionNameBySerialNumber(
    ...
    NumberOfCharacters = Swprintf_s(Name, NameSize, L"%a", &NvmeControllerData[4]);
...
//<kuaibai-20181013-Delete AMIFWUpdate option if none request +>
        if(!(StrCmp(Option->Description, L"AMIFWUpdate")))
        {
#if HIDE_FWUPDATE_BOOT_OPTION
            Option->Attributes = LOAD_OPTION_HIDDEN | LOAD_OPTION_ACTIVE;
#else
            Option->Attributes = LOAD_OPTION_ACTIVE;
#endif
        }
