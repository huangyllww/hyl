...
VOID ImportVariableToNVRAM()
{
	...
	for ( ; Variable; Variable = Variable->Next )
	{
		...
		if ( CompareMem( Buffer, Variable->VariableBuffer, ( size_t )BufferSize ) != 0 )
		{
			...
			{
				//if gAmiSceReboot false then only check for reboot condition
				if (gAmiSceReboot == FALSE)
				{
					ResetControlInfo = (CONTROL_RESET_FLAGINFO *)gResetControlInfo;
	
					while (ResetControlInfo)
					{
						if (Variable->VariableIndex == ResetControlInfo->VariableIndex)
						{
							UINT8 *ImportBuffer = Variable->VariableBuffer;
	
							if (memcmp(Buffer + (ResetControlInfo->DataOffset), ImportBuffer + (ResetControlInfo->DataOffset), ResetControlInfo->DataWidth))
							{
								//If ControlInfodata and NVRAM data differs set flag to TRUE
								gAmiSceReboot = TRUE;
								break;
							}
						}
        ...
        Status = VarSetNVRAMVariable(Variable, Variable->VariableBuffer, (UINTN)BufferSize);
			...
			{
			    DEBUG((DEBUG_ERROR, "WARNING : Error in writing variable %a to NVRAM\n", Variable->VariableName));
