  ...
  )
{
  ...
  if (PcieCapPtr != 0) {
    ...
    if (Value & BIT24) {
      GnbLibPciRMW ( Device.AddressValue | (PcieCapPtr + PCIE_SLOT_CAP_REGISTER) ,
                     AccessS3SaveWidth32,
                     0xFFFF807F,
                     (UINT32)(PowerLimit << 7),
                      NULL
  ...
  )
{
  ...
  while (PcieWrapper != NULL) {
    ...
    while (PcieEngine != NULL) {
      if ((PcieLibIsEngineAllocated (PcieEngine)) && (PcieEngine->Type.Port.SlotPowerLimit != 0)) {
        PcieSlotPwrLimitCallback (PcieEngine, PcieEngine->Type.Port.SlotPowerLimit);
      }
