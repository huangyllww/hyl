...
UINT16 PhyParameterTable_H4[2][9] = {
    //VERG_CTRL2,AFE_CTLE,AFE_OVER_RTRIM,AFE_OVER_ICTRL,CAL_MUXB,CAL_MUXA,CDR_CTL_3_3_5,CTL_4_0_2
...
)
{
    ...
    IDS_HDT_CONSOLE (PCIE_MISC, "DxioPhyReCfgH3 Entry\n");
    ...
    *RequestRedoEq = TRUE;
EXIT:
    IDS_HDT_CONSOLE (PCIE_MISC, "DxioPhyReCfgH3 Exit\n");
    return;
}
/*----------------------------------------------------------------------------------------*/
/**
 * PHY register reconfigure for H4
 * @param[in]  GnbHandle      Pointer to the Silicon Descriptor for this node
 * @param[in]  PcieEngine     Pointer to the PcieEngine
 * @param[in]  UserConfig     Pointer to the platform BIOS supplied platform configuration
 * @retval
...
)
{
    ...
    IDS_HDT_CONSOLE (PCIE_MISC, "DxioPhyReCfgH4 Entry\n");    
    ...
    *RequestRedoEq = FALSE;
    ...
    //H4 Set Ctl3/Ctl4 config for Gen4
...
EXIT:
    IDS_HDT_CONSOLE (PCIE_MISC, "DxioPhyReCfgH4 Exit\n");
    return;
}
/*----------------------------------------------------------------------------------------*/
/**
      ...
      if (PcieEngineList != NULL) {
        ...
        if ((TrainingSuccess != TRUE) || (DxioEnginePointer->LinkDesc.Pcie.Capabilities.portPresent == 0)) {
          ...
          if (PcieWaitToL0 (GnbHandle, PcieEngineList, 200)) {
            ...
            //Check if H3 Platform
            if(LogicalId.Revision & (HYGON_REV_F18_DN_Cx)){
              DxioPhyReCfgH3 (GnbHandle, PcieEngineList, UserCfgPortPointer, &RequestRedoEq);
              RedoEq |= RequestRedoEq;
            }
            
            //Check if H4 Platform
            if(LogicalId.Revision & (HYGON_REV_F18_DN_Dx)){               
              DxioPhyReCfgH4 (GnbHandle, PcieEngineList, UserCfgPortPointer, &RequestRedoEq);
              RedoEq |= RequestRedoEq;
             }
