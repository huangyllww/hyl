...
#define MAX_2D_DQS_SEED_COUNT 4                      ///< Max Seed count
#define MAX_2D_RDDQS_CS_PER_CHANNEL      8   ///< Max CS per RdDQS delay group

#define EYERIM_PARALLEL_SAMPLING TRUE
#define EYERIM_BROADCAST_DELAYS TRUE
#define SAMPLE_INDEX(l, y, x) (((l) * MAX_2D_VREF_ENTRIES * MAX_RD_DQS_ENTRIES) + ((y) * MAX_RD_DQS_ENTRIES) + (x))
...
typedef enum {
  ...
  DataEyeSaveCompositeEyes,         ///< Perform saving the composite data eyes to buffer
  DataEyeCompressEyes,              ///< Perform compress the composite data eyes.
...
typedef struct {
  RD_DQS_2D  *Vref;///< pointer to Vref Entries
  UINT16 Convolution[MAX_RD_DQS_ENTRIES]; ///< Total number of passes in each mask
...
typedef struct {
  VREF_2D  Lane[MAX_NUMBER_LANES]; ///< Bytelane or Nibble
  UINT8 Vnom;  // Nominal Vref value
  UINT8 MaxRdDqsSweep; // Maximum RdDqs Sweep size
  UINT8 SmallestPosMaxVrefperCS[MAX_2D_RDDQS_CS_PER_CHANNEL];  // Smallest Positive Max Vref per CS
  UINT8 SmallestNegMaxVrefperCS[MAX_2D_RDDQS_CS_PER_CHANNEL];  // Smallest Negative Max Vref per CS
...
/// Structure for RD DQS 2D training Nibbles
typedef struct {
  VREF_2D  LaneSaved[MAX_NUMBER_LANES]; ///< Bytelane or Nibble that was saved
  ...
  UINT32 RdDqsDlyUpdates;           ///< Count of updates to RdDqsDly
...
typedef struct _MEM_TECH_BLOCK {
  ...
  MEM_2D_ENTRY *Local2DData;   ///< Pointer to local data structure
  MEM_2D_RIM_ENTRY *SavedData;     ///< Eye rim search save data
  VOID *RdWr2DData; ///< 2D Read/Write Training Feature Data Struct Pointer
