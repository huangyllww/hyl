...
//------------------------------
    ...
    if((DnStreamPort->PciExpress->Pcie2->Pcie2Setup.LnkSpeed!=PCI_SETUP_DONT_TOUCH) && 
        ...
        DEBUG((EFI_D_ERROR, "[OemDbg][%a][%d]maxls:%x\r\n",__FUNCTION__,__LINE__,maxls));
        ...
        if(maxls>=PCIE_LINK_SPEED_80G){
            //Mizl-20211027- Update Auto Policy For No LINK3 Support+>
            if((!Pcie3CheckPcie3Compatible(DnStreamPort)|| !Pcie3CheckPcie3Compatible(UpStreamPort)) && (LnkSpeedOption !=PCI_SETUP_AUTO_VALUE)) maxls=PCIE_LINK_SPEED_50G;
            else{
               if((maxls>PCIE_LINK_SPEED_80G) && (LnkSpeedOption==3))ls=PCIE_LINK_SPEED_80G; 
            }
        }
		//<xuyj120210918 sync Hygon Crb016 Code for GEN4 +>
        if(maxls>=PCIE_LINK_SPEED_80G){
            //Mizl-20211027- Update Auto Policy For No LINK3 Support+>
            if((!Pcie3CheckPcie3Compatible(DnStreamPort)|| !Pcie3CheckPcie3Compatible(UpStreamPort)) && (LnkSpeedOption !=PCI_SETUP_AUTO_VALUE))
            { 
                ...
                DEBUG((EFI_D_ERROR, "[OemDbg][%a][%d]maxls:%x\r\n",__FUNCTION__,__LINE__,maxls));
               if((maxls>PCIE_LINK_SPEED_80G) && (LnkSpeedOption==3))ls=PCIE_LINK_SPEED_80G; 
               if((maxls==PCIE_LINK_SPEED_16G) && (LnkSpeedOption==4))
               {
