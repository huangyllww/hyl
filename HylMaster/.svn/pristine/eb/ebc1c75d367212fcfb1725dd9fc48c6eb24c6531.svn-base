...
*/
GLOBAL_REMOVE_IF_UNREFERENCED UINTN  gSbUartBASE = FCH_UART_BASE_ADDRESS;
GLOBAL_REMOVE_IF_UNREFERENCED UINTN  gSbUartBASE1 = FCH_UART1_BASE_ADDRESS;
...
*/
GLOBAL_REMOVE_IF_UNREFERENCED UINTN  gSbUartBASE  = 0x3F8;
GLOBAL_REMOVE_IF_UNREFERENCED UINTN  gSbUartBASE1 = 0x2F8;
#else
/*
#if (defined(FCH_UART_DEBUG_SELECT) && (FCH_UART_DEBUG_SELECT == 1))
GLOBAL_REMOVE_IF_UNREFERENCED UINTN  gSbUartBASE = FCH_UART_BASE_ADDRESS;
...
/**
   Get the FCH Uart1 base address
...
UINT8
SerialPort1ReadRegister (
  UINTN  Offset
  )
{
    if(!gUartLegacyIo){
        return MmioRead8 (GetSbUart1Base()+ Offset*4);
    } else {
        return IoRead8(GetSbUart1Base()+ Offset);
    }
}
/**
...
UINT8
SerialPort1WriteRegister (
  UINTN  Offset,
  UINT8  Value
  )
{
    if(!gUartLegacyIo){
        return MmioWrite8 (GetSbUart1Base()+ Offset*4, Value);
    } else {
        return IoWrite8(GetSbUart1Base() + Offset, Value);
    }
}
  ...
  )
{
  if (FchSerialUseHardwareFlowControl) {
    if (FchSerialDetectCable) {
      ...
      //    1    1   Cable connected, and clear to send.       Transmit
      //
      return (BOOLEAN) ((SerialPort1ReadRegister (FCH_UART_MSR) & (FCH_UART_MSR_DSR | FCH_UART_MSR_CTS)) == (FCH_UART_MSR_DSR | FCH_UART_MSR_CTS));
    } else {
      //
      // Wait for both DSR and CTS to be set OR for DSR to be clear.  
      //   DSR is set if a cable is connected.
      ...
      //    1    1   Cable connected, and clar to send.        Transmit
      //
      return (BOOLEAN) ((SerialPort1ReadRegister (FCH_UART_MSR) & (FCH_UART_MSR_DSR | FCH_UART_MSR_CTS)) != (FCH_UART_MSR_DSR));
    }
  }
...
//HYGON: Not for Zeppelin  SbUartPowerOn();
  ...
  if (!gUartLegacyIo) {
    ...
    MmioWrite8 (GetSbUart1Base() + FCH_SRR_OFFSET, 0x01); //huangjin enable both UART0 and UART1 //reset UARTx
  ...
  SerialPort1WriteRegister (FCH_UART_LCR, FCH_UART_LCR_DLAB);//huangjin enable both UART0 and UART1
  SerialPortWriteRegister (FCH_UART_BAUD_HIGH, 00);
  SerialPort1WriteRegister (FCH_UART_BAUD_HIGH, 00);
  if(!gUartLegacyIo){
    SerialPortWriteRegister (FCH_UART_BAUD_LOW, 0x1a); //set 115200, if inp =48MHz
    SerialPort1WriteRegister (FCH_UART_BAUD_LOW, 0x1a);//set 115200, if inp =48MHz
  } else {
    SerialPortWriteRegister (FCH_UART_BAUD_LOW, 0x01); //set 115200, if inp = 1.8432MHz
    SerialPort1WriteRegister (FCH_UART_BAUD_LOW, 0x01);//set 115200, if inp = 1.8432MHz
  }
  ...
  SerialPort1WriteRegister (FCH_UART_LCR, (UINT8)(FchSerialLineControl & 0x3F));//huangjin enable both UART0 and UART1
  ...
  SerialPort1WriteRegister (FCH_UART_FCR, (UINT8)(FchSerialFifoControl & 0x27));//huangjin enable both UART0 and UART1
  ...
  //add for recv data
  SerialPortWriteRegister (FCH_UART_MCR, 0x00);
  SerialPort1WriteRegister (FCH_UART_MCR, 0x00);//huangjin enable both UART0 and UART1
...
//HYGON #if (defined(FCH_UART_DEBUG_SELECT) && (FCH_UART_DEBUG_SELECT == 1))     
  ...
  //PS-285, Hygon 65N32 C01 board use GPIO137 control DDR_SPD_SWITCH
...
)
{
  ...
  //huangjin enable both UART0 and UART1 +>
  if ((SerialPort1ReadRegister (FCH_UART_LCR) & 0x3F) !=FchSerialLineControl) {
  //re initial  
  SerialPortInitialize();
  }
  ...
  if (NumberOfBytes == 0) {
    ...
    //huangjin enable both UART0 and UART1 +>
     while ((SerialPort1ReadRegister (FCH_UART_LSR) & FCH_UART_LSR_TEMT) == 0);

    while (!SerialPort1Writable ());
    //huangjin enable both UART0 and UART1 ->
  ...
  while (NumberOfBytes != 0) {
    ...
    //
    while ((SerialPortReadRegister (FCH_UART_LSR) & FCH_UART_LSR_TEMT) == 0);
    while ((SerialPort1ReadRegister(FCH_UART_LSR) & FCH_UART_LSR_TEMT) == 0);//huangjin enable both UART0 and UART1
...
//#endif
    for (Index = 0; Index < FifoSize && NumberOfBytes != 0; Index++, NumberOfBytes--, Buffer++) {
     ...
     // while (!SerialPortWritable ());
      ...
      while (!SerialPort1Writable());//huangjin enable both UART0 and UART1
      SerialPort1WriteRegister (FCH_UART_TXBUF, *Buffer);//huangjin enable both UART0 and UART1
    }
  }
 ...
 //#endif
     for (Index = 0; Index < FifoSize && NumberOfBytes != 0; Index++, NumberOfBytes--) {
      ...
      // while (!SerialPortWritable ());
       ...
       while (!SerialPort1Writable ());//huangjin enable both UART0 and UART1
       SerialPort1WriteRegister (FCH_UART_TXBUF, code[Index]);//huangjin enable both UART0 and UART1
     }
   }
   //debug postcode end
...
)
{
  ...
  for (Result = 0; NumberOfBytes-- != 0; Result++, Buffer++) {
      ...
      while ((SerialPortReadRegister (FCH_UART_LSR) & FCH_UART_LSR_RXRDY) == 0) {
          ...
          SerialPort1WriteRegister (FCH_UART_MCR, (UINT8)(Mcr | FCH_UART_MCR_RTS));//huangjin enable both UART0 and UART1
         }
      }
  ...
  )
{
  ...
  if ((SerialPortReadRegister (FCH_UART_LSR) & FCH_UART_LSR_RXRDY) != 0) {
      ...
      SerialPort1WriteRegister (FCH_UART_MCR, (UINT8)(SerialPortReadRegister (FCH_UART_MCR) & ~FCH_UART_MCR_RTS));//huangjin enable both UART0 and UART1
    }
    return TRUE;
  }    
