  ...
  )
{
  ...
  if (BlockIo->Media->MediaPresent ||
	...
	//*** AMI PORTING BEGIN ***//
	// Improvement: Detection of removable devices takes time.
	// In order to improve overall boot time, physical block I/O drivers don't perform full device detection 
	// when publishing a Block I/O instance. They perform full detection on the first device access.
	// That's why first device operation may return EFI_NO_MEDIA. We re-try when it happens.
	EFI_STATUS PreviousStatus = EFI_SUCCESS;
	// Improvement(EIP 204664): Workaround for buggy third party block I/O drivers.
	// Some third party block I/O drivers do not properly return media status on the first read.
	// It takes several read operations before correct status is returned.
	// The AmiPcdNumberOfRetriesOnNoMedia PCD defines a number of read operations that partition driver 
	// will attempt before returning an error if Block I/O read operation returns EFI_NO_MEDIA.
	UINT32 NumberOfNoMediaRetries = PcdGet32(AmiPcdNumberOfRetriesOnNoMedia);
	//*** AMI PORTING END *****//
    ...
    while (*Routine != NULL) {
      ...
      //*** AMI PORTING BEGIN ***//
      // Try again on EFI_MEDIA_CHANGED (see comments above for more details).
      if (Status == EFI_MEDIA_CHANGED && PreviousStatus != EFI_MEDIA_CHANGED) {
    	PreviousStatus = Status;
        continue;
      // Workaround for buggy third party block I/O drivers (see comments above for more details).
      }else if (Status == EFI_NO_MEDIA && NumberOfNoMediaRetries > 1){
    	NumberOfNoMediaRetries--;
    	continue;
      }
      //*** AMI PORTING END *****//
  ...
  )
{
  ...
  for (Index = 0; Index < NumberOfChildren; Index++) {
    ...
    if (EFI_ERROR (Status)) {
      ...
      //*** AMI PORTING BEGIN ***//
        remove_part (ControllerHandle, Private->DevicePath);
      //*** AMI PORTING END ***//
  ...
  )
{
  ...
  if ((PartitionInfo->System == 1)) {
    ...
    if (PartitionInfo->Type == PARTITION_TYPE_GPT){
      Private->EspGuid = &PartitionInfo->Info.Gpt.PartitionTypeGUID;
	} else
    //*** AMI PORTING END ***//
    //
