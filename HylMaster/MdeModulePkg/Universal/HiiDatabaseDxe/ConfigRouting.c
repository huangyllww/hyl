  ...
  )
{
  ...
  ASSERT (HiiHandle != NULL);
  //ASSERT (StringId != 0); //<ChengXW001-20170803 Mantis #15124 For 3008 Setup Hang>
  ...
  //<ChengXW001-20170803 Mantis #15124 For 3008 Setup Hang +>
  if (StringId == 0) {
    DEBUG (( EFI_D_ERROR, "[OemDbg]: StringId is 0\n"));
    return String;
  }
  //<ChengXW001-20170803 Mantis #15124 For 3008 Setup Hang ->
  ...
  while (*StringPtr != 0 && StrnCmp (StringPtr, L"GUID=", StrLen (L"GUID=")) == 0) {
    ...
    *(ConfigRequest + Length) = 0;
    ...
    if (IsEfiVarStore) {
      ...
      if (EFI_ERROR (Status)) {
	    //*** AMI PORTING BEGIN ***//
		// Bug fix: when protocol is not installed on a handle, HandleProtocol returns EFI_UNSUPPORTED.
		// However, ExtractConfig has to return EFI_NOT_FOUND when ConfigAceess instance is not found.
    ...
    if (EFI_ERROR (Status)) {
      ...
      //
      //*** AMI PORTING BEGIN ***//
      // Workaround for a buggy instance of ConfigAccess that sets AccessProgress to NULL, 
      // which is illegal according to UEFI specification.
      ASSERT (AccessProgress != NULL);
