  ...
  IN  UINT8                     TableSize )
{
    ...
    SERVER_IPMI_DEBUG ((EFI_D_INFO, "ViewSelLog: Inside %a\n", __FUNCTION__));
    ...
    SERVER_IPMI_DEBUG ((EFI_D_INFO, "ViewSelLog: Exit %a\n", __FUNCTION__));
  ...
  IN  UINT32    TimeStamp )
{
    ...
    SERVER_IPMI_DEBUG ((EFI_D_INFO, "ViewSelLog: Exiting %a\n", __FUNCTION__));
  ...
  IN  IPMI_SEL_EVENT_RECORD_DATA    SelRecordData )
{
    ...
    if ((SelRecordData.SensorType >= RESERVED_SENSOR_TYPE_START) && \
        ...
        String = GetDescStringFromValue (
                    ...
                    gSensorTypeTable,
                    SENSOR_TYPE_COUNT );
        if (StrCmp(String,RESERVED_UNKNOWN_STRING) == 0x00)
        {
  ...
  IN  IPMI_SEL_EVENT_RECORD_DATA    SelRecordData )
{
    ...
    SensorDetailsToken = BuildSelSensorDetailsWithTimeStamp (
                            HiiHandle,
                            SelRecordData );
    SERVER_IPMI_DEBUG ((EFI_D_INFO, "ViewSelLog: SensorDetailsToken: %X \n", SensorDetailsToken));
    ...
    SelRecordDescriptionToken = BuildSelRecordDescription (
                                    HiiHandle,
                                    SelRecordData );
    SERVER_IPMI_DEBUG ((EFI_D_INFO, "ViewSelLog: SelRecordDescriptionToken: %X \n", SelRecordDescriptionToken));
  ...
  IN  VIEW_SEL_LOG_FLAG   ViewSelLogFlag )
{
    ...
    Status = HiiLibGetBrowserData (
                ...
                L"ServerSetup" );
    SERVER_IPMI_DEBUG ((EFI_D_INFO, "ViewSelLog: Status of HiiLibGetBrowserData(): %r\n",Status));
    ...
    Status = HiiLibSetBrowserData (
                ...
                &gEfiServerMgmtSetupVariableGuid,
                L"ServerSetup" );
    SERVER_IPMI_DEBUG ((EFI_D_INFO, "ViewSelLog: Status of HiiLibSetBrowserData(): %r\n",Status));
    ASSERT_EFI_ERROR (Status);
    if ( EFI_ERROR (Status) ) {
        return;
    }
  ...
  IN  UINT16            Key )
{
    ...
    if ((Key != VIEW_SEL_LOG_FORM_KEY) && \
        ...
        SERVER_IPMI_DEBUG ((EFI_D_ERROR, "ViewSelLog: Callback function is called with \
    ...
    if (CallbackParameters->Action != EFI_BROWSER_ACTION_CHANGING) {
        SERVER_IPMI_DEBUG ((EFI_D_INFO, "ViewSelLog: CallbackParameters->Action != \
                EFI_BROWSER_ACTION_CHANGING... so return EFI_SUCCESS\n"));
        return  EFI_SUCCESS;
    }

    SERVER_IPMI_DEBUG ((EFI_D_INFO, "ViewSelLog: Key: %x \n", Key));
    ...
    SERVER_IPMI_DEBUG ((EFI_D_INFO, "ViewSelLog: LocateProtocol for gAmiPostManagerProtocolGuid Status: %r.\n", Status));
    ...
    if (!EFI_ERROR (Status)) {
        ...
        if (!EFI_ERROR (Status)) {
            ...
            if (!EFI_ERROR (Status)) {
                ...
                SERVER_IPMI_DEBUG ((EFI_D_INFO, "ViewSelLog: DisplayMsgBox Status: %r.\n", Status));
                SERVER_IPMI_DEBUG ((EFI_D_INFO, "ViewSelLog: MsgBoxSel value: %x.\n", MsgBoxSel));
                //
    ...
    Status = SendIpmiCommand (
                ...
                (UINT8 *) &SelInfoSize,
                &CompletionCode );
    SERVER_IPMI_DEBUG ((EFI_D_INFO, "ViewSelLog: IPMI_STORAGE_GET_SEL_INFO Status: %r\n", Status));
    ...
    SERVER_IPMI_DEBUG ((EFI_D_INFO, "ViewSelLog: SelInfo.NoOfEntries: %x \n", SelInfo.NoOfEntries));
    ...
    SERVER_IPMI_DEBUG ((EFI_D_INFO, "ViewSelLog: IsFirstCall: %x\n", IsFirstCall));
    ...
    if (IsFirstCall == TRUE) {
        ...
        if (gSensorDetailsSavedToken == NULL) {
            SERVER_IPMI_DEBUG((EFI_D_ERROR, "Failed to allocate memory for gSensorDetailsSavedToken! \n"));
            return EFI_OUT_OF_RESOURCES;
        }
        ...
        if (gSelRecordDescSavedToken == NULL) {
            SERVER_IPMI_DEBUG((EFI_D_ERROR, "Failed to allocate memory for gSelRecordDescSavedToken! \n"));
            return EFI_OUT_OF_RESOURCES;
        }
    ...
    if (gErrorManager == NULL) {
        SERVER_IPMI_DEBUG((EFI_D_ERROR, "Failed to allocate memory for gErrorManager! \n"));
        return EFI_OUT_OF_RESOURCES;
    }
    ...
    SERVER_IPMI_DEBUG ((EFI_D_INFO, "ViewSelLog: MaxSelPerForm: %x\n", MaxSelPerForm));
    ...
    while (SelResponseData.NextSelRecordId != 0xFFFF) {
        ...
        SERVER_IPMI_DEBUG ((EFI_D_INFO, "ViewSelLog: RecordId of the event to be read: 0x%02X%02X \n", SelEntry.SelRecID[1], SelEntry.SelRecID[0] ));
        ...
        SERVER_IPMI_DEBUG ((EFI_D_INFO, "ViewSelLog: EFI_STORAGE_GET_SEL_ENTRY Status: %r Next RecordId: %x \n", Status, SelResponseData.NextSelRecordId ));
        //
...
DONE:
    SERVER_IPMI_DEBUG ((EFI_D_INFO, "ViewSelLog: ViewSelLogFlag: %x \n", ViewSelLogFlag));
    //
    ...
    Status = gRT->GetVariable (
                ...
                &Size,
                gErrorManager);
    SERVER_IPMI_DEBUG ((EFI_D_INFO, "ViewSelLog: gRT->GetVariable Status: %r \n", Status));
    //
    ...
    SERVER_IPMI_DEBUG ((EFI_D_INFO, "ViewSelLog: gRT->SetVariable Status: %r \n", Status));
    //
    // Free the memory allocated for Error Manager
    //
    gBS->FreePool(gErrorManager);

    SERVER_IPMI_DEBUG ((EFI_D_INFO, "ViewSelLog: ViewSelLogCallbackFunction() Exit\n"));
