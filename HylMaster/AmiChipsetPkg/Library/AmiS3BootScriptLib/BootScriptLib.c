...
//*************************************************************************
/** @file BootScriptLib.c
    The function implementation for EFI S3 boot script.
...
/**
    ...
    @param DataMask A pointer to the data mask to be AND-ed with the data
...
/**
    Adds a record for a PCI configuration space write operation 
...
/**
    Adds a record for a PCI configuration space modify operation 
    into a specified boot script table.
...
/**
    ...
    @param Operation Indicates which particular SMBus protocol it will
        use to execute the SMBus transactions.
    @param Length A pointer to signify the number of bytes that this
        operation will do.
    @param Buffer Contains the value of data to execute to the SMBUS
        slave device.
...
/**
    ...
    @param EntryPoint Entry point of the code to be dispatched.
    @param Context Argument to be passed into the EntryPoint of the
...
/**
    Adds a record for memory reads of the memory location and continues 
    when the exit criteria is satisfied or after a defined duration.

    @param Width The width of the memory operations.
    @param Address The base address of the memory operations.
    @param BitMask A pointer to the bit mask to be AND-ed with the data
        read from the register.
...
/**
    Store arbitrary information in the boot script table. This opcode 
...
/**
    Adds a record for I/O reads the I/O location and continues when 
    ...
    @param DataMask Mask used for the polling criteria. The bits in the
...
/**
    Adds a record for PCI configuration space reads and continues when 
    the exit criteria is satisfied or after a defined duration.
...
/**
    Save ASCII string information specified by Buffer to boot script
    ...
    @retval RETURN_SUCCESS BootScriptSave Protocol exist, always returns 
        RETURN_SUCCESS.
**/
...
/**
    ...
    @retval EFI_INVALID_PARAMETER The parameter is illegal or the given boot script is 
