...
//
// AMI Porting Start - PXE boot fails after booting to an unbootable legacy device.
//
#if (NET_PKG_AMI_PORTING_ENABLE == 1)

EFI_EVENT   EventForPxeShutdown = NULL; // AMI PORTING - for CTRL_ALT_DEL Key Combination.
...
/**
  This function start SNP driver when legacy to EFI boot called.
...
)
{
    ...
    //
    // Backup Filter Settings to restore later
    //
    MCastFilter = NULL;
    ...
    //
   Status = SnpUndi32Initialize (&Snp->Snp, 0, 0);
   ...
   if (EFI_ERROR (Status)) {
    ...
    //
    // If initialization is successful, restore filter settings.
    //
    if (Snp->Mode.State == EfiSimpleNetworkInitialized){
        //
        // Restore SNP receive filter settings
        //
        Status = SnpUndi32ReceiveFilters(
                       &Snp->Snp,
                       EnableFilterBits,
                       DisableFilterBits,
                       ResetMCastFilters,
...
Exit:
    if (EFI_ERROR (Status)) {
     SnpUndi32Shutdown(&Snp->Snp);
     SnpUndi32Stop (&Snp->Snp);
    }
...
//
// AMI Porting End - PXE boot fails after booting to an unbootable legacy device.
//
  ...
  )
{
...
#if (NET_PKG_AMI_PORTING_ENABLE == 1)
  EFI_ACPI_QWORD_ADDRESS_SPACE_DESCRIPTOR   *BarDesc = NULL;  // AMI Porting - PXE boot fails with cards having non-continuous BAR.
  EFI_GUID                                  pLTEBGuid = LTEB_GUID;  // AMI Porting - PXE boot fails after booting to an unbootable legacy device
#else
  ...
  for (BarIndex = 0; BarIndex < PCI_MAX_BAR; BarIndex++) {
	...
	// AMI Porting start - PXE boot fails with cards having non-continuous BAR.
#if (NET_PKG_AMI_PORTING_ENABLE == 1)
...
#endif	// NET_PKG_AMI_PORTING_ENABLE
    // AMI Porting end - PXE boot fails with cards having non-continuous BAR.
  ...
  //  
#if (NET_PKG_AMI_PORTING_ENABLE == 1)
  if(EventForPxeShutdown == (EFI_EVENT)NULL){
	  Status = gBS->CreateEventEx(
          EVT_NOTIFY_SIGNAL,
          TPL_NOTIFY,
          TriggerPxeShutdown,
  ...
  DEBUG((DEBUG_INFO,"\n SNPDEBUG: CreateEventEx for EventForPxeShutdown Status :: %r\n",Status));
  //
  ...
  //
  // Register a Legacy boot event to stop UNDI driver.
  //
  ...
  )
{
  ...
  gBS->CloseEvent(Snp->LegacyBootEvent);   // AMI PORTING : Close Legacy Boot Event for Pxe Shutdown.

  gBS->CloseEvent(Snp->LegacyToEfiSnpEvent);   // AMI PORTING : Close LegacyToEfi Event for Pxe Restart.
...
// AMI PORTING STARTS  
#if (NET_PKG_AMI_PORTING_ENABLE == 1)  
  if((EventForPxeShutdown != NULL ) && NumberOfChildren == 0)
  {
      gBS->CloseEvent(EventForPxeShutdown);
      EventForPxeShutdown = NULL;
  }
#endif	// NET_PKG_AMI_PORTING_ENABLE
// AMI PORTING ENDS
...
//USI-Paul0025-2017-08-16 To support Legacy PXE Retry boot.+>
    //#define CMOS_PxeRetryCount      0x5F
    //#define CMOS_BootCount          0x5E
    //#define CMOS_PxeRetryFlag       0x5D

    // CMOS_PxeRetryCount : 0x5F
    outp(0x72, 0x5F);             // CMOS_PxeRetryCount
    //outp(0x73, NetworkStackSetupData.UefiPxeRetryCount);
    outp(0x73, 0xff);
    // CMOS_PxeRetryCount : 0x5E
    outp(0x72, 0x5E);             // CMOS_BootCount
    //outp(0x73, NetworkStackSetupData.UefiPxeRetryCount);
    outp(0x73, 0xff);   
    // CMOS_PxeRetryCount : 0x5D
    // BIT0: PxeRetryFlag, <Pxe Retry flag>
    // /0: Non-Pxe Retry flag, /1: Pxe Retry flag
    // BIT7: EndlessBoot flag
    // /0: Disabled Auto Pxe retry behaviorin End of counting, it will display message.
    // /1: Enable Auto Pxe retry behavior in End of counting, it don't dispay any message.       
    outp(0x72, 0x5D);             // CMOS_PxeRetryFlag - BIT0/BIT7 
    //outp(0x73, (NetworkStackSetupData.EndlessBootSupport << 7) + 0);
    outp(0x73, ((NetworkStackSetupData.UefiPxeRetryCount / 255) << 7) + 0);
    //outp(0x73, ((1) << 7) + 0);
    //USI-Paul0025-2017-08-16 To support Legacy PXE Retry boot.->
#endif    
//<Ouyyx-20190831 To support UEFI PXE Retry boot.->
...
#endif	// LOAD_DRIVER_ONLY_ON_NWSTACK_ENABLE
  //
  // AMI PORTING END
  //
