  ...
  )
{
  ...
  // AMI PORTING START
  //
#if (NWSTACK_CONFIGURE_IP)
  UINTN                   VarSize;
  IP4_CONFIG2_IFR_NVDATA  *IfrFromNvData=0;
#endif 
  //
  // AMI PORTING END
  //
  ...
  if (Status == EFI_BUFFER_TOO_SMALL) {
	  IfrFromNvData = AllocateZeroPool (VarSize);
       if (IfrFromNvData == NULL) {
         return EFI_OUT_OF_RESOURCES;
       }
       Status = gRT->GetVariable (
		               IpSb->MacString,
  ...
  //
#if (NWSTACK_CONFIGURE_IP)
  if (IfrFromNvData->DhcpEnable == 0) {
    IfrNvData->DhcpEnable = FALSE;
  } else if (IfrFromNvData->DhcpEnable == 1) {
  ...
  if (Action == EFI_BROWSER_ACTION_CHANGED) {
   ...
   IpSb = IP4_SERVICE_FROM_IP4_CONFIG2_INSTANCE (Instance);
    ...
    case KEY_SAVE_CHANGES:
	  ...
	  //
#if (NWSTACK_CONFIGURE_IP)
      Status =gRT->SetVariable(IpSb->MacString,
         	    	         &gIp4Config2NvDataGuid,
  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    ...
    //
    // ADD MAC string in the menu Title string
    //  
    OldMenuString = HiiGetString (
                      CallbackInfo->RegisteredHandle, 
                      STRING_TOKEN (STR_IP4_CONFIG2_FORM_TITLE), 
                      NULL
                      );
...
#else
  {
      ...
      CopyGuid(&PackageListHeader->PackageListGuid, &gEfiIp4Config2ProtocolGuid);
      ...
      Status = gHiiDatabase->NewPackageList(
                                  gHiiDatabase, 
                                  PackageListHeader, 
                                  Instance->CallbackInfo.ChildHandle,	// AMI PORTING (UEFI 2.5)
//                                Instance->ChildHandle, // AMI PORTING (EIP82809)
                                  &OutHiiHandle
                                  );
      ...
      Instance->CallbackInfo.RegisteredHandle = OutHiiHandle;
