...
// AMI PORTING START.
//
BOOLEAN                     gWaitforIp4 = FALSE;
BOOLEAN                     gWaitforIp6 = FALSE;
EFI_HANDLE 	                gControllerHandle = NULL;

//
// This variable is consumed in GetCongifData function to return and 
// also to process MPIO configuration
//
  ...
  )
{
  ...
  if (ControllerHandle == gControllerHandle) return EFI_SUCCESS;
  ...
  //
  Status = NetLibGetMacAddress (ControllerHandle, &MacAddress, &HwAddressSize);
  ASSERT (Status == EFI_SUCCESS);
  VlanId = NetLibGetVlanId (ControllerHandle);
  ...
  //
  // Get the Attempt configuration.
  //
  for (Index = 0; Index < AttemptConfigOrderSize / sizeof (UINT8); Index++) {
      //
      // Append the Mac Address and Index
      //
      UnicodeSPrint (PortString,(UINTN) 128,L"%s%d",MacString,(UINTN) AttemptConfigOrder[Index]);

      //
      // Get the normal configuration.
      //
      AttemptTmp = IScsiGetVariableAndSize (
                       PortString,
...
// AMI PORTING ENDS		
    ...
    if (AttemptTmp->SessionConfigData.IpMode == IP_MODE_IP4)
    {
        gWaitforIp4 = TRUE;
//      DEBUG ((EFI_D_ERROR, "gWaitforIp4 = TRUE.\n"));
    }  
    if (AttemptTmp->SessionConfigData.IpMode == IP_MODE_IP6)
    {
        gWaitforIp6 = TRUE;
//      DEBUG ((EFI_D_ERROR, "gWaitforIp6 = TRUE.\n"));
    }
  }
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  if (mPrivate->OneSessionEstablished && mPrivate->EnableMpio) {
	...
	//
    //
    // Control comes here only for IPV6 case: when MPIO is enabled and established session in Ipv4
    // As the session is already established in IPv4, there is no need to process the IPv6 case
    // so returning EFI_ALREADY_STARTED.
    //
    if ( SameNicIndex ) {
        return EFI_ALREADY_STARTED;
    }
...
// AMI PORTING STARTS - RSA iSCSI Support.
      ...
      {
          Status = RSAUpdateBMCIScsiAttemptDataCache(mPrivate->PortString, AttemptConfigData); 
      }
#endif	// RSA_ISCSI_SUPPORT
// AMI PORTING ENDS - RSA iSCSI Support.
// AMI PORTING STARTS
...
//
//  AMI Porting Start - Fix for block device not detected when MPIO attempts' order 
...
//
//  AMI Porting End - Fix for block device not detected when MPIO attempts' order 
//  is changed and first Attempt is made invalid.
//
  }
...
//      IScsiRemoveNic (ExistPrivate->Controller);   // AMI PORTING - Commenting so that attempts/MAC will not be removed in MPIO configured attempts(EIP 148500)
