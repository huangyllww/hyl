...
//<Ouyyx-20190831 To support UEFI PXE Retry boot +>
#if defined(UEFI_PXERetryBoot_SUPPORT) && (UEFI_PXERetryBoot_SUPPORT == 1) 
    UINT8 F12_Retry_Flag = 0;
    UINTN NetworkStackSize;
    UINT32 Attributes;
    NETWORK_STACK mNetworkStackSetupData;
#endif
//<Ouyyx-20190831 To support UEFI PXE Retry boot -> 
    Status = GetEfiVariable( L"BootOrder", &EfiVarGuid, NULL, &BootOrderSize, &BootOrder );
    if (EFI_ERROR(Status))
...
//<Ouyyx-20190831 To support UEFI PXE Retry boot +>
        ...
        Status = pRS->GetVariable(
                        ...
                        &Attributes, 
                        &NetworkStackSize, 
                        &mNetworkStackSetupData
                        ); 
        if (!EFI_ERROR(Status)) 
        {
            if (F12_Retry_Flag == 1)
            {
                mNetworkStackSetupData.F12_Hot_Key = 1;
                F12_Retry_Flag = 0;
            }
            NetworkStackSize = sizeof(NETWORK_STACK);
        ...
        {
            TRACE((TRACE_ALWAYS,"Get NetworkStackVar Variable is failed ... \n"));
        }
#endif
//<Ouyyx-20190831 To support UEFI PXE Retry boot ->
...
EFI_STATUS NetBootHotKey(VOID)
{
    //<Ouyyx-20190831 To support Uefi PXE Retry boot +>
        ...
        DEBUG((DEBUG_INFO,"Try the EFI boot option first\n"));
        EfiPxeBoot();
        // Attempt for Legacy PXE boot
        DEBUG((DEBUG_INFO,"Attempt for Legacy PXE boot\n"));
        Status = LegacyPxeBoot();
        return Status;
#else //<Ouyyx-20190831 To support Uefi PXE Retry boot ->
    UINT8 i;
    ...
    return EFI_SUCCESS;
#endif//<Ouyyx-20190831 To support Uefi PXE Retry boot>
}
