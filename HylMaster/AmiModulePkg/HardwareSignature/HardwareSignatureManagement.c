...
/*
 * This function will check SOC vendor ID and family ID for Hygon Dhyana SOC release BIOS
 * CPUID 0x80000000 return SOC vendor ID must be  HygonGenuine
 * CPUID 0x80000001 return SOC family ID must be 01h 
 * If not, BIOS will pop up mailbox to warining user
 */
VOID VerifyHygonSocID (
        VOID
)
{
    ...
    //check SOC vendor ID, must be  HygonGenuine
    AsmCpuid (
        0x80000000,
        &EAX_Reg,
        &EBX_Reg,
    ...
    HWSIG_TRACE((-1,"Soc Vendor ID ASCII = 0x%08x, 0x%08x, 0x%08x \n", EBX_Reg, EDX_Reg, ECX_Reg));
    if ((EBX_Reg == 0x6F677948) && (EDX_Reg == 0x6E65476E) && (ECX_Reg == 0x656E6975)) {
        //0x6F677948 = "ogyH" ASCII code
        //0x6E65476E = "neGn" ASCII code
        //0x656E6975 = "eniu" ASCII code
        HWSIG_TRACE((-1,"Soc Vendor ID is HygonGenuine \n"));
        
        //check SOC family ID, must be 01h 
        AsmCpuid (
            0x80000001,
            &EAX_Reg,
            &EBX_Reg,
        ...
        HWSIG_TRACE((-1,"Soc Family, Model, Stepping ID = 0x%08x \n", EAX_Reg));  
        
        SocFamilyID = EAX_Reg & RAW_FAMILY_ID_MASK;           
        if (SocFamilyID != F18_DN_RAW_ID) {
            gST->ConOut->ClearScreen (gST->ConOut);
            CreatePopUp (EFI_LIGHTGRAY | EFI_BACKGROUND_BLUE, &Key, L"Hygon Dhyana SOC Family ID error. BIOS will cold reset !", NULL);
			gRT->ResetSystem (EfiResetCold, EFI_SUCCESS, 0, NULL);
        }
        
    } else {
        HWSIG_TRACE((-1,"Soc Vendor ID is not HygonGenuine \n"));
        gST->ConOut->ClearScreen (gST->ConOut);
        CreatePopUp (EFI_LIGHTGRAY | EFI_BACKGROUND_BLUE, &Key, L"Hygon Dhyana SOC Vendor ID error. BIOS will cold reset !", NULL);
		gRT->ResetSystem (EfiResetCold, EFI_SUCCESS, 0, NULL);
    }           
#endif
