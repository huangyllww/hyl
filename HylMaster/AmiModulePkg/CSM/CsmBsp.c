...
EFI_STATUS GetConsoleMode(BOOLEAN*);
    ...
    //
    {
        if (gVgaHandle != NULL)
        {
            ...
            if(i < Count) {
                ...
                if (EFI_ERROR(Status)) {
                    DEBUG((DEBUG_ERROR, "UEFI Video driver is connected: suppress video switching for legacy Option ROM.\n"));
                    *SetTxtSwitchingMode = 2;
                }
...
/**
    ...
    specific information related to NMI. By default the function reads port 0x61
    If either SERR NMI or IOCHK# NMI is disabled, function reports NMI disable.
    
    The default funcionality can be overridden by the OEM porting hook using eLink.
        
    @retval        NMI_information:
        BIT0    0 - NMI should be kept disabled
        BIT0    1 - enable NMI
    @retval        EFI_SUCCESS         Indication that NmiInfo is valid.
...
)
{
    ...
    // Read port 0x61 to see if NMI enabled for SERR and IOCHK#
    Data8 = IoRead8(0x61);
    Data8 &= 0xc;   // isolate BIT2 and BIT3
    *NmiInfo = (Data8 == 0)? 1 : 0;
...
/**
    This function is called from LegacyBios entry point and it returns the board
    specific switch to adjust HDD priorities before legacy boot. By default the
    adjustment is on; the purpose of this function is to be able to turn it off.
    
    The adjustment of the HDD connection order is implemented for the problems
    related to OS (Windows) installation. It is a known issue where OS may not
    be installed on a drive other than drive 0x80. Adjustment is needed to make
    this drive 0x80.

        
    @retval        HDD adjustment information:
        0 - HDD adjustment will be performed during legacy boot
        1 - HDD order adjustment will be suppressed
    @retval        EFI_SUCCESS
    @retval        EFI_UNSUPPORTED     Function is not supported, CSM will use
   									   the default HDD adjustment policy.

**/

EFI_STATUS
GetHddOrderAdjustmentInformation(
	OUT UINTN *HddAdjustmentInfo
)
{
	// Function will use BIOS Setup values to return the HDD adjustment policy
	*HddAdjustmentInfo = (UINTN)gSetup.HddOrderAdjustment;
...
/* By default this function returns EFI_UNSUPPORTED to make every Option ROM execute
    ...
    for (er = ExecutedRom; er->Seg | er->Bus | er->Dev | er->Fun; er--) {
        if (er->Bus == Bus && er->Dev == Dev) {
            DEBUG((DEBUG_ERROR, "CSM OPROM: skip OpROM execution for device B%x/d%x/F%x.\n",
                Bus, Dev, Func));
            return EFI_SUCCESS;
        }
    }
  ...
  )
{
    ...
    NumberOfPages = EFI_SIZE_TO_PAGES(sizeof (SMM_TABLE) + NUMBER_OF_CSM_SUPPORTED_SW_SMIS * sizeof(SMM_ENTRY));
    Address = 0xffffffff;
    ...
    SmmTable = (SMM_TABLE*)(UINTN)Address;
...
#pragma pack(pop)
    ...
    //
    do {
        DEBUG((DEBUG_INFO, "CsmBsp: Installing service ROM, %x...", RomImageStart));
