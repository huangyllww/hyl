...
BOOLEAN                      gMmioAddressUpdated = FALSE;

UINTN                        gUartInputClock=0;

UINT8      *gLegacySerialCommBuffer = NULL;
UINTN      gLegacySerialCommBufferSize;
...
)
{
    ...
    EbdaAddress = (UINT8*)((UINTN)EbdaSeg<<4);
    EbdaSize = (*EbdaAddress)<<10;
...
)
{
    ...
    BinSize = (UINT16)( ((gSreDirImageSize) >> 10) + ( (gSreDirImageSize & 0x3FF) ? 1 : 0));
    // Increase 4K in case of no VGA, to maintaing Temp Video buffer
    if(!IsVgaPresent()) {
        BinSize += 4;
    }
    BinSize = BinSize << 10;
    ...
    NewEbdaAdd = (EbdaSeg<<4)-BinSize;       
    NewEbdaSeg = NewEbdaAdd>>4;

    //We are taking memory for the Sredir.bin from the base memory. 
    //So adjust the Base Memory
    *(UINT16*)0x413 = ((*(UINT16*)0x413) - (BinSize >> 10));
    ...
    *(UINT16*)0x40e = (UINT16)NewEbdaSeg;
    ...
    // Report BinSize to csm16 as per calculated
    gAmiComParameters.SredirBinSize = (UINT16)BinSize;
...
/**
    ...
    Transfer the Mmio adress information to SMRAM area that would be used
	in SMI handler.  

    @param MmioBaseAddress 
...
VOID
TransferRedirectionDataToSmram (
...
)
{
    ...
    EFI_GUID                           gAmiLegacySredirCommunicationGuid 
                                            = AMI_LEGACY_SREDIR_COMMUNICATION_GUID;
    // Send only once
    if ( gMmioAddressUpdated ) {
        return;
    }
    ...
    CommHeader = (EFI_SMM_COMMUNICATE_HEADER *)gLegacySerialCommBuffer;
    pBS->CopyMem (&CommHeader->HeaderGuid, &gAmiLegacySredirCommunicationGuid, sizeof(gAmiLegacySredirCommunicationGuid));
    CommHeader->MessageLength = sizeof(MmioBaseAddress);
    pBS->CopyMem (&(CommHeader->Data[0]), &MmioBaseAddress, sizeof(MmioBaseAddress));

    // Transfer MMIO Address
    SmmStatus = SmmCommunication->Communicate (
                                 SmmCommunication,
                                 gLegacySerialCommBuffer,
                                 &gLegacySerialCommBufferSize
                                 );
    ASSERT_EFI_ERROR(SmmStatus);
    gMmioAddressUpdated = TRUE;
    return;
...
)
{
    ...
    if(gSreDirImageDataInit == FALSE) {
        ...
        // Pass Mmio base address to SMRAM
        TransferRedirectionDataToSmram(gAmiComParameters.MMIOBaseAddress);
...
/**
    ...
    Function to send MMIO Address, if already sent it does nothing.
	This is a dummy call with address 0 to make sure the SMI Handler is UnRegistered if not sent already.
...
VOID
EFIAPI
TransferMmioAddrtoSmram (
    IN  EFI_EVENT   Event,
    IN  VOID        *Context
)
{
    
   if(EFI_ERROR(GetSetupValuesForLegacySredir(&gAmiComParameters))) {
        TransferRedirectionDataToSmram(0);
        return;
    }
...
)
{
    ...
    // Buffer allocated for SMM Communication as new core doesn't allow buffers which are allocated after EndofDXE event.
    gLegacySerialCommBufferSize  = sizeof(EFI_GUID) + sizeof(UINTN) + sizeof(UINTN);   

    // Allocate Memory for Communication buffer.   
    Status = pBS->AllocatePool( EfiRuntimeServicesData,
                                gLegacySerialCommBufferSize,
                                (VOID**)&gLegacySerialCommBuffer );
