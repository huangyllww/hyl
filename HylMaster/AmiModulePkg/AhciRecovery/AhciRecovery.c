...
)
{
    ...
    // Dhyana SOC porting START
    // Assuming AHCI_ROOT_BRIDGE_LIST list is unchanged
    // 0x00, 0x08, 0x01},{0x10, 0x08, 0x01},{0x20, 0x08, 0x01},{0x30, 0x08, 0x01},{0x40, 0x08, 0x01},{0x50, 0x08, 0x01}, {0x60, 0x08, 0x01},{0x70, 0x08, 0x01}
    ...
    if(Index > 1){
        ...
        if(EntryCount > SystemDieCount){
            for(Index = SystemDieCount; Index < EntryCount; Index++){
                RootBridgeList[Index].Bus = 0xFF;
                RootBridgeList[Index].Dev = 0xFF;
                RootBridgeList[Index].Func = 0xFF;
            }
        }
		
        //Get and calculate DieNumberOerSocket
        (*PeiServices)->PciCfg->Read(PeiServices,(*PeiServices)->PciCfg,
                            EfiPeiPciCfgWidthUint8, EFI_PEI_PCI_CFG_ADDRESS(0, 0x18, 1, 0x200),&DieNumberPerSocket);
		DieNumberPerSocket = (UINT8)LowBitSet32((DieNumberPerSocket & 0x0F)+1);
		
        for (Index = 1; Index < SystemDieCount; Index++) {
			if (Index < DieNumberPerSocket) {
        		(*PeiServices)->PciCfg->Read(PeiServices,(*PeiServices)->PciCfg,
                    	EfiPeiPciCfgWidthUint8, EFI_PEI_PCI_CFG_ADDRESS(0, 0x18 + Index, 0, 0x84),&Bus);
			} else {
				(*PeiServices)->PciCfg->Read(PeiServices,(*PeiServices)->PciCfg,
                    	EfiPeiPciCfgWidthUint8, EFI_PEI_PCI_CFG_ADDRESS(0, 0x1C + (Index - DieNumberPerSocket), 0, 0x84),&Bus);
			}
			RootBridgeList[Index].Bus = Bus;
        }
    }
    // Dhyana SOC porting END
    for( Index=0; RootBridgeList[Index].Func != 0xff ; Index++) {
