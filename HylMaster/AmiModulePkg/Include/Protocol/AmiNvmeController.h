...
/// Defines NVMe device operational state
typedef enum {
    Functional=0,
    Fatal_UnRecovered,
    Fatal_Recovery_state
} NVME_DEVICE_STATE;
...
struct _AMI_NVME_CONTROLLER_PROTOCOL{
    ...
    NVME_DEVICE_STATE           DeviceState;

//  Flags used to reinitialize Nvme device when FATAL error occurs for the commands "SET_FEATURE, 
//  CREATE_IO_COMPLETION_QUEUE and CREATE_IO_SUBMISSION_QUEUE" respectively
    BOOLEAN                     SetNumberOfQueuesInProgress;
    BOOLEAN                     CreateIoCompletionQueueInProgress;
    BOOLEAN                     CreateIoSubmissionQueueInProgress;
...
//----------------------------Queue1 for Nvme Cmds------------------------------------------------------
  ...
  //-----------------------------------------------------------------------------------------------------  
    ...
    VOID                        *SupportedSpList;                    // Pointer to NVME_SP0_TRUSTED_RECEIVE_PARAMETER_DATA
