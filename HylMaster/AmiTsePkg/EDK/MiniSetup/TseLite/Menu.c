...
EFI_STATUS MenuDraw( MENU_DATA *menu )
{
    ...
    for( k = 0 ; k < menu->NumEntries ; k++ )
	{
        ...
        {
        		BoundaryLength=HiiFindStrPrintBoundary(text,(menu->MenuWidth-5));
        		SLength=EfiStrLen(L"...")*2;
        		if((BoundaryLength +SLength) >  length)
        		{
        			EfiStrCpy( &text[BoundaryLength-(BoundaryLength+SLength - length)],L"...");
        		}
        		else
