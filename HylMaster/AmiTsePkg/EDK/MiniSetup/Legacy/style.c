...
VOID OverRideStyleLabelCallback( FRAME_DATA *frame, LABEL_DATA *label, VOID *cookie )
{
    	...
    	switch(token)
	{
	   ...
	   case STRING_TOKEN(STR_SAVE_SHUTDOWN):
			_SubMenuAmiCallback( SAVE_AND_SHUTDOWN_VALUE ); 
			//HandleSaveAndReset();
			break;	

       case STRING_TOKEN(STR_SHUTDOWN):
			_SubMenuAmiCallback( DISCARD_AND_SHUTDOWN_VALUE ); 	
			//HandleResetSys();
			break;
       #endif		
       //<Chendl001-20191221 System can power off under BIOS Setup ->
