...
//Mizl-20220121-SZPZT#201 Workload Setting - Advanced P-State Setting+>
#include <Setup.h>
extern  EFI_BOOT_SERVICES *gBS;
extern  EFI_RUNTIME_SERVICES *gRT;
UINT8   gPStateSettingValue=0xFF;
void GetAdvancePStateSetting();
//Mizl-20220121-SZPZT#201 Workload Setting - Advanced P-State Setting->
/*----------------------------------------------------------------------------------------
...
UINT8 gAesSettingValue=0xFF ; //<hety-20220521 Add BIOS option to control AES. >
...
//<hety-20220521 Add BIOS option to control AES. +>
VOID
GetAesSetting () {
  EFI_STATUS   Status;
  ...
  Status = gRT->GetVariable(L"Setup", &gSetupGuid, NULL, &VariableSize, &SetupData);

  if(EFI_ERROR(Status)) {
    IDS_HDT_CONSOLE (CPU_TRACE, "Get AES Setting fail!!!\n");
    return;
  }

  gAesSettingValue = SetupData.AesMode;
  IDS_HDT_CONSOLE (CPU_TRACE, "Get AES Mode Setting = %x\n", gAesSettingValue);
}
//<hety-20220521 Add BIOS option to control AES. ->
  ...
  if (CcxIsBsp (&StdHeader)) {
    ...
    if (mPackageId == (1 << ZP_SOCKET_SP4)) {
        if ((mLogicalId.Family == HYGON_FAMILY_18) && (mLogicalId.Revision & HYGON_REV_F18_DN_Dx)) {
            NbioSmuServices->AmdSmuRegisterRead (NbioSmuServices, 0, HG4_OPNFUSEPKG_SP3R2, &OpnFuse);
            if ((OpnFuse & HG4_OPEN_FUSE_BIT) != 0) {
  ...
  )
{
  ...
  //Mizl-20220121-SZPZT#201 Workload Setting - Advanced P-State Setting+>
  GetAdvancePStateSetting();
  //Mizl-20220121-SZPZT#201 Workload Setting - Advanced P-State Setting->
  GetAesSetting(); //<hety-20220521 Add BIOS option to control AES. >
  ...
  )
{
  ...
  //<hety-20220521 Add BIOS option to control AES. +>
  //Control AES by AES Mode Option
  if (gAesSettingValue == 0) {
    AsmMsrAnd64(MSR_CPUID_FEATS, ~BIT57);
  }
  //<hety-20220521 Add BIOS option to control AES. ->
}
...
GetAdvancePStateSetting()
{
    ...
    VariableSize = sizeof(SETUP_DATA);
    Status = gRT->GetVariable(L"Setup", &gSetupGuid, NULL, &VariableSize, 
                              &SetupData);
    if(EFI_ERROR(Status)){
        IDS_HDT_CONSOLE (CPU_TRACE, "Get Advance P-State Setting fail!!!\n");
        return;
    }
    gPStateSettingValue = SetupData.ADVANCE_P_STATE;
    IDS_HDT_CONSOLE (CPU_TRACE, "Get Advance P-State Setting = %x\n",gPStateSettingValue);
