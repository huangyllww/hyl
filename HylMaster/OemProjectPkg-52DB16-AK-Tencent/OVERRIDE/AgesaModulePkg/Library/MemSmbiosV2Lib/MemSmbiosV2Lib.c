...
//<Fancf001-20201012 Distinguish SN of Unilc memory by Manufacturing Date +>
VOID
STATIC
IntToStringDEC (
  IN OUT   CHAR8 *String,
...
//<Lvshh001-20200911 Modify Memory SN follow Tencent spec V1.5 +> 
/**
  ...
  )
{
  ...
  UINT8                 VoltageMap;
  //<Lvshh001-20200911 Modify Memory SN follow Tencent spec V1.5 +>
  CHAR8                 TempString[23]={0};
  UINT8                 MfgSpecialData;
  UINTN                 StringBufferSize;
  UINT16                ManufacturerIdCode = 0;
  UINT32                Temp;  //<Fancf001-20201012 Distinguish SN of Unilc memory by Manufacturing Date>
  //<Lvshh001-20200911 Modify Memory SN follow Tencent spec V1.5 -> 
  UINT8 				DieCount = 0;
...
//  UINT32 ManufacturerIdCode;
  ...
  if (Socket == 0){
      ...
      default:
		  ...
		  */
		  T17->DeviceLocator[0] = 'C';
		  T17->DeviceLocator[1] = 'P';
		  T17->DeviceLocator[2] = 'U';
		  T17->DeviceLocator[3] = Socket + 0x30;
		  T17->DeviceLocator[4] = '_';
		  T17->DeviceLocator[5] = 'D';
		  T17->DeviceLocator[6] = 'I';
		  T17->DeviceLocator[7] = 'M';
		  T17->DeviceLocator[8] = 'M';
		  T17->DeviceLocator[9] = Channel + 0x41;
		  if (Dimm == 1) {
            T17->DeviceLocator[10] = 0 + 0x30;
		  } else if (Dimm == 0) {
		    T17->DeviceLocator[10] = 1 + 0x30;
		  } else {
		    T17->DeviceLocator[10] = Dimm + 0x30;
		  }
		    T17->DeviceLocator[11] = '\0';
      }
  }
  ...
  if (PhysicalDimm->DimmPresent) {
    ...
    PcdSet8(PcdSystemDramWidth, DimmSpd[SPD_DEVICE_WIDTH] & DEVICE_WIDTH_MASK); //<Malj-20181106 Fixed Mem Info Send to BMC> 
    PcdSet8(PcdSystemModuleType, DimmSpd[SPD_BASE_MODULE_TYPE] & MODULE_TYPE_MASK); //<Malj-20181106 Fixed Mem Info Send to BMC> 
    //<Malj-20181105 Add Mem Info Send to BMC ->
    // 3DS Support
    ...
    case 0x02:    // UDIMM
      ...
      break;
    case 0x04:    // LRDIMM
	//<Kangmm-20220128 Support for LRDIMM +> 
      //T17->TypeDetail.Registered = 1;
	  T17->TypeDetail.LRDIMM = 1;
	//<Kangmm-20220128 Support for LRDIMM -> 
      T17->FormFactor = DimmFormFactorFormFactor;
      break;
    case 0x03:    // SO-DIMM
    ...
    //liutz001-20200324 TX memory sn design change spd data from 325-328 to 320-328++
    //<Lvshh001-20200911 Modify Memory SN follow Tencent spec V1.5 +>
    ManufacturerIdCode = (DimmSpd[SPD_MANUFACTURER_ID_MSB] << 8) | DimmSpd[SPD_MANUFACTURER_ID_LSB];
    ManufacturerIdCode = ManufacturerIdCode & 0xFF7F;
    if (ManufacturerIdCode == 0x2C00) {
        StringBufferSize = (sizeof (CHAR8)) * 23;    
        IntToString (T17->SerialNumber, &DimmSpd[320], (sizeof (T17->SerialNumber) - 1) / 2);
        LibAmdMemFill(TempString, 0x00, StringBufferSize, StdHeader);
        AsciiStrCpyS(TempString, StringBufferSize, "S");
        AsciiStrCatS (TempString, StringBufferSize,T17->SerialNumber);
        LibAmdMemFill(T17->SerialNumber, 0x00, 20, StdHeader);
        AsciiStrCatS (T17->SerialNumber, 20,TempString );
        IDS_HDT_CONSOLE (MAIN_FLOW, "[OemDbg]T17->SerialNumber %a\n", T17->SerialNumber);
    } else if (ManufacturerIdCode == 0xCE00) {
        for (i=0;i<7;i++) {
            MfgSpecialData = DimmSpd[353+i];
            T17->SerialNumber[i] = FilterCharacter(MfgSpecialData);
        }
        T17->SerialNumber[i] = '\0';
        StringBufferSize = (sizeof (CHAR8)) * 23; 
        LibAmdMemFill(TempString, 0x00, StringBufferSize, StdHeader);
        
        IntToString (&(T17->SerialNumber[8]), &DimmSpd[323], 6);
        AsciiStrCatS (TempString, StringBufferSize,&(T17->SerialNumber[9]));
        AsciiStrCatS (T17->SerialNumber, 20,TempString );
        IDS_HDT_CONSOLE (MAIN_FLOW, "[OemDbg]T17->SerialNumber %a\n", T17->SerialNumber);
    } else if (ManufacturerIdCode == 0x1A08) {
        //<Fancf001-20201012 Distinguish SN of Unilc memory by Manufacturing Date +>
        if(DimmSpd[323] > 0x19) {
            IntToString (T17->SerialNumber, &DimmSpd[325], 4);
        } else {
            Temp = DimmSpd[325] | ((DimmSpd[326] << 8) & 0xFF00) | ((DimmSpd[327] << 16) & 0xFF0000) | ((DimmSpd[328] << 24) & 0xFF000000);
            IntToStringDEC(T17->SerialNumber, Temp, 8);
        }
        IDS_HDT_CONSOLE (MAIN_FLOW, "[OemDbg]T17->SerialNumber %a\n", T17->SerialNumber);
        //<Fancf001-20201012 Distinguish SN of Unilc memory by Manufacturing Date ->
    } else {
 ...
 *
 *  Description:
 *    Translate UINT32 to CHAR array.
 *
 *  Parameters:
 *    @param[in, out]    *String       Pointer to CHAR array
 *    @param[in]         Integer       UINT32
 *    @param[in]         SizeInByte    The size of CHAR array
 *
 *  Processing:
 *
 */
  ...
  )
{
  ...
  for (Index = 0; Index < SizeInByte; Index++) {
    for (N = 1, i = SizeInByte - Index; i > 1; i--) {
      N = N * 10;
    }
    *(String + Index) = Integer / N + 0x30;
    Integer = Integer % N;
  }
  *(String + SizeInByte) = 0x0;
}
//<Fancf001-20201012 Distinguish SN of Unilc memory by Manufacturing Date ->
