...
//<zhuhan-20180208 No Display AMI +>
...
CHAR16 *gMainTitle =  L" ";//<Lvshh001-20200421 Add token for Setup interface style,0 is normal(AMI default) ,1 is EzportPlus(Byo) >
CHAR16 *gMainCopyright = L" ";
//<zhuhan-20180208 No Display AMI +>
#define STR_SIZE   0x200  //<Lvshh001-20200415 Sinicize TITLE info>
...
VOID OverrideTitleString(void)
{
	...
	//<Lvshh001-20200415 Add token for Setup interface style,0 is normal(AMI default) ,1 is EzportPlus(Byo) +>
	UINTN  Size = STR_SIZE;
	CHAR16 StrBuf[STR_SIZE]=L"";
	//<Lvshh001-20200415 Add token for Setup interface style,0 is normal(AMI default) ,1 is EzportPlus(Byo) ->
	...
	for (; *Supportedlanguage != '\0'; ) 
	{
		...
		HiiLibGetString(gHiiHandle, STRING_TOKEN(STR_MAIN_TITLE_AK), &Size, StrBuf);
        #else
		HiiLibGetString(gHiiHandle, STRING_TOKEN(STR_MAIN_TITLE), &Size, StrBuf);
		#endif
		HiiChangeStringLanguage( gHiiHandle, STR_MAIN_TITLE,Language ,StrBuf );
