  ...
  )
{
  ...
  switch (Action) {
    ...
    case EFI_BROWSER_ACTION_CHANGED:
      switch (QuestionId) {
      case KEY_OC_MODE:
      case KEY_FIX_PSTATE: //<hujf1001-20200512 Add Fix P state feature +>
  ...
  )
{
  ...
  if (SocFamilyID != F15_BR_RAW_ID) {
    ...
    ValueSize = 0;
    DataStream = NULL;
    PrepareDbgTypeAttrib (&AttributeSize, (APCB_PARAM_ATTRIBUTE *)DataStream, (CBS_CONFIG *)IfrData);
    PrepareDbgTypeValue (&ValueSize, DataStream, (CBS_CONFIG *)IfrData);
    TotalSizeExt = AttributeSize + ValueSize;
    TotalSizeExt += ((ALIGN_SIZE_OF_TYPE - (TotalSizeExt % ALIGN_SIZE_OF_TYPE)) & (ALIGN_SIZE_OF_TYPE - 1));

    if (0 != TotalSizeExt) {
      DEBUG ((EFI_D_INFO, "Replace CBS DBG Options\n"));
      // Allocate buffer for new Type data stream
      DataStream = NULL;
      DataStream = AllocateZeroPool (TotalSizeExt);
      ASSERT (DataStream != NULL);
      ...
      PrepareDbgTypeAttrib (&AttributeSize, (APCB_PARAM_ATTRIBUTE *)DataStream, (CBS_CONFIG *)IfrData);
      ValueStream = &DataStream[AttributeSize];
      PrepareDbgTypeValue (&ValueSize, ValueStream, (CBS_CONFIG *)IfrData);

      Status = mApcbDxeServiceProtocol->ApcbUpdateCbsData (mApcbDxeServiceProtocol, DataStream, TotalSizeExt, TRUE);
      if (EFI_ERROR (Status)) {
        ASSERT (FALSE);
      }
