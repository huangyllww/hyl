 ...
 * @file
 *
 * NbioTopMemZP - Set top of memory for NBIO
 *
 *
...
#define FILECODE        NBIO_NBIOBASE_AMDNBIOBASEZPPEI_NBIOTOPMEMZP_FILECODE
  ...
  ///@todo  if (AmdMemoryInfoHob != NULL) {
    if (FALSE) {
      ...
      for (Index = 0; Index < AmdMemoryInfoHob->NumberOfDescriptor; Index++) {
        ...
        if ((Range->Attribute & AMD_MEMORY_ATTRIBUTE_AVAILABLE) == AMD_MEMORY_ATTRIBUTE_AVAILABLE) {
          if (Range->Base == 0) {
            D0F0_090.Field.TOP_OF_DRAM = (UINT32)RShiftU64 ((Range->Size & 0xFF800000), 23);     //Keep bits 31:23
  ///@todo - set ranges for TOM2 and TOM3 if availabe in AMD_MEMORY_INFO_HOB
  //        } else if (Range->Base > 0xFFFFFFFF) {
          }
        }
      }
    } else {
    ...
    if ((MsrData & BIT21) != 0) {
      ...
      if ((MsrData & (UINT64)0x0000FFFFFF800000) > ((UINT64)0x0000010000000000)) {
        // If TOP_MEM2 is above 1TB, enable GNB TOM2 and TOM3
        // Set TOM2 for below 1TB limit
        GnbTom2 = 0x000000FD00000000;
        // TOM3 is INCLUSIVE, so set it to TOM2 - 1 using bits 47:23
        GnbTom3 = (MsrData & (UINT64)0x0000FFFFFF800000) - 1;
      } else {
        // If TOP_MEM2 is below 1TB, set TOM2 using bits 39:23
        GnbTom2 = MsrData & (UINT64)0x000000FFFF800000;       //Keep bits 39:23
        // If TOP_MEM2 is below 1TB, disable GNB TOM3
        GnbTom3 = 0;
      }
    }
    IDS_HDT_CONSOLE (GNB_TRACE, "GnbSetTomZP setting GnbTom2 to 0x%lx\n", GnbTom2);
    IDS_HDT_CONSOLE (GNB_TRACE, "GnbSetTomZP setting GnbTom3 to 0x%lx\n", GnbTom3);
    if (GnbTom2 != 0) {
    ...
    if (GnbTom3 != 0) {
      ...
      NB_MMIOBASE.Value = (UINT32) RShiftU64 ((GnbTom3 + 1), 16);
      // Set TOM2 to all 1's
