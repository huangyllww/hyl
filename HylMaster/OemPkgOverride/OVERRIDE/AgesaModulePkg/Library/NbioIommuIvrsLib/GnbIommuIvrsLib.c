...
//<Eip#491126: Assert when multiple LSI iMR enabled cards were installed on Rome+>
#include <Protocol/HostMemoryReportProtocol.h>
...
//<Eip#491126: Assert when multiple LSI iMR enabled cards were installed on Rome+>
UINT16
OemSpecGetHandleCount ();

EFI_STATUS
OemSpecIommuIvrsCollectLegacyInfo (
        EFI_HOST_MEMORY_REPORT_INFO     *LsiHostMemoryInfo
);

EFI_STATUS
OemSpecIommuIvrsCollectUefiInfo (
        EFI_HOST_MEMORY_REPORT_INFO     *LsiHostMemoryInfo
);
//<Eip#491126: Assert when multiple LSI iMR enabled cards were installed on Rome->
...
OemSpecGetHandleCount ()
{
    ...
    IDS_HDT_CONSOLE (MAIN_FLOW, "OemSpecGetHandleCount entered  \n");
    ...
    Status = gBS->LocateHandleBuffer (
                 ...
                 (UINTN*)&HandleCount,
                 &HandleBuffer );
    IDS_HDT_CONSOLE (MAIN_FLOW, "UEFI HandleCount: %X \n", HandleCount);

    if ( EFI_ERROR(Status) ) {
        IDS_HDT_CONSOLE (MAIN_FLOW, "LocateHandleBuffer gEfiHostMemoryReportProtocolGuid. Status: %r \n", Status);
        ...
        Status = gBS->LocateProtocol (
                    ...
                    NULL,
                    &LegacyBios );
        IDS_HDT_CONSOLE (MAIN_FLOW, "LocateProtocol gEfiLegacyBiosProtocolGuid. Status: %r \n", Status);
        if ( EFI_ERROR(Status) ) {
            IDS_HDT_CONSOLE (MAIN_FLOW, "gEfiLegacyBiosProtocolGuid. Status: %r \n", Status);
            return 0;
        }
        ...
        RegSet.H.AH = 0xDE;
        RegSet.H.AL = 0;
        RegSet.E.EDX = 0xABBADADA;
    
        Int86Status = LegacyBios->Int86 (
                        LegacyBios, 
                        0x15, 
                        &RegSet );
        IDS_HDT_CONSOLE (MAIN_FLOW, "LegacyBios->Int86. OemSpecGetControllerCount Int86Status: %X \n", Int86Status);
        IDS_HDT_CONSOLE (MAIN_FLOW, "RegSet.X.Flags.CF: %X \n", RegSet.X.Flags.CF);
        IDS_HDT_CONSOLE (MAIN_FLOW, "RegSet.H.AH: %X \n", RegSet.H.AH);
        IDS_HDT_CONSOLE (MAIN_FLOW, "RegSet.X.CX: %X \n", RegSet.X.CX);
    
        if ( Int86Status || RegSet.X.Flags.CF ) {
            IDS_HDT_CONSOLE (MAIN_FLOW, "OEM SPEC Function OemSpecGetControllerCount Status: %r \n", EFI_UNSUPPORTED);
            return 0;
        }
        ...
        IDS_HDT_CONSOLE (MAIN_FLOW, "Legacy HandleCount: %X \n", HandleCount);
    }
...
)
{
    ...
    //<Eip#491126: Assert when multiple LSI iMR enabled cards were installed on Rome +>
    EFI_HOST_MEMORY_REPORT_INFO             *TempPtr = NULL;
    EFI_HOST_MEMORY_REPORT_INFO             *TempPtr2 = NULL;
    //<Eip#491126: Assert when multiple LSI iMR enabled cards were installed on Rome ->
    ...
    IDS_HDT_CONSOLE (MAIN_FLOW, "OemSpecIommuIvrsCollectLegacyInfo entered  \n");
    ...
    TempPtr = AllocateZeroPool(HandleCount * sizeof (EFI_HOST_MEMORY_REPORT_INFO));
    
    //<Eip#491126: Assert when multiple LSI iMR enabled cards were installed on Rome +>
    if (TempPtr == NULL) {
      IDS_HDT_CONSOLE (MAIN_FLOW, "AllocateZeroPool TempPtr Failed!!\n");   
    ...
    for (Index = 0; Index < HandleCount; Index ++) {
        ...
        Int86Status = LegacyBios->Int86 (
                        ...
                        0x15, 
                        &RegSet );
        IDS_HDT_CONSOLE (MAIN_FLOW, "LegacyBios->Int86. OemSpecGetMemoryReqBDFInfo Int86Status: %X \n", Int86Status);
        IDS_HDT_CONSOLE (MAIN_FLOW, "RegSet.X.Flags.CF: %X \n", RegSet.X.Flags.CF);
        IDS_HDT_CONSOLE (MAIN_FLOW, "RegSet.H.AH: %X \n", RegSet.H.AH);
        IDS_HDT_CONSOLE (MAIN_FLOW, "RegSet.X.CX: %X \n", RegSet.X.CX);
        IDS_HDT_CONSOLE (MAIN_FLOW, "RegSet.E.EBX: %X \n", RegSet.E.EBX);
        IDS_HDT_CONSOLE (MAIN_FLOW, "RegSet.E.ESI: %X \n", RegSet.E.ESI);

        if ( Int86Status || RegSet.X.Flags.CF ) {
            IDS_HDT_CONSOLE (MAIN_FLOW, "OEM SPEC Function OemSpecGetMemoryReqBDFInfo Status: %r \n", EFI_DEVICE_ERROR);
            continue;
        }
        ...
        IDS_HDT_CONSOLE (MAIN_FLOW, "HostMemoryInfo.Size: %lX \n", HostMemoryInfo.Size);

        //
        // Allocate 32 bit memory for HOST
        //
        SizeinPages = EFI_SIZE_TO_PAGES(HostMemoryInfo.Size);
        IDS_HDT_CONSOLE (MAIN_FLOW, "SizeinPages: %lX \n", SizeinPages);
        ...
        IDS_HDT_CONSOLE (MAIN_FLOW, "gBS->AllocatePages  Status: %r \n", Status);
        if ( EFI_ERROR(Status) ) {
            IDS_HDT_CONSOLE (MAIN_FLOW, "Failed to allocate memory for Host! \n");
            continue;
        }

        IDS_HDT_CONSOLE (MAIN_FLOW, "HostMemoryInfo.HostMemoryAddress: %lX \n", HostMemoryInfo.HostMemoryAddress);
        IDS_HDT_CONSOLE (MAIN_FLOW, "HostMemoryInfo.Bus: %X \n", HostMemoryInfo.Bus);
        IDS_HDT_CONSOLE (MAIN_FLOW, "HostMemoryInfo.Device: %X \n", HostMemoryInfo.Device);
        IDS_HDT_CONSOLE (MAIN_FLOW, "HostMemoryInfo.Func: %X \n", HostMemoryInfo.Func);
        ...
        Int86Status = LegacyBios->Int86 (
                        ...
                        0x15, 
                        &RegSet );
        IDS_HDT_CONSOLE (MAIN_FLOW, "LegacyBios->Int86. OemSpecSetMemory Int86Status: %X \n", Int86Status);
        IDS_HDT_CONSOLE (MAIN_FLOW, "RegSet.X.Flags.CF: %X \n", RegSet.X.Flags.CF);
        IDS_HDT_CONSOLE (MAIN_FLOW, "RegSet.H.AH: %X \n", RegSet.H.AH);

        if ( Int86Status || RegSet.X.Flags.CF ) {
            IDS_HDT_CONSOLE (MAIN_FLOW, "OEM SPEC Function OemSpecSetMemory Status: %r \n", EFI_DEVICE_ERROR);
            continue;
        }
        ...
        CopyMem(TempPtr2, &HostMemoryInfo, sizeof(EFI_HOST_MEMORY_REPORT_INFO));
        
        TempPtr2++; //<Eip#491126: Assert when multiple LSI iMR enabled cards were installed on Rome >
        gDevCount++;
    }

    IDS_HDT_CONSOLE (MAIN_FLOW, "ReturnSuccess: %x \n", ReturnSuccess);

    if ( ReturnSuccess ) {
        CopyMem(LsiHostMemoryInfo, TempPtr, gDevCount * sizeof(EFI_HOST_MEMORY_REPORT_INFO));
        Status = EFI_SUCCESS;
    } else {
        Status = EFI_UNSUPPORTED;
    }

    IDS_HDT_CONSOLE (MAIN_FLOW, "OemSpecIommuIvrsCollectLegacyInfo exiting  Status: %r \n", Status);

    //<Eip#491126: Assert when multiple LSI iMR enabled cards were installed on Rome +>
errExit:
      if(TempPtr)
          gBS->FreePool(TempPtr);
    //<Eip#491126: Assert when multiple LSI iMR enabled cards were installed on Rome ->
...
)
{
    ...
    UINTN                                   HandleCount;
    EFI_HANDLE                              *HandleBuffer = NULL;//<Eip#491126: Assert when multiple LSI iMR enabled cards were installed on Rome >
    UINTN                                   Index;
    BOOLEAN                                 ReturnSuccess = FALSE;

    IDS_HDT_CONSOLE (MAIN_FLOW, "OemSpecIommuIvrsCollectUefiInfo entered  \n");
    ...
    Status = gBS->LocateHandleBuffer (
                 ...
                 &HandleBuffer );
    IDS_HDT_CONSOLE (MAIN_FLOW, "LocateHandleBuffer gEfiHostMemoryReportProtocolGuid. Status: %r \n", Status);
    IDS_HDT_CONSOLE (MAIN_FLOW, "HandleCount: %X \n", HandleCount);

    if ( EFI_ERROR(Status) ) {
        IDS_HDT_CONSOLE (MAIN_FLOW, "LocateHandleBuffer gEfiHostMemoryReportProtocolGuid. Status: %r \n", Status);
        goto errExit; //<Eip#491126: Assert when multiple LSI iMR enabled cards were installed on Rome >
    }
    ...
    //<Eip#491126: Assert when multiple LSI iMR enabled cards were installed on Rome +>
    if (TempPtr == NULL) {
      IDS_HDT_CONSOLE (MAIN_FLOW, "TempPtr is NULL, ERROR!!\n");
      Status = EFI_OUT_OF_RESOURCES;
      goto errExit;
    }
    //<Eip#491126: Assert when multiple LSI iMR enabled cards were installed on Rome ->
    ...
    for (Index = 0; Index < HandleCount; Index ++) {
        ...
        Status = gBS->HandleProtocol (
                    ...
                    &gEfiHostMemoryReportProtocolGuid,
                    &HostMemoryReport );
        IDS_HDT_CONSOLE (MAIN_FLOW, "HandleProtocol gEfiHostMemoryReportProtocolGuid. Status: %r \n", Status);
        if ( EFI_ERROR(Status) ) {
            IDS_HDT_CONSOLE (MAIN_FLOW, "HandleProtocol gEfiHostMemoryReportProtocolGuid. Status: %r \n", Status);
            continue;
        }
        ...
        IDS_HDT_CONSOLE (MAIN_FLOW, "sizeof (HostMemoryInfo): %d \n", sizeof (HostMemoryInfo));
        //
        ...
        Status = HostMemoryReport->GetInfo (
                    HostMemoryReport,
                    &HostMemoryInfo );
        IDS_HDT_CONSOLE (MAIN_FLOW, "HostMemoryReport->GetInfo. Status: %r \n", Status);
        if ( EFI_ERROR(Status) ) {
            IDS_HDT_CONSOLE (MAIN_FLOW, "HostMemoryReport->GetInfo. Status: %r \n", Status);
            continue;
        }
    ...
    IDS_HDT_CONSOLE (MAIN_FLOW, "OemSpecIommuIvrsCollectUefiInfo exiting  Status: %r \n", Status);
  ...
  )
{
  ...
  EFI_HOST_MEMORY_REPORT_INFO         *LsiMemInfo=NULL;
  EFI_HOST_MEMORY_REPORT_INFO         *LsiMemInfotemp=NULL;
  UINT8                               i;
  
  IOMMU_EXCLUSION_RANGE_DESCRIPTOR    *LsiExclusionRangeList=NULL;
  IOMMU_EXCLUSION_RANGE_DESCRIPTOR    *LsiExclusionRangeListtemp=NULL;
  
  UINT16                               HandleCount;
  AMD_LATE_PARAMS                     *LsiParamsPtr=NULL;
  UINT8                               StartBusNumber;
  UINT8                               EndBusNumber;
  
  gDevCount = 0;
  HandleCount = OemSpecGetHandleCount();
  
  if (HandleCount > 0) {
      LsiMemInfo = AllocateZeroPool(HandleCount * sizeof (EFI_HOST_MEMORY_REPORT_INFO));
      
      if (LsiMemInfo == NULL) {
        IDS_HDT_CONSOLE (MAIN_FLOW, "LsiMemInfo is NULL, ERROR!!\n");
        Status = AGESA_ERROR;
        goto errExit;
      }
      LsiMemInfotemp=LsiMemInfo;
      
      EfiStatus = OemSpecIommuIvrsCollectUefiInfo(LsiMemInfotemp);
      if ( EFI_ERROR(EfiStatus) ) {
          EfiStatus = OemSpecIommuIvrsCollectLegacyInfo(LsiMemInfotemp);
      }
  ...
  LsiParamsPtr = AllocateZeroPool(sizeof(AMD_LATE_PARAMS));
  if (LsiParamsPtr == NULL) {
    IDS_HDT_CONSOLE (MAIN_FLOW, "LsiParamsPtr is NULL, ERROR!!\n");
    Status = AGESA_ERROR;
    goto errExit;
  }
  //<Eip#491126: Assert when multiple LSI iMR enabled cards were installed on Rome ->
  ...
  LateParamsPtr = (AMD_LATE_PARAMS*) NULL; //<Eip#491126: Assert when multiple LSI iMR enabled cards were installed on Rome +>
  IvrsSupport = FALSE;
  Counter = 0;
//<Eip#491126: Assert when multiple LSI iMR enabled cards were installed on Rome +>
#if 0
  ...
  if (Ivrs == NULL) {
    IDS_HDT_CONSOLE (MAIN_FLOW, "%a() - The locate Ivrs buffer fail!\n", __FUNCTION__);
    return AGESA_ERROR;
  }
#endif
  Ivrs = NULL;
  ...
  LsiExclusionRangeList = AllocateZeroPool((gDevCount+1) * sizeof (IOMMU_EXCLUSION_RANGE_DESCRIPTOR));

  //<Eip#491126: Assert when multiple LSI iMR enabled cards were installed on Rome +>
  if (LsiExclusionRangeList == NULL) {
    IDS_HDT_CONSOLE (MAIN_FLOW, "LsiExclusionRangeList is NULL, ERROR!!\n");
    Status = AGESA_ERROR;
    goto errExit;
  }
  
  LsiExclusionRangeListtemp = LsiExclusionRangeList;
  //<Eip#491126: Assert when multiple LSI iMR enabled cards were installed on Rome ->
  ...
  while (GnbHandle != NULL) {
    if (GnbCheckIommuPresent (GnbHandle, (AMD_CONFIG_PARAMS *)NULL)) {
      ...
      IvrsSupport = TRUE;
      gListCount = 0; //<Eip#491126: Assert when multiple LSI iMR enabled cards were installed on Rome >
      ...
      GnbCreateIvrsEntry (GnbHandle, IvrsIvhdBlock10h, Ivrs, EfrAndMask, EfrOrMask, (AMD_CONFIG_PARAMS *)NULL);
      GnbCreateIvrsEntry (GnbHandle, IvrsIvhdBlock11h, Ivrs, EfrAndMask, EfrOrMask,(AMD_CONFIG_PARAMS *)NULL);
      GnbBuildIvmdList (IvrsIvmdBlock, Ivrs, (AMD_CONFIG_PARAMS *)NULL);
      if (IvrsRelativeAddrNamesSupport) {
        GnbCreateIvrsEntry (GnbHandle, IvrsIvhdrBlock40h, Ivrs, EfrAndMask, EfrOrMask, (AMD_CONFIG_PARAMS *)NULL);
        GnbCreateIvrsEntry (GnbHandle, IvrsIvhdrBlock41h, Ivrs, EfrAndMask, EfrOrMask, (AMD_CONFIG_PARAMS *)NULL);
        GnbBuildIvmdList (IvrsIvmdrBlock, Ivrs, (AMD_CONFIG_PARAMS *)NULL);
      }
      
      //<Eip#491126: Assert when multiple LSI iMR enabled cards were installed on Rome +>
      // and check bus/address
      Status = GnbFmGetBusDecodeRange (GnbHandle, &StartBusNumber, &EndBusNumber, NULL);
      ASSERT (Status == AGESA_SUCCESS);
      
      LsiExclusionRangeListtemp = LsiExclusionRangeList;
      ZeroMem(LsiExclusionRangeListtemp, (gDevCount+1) * sizeof (IOMMU_EXCLUSION_RANGE_DESCRIPTOR));
      LsiMemInfotemp=LsiMemInfo;
      
      for (i = 0; i < gDevCount; i++) {
          if (LsiMemInfotemp != NULL) {
              if (LsiMemInfotemp->HostMemoryAddress != 0) {
                  if (LsiMemInfotemp->Bus >= StartBusNumber && LsiMemInfotemp->Bus <= EndBusNumber) {
                      LsiExclusionRangeListtemp->Flags = 0;
                      LsiExclusionRangeListtemp->RangeBaseAddress = LsiMemInfotemp->HostMemoryAddress;
                      LsiExclusionRangeListtemp->RangeLength = LsiMemInfotemp->Size;
                      LsiExclusionRangeListtemp->RequestorIdStart.Bus = (UINT16)LsiMemInfotemp->Bus;
                      LsiExclusionRangeListtemp->RequestorIdStart.Device = (UINT16)LsiMemInfotemp->Device;
                      LsiExclusionRangeListtemp->RequestorIdStart.Function = (UINT16)LsiMemInfotemp->Func;
                      LsiExclusionRangeListtemp->RequestorIdEnd.Bus = (UINT16)LsiMemInfotemp->Bus;
                      LsiExclusionRangeListtemp->RequestorIdEnd.Device = (UINT16)LsiMemInfotemp->Device;
                      LsiExclusionRangeListtemp->RequestorIdEnd.Function = (UINT16)LsiMemInfotemp->Func;
                      LsiExclusionRangeListtemp++;
                      gListCount++;
                  }
              }
              LsiMemInfotemp++; 
      ...
      if (gListCount > 0) {
          ...
          LsiExclusionRangeListtemp->Flags = DESCRIPTOR_TERMINATE_LIST;      
          LsiParamsPtr->IvrsExclusionRangeList = (IOMMU_EXCLUSION_RANGE_DESCRIPTOR*)LsiExclusionRangeList;
          
          IDS_HDT_CONSOLE (MAIN_FLOW, "Build IVMD List \n");
          
          //GnbBuildIvmdList (IvrsIvmdBlockSingle, Ivrs, (AMD_CONFIG_PARAMS *)LsiParamsPtr);
          GnbBuildIvmdList (IvrsIvmdBlock, Ivrs, (AMD_CONFIG_PARAMS *)LsiParamsPtr);
      }
      // AMI PORTING
      //<Eip#491126: Assert when multiple LSI iMR enabled cards were installed on Rome ->
    }
  ...
  //<Eip#491126: Assert when multiple LSI iMR enabled cards were installed on Rome +>
errExit:
  if(LsiExclusionRangeList)
     gBS->FreePool(LsiExclusionRangeList);
  if(LsiMemInfo) 
     gBS->FreePool(LsiMemInfo);
  if (LsiParamsPtr)  
     gBS->FreePool(LsiParamsPtr);
  
  if(Ivrs) gBS->FreePool(Ivrs);
  //<Eip#491126: Assert when multiple LSI iMR enabled cards were installed on Rome ->
  ...
  )
{
  ...
  UINT8                             Index = 0;//<Eip#491126: Assert when multiple LSI iMR enabled cards were installed on Rome +>
...
#endif
  if (gListCount > 0) {
      IDS_HDT_CONSOLE (GNB_TRACE, "Get exclusion list \n");
      IvrsExclusionRangeList = ((AMD_LATE_PARAMS*)StdHeader)->IvrsExclusionRangeList;
  } else {
      IvrsExclusionRangeList = NULL;
  }
  ...
  if (IvrsExclusionRangeList != NULL) {
    ...
    while ((IvrsExclusionRangeList->Flags & DESCRIPTOR_TERMINATE_LIST) == 0) {
      if ((IvrsExclusionRangeList->Flags & DESCRIPTOR_IGNORE) == 0) {
          IDS_HDT_CONSOLE (GNB_TRACE, "IvrsExclusionRangeList->Flags = %x \n", IvrsExclusionRangeList->Flags);
          IDS_HDT_CONSOLE (GNB_TRACE, "Adding IVMD entry\n");
      ...
      Index++;//<Eip#491126: Assert when multiple LSI iMR enabled cards were installed on Rome +>
    }
  }
