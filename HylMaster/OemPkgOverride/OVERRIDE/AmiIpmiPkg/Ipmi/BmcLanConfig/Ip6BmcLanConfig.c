  ...
  IN EFI_SYSTEM_TABLE       *SystemTable )
{
    ...
    //
    Status = GetIp6Supported (BMC_LAN1_CHANNEL_NUMBER,&(gServerMgmtConfiguration.Ip6Support1));
    DEBUG ((EFI_D_INFO, "IPMI : BMC LAN1 Ip6 configuration Status: %r\n", Status));    if (EFI_ERROR(Status)) {
       gServerMgmtConfiguration.Ip6Support1 = 0;
    }
    SERVER_IPMI_DEBUG ((EFI_D_INFO, "gServerMgmtConfiguration.Ip6Support1 %d \n", gServerMgmtConfiguration.Ip6Support1));
    if (gServerMgmtConfiguration.Ip6Support1) {
    //
    ...
    //<ChengXW001-20171024 65N32 Mantis #15154, IPV6 Function do not work well ->
    ...
    // Check if Ipv6 supported or not.
    //
    Status = GetIp6Supported (BMC_LAN2_CHANNEL_NUMBER,&(gServerMgmtConfiguration.Ip6Support2));
    DEBUG ((EFI_D_INFO, "IPMI: BMC LAN2 Ip6 configuration Status: %r\n", Status));
    //<ChengXW001-20171024 65N32 Mantis #15154, IPV6 Function do not work well +>
    if (EFI_ERROR(Status)) {
       gServerMgmtConfiguration.Ip6Support2 = 0;
    }
      SERVER_IPMI_DEBUG ((EFI_D_INFO, "gServerMgmtConfiguration.Ip6Support2 %d \n", gServerMgmtConfiguration.Ip6Support2));
    if (gServerMgmtConfiguration.Ip6Support2) {
        //
        // Setting to unspecified since request is processed.
        //
  ...
  IN  UINT8      IsStaticSource )
{
    ...
    //<ChengXW001-20171024 65N32 Mantis #15154, IPV6 Function do not work well, Confirmed with BMC +>
  ...
  )
{
    ...
    //<ChengXW001-20171024 65N32 Mantis #15154, IPV6 Function do not work well, Confirmed with BMC +>
    //DataSize = sizeof (SetParamCommand) - 1; // Since the Last data is read only parameter.
    DataSize = sizeof (SetParamCommand); 
    //<ChengXW001-20171024 65N32 Mantis #15154, IPV6 Function do not work well, Confirmed with BMC ->
  ...
  OUT  UINT8      *GetIPV6Status )
{
    ...
    if (!EFI_ERROR(Status)) {
        if(ResponseData.Data1 == EFI_BMC_IPV6_DIS){
            SERVER_IPMI_DEBUG ((DEBUG_ERROR, "IPMI : Ipv6 Support Disabled \n"));
            *GetIPV6Status = IPV6_SUPPORT_DISABLED;
        //<hujf1001-20201127 Fix IPV6 can't be enable +>
        } else if ((ResponseData.Data1 == EFI_BMC_IPV4_DIS) || (ResponseData.Data1 == EFI_BMC_IPV_4_6_SUPP)) {
            SERVER_IPMI_DEBUG ((DEBUG_ERROR, "IPMI : Ipv6 Support Enabled \n"));
            *GetIPV6Status = IPV6_SUPPORTED;
        //<hujf1001-20201127 Fix IPV6 can't be enable ->
        }
    }
    SERVER_IPMI_DEBUG ((EFI_D_INFO, "%a Exiting...\n", __FUNCTION__));
