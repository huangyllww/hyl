...
//*** AMI PORTING BEGIN ***//
#include <Token.h>
// Structures needed for saving performance data in S3 resume
#if FPDT_BUILD
  ...
  )
{
  ...
  if (Facs->XFirmwareWakingVector != 0) {
    if ((Facs->Version == EFI_ACPI_4_0_FIRMWARE_ACPI_CONTROL_STRUCTURE_VERSION) &&
		...
		//*** AMI PORTING BEGIN ***//
        ((Facs->OspmFlags & EFI_ACPI_4_0_OSPM_64BIT_WAKE__F) != 0)) {//OspmFlags field should be used instead of Flags
		//*** AMI PORTING END ***//
...
//*** AMI PORTING BEGIN ***//
#if FPDT_BUILD
// Functions needed for saving performance data in S3 resume
...
VOID FillFpdtOnS3()
{
	...
	if (!EFI_ERROR (Status))
	{
	    if (((PERF_TAB_HEADER*)(UINT8*)(FpdtVar->S3Pointer))->Signature == 0x54503353)
	    {
	        ...
	        {
	        	NanoTime = DivU64x32 (MultU64x64 (GetPerformanceCounter (), FpdtVar->NanoFreq), 1000000);
	            
	            S3PerRec->AverageResume = DivU64x32 ((MultU64x32 (S3PerRec->AverageResume, S3PerRec->ResumeCount) + NanoTime), 
	                                                        (S3PerRec->ResumeCount + 1));
  ...
  )
{
  ...
  if (PeiS3ResumeState->ReturnStatus != EFI_SUCCESS) {
    ...
    //*** AMI PORTING BEGIN ***//	 
    DEBUG (( EFI_D_ERROR, "BootScript Executor Returned an Error! Reseting System!\r\n"));
    {
    	//Reseting System, because BootScript Executed with errors
          ...
          if ((AcpiResetVar.AccessSize == 2) && (AcpiResetVar.Type < 2) && (AcpiResetVar.Addess < 0xffffffff))
          {
        	  ...
        	  else 
        	  {
        		  ...
        		  UINT32 MemoryMappedReset = *(UINT32*)AcpiResetVar.Addess;
        		  *(UINT32*)AcpiResetVar.Addess = MemoryMappedReset & ~(0x1c00);
  ...
  //*** AMI PORTING BEGIN ***//  
#if FPDT_BUILD
  FillFpdtOnS3(); //Saving performance data
#endif
  //*** AMI PORTING END *****//
