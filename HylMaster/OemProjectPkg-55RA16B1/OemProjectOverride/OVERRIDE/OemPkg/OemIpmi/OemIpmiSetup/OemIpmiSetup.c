...
//<huangjin001-20190427 Remove fan profile function + > 
/*
EFI_STATUS
...
)
{
  ...
  if(!EFI_ERROR(Status)) {
    Status = GetSetACPowerLossPolicyFromStdIPMI(IpmiTransportProtocol,TRUE,(UINT8*)&ServerMgmtConfiguration.AcPowerCtl);
//<huangjin001-20190427 Remove fan profile function >       Status = GetSetFanCtrlModeFromStdIPMI(IpmiTransportProtocol,TRUE,(UINT8*)&ServerMgmtConfiguration.FanCtl_Mode);
  }
...
//<Lvshh001-20190621 Network SRIOV Function modified by OOB ->
  ...
  // Type 0 -- Cpu
  //-----------------------------------------------------------------
  pIpmiSetupData->CpuOptions->SmtMode                   = NvDataSetup.SmtMode;
...
//<huangjin001-20191018 Fix build error for svm mode +>    
  ...
  // Type 4 -- Thermal
  //-----------------------------------------------------------------
  //<huangjin001-20190427 Remove fan profile function >     pIpmiSetupData->ThermalOptions->FanCtl_Mode           = NvDataServerMgmtSetup.FanCtl_Mode;
  ...
  //zanggy-20200918-Fix Console Ridirection(SOL) sync with BMC fail+>
  pIpmiSetupData->ManagementOptions->ConsoleRedirection = NvDataSetup.ConsoleRedirectionEnable[SOL_COM_PORT];
  pIpmiSetupData->ManagementOptions->TerminalType       = NvDataSetup.TerminalType[SOL_COM_PORT];
  pIpmiSetupData->ManagementOptions->BaudRate           = NvDataSetup.BaudRate[SOL_COM_PORT] - 3;
  pIpmiSetupData->ManagementOptions->FlowControl        = NvDataSetup.FlowControl[SOL_COM_PORT];
  pIpmiSetupData->ManagementOptions->DataBits           = NvDataSetup.DataBits[SOL_COM_PORT] - 7;
  pIpmiSetupData->ManagementOptions->Parity             = NvDataSetup.Parity[SOL_COM_PORT] - 1;
  ...
  switch (NvDataSetup.StopBits[SOL_COM_PORT]) {
  ...
  )
{
  ...
  if (!EFI_ERROR(Status)) {
    ...
    if (pIpmiSetupData->ValidIpmiData[OemIpmiSetupCpuOptions]) {
      OPTION_PUT(NvDataSetup.SmtMode,            pIpmiSetupData->CpuOptions->SmtMode,                UpdatedFlag);
//<huangjin001-20191018 Fix build error for svm mode +>      
...
//<huangjin001-20191018 Fix build error for svm mode ->           
    ...
    if (pIpmiSetupData->ValidIpmiData[OemIpmiSetupManagementOptions]) {
      OPTION_PUT(NvDataSetup.ConsoleRedirectionEnable[SOL_COM_PORT], pIpmiSetupData->ManagementOptions->ConsoleRedirection, UpdatedFlag);
      OPTION_PUT(NvDataSetup.TerminalType[SOL_COM_PORT],    pIpmiSetupData->ManagementOptions->TerminalType,    UpdatedFlag);
      OPTION_PUT(NvDataSetup.BaudRate[SOL_COM_PORT],        pIpmiSetupData->ManagementOptions->BaudRate + 3,    UpdatedFlag);
      OPTION_PUT(NvDataSetup.FlowControl[SOL_COM_PORT],     pIpmiSetupData->ManagementOptions->FlowControl,     UpdatedFlag);
      OPTION_PUT(NvDataSetup.DataBits[SOL_COM_PORT],        pIpmiSetupData->ManagementOptions->DataBits + 7,    UpdatedFlag);
      OPTION_PUT(NvDataSetup.Parity[SOL_COM_PORT],          pIpmiSetupData->ManagementOptions->Parity + 1,      UpdatedFlag);

      if (OPTION_VALIDATE(pIpmiSetupData->ManagementOptions->StopBits)) {
        OPTION_PUT(NvDataSetup.StopBits[SOL_COM_PORT], pIpmiSetupData->ManagementOptions->StopBits, UpdatedFlag);
        switch (NvDataSetup.StopBits[SOL_COM_PORT]) {
          // IPMI Value         BIOS SETUP Value          Stop bits
          // 0                  1                         1
          // 1                  3                         2
          case 0:  NvDataSetup.StopBits[SOL_COM_PORT] = 1;  break;
          case 1:  NvDataSetup.StopBits[SOL_COM_PORT] = 3;  break;
        }
      }
    }
    //zanggy-20200918-Fix Console Ridirection(SOL) sync with BMC fail->
    ...
    if (pIpmiSetupData->ValidIpmiData[OemIpmiSetupBootOptions]) {
      ...
      //<Lvshh001-20200811 Realizes the linkage of 'Boot option filter' and 'Boot mode select' +>
#if defined(FIXED_BOOT_ORDER_SUPPORT) && (FIXED_BOOT_ORDER_SUPPORT == 1)
      if (NvDataSetup.BootOptionFilter == 0){ // Boot Option Filter set to "Uefi and Legacy" 
          NvDataSetup.BootMode = 2;
          OemOpROMEN[VIDEOINDX] = 2;
