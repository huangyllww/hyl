...
CPU_OEM_SETUP_CALLBACK* CpuOemSetupCallbackList[] = \
                                            {CPU_OEM_SETUP_CALLBACK_LIST NULL};
...
)
{
    ...
    if(EFI_ERROR(Status))
    {
        ...
        CpuSetupData->PSTATE_CTRL = 0;
        CpuSetupData->PPC_CTRL = 0;
        CpuSetupData->SVM_MODE = 1;
        CpuSetupData->NX_MODE = 1;

#if CORE_LEVELING_SUPPORT
        CpuSetupData->CORE_LEVELING = 0;
#endif

        CpuSetupData->C6Mode = 1;
        CpuSetupData->CpbMode = 0;
...
/*        CpuSetupData->PNOW_ASL_MODE = SetupData.PNOW_ASL_MODE;
        CpuSetupData->PSTATE_CTRL = SetupData.PSTATE_CTRL;
...
#if CORE_LEVELING_SUPPORT
        CpuSetupData->CORE_LEVELING = SetupData.CORE_LEVELING;
#endif
/*
        CpuSetupData->C6Mode = SetupData.C6Mode;
        CpuSetupData->CpbMode = SetupData.CpbMode;*/
