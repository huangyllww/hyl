  ...
  )
{
  ...
  //Ram Added
  EFI_MP_SERVICES_PROTOCOL   *gEfiMpServiceProtocol;
  //Ram added ends
  ...
  //Ram changes to show cpu count
  Status = gBS->LocateProtocol (
                  &gEfiMpServiceProtocolGuid,
                  NULL,
                  (VOID **) &gEfiMpServiceProtocol
                  );
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  gEfiMpServiceProtocol->GetNumberOfProcessors(gEfiMpServiceProtocol,
      &NumberOfCPUs, &NumberOfEnabledProcessors);

  ShellPrintHiiEx(0, gST->ConOut->Mode->CursorRow++, NULL, STRING_TOKEN (STR_PROCESSOR_COUNT), ShellInfoObject.HiiHandle, NumberOfCPUs);
  gST->ConOut->Mode->CursorRow++;
  //Ram changes to show cpu count ends
  //
