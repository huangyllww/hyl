...
When SMBIOS_PRESERVE_NVRAM token is disable, OEM engineer can set OEMSETUP_SUPPORT token
...
#endif
#include "SaveBiosConfAfterAfuFlash.h"
#include "../Ofbd.h"
...
BOOLEAN   IpmitoolSendFlag = TRUE;
static EFI_IPMI_TRANSPORT   *gIpmiTransport;

EFI_STATUS SetBIOSSaveFlagIpmiCommand(UINT8 SetBIOSSaveFlag){
    EFI_STATUS  Status;
    //UINT8                                               SetBIOSSaveFlag;
    ...
    Status = gIpmiTransport->SendIpmiCommand (
                             ...
                             NETFN_OEM,
                             BMC_LUN,
                             CMD_OEM_SET_SAVE_BIOS_CONFIG_FLAG, 
                             &SetBIOSSaveFlag,
                             1,
                             ResponseData,
                             &ResponseDataSize
        ...
        DEBUG((DEBUG_INFO,"SetBIOSSaveFlagIpmiCommand Locate IpmiTransport\n"));
    }
    ...
    IN OUT OFBD_HDR *pOFBDHdr)
{
	...
	SETUPStructPtr = (OFBD_TC_56_SETUP_STRUCT *)((UINT8 *)pOFBDHdr + sizeof(OFBD_HDR) + sizeof(OFBD_EXT_HDR)); 
    ...
    IN OUT OFBD_HDR *pOFBDHdr)
{
    ...
    //OEM add ...
	 SetBIOSSaveFlagIpmiCommand(1);
	 IpmitoolSendFlag=FALSE;
    if (!EFI_ERROR(Status))
		SETUPStructPtr->dbRetSts =  OFBD_TC_SETUP_OK;
	else
...
/**
	...
	@retval	0xFE means Function error occurred
*/ 
    ...
    IN OUT UINT8        *pOFBDDataHandled )
{
    ...
    if(*pOFBDDataHandled == 0)
    {
        ...
        else if(pOFBDHdr->OFBD_FS ==0){
            IpmitoolSendFlag=TRUE; //SMI First entry;so we will send not save flag to BMC
        }
        if(IpmitoolSendFlag){
            SetBIOSSaveFlagIpmiCommand(0);
            IpmitoolSendFlag=FALSE;
        }
    }
