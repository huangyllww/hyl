...
#define PStateMode(Mode) "Advanced P-State",Mode,PSTATE_NORMAL,"Normal",PSTATE_PERFORMANCE,"Performance",InvalidSetting,"NULL"
...
WORKLOAD_TUNING_MODE WorkLoadTuning[] = {
    //Balanced Mode
    {
        BalancedMode, PStateMode(PSTATE_PERFORMANCE), DeterminismSlider(DETERMINISM_SLIDER_PERFORMANCE), cTDPControl(CTDP_AUTO),
    ...
    {
        PowerEfficiencyMode, PStateMode(PSTATE_NORMAL), DeterminismSlider(DETERMINISM_SLIDER_POWER), cTDPControl(CTDP_AUTO),
    ...
    {
        HighPerformanceMode, PStateMode(PSTATE_PERFORMANCE), DeterminismSlider(DETERMINISM_SLIDER_PERFORMANCE), cTDPControl(CTDP_MANUAL),
        SmtMode(SMT_ENABLE), IOMMU(IOMMU_DISABLE), SVM(SVM_DISABLE), SRIOV(SRIOV_DISABLE), CorePerformanceBoost(PERFORMANCE_BOOST_AUTO), GlobalCState(GLOBAL_C_DISABLED)},
    //Virtualization Mode
    {
        VirtualizationMode, PStateMode(PSTATE_PERFORMANCE), DeterminismSlider(DETERMINISM_SLIDER_PERFORMANCE), cTDPControl(CTDP_AUTO),
    ...
    //BIOSDefaultSetting
    {
        BiosDefaultMode, PStateMode(DEFAULT_ADVANCED_PSTATE_POLICY), DeterminismSlider(DEFAULT_DETERNIBISM_SLIDER_POLICY), cTDPControl(DEFAULT_CTDP_CONTROL_POLICY),
    ...
    //Manual Mode
    {
        ManualMode, PStateMode(DEFAULT_ADVANCED_PSTATE_POLICY), DeterminismSlider(DEFAULT_DETERNIBISM_SLIDER_POLICY), cTDPControl(DEFAULT_CTDP_CONTROL_POLICY),
        SmtMode(DEFAULT_SMT_CONTROL_POLICY), IOMMU(DEFAULT_CBS_IOMMU), SVM(DEFAULT_SVM_CONTROL_POLICY), SRIOV(DEFAULT_PCIBUS_ONEOF_SRIOV), CorePerformanceBoost(DEFAULT_CORE_PERFORMANCE_BOOST), GlobalCState(DEFAULT_GLOBAL_C_STATE_CONTROL)},
};
#endif