...
//<hety-20220921 Support sync BIOS vendor by fru +>
#if defined(SYNC_BIOS_VENDOR_FORM_FRU)&&(1 == SYNC_BIOS_VENDOR_FORM_FRU)
    SMBIOS_TABLE_TYPE0              *Type0Structure;//<Lvshh002-20190420 Update Smbios Type 0 Vendor from Type 1 Manufacturer>////dengyh_20220401_change BIOS Vendor to Hygon
#endif
//<hety-20220921 Support sync BIOS vendor by fru ->
...
//<hety-20220921 Support sync BIOS vendor by fru +>
#if defined(SYNC_BIOS_VENDOR_FORM_FRU)&&(1 == SYNC_BIOS_VENDOR_FORM_FRU)  
    //dengyh_20220401_change BIOS Vendor to Hygon+
    //<Lvshh002-20190420 Update Smbios Type 0 Vendor from Type 1 Manufacturer +>
    //
    // Smbios Type 0 structure process start
    //
    ...
    if (EFI_ERROR(StatusOfRequestedSmbiosType)) {
        //
        // Type 0 structure is not available, needs to be created.
        //
        Type0Structure = AllocateZeroPool (sizeof (SMBIOS_TABLE_TYPE0) + SIZE_OF_SMBIOS_STRUCTURE_END_MACRO);
        if (Type0Structure == NULL) {
            return;
        }
        Type0Structure->Hdr.Type = EFI_SMBIOS_TYPE_SYSTEM_INFORMATION;
        Type0Structure->Hdr.Length = sizeof (SMBIOS_TABLE_TYPE0);
        Type0Structure->Hdr.Handle = 0xFFFF;//To be updated by SMBIOS driver.
    }else {
        Type0Structure = (SMBIOS_TABLE_TYPE0 *)Record;
    }
    Type0Structure->Vendor = 0x01;
    
    //
    // Creating the Smbios Type 0 Structure.
    //
    if (EFI_ERROR (StatusOfRequestedSmbiosType)) {
        SmbiosHandle = SMBIOS_HANDLE_PI_RESERVED;
        Status = SmbiosProtocol->Add (
                     ...
                     NULL,
                     &SmbiosHandle,
                     (EFI_SMBIOS_TABLE_HEADER*) Type0Structure);
    ...
    SmbiosHandle = Type0Structure->Hdr.Handle;
    StringNumber = 0x01;
...
//<hety-20220921 Support sync BIOS vendor by fru ->
    ...
    Type1Structure->SKUNumber = 0x05;//<ChengXW001-20170927 Update Smbios Type1 SKU number>
    Type1Structure->Family = 0x06;//<huangjin001-20200305 Update Type1 Family and Type2 LocationInChassis>
    //
    // Copy the Uuid
    //
    ...
    //
    // Type 1 Product Part Number String
    //
    SmbiosHandle = Type1Structure->Hdr.Handle;
    StringNumber = 0x05;
    UnicodeString = (CHAR16 *) PcdGetPtr (PcdSystemProductPartNumber);
    if ( StrCmp (UnicodeString, PcdDefaultString) == 0x00 ) {
        AsciiStrCpy (AsciiString, SmBiosDefaultString);
    } else {
        GetValidSMBIOSString (AsciiString, UnicodeString);
    ...
    //<huangjin001-20200305 Update Type1 Family and Type2 LocationInChassis +>
    SmbiosHandle = Type1Structure->Hdr.Handle;
    StringNumber = 0x06; 
    switch(PcdGet8 (PcdChassisType))
    {
    case 0x17:    
        UnicodeString = (CHAR16 *) L"Rack";   
        break;
    case 0x1c:    
        UnicodeString = (CHAR16 *) L"Blade";
        break;
    default:
        UnicodeString = (CHAR16 *) L"Blade";
      break;
    ...
    Type2Structure->LocationInChassis = 0x06;    //<huangjin001-20200305 Update Type1 Family and Type2 LocationInChassis >
    ...
    SmbiosHandle = Type2Structure->Hdr.Handle;
    StringNumber = 0x06; 
    UnicodeString = (CHAR16 *) L"To be filled by O.E.M.";      
    if ( StrCmp (UnicodeString, PcdDefaultString) == 0x00 ) {
        AsciiStrCpy (AsciiString, SmBiosDefaultString);
    } else {
        GetValidSMBIOSString (AsciiString, UnicodeString);
