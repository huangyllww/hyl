...
// Register SetVirtualAddressMap () notify function
    ...
    if (EFI_ERROR (Status)) {
        DEBUG((DEBUG_ERROR, "Create Event to gEfiEventVirtualAddressChangeGuid is Failed\n"));
    }
...
// Get the Current time
        if ((pRS != NULL) && (pRS->GetTime != NULL)) {
            ...
            Status = pRS->GetTime (&EfiTimeStamp, NULL);
        ...
        if (Status != EFI_SUCCESS) {
            ...
            #if OEMRtc_Enabled
            Status = GpnvRtcRead (&EfiTimeStamp);
            #endif

        }

        ConvertTimetoBCD (&EfiTimeStamp);

        if ((EFI_ERROR(Status)) || ((EfiTimeStamp.Year < 0x1980) || (EfiTimeStamp.Year > 0x2079))) {

            CurrentTime.Month  = 1;
            CurrentTime.Day    = 1;
            CurrentTime.Year   = 0x80;
            CurrentTime.Hour   = 0;
            ...
            CurrentTime.Year   = (UINT8)EfiTimeStamp.Year;
            CurrentTime.Month  = EfiTimeStamp.Month;
            CurrentTime.Day    = EfiTimeStamp.Day;
            CurrentTime.Hour   = EfiTimeStamp.Hour;
            CurrentTime.Minute = EfiTimeStamp.Minute;
            CurrentTime.Second = EfiTimeStamp.Second;
