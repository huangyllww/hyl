  ...
  )
{
  ...
  NbioSmuServices->SmuRegisterRead (NbioSmuServices, SystemDieCount, CCX_BIST_EN, &BistEnFuse);
  BistEn = (BistEnFuse >> BIST_EN_BITS) & 0x1;

  NbioSmuServices->SmuRegisterRead (NbioSmuServices, SystemDieCount, CCX_COLD_RESET_MBIST_EN, &ColdResetMBistEnFuse);
  ColdResetMBistEn = (ColdResetMBistEnFuse >> COLD_RESET_MBIST_EN_BITS) & 0x1;
  ...
  )
{
  ...
  if ((LogicalId.Family == HYGON_FAMILY_18) && (LogicalId.Revision & HYGON_REV_F18_DN_Dx)) {
      NbioSmuServices->SmuRegisterRead (NbioSmuServices, DieNumber, HYGON_DX_MP0_RUN_MBIST, &RunMBistFuse);
      RunMBist = (RunMBistFuse >> HYGON_DX_RUN_MBIST_FUSE_BITS) & 0x1;
  } else {
      NbioSmuServices->SmuRegisterRead (NbioSmuServices, DieNumber, MP0_RUN_MBIST, &RunMBistFuse);
      RunMBist = (RunMBistFuse >> RUN_MBIST_FUSE_BITS) & 0x1;
  }
