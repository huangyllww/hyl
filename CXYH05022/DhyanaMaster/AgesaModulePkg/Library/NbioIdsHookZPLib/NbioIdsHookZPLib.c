  ...
  )
{
  ...
  IDS_NV_READ_SKIP (IDSNVID_CMNC_TDP_CTL, &IdsNvValue) {
    ...
    case IDSOPT_CMNC_TDP_CTL_MANUAL:
      IDS_NV_READ_SKIP (IDSNVID_CMNC_TDP_LIMIT, &IdsNvValue) {
        PcdSet32S (PcdAmdcTDP, (UINT32)IdsNvValue);
      }
      break;
  ...
  // CLCK Control
  IDS_NV_READ_SKIP (IDSNVID_CMN_NBIO_LCLK_FREQ, &IdsNvValue) {
	switch (IdsNvValue) {
	case IDSOPT_CMM_NBIO_LCLK_FREQ_AUTO :
	case IDSOPT_CMM_NBIO_LCLK_FREQ_1000MHz:
	case IDSOPT_CMM_NBIO_LCLK_FREQ_889MHz: 
	case IDSOPT_CMM_NBIO_LCLK_FREQ_800MHz:
	case IDSOPT_CMM_NBIO_LCLK_FREQ_727MHz:
	case IDSOPT_CMM_NBIO_LCLK_FREQ_667MHz:
	case IDSOPT_CMM_NBIO_LCLK_FREQ_615MHz: 
	case IDSOPT_CMM_NBIO_LCLK_FREQ_571MHz: 
	case IDSOPT_CMM_NBIO_LCLK_FREQ_533MHz:
	case IDSOPT_CMM_NBIO_LCLK_FREQ_500MHz:   
	  PcdSet8S (PcdNbioLclkFrequency, (UINT8)IdsNvValue);
	  break;
	default:
	  ASSERT (FALSE);
	  break;
  ...
  )
{
  ...
  IDS_NV_READ_SKIP (IDSNVID_DBG_POISON_CONSUMPTION, &IdsNvValue) {
    if (IdsNvValue != IDSOPT_DBG_POISON_CONSUMPTION_AUTO) {
      PcdSetBoolS (PcdAmdNbioPoisonConsumption, (BOOLEAN)IdsNvValue);
    }
  }
  ...
  IDS_NV_READ_SKIP (IDSNVID_DBG_RAS_CONTROL, &IdsNvValue) {
    if (IdsNvValue != IDSOPT_DBG_RAS_CONTROL_AUTO) {
      PcdSetBoolS (PcdAmdNbioRASControl, (BOOLEAN)IdsNvValue);
    }
  }
