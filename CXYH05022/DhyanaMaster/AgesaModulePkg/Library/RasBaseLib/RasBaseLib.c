 ...
 *----------------------------------------------------------------------------------------
 */
#define FILECODE LIBRARY_RASBASELIB_RASBASELIB_FILECODE
  ...
  )
{
  ...
  if (numInterleaveBits > 0) {
    ...
    tempAddrB = ((UINTN)csId << intLvAddrBit);
  ...
  )
{
  ...
  RegIndex = SmnAddress;
  PciAddress = (IohcBus << 20) + NB_SMN_INDEX_2;
  PciWrite32 (PciAddress, RegIndex);
  PciAddress = (IohcBus << 20) + NB_SMN_DATA_2;
  *Value = PciRead32 (PciAddress);
}
