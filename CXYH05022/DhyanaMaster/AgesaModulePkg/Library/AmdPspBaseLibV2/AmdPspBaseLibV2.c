    ...
    if (FirmwareTableBase->Signature  == FIRMWARE_TABLE_SIGNATURE) {
      ...
      case F18_DN_RAW_ID:
        ...
        if (LogicalId.Revision & HYGON_REV_F18_DN_Ax) {
          *BiosDirBase = FirmwareTableBase->Soc0BiosDirBase;
          PspChipId = SOC0_PSP_CHIP_ID;
        } else if (LogicalId.Revision & HYGON_REV_F18_DN_Bx) {
          *BiosDirBase = FirmwareTableBase->Soc1BiosDirBase;
          PspChipId = SOC1_PSP_CHIP_ID;
        } else if (LogicalId.Revision & HYGON_REV_F18_DN_Cx) {
          *BiosDirBase = FirmwareTableBase->Soc2BiosDirBase;
          PspChipId = SOC2_PSP_CHIP_ID;
        } else if (LogicalId.Revision & HYGON_REV_F18_DN_Dx) {
          *BiosDirBase = FirmwareTableBase->Soc3BiosDirBase;
          PspChipId = SOC3_PSP_CHIP_ID;
        }
        else {
          ASSERT(FALSE);
          return FALSE;
