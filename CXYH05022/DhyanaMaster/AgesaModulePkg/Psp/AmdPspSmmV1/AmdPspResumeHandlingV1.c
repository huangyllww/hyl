 ...
 * @e project:      AGESA
 * @e sub-project:  PSP
 * @e \$Revision: 317842 $   @e \$Date: 2015-04-29 00:06:52 -0700 (Wed, 29 Apr 2015) $
 */
/*****************************************************************************
 *
 * 
...
#define FILECODE PSP_AMDPSPSMMV1_AMDPSPRESUMEHANDLINGV1_FILECODE
  ...
  )
{
  ...
  PspResumeServiceCallBackV1 (ResumeFromConnectedStandby);
}
  ...
  )
{
  ...
  PspResumeServiceCallBackV1 ( ResumeFromS3);
  ...
  )
{
  if (IsS0i3Resume ()) {
    ...
    if (!mCapsuleUpdateFlag) {
      ...
      IDS_HDT_CONSOLE_PSP_TRACE ("AGESA Capsule Flag Set\n");      
      return ResumeFromS3;
    }
  }
}
  ...
  )
{
  ...
  if (++ApExitS3Count >= (gSmst->NumberOfCpus)) {
    ApExitS3Count = 0;
    PciAddress = MAKE_SBDFO (0, 0, 0x18, 5, 0x204);
    do {
      PspMboxBiosClearS3Status (mPspMboxSmmBuffer, mPspMboxSmmFlagAddr);
      S0i3Ctrl = PciRead32 (PciAddress);
    } while ((S0i3Ctrl & BIT2) != 0);
  }
}
  ...
  )
{
  ...
  //Get LocalApicId:[31:24]
  return Cpuid.EBX_Reg >> 24;
}
  ...
  )
{
  UINT32    CuStatus;
  UINTN     PciAddress;
  //Read DualCore information from D18F5x80.DualCore
  PciAddress = MAKE_SBDFO (0, 0, 0x18, 5, 0x80);
  CuStatus = PciRead32 (PciAddress);
  return ((CuStatus >> 16) & 0xFF);
}

VOID
SetCombineCr0Cd (
  IN       BOOLEAN                CombineCr0Cd
  )
{
  //MSR C001_102B[49] CombineCr0Cd
  AsmMsrBitFieldAndThenOr64 (0xC001102B, 49, 49, 0, (CombineCr0Cd ? 1 : 0));
}
  ...
  )
{
  ...
  AGESA_TESTPOINT (TpPspSmmV1ApRsmEntry, NULL);
  IDS_HDT_CONSOLE_PSP_TRACE ("PSP.ApSmmResumeVector\n");
  //Clear CombineCr0Cd on Core 0, Set on Core 1
  SetCombineCr0Cd (GetCoreApicId () % 2);
  ResumeType = GetResumeType (); //Check resume type is S3 or connected standby

  if (ResumeType == ResumeFromS3) {
    ClearS3Exit ();
    AGESA_TESTPOINT (TpPspSmmV1ApRsmExit, NULL);
    
    GetApSyncFlagAddress(&ApSyncFlagAddress);
    (UINT32)ApSyncFlag = ApSyncFlagAddress;
...
{
  UINT8 ResumeType;
  AGESA_TESTPOINT (TpPspSmmV1BspRsmEntry, NULL);
  IDS_HDT_CONSOLE_PSP_TRACE ("PSP.BspSmmResumeVector\n");

  ResumeType = GetResumeType (); //Check resume type is S3 or connected standby
  //Set CombineCr0Cd to 0, if dual core in Compute unit 0
  if ((GetDualCore () & BIT0) == 1) {
    SetCombineCr0Cd (0);
  }
  if (ResumeType == ResumeFromS3) {
    ...
    ClearS3Exit ();
    PspS3ResumeHandling ();
    AGESA_TESTPOINT (TpPspSmmV1BspRsmExit, NULL);
    //Clear Sleep Type to force S5 Path, Specail value is set in RBX to identify CapsuleUpdate mode
    if (mCapsuleUpdateFlag) {
      FchWriteSleepType (0);
    }
    S3ExitToBiosV1 ();
