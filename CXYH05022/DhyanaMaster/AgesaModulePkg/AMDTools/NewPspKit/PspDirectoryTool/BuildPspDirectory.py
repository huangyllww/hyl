...
#                         Type:   DIR, Entry Type    ShortName    Full description  Modifiable 
                          ...
                          0x38: ['PSP' ,'POINT_ENTRY','PspSecureFwData', 'Psp Security Firmware Data', 1],
                          0x40: ['PSP' ,'POINT_ENTRY','PspDirLv2', 'Point to PSP level 2 directory', 0],
                          0x49: ['PSP' ,'IMAGE_ENTRY','PspSecureFw', 'Psp Security Firmware', 1],
                          0x5e: ['PSP' ,'IMAGE_ENTRY','PspTpcmFw', 'Psp Tpcm Firmware', 1],
                          ...
                          0x19023: ['BIOS','IMAGE_ENTRY','UCodePatch','Microcode Patch', 0],
...
ProgramTable = {  
  ...
  'HYGONAX': {"PSPID": 0xBC090000, "BIOSDIR_OFFSET": 6},
  'HYGONBX': {"PSPID": 0xBC090200, "BIOSDIR_OFFSET": 7},
  'HYGONCX': {"PSPID": 0xBC090201, "BIOSDIR_OFFSET": 8},
  'HYGONDX': {"PSPID": 0xBC090300, "BIOSDIR_OFFSET": 9},
}
...
def IsPspSecureFwDataEntry(entry):
  if ((entry['Entry'] == 'POINT_ENTRY') and (entry['Type'] == 0x38)) :
    return True
  return False

def IsPointFileEntry(entry):
  if ((entry['Entry'] == 'POINT_ENTRY') and (entry['File'] != None)) :
    return True
  return False
...
def GetOutEntryFileBaseName (entry, DirStr, Level):
  if (IsImageEntry (entry)):
    ...
    #Use constant name for RTM signature
      ...
      return "ResetImage"
  elif (IsPspSecureFwDataEntry(entry)):
...
def ParseArg ():
  ...
  DPParser.add_argument ("-p", "--program", help="Specify the program name, Valid choices: HYGONAX, HYGONBX, HYGONCX, HYGONDX", required=True)
  ...
  if (args.subcommand == 'dp'):
    if args.program.upper() not in ["ZP", "RV", "HYGONAX", "HYGONBX", "HYGONCX", "HYGONDX"]:
      log.info ("[Error] Not a valid program string, only HYGONAX & HYGONBX & HYGONCX & HYGONDX are supported")
      FatalErrorExit ()
...
def CheckComboEntries(Entries):
  ...
  for entry in Entries:
    if entry['Id'] == ProgramTable['HYGONBX']['PSPID']:
      IsHygonBx = True
      HygonBxEntryIndex = Index
    if entry['Id'] == ProgramTable['HYGONCX']['PSPID']:
      IsHygonCx = True
      HygonCxEntryIndex = Index
    Index += 1
  if (IsHygonBx and IsHygonCx) and (HygonBxEntryIndex < HygonCxEntryIndex):
    Entries[HygonBxEntryIndex], Entries[HygonCxEntryIndex] = Entries[HygonCxEntryIndex], Entries[HygonBxEntryIndex]
    ...
    for PspDirectoryInfo in PspDirectoryInfos:
      ...
      for entry in PspDirectoryInfo['Entries']:
        ...
        if (IsValueEntry (entry)):
          ...
          EntryDict['Size'] = entry ['Size']
          EntryDict['Address'] = entry ['Address']  
          if (IsPspSecureFwDataEntry(entry)): 
            EntryDict['File'] = OutputPath + entry ['File']
    ...
    if (Args.subcommand == 'bd'):
      ...
      if (len(PspDicts) != 0):
        for PspDict in PspDicts:
          ...
          # Process each PSP entry
          for Entry in PspDict['Entries']:
            if (IsImageEntry(Entry) or IsPointFileEntry(Entry)):
