  ...
  )
{
  ...
  for (PortNum = 0; PortNum < TAISHAN_SATA_PORT_NUM; PortNum++) {
    FchSmnRW (DieBusNum, FCH_TS_SATA_AOAC_CONTROL, ~(UINT32) (BIT0 << PortNum), 0x00, LocalCfgPtr->StdHeader);
  }
  ...
  if ( LocalCfgPtr->FchReset.SataEnable ) {
    FchSmnRW (DieBusNum, FCH_TS_SATA_MISC_CONTROL, 0xfffffffe, 0, LocalCfgPtr->StdHeader);
  } else {
    FchSmnRW (DieBusNum, FCH_TS_SATA_MISC_CONTROL, 0xfffffffe, BIT0, LocalCfgPtr->StdHeader);
    FchSmnRW (DieBusNum, FCH_TS_SATA_MISC_CONTROL, ~(UINT32) BIT11, BIT11, LocalCfgPtr->StdHeader);
    return;
  }
  ...
  //
  for (PortNum = 0; PortNum < TAISHAN_SATA_PORT_NUM; PortNum++) {
    FchTSSataInitPortOffline (DieBusNum, PortNum, FchDataPtr);
  }
}
