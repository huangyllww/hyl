  ...
  )
{
  ...
  if ( LocalCfgPtr->Sata.SataMode.SataEnable == 0 ) {
    FchSmnRW (0, FCH_SMN_SATA_NBIF_STRAP0, ~(UINT32)BIT28, 0, StdHeader);
  ...
  SataEnableWriteAccessTS (0, LocalCfgPtr);
  ...
  if (( LocalCfgPtr->Sata.SataClass == SataAhci7804) || (LocalCfgPtr->Sata.SataClass == SataAhci )) {
    FchInitEnvSataAhciTS ( 0, FchDataPtr );
  }

  if ( LocalCfgPtr->Sata.SataClass == SataRaid) {
    FchInitEnvSataRaidTS ( 0, FchDataPtr );
  }
  ...
  SataDisableWriteAccessTS (0, LocalCfgPtr);
}
