...
GetBarXSize(
 ...
 OUT UINT32 *BarXSizeLo,
 OUT UINT32 *BarXSizeHi
)
{
  if (NTBBarSizeIndex > 0x13 || BarXSizeLo == NULL) {
    *BarXSizeLo = 0;
    *BarXSizeHi = 0;
    return;
  }
  if (NTBBarSizeIndex < 0xc ) {
    *BarXSizeLo = (1 << NTBBarSizeIndex) * 1024 * 1024 - 1;
    if (BarXSizeHi != NULL)
      *BarXSizeHi = 0;
  } else {
    *BarXSizeLo = 0xFFFFFFFF;
	if (BarXSizeHi != NULL)
      *BarXSizeHi = (1 << (NTBBarSizeIndex - 0xc)) - 1;
  }
}
/**
        ...
        while (GnbHandle != NULL) {
          if (GnbBuildOptionsZPDataHob->CfgNTBLocation == GnbHandle->InstanceId) {
            ...
            if (GnbBuildOptionsZPDataHob->CfgNTBMode == 1) {
              ...
              GetBarXSize(GnbBuildOptionsZPDataHob->CfgNTBBAR1Size, &PhysicalBar1Size, NULL);
			  GetBarXSize(GnbBuildOptionsZPDataHob->CfgNTBBAR23Size, &PhysicalBar23SizeLo, &PhysicalBar23SizeHi);
			  GetBarXSize(GnbBuildOptionsZPDataHob->CfgNTBBAR45Size, &PhysicalBar45SizeLo, &PhysicalBar45SizeHi);

			  IDS_HDT_CONSOLE (GNB_TRACE, " NTB BAR SIZE: BAR1:%x - BAR23LO:%x - BAR23Hi:%x - BAR45LO:%x - BAR45HI:%x\n",
                      PhysicalBar1Size,PhysicalBar23SizeLo,PhysicalBar23SizeHi,PhysicalBar45SizeLo,PhysicalBar45SizeHi);
			  //BAR LMT SET
              NbioRegisterRMW (GnbHandle,
                                TYPE_SMN,
                                NTB_CTRL_ADDRESS,
