...
#include <Token.h> //<ChengXW-20190111 Hygon solution: Set RedirectForReturn to 1 to fix "cpu soft lockup" error ->
  ...
  )
{
  ...
  //<ChengXW-20190111 Hygon solution: Set RedirectForReturn to 1 to fix "cpu soft lockup" error +>
  //Setup_Config->CbsCmnCpuGenWA05 = 1;                                   // RedirectForReturnDis
  Setup_Config->CbsCmnCpuGenWA05 = DEFAULT_CBS_REDIRECTFORRETURNDIS;                                // RedirectForReturnDis
  //<ChengXW-20190111 Hygon solution: Set RedirectForReturn to 1 to fix "cpu soft lockup" error ->
  Setup_Config->CbsCmnCpuL2TlbWayAlloc = 3;                             // L2 TLB Associativity
  //<Lvshh001-20190613 set PFEH "Auto" +>
  //Setup_Config->CbsCmnCpuPfeh = 0;                                      // Platform First Error Handling
  Setup_Config->CbsCmnCpuPfeh = DEFAULT_PLATFORM_FIRST_ERROR_HANDLING;                                      // Platform First Error Handling
  //<Lvshh001-20190613 set PFEH "Auto" ->
  
  //<Chendl001-20190808 20190808 Hygon CBS Default Setting, Core Performance Boost=Auto +>
  //Setup_Config->CbsCmnCpuCpb = 1;                                       // Core Performance Boost
  Setup_Config->CbsCmnCpuCpb = DEFAULT_CORE_PERFORMANCE_BOOST;                                       // Core Performance Boost
  //<Chendl001-20190808 20190808 Hygon CBS Default Setting, Core Performance Boost=Auto ->
  
  Setup_Config->CbsCmnCpuEnIbs = 3;                                     // Enable IBS
  //<Chendl001-20190808 20190808 Hygon CBS Default Setting, Global C-State=Disbled +>
  //Setup_Config->CbsCmnCpuGlobalCstateCtrl = 3;                          // Global C-state Control
  Setup_Config->CbsCmnCpuGlobalCstateCtrl = DEFAULT_GLOBAL_C_STATE_CONTROL;                          // Global C-state Control
  //<Chendl001-20190808 20190808 Hygon CBS Default Setting, Global C-State=Disbled ->
  ...
  Setup_Config->CbsCmnSmeeCtrl = DEFAULT_CPU_SMEE_CONTROL_POLICY;       // Enable/Disable SMEE //#Yanshp-20220516-Add more default option set>
  Setup_Config->CbsCmnCpuSmeeCap = 1;                                   // Enable/Disable SMEE
  Setup_Config->CbsCmnSVMCtrl = DEFAULT_SVM_CONTROL_POLICY; // Enable/Disable SVM //#Mizl-20220121-SGPZT#201:Work Loading Setting>
  ...
  Setup_Config->CbsCpuPstateFix = 0;                                    // Fix Pstate
  //<hujf1001-20200512 Add Fix P state feature ->
  ...
  //<Chendl001-20190808 20190808 Hygon CBS Default Setting,L1 && L2 Stream HW Prefetcher=Auto +>
  ...
  Setup_Config->CbsCmnCpuL1StreamHwPrefetcher = DEFAULT_L1_STREAM_HW_PREFETCHER;                      // L1 Stream HW Prefetcher
  Setup_Config->CbsCmnCpuL2StreamHwPrefetcher = DEFAULT_L2_STREAM_HW_PREFETCHER;                      // L2 Stream HW Prefetcher
  //<Chendl001-20190808 20190808 Hygon CBS Default Setting,L1 && L2 Stream HW Prefetcher=Auto ->
  ...
  //<Chendl001-20190808 20190808 Hygon CBS Default Setting, Memory Interleaving=Auto +>
  ...
  Setup_Config->CbsDfCmnSysProbeFilter = DEFAULT_SYSTEM_PROBE_FILTER;                             // System probe filter
  Setup_Config->CbsDfCmnMemIntlv = DEFAULT_MEMORY_INTERLEAVING;                                   // Memory interleaving
  Setup_Config->CbsDfCmnMemIntlvSize = DEFAULT_MEMORY_INTERLEAVING_SIZE;                               // Memory interleaving size
  //<Chendl001-20190808 20190808 Hygon CBS Default Setting, Memory Interleaving=Auto ->
  ...
  Setup_Config->CbsCmnMemDataPoisoningDdr4 = DEFAULT_AMD_CBS_DATA_POISONING;                      // Data Poisoning
  ...
  Setup_Config->CbsDbgMemCtrllerWrCrcEnDdr4 = 0;                        // Write CRC Enable
  Setup_Config->CbsDbgMemWriteCrcRetryDdr4 = 0;                         // Writr CRC Retry
  Setup_Config->CbsDbgMemWriteCrcErrorMaxReplayDdr4 = 0x3F;             // Writr CRC Error Max Replay
  Setup_Config->CbsCmnMemTsmeDdr4 = 0xFF;                               // TSME
  Setup_Config->CbsCmnMemCtrllerDataScrambleDdr4 = 0xFF;                // Data Scramble
  //<Chendl001-20190808 Hygon CBS Default Setting,ChipSelect Interleave +>
  ...
  Setup_Config->CbsCmnMemMappingBankInterleaveDdr4 = DEFAULT_CHIPSELECT_INTERLEAVING;              // Chipselect Interleaving
  Setup_Config->CbsCmnMemCtrllerBankGroupSwapDdr4 = DEFAULT_CBS_BANKGROUPSWAP;               // BankGroupSwap
  //<Chendl001-20190808 Hygon CBS Default Setting,ChipSelect Interleave ->
  ...
  Setup_Config->CbsCmnDeterminismSlider = DEFAULT_DETERNIBISM_SLIDER_POLICY; // Determinism Slider //#Mizl-20220121-SGPZT#201:Work Loading Setting>
  Setup_Config->CbsCmncTDPCtl = DEFAULT_CTDP_CONTROL_POLICY;                 // cTDP Control //#Mizl-20220121-SGPZT#201:Work Loading Setting>
  Setup_Config->CbsCmncTDPLimit = 0;                                    // cTDP
  Setup_Config->CbsCmnNbioEfficiencyOptimizedMode = DEFAULT_EFFICIENCY_OPTIMIZED_MODE_POLICY; // Efficiency Optimized Mode //#Yanshp-20220516-Add more default option set>
  ...
  //<Chendl001-20190808 20190808 Hygon CBS Default Setting, IOMMU=Enabled +>
  //Setup_Config->CbsCmnGnbNbIOMMU = 0xf;                                 // IOMMU
  Setup_Config->CbsCmnGnbNbIOMMU = DEFAULT_CBS_IOMMU;
  //<Chendl001-20190808 20190808 Hygon CBS Default Setting, IOMMU=Enabled ->
  ...
  //<Chendl001-20190808 20190808 Hygon CBS Default Setting, SATA Controller=Auto +>
  //Setup_Config->CbsCmnFchSataEnable = 0xf;                              // SATA Controller
  Setup_Config->CbsCmnFchSataEnable = DEFAULT_SATA_CONTROLLER;                              // SATA Controller
  //<Chendl001-20190808 20190808 Hygon CBS Default Setting, SATA Controller=Auto ->
  ...
  Setup_Config->CbsCmnFchUsbXHCI0Enable = DEFAULT_AMD_CBS_XHCI_CONTROLLER;                          // XHCI controller enable
  ...
  Setup_Config->CbsCmnSPISpeedCtrl = 0xf;                               // SPI Speed Value of APCB_TOKEN_CBS_CMN_BIOS_SPI_SPEED_MODE_VALUE

  //mizl-2021-12-09-Sync Setup Setting With BIOS Setup Layout V0.5+>
  Setup_Config->CbsCpuSmtCtrl = DEFAULT_SMT_CONTROL_POLICY; // SMT Control //#Mizl-20220121-SGPZT#201:Work Loading Setting>
  Setup_Config->McaMemErrThreshCount = DEFAULT_MEM_ERR_THRESHOLD_COUNTER;
  Setup_Config->McaProErrThreshCount = DEFAULT_PRO_ERR_THRESHOLD_COUNTER;
  Setup_Config->MemLeakyBucketInterval = DEFAULT_MEM_LEAKY_BUCKET_INTERVAL;
  Setup_Config->MemLeakyBucketDropOutCnt = DEFAULT_MEM_LEAKY_BUCKET_DROP_OUT_COUNTER;

  //mizl-2021-12-09-Sync Setup Setting With BIOS Setup Layout V0.5->
  Setup_Config->CbsDfCmnNumaMode = DEFAULT_NUMA_MODE;                  // Numa Mode //<Yaosr-20211104 Control NUMA enable or disable.>
  //SGEZT#23035:PCIe Leaky Buckey +>
  #if defined(PCIE_LEAKY_BUCKET_SUPPORT) && (PCIE_LEAKY_BUCKET_SUPPORT ==1)  
  Setup_Config->McaPcieErrThreshCount = DEFAULT_PCIE_ERR_THRESHOLD_COUNTER;
  Setup_Config->PcieLeakyBucketInterval = DEFAULT_PCIE_LEAKY_BUCKET_INTERVAL;
  Setup_Config->PcieLeakyBucketDropOutCnt = DEFAULT_PCIE_LEAKY_BUCKET_DROP_OUT_COUNTER;
  Setup_Config->SeparateAERLog = DEFAULT_SEPARATE_AER_LOG;
  #endif
  //SGEZT#23035:PCIe Leaky Buckey ->
  //SGEZT#26230:Mask ECC Error For OS +>
  #if defined(DEFAULT_SEPARATE_DIMM_ECC_LOG)
  Setup_Config->SeparateDIMMECCLog = DEFAULT_SEPARATE_DIMM_ECC_LOG; 
  #endif
  //SGEZT#26230:Mask ECC Error For OS >
