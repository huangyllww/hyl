...
#include <Token.h>//<Chendl001-20180626 Memory Leaky Bucket Design
/*----------------------------------------------------------------------------------------
...
PlatformConfigInit ( VOID )
{
  ...
  mPlatformApeiPrivate->PlatRasPolicy.McaMemErrThreshCount = PcdGet16 (PcdMcaMemErrThreshCount);
  mPlatformApeiPrivate->PlatRasPolicy.McaProErrThreshCount = PcdGet16 (PcdMcaProErrThreshCount);
  //<Chendl001-20181103 Optimized Memory Leaky Bucket_2 +>
  mPlatformApeiPrivate->PlatRasPolicy.McaMemErrInterval = PcdGet32 (PcdMcaMemErrInterval);
  mPlatformApeiPrivate->PlatRasPolicy.McaMemErrDropOut = PcdGet16 (PcdMcaMemErrDropOut);
  //<Chendl001-20181103 Optimized Memory Leaky Bucket_2 ->
  //<Chendl001-20180626 Memory Leaky Bucket Design ->
#else
  ...
  mPlatformApeiPrivate->PlatRasPolicy.SeparateDIMMECCLog = PcdGet8 (PcdSeparateDIMMECCLog);
  #endif
  //SGEZT#26230:Mask ECC Error For OS ->
//SGEZT#23035:PCIe Leaky Buckey +>
#if defined(PCIE_LEAKY_BUCKET_SUPPORT) && (PCIE_LEAKY_BUCKET_SUPPORT ==1)  
  mPlatformApeiPrivate->PlatRasPolicy.McaPcieErrThreshCount = PcdGet16 (PcdMcaPcieErrThreshCount);
  mPlatformApeiPrivate->PlatRasPolicy.PcieLeakyBucketInterval = PcdGet32 (PcdMcaPcieErrInterval);
  mPlatformApeiPrivate->PlatRasPolicy.PcieLeakyBucketDropOutCnt = PcdGet16 (PcdMcaPcieErrDropOut);
  mPlatformApeiPrivate->PlatRasPolicy.SeparateAERLog = PcdGet8 (PcdSeparateAERLog);
#endif
//SGEZT#23035:PCIe Leaky Buckey ->
  ...
  )
{
  ...
  if (SecPcieExtCapPtr !=0 ) {
    PcieLinkStatus = PciRead32 (PciPortAddr.AddressValue + SecPcieExtCapPtr + PCIE_LANE_ERR_STATUS_PTR);
    while (PcieLinkStatus != 0) {
      if (RasRetryCounter == 0) {
        break;
      } else {
        RasRetryCounter--; 
      }
      PciWrite32 (PciPortAddr.AddressValue + SecPcieExtCapPtr + PCIE_LANE_ERR_STATUS_PTR, PcieLinkStatus);
      PcieLinkStatus = PciRead32 (PciPortAddr.AddressValue + SecPcieExtCapPtr + PCIE_LANE_ERR_STATUS_PTR);
...
//<Chendl001-20180626 Memory Leaky Bucket Design +>
  ...
  mAmiHygonElogProtocol->SendElogEventMca((UINT8*)RasMcaErrorInfo, NULL,BankIndex); // AMI PORTING ELog
#else
...
//<Chendl001-20180626 Memory Leaky Bucket Design ->
  #if defined(DEFAULT_SEPARATE_DIMM_ECC_LOG)  
    if(mPlatformApeiPrivate->PlatRasPolicy.SeparateDIMMECCLog == 0)
    {
  #endif
      // Add a new record to the BERT table
...
McaErrorDetection ( VOID )
{
  ...
  UINT8                         BankIndex;
  UINT64                        MsrData;
//<fengning-202020921 Workaround for bug409 +>
  BOOLEAN                       S5ResetStatSyncFlood;
//<fengning-202020921 Workaround for bug409 ->
  // Get EFI MP service
  if (MpServices == NULL) {
  ...
  for (ProcessorNumber = 0; ProcessorNumber < NumberOfLogicProcessors; ProcessorNumber++) {
    ...
    for (BankIndex = 0; BankIndex < BankNum; BankIndex++) {
      ...
      if (RasMcaErrorInfo.McaBankErrorInfo[BankIndex].McaStatusMsr.Field.Val) {
        ...
        //<fengning-202020921 Workaround for bug409 +>
        case MCA_PIE_BANK:
          S5ResetStatSyncFlood = (MmioRead32(0xFED803C0) & 0x08000000) >> 27;
          if(RasMcaErrorInfo.McaBankErrorInfo[BankIndex].McaStatusMsr.Field.UECC == TRUE && S5ResetStatSyncFlood == FALSE){
             break;
          }
        //<fengning-202020921 Workaround for bug409 ->
        case MCA_SMU_BANK:
        ...
        //case MCA_CS1_BANK:
        //case MCA_PIE_BANK:  //<fengning-202020921 Workaround for bug409 >
          LogProcessorError(MpServices, &RasMcaErrorInfo, BankIndex, ProcessorNumber);
          break;
        }
  ...
  )
{
  ...
  BOOLEAN       PcieUncorrErrorValid = FALSE;
  BOOLEAN       PcieCorrErrorValid = FALSE;
  ...
  if (!IsRootPortError && RootErrStatusSet) {
     ...
     if (AerCapPtr != 0) {
       ...
       if (0 != (PcieUncorrStatus & ~PcieUncorrMask)) {
         ...
         PcieUncorrErrorValid = TRUE;
       } else {
         //Correctable error
         PcieCorrErrorValid = PcieAerCorrErrCheck (PciPortAddr, AerCapPtr);
         if (PcieCorrErrorValid) {
           GenPcieAerErrEntry->GenErrorDataEntry.ErrorSeverity = ERROR_SEVERITY_CORRECTED;
      	 }
       }
       
       if (PcieUncorrErrorValid || PcieCorrErrorValid) {
         FillPcieInfo(PciPortAddr, PcieErrorSection);
         // Add a new record to the BERT table
         
         //SGDFT#96:Mask AER Log For OS +>
         if((mPlatformApeiPrivate->Valid == TRUE) && (mPlatformApeiPrivate->PlatRasPolicy.SeparateAERLog ==0))
         {
