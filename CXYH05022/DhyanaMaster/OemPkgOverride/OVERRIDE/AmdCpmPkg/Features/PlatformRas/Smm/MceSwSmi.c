...
UINT8 fMaskOSDIMMECCError = 0; //SGEZT#26230:Mask ECC Error For OS +>
/*----------------------------------------------------------------------------------------
...
VOID McaThresholdInit(
    IN RAS_MCA_ERROR_INFO *RasMcaErrorInfo,
    IN UINT8 BankIndex);

VOID ErrorThresholdInit(
    IN VOID *Buffer);
...
VOID CpmSetMcaThreshold(
    IN RAS_THRESHOLD_CONFIG *RasThresholdConfig);

VOID CpmSetMpMcaThreshold(
    IN RAS_THRESHOLD_CONFIG *RasThresholdConfig);
...
//<Hujf1001-20210304 Memory Leaky Bucket Design V2 +>
  ...
  fMaskOSDIMMECCError = mPlatformApeiData->PlatRasPolicy.SeparateDIMMECCLog; //#SGEZT#26230:Mask ECC Error For OS +>
  #else
  fMaskOSDIMMECCError=0;
  #endif
  ...
  else
  {
    ...
    for (ProcessorNumber = 0; ProcessorNumber < gSmst->NumberOfCpus; ProcessorNumber++)
    {
      //@todo
      //Get SMI Source
      //MceRedirSts - Machine Check Error
      //SmiSrcMca - Deferred Or Threshold Interrupt
      //WrMsr - MASK OS Error,SKIP this Setting
      if (pLocalSmiStatusList[ProcessorNumber].Field.MceRedirSts ||
          pLocalSmiStatusList[ProcessorNumber].Field.SmiSrcMca)
      {
        ...
        DEBUG((EFI_D_ERROR, "[OemDbg][%a][%d]MceRedirSts:%x SmiSrcMca:%x\r\n", __FUNCTION__, __LINE__,
               pLocalSmiStatusList[ProcessorNumber].Field.MceRedirSts,
               pLocalSmiStatusList[ProcessorNumber].Field.SmiSrcMca));
        //Check MCA Error Status
        mAmdRasSmmProtocol->SearchMcaError(&RasMcaErrorInfo);
        //Log MCA Error
        LogMCAError(&RasMcaErrorInfo);
        ...
        if (pLocalSmiStatusList[ProcessorNumber].Field.SmiSrcMca)
        {
          for (BankIndex = 0; BankIndex < BankNum; BankIndex++)
          {
            if (RasMcaErrorInfo.McaBankErrorInfo[BankIndex].McaStatusMsr.Field.Val)
            {
              ...
              {
                McaThresholdInit(&RasMcaErrorInfo, BankIndex);
                mAmdRasSmmProtocol->ClrMcaStatus(ProcessorNumber, RasMcaErrorInfo.McaBankErrorInfo[BankIndex].McaBankNumber, FALSE);
              }
            }
  ...
  for (BankIndex = 0; BankIndex < BankNum; BankIndex++)
  {
    if (RasMcaErrorInfo->McaBankErrorInfo[BankIndex].McaStatusMsr.Field.Val)
    {
      ...
      if ((RasMcaErrorInfo->McaBankErrorInfo[BankIndex].McaMisc0Msr.Field.CntEn && RasMcaErrorInfo->McaBankErrorInfo[BankIndex].McaMisc0Msr.Field.Ovrflw && (mSmiMode != POLLING_MODE)) ||
          (RasMcaErrorInfo->McaBankErrorInfo[BankIndex].McaMisc1Msr.Field.Valid && RasMcaErrorInfo->McaBankErrorInfo[BankIndex].McaMisc1Msr.Field.CntEn && RasMcaErrorInfo->McaBankErrorInfo[BankIndex].McaMisc1Msr.Field.Ovrflw && (mSmiMode != POLLING_MODE)))
      { //<Lvshh001-20210507 Optimized Memory Leaky Bucket_3>
        if ((RasMcaErrorInfo->McaBankErrorInfo[BankIndex].McaBankNumber == MCA_UMC0_BANK) || (RasMcaErrorInfo->McaBankErrorInfo[BankIndex].McaBankNumber == MCA_UMC1_BANK))
        {
          ...
          if (mPlatformApeiData->PlatRasPolicy.McaMemErrInterval > 0)
          { // Memory Leaky Bucket is disabled if McaMemErrInterval <=0
          ...
          if (mTotalErrorNum[FailedSocket][FailedDie][FailedUMC] == 0xFFFF)
          { //First record MemErrThreshCount
    ...
    VOID)
{
  ...
  else
  {
    for (ProcessorNumber = 0; ProcessorNumber < gSmst->NumberOfCpus; ProcessorNumber++)
    {
      ...
      for (BankIndex = 0; BankIndex < BankNum; BankIndex++)
      {
        DEBUG((EFI_D_ERROR, "[OemDbg][%a][%d]BankIndex:%x  McaStatusMsr:%x\r\n", __FUNCTION__, __LINE__,
               BankIndex, RasMcaErrorInfo.McaBankErrorInfo[BankIndex].McaStatusMsr.Field.Val));
