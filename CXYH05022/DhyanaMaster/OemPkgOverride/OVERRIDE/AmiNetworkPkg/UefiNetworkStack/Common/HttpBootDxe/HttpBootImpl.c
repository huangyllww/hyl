...
//SGMTB#163016:NO MAC Info During PXE Loop->
// AMI PORTING STARTS
#define FIVE_HUNDRED_MS  (500)   // 500 milli seconds for HTTP boot delay
  ...
  )
{
  ...
  if (UsingIpv6 && Private->Ip6Nic != NULL) {
    ...
    AsciiPrint ("\n>>Start HTTP Boot over IPv6");	// AMI PORTING.
  } else if (!UsingIpv6 && Private->Ip4Nic != NULL) {
    Private->UsingIpv6 = FALSE;
    AsciiPrint ("\n>>Start HTTP Boot over IPv4");	// AMI PORTING.
  } else {
    if (Uri != NULL) {
      FreePool (Uri);
    }
...
/**
  Attempt to complete a DHCPv4 D.O.R.A or DHCPv6 S.A.R.R sequence to retrieve the boot resource information.
  ...
  )
{
  ...
  UINTN                       	NoofIterations;	// AMI PORTING
//SGMTB#163016:NO MAC Info During PXE Loop +>  
  STATIC UINT32               FilePathIndex = 1;

  MAC_ADDR_DEVICE_PATH         *Mac=NULL;
  EFI_DEVICE_PATH_PROTOCOL     *MacDevicePath=NULL;
  PCI_DEV_SETUP_FORM_DATA     *DevListData;
  OEM_PCIE_DEV_DATA_PROTOCOL  *gPcieDevProtocol = NULL ;
...
//SGMTB#163016:NO MAC Info During PXE Loop ->
  ...
  //SGMTB#163016:NO MAC Info During PXE Loop+>
  Status = gBS->HandleProtocol(VirtualNic->Controller, &gEfiDevicePathProtocolGuid, (VOID**)&MacDevicePath);
  // MacDevicePath = VirtualNic->Private->ParentDevicePath;
  if(EFI_ERROR (Status) || (MacDevicePath == NULL))
    return EFI_INVALID_PARAMETER;
  DEBUG ((DEBUG_ERROR, "%a %d  \n", __FUNCTION__, __LINE__));  
  while (! IsDevicePathEnd(MacDevicePath) &&\
        ((DevicePathType(MacDevicePath)!=MESSAGING_DEVICE_PATH) ||\
        (DevicePathSubType(MacDevicePath) != MSG_MAC_ADDR_DP)))
  {
    /* code */
    MacDevicePath=NextDevicePathNode(MacDevicePath);
  }
  Mac = (MAC_ADDR_DEVICE_PATH *) MacDevicePath;


  Status=gBS->LocateProtocol(&gOemPcieDevProtocolGuid,NULL,(VOID **)&gPcieDevProtocol);
  if(EFI_ERROR (Status))
  {
    DEBUG ((DEBUG_ERROR, "%a %d Locate Protocol %r \n", __FUNCTION__, __LINE__,Status));
    return Status;
  }

  Status=gPcieDevProtocol->GetPcieDevList(&DevListData);
  if (EFI_ERROR(Status))
  {
    DEBUG ((DEBUG_ERROR, "%a %d GetPcieDevList %r \n", __FUNCTION__, __LINE__,Status));
    return Status;
  }
  
  for (i = 0; i < DevListData->PciDevCount; i++)
  {
    IsCurNIC = 0;
    IdxDev=DevListData->PciDevIdxData[i];
    ...
    for (j = 0; j < IdxDev->PortCount ; j++)
    {
      ...
      if(dev->Class.BaseClassCode == PCI_CLASS_NETWORK)
      {
        Status=GetNicMacByUefi(gBS,dev->Handle,NicMac);
        if(EFI_ERROR (Status)){
          DEBUG ((DEBUG_ERROR, "%a %d Read NIC Mac Status:%r \n", __FUNCTION__, __LINE__,Status));
        }

        if((NicMac[0] == Mac->MacAddress.Addr[0]) &&
           (NicMac[1] == Mac->MacAddress.Addr[1]) &&
           (NicMac[2] == Mac->MacAddress.Addr[2]) &&
           (NicMac[3] == Mac->MacAddress.Addr[3]) &&
           (NicMac[4] == Mac->MacAddress.Addr[4]) &&
           (NicMac[5] == Mac->MacAddress.Addr[5]))
           {
            IsCurNIC = 1;
            PortNum = DevItem->PortIdx;
            break;
           }
      }
      /* code */
  ...
  //
  //AsciiPrint ("\n>>Checking Media Presence......");//SGMTB#163016:NO MAC Info During PXE Loop+>
 
  AsciiPrint("\n\r>>%s Port%d MAC:%02x%02x%02x%02x%02x%02x",DevItem->BrandName,PortNum,Mac->MacAddress.Addr[0],\
                                                                Mac->MacAddress.Addr[1],Mac->MacAddress.Addr[2],Mac->MacAddress.Addr[3],\
                                                                Mac->MacAddress.Addr[4],Mac->MacAddress.Addr[5]); //SGMTB#163016:NO MAC Info During PXE Loop+>
  
  MediaPresent = TRUE;
  NoofIterations = NetworkStackSetupData.MediaDetectCount;
  while(TRUE){
    //<Wangyia001-20180116 Mantis 18274: User can not get the effect when Media detect count is set to different value +>
    AsciiPrint ("\n\r>>Checking Media Presence(Http:%d)......",FilePathIndex);//SGMTB#163016:NO MAC Info During PXE Loop+>
    //<Wangyia001-20180116 Mantis 18274: User can not get the effect when Media detect count is set to different value ->
  ...
  FilePathIndex++;//SGMTB#163016:NO MAC Info During PXE Loop+>
  //
  // AMI PORTING END.
  //
