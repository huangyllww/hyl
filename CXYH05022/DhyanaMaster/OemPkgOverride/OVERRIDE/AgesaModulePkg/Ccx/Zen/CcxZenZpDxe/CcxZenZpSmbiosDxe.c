...
#include "Protocol/AmiCpuInfo.h" //SGEZT#25449:Version C CPUs are mixed, and CPU information displays are inconsistent
  ...
  )
{
  ...
  INT16                                 NoneZoroIndex; //<Wangyia002-20170412 Setup info display>
  ...
  Status = gBS->LocateProtocol (&gAmiCpuInfoProtocolGuid, NULL, &AmiCpuInfo);
...
//SGEZT#25449:Version C CPUs are mixed, and CPU information displays are inconsistent->
  ...
  if (NbioSmuServices == NULL) {
    ...
    if ((LogicalId.Family == HYGON_FAMILY_18) && (LogicalId.Revision & HYGON_REV_F18_DN_Dx)) {
        NbioSmuServices->AmdSmuRegisterRead (NbioSmuServices, (UINT32) DieNumber, HYGON_DX_MP0_CORE_DISABLE, &CoreDisFuse);
        NbioSmuServices->AmdSmuRegisterRead (NbioSmuServices, (UINT32) DieNumber, HYGON_DX_MP1_FMAX, &Fmax);
        Fmax = (Fmax >> HYGON_DX_MP1_FMAX_START_BITS) & 0xFF;
    } else {
  ...
  //SGEZT#28111--CPU-Enabled-Info-Error--Once-SMT-is-disabled
  CoreDmiInfo->CoreEnabled = (UINT16) (((CpuId.ECX_Reg & 0xFF) + 1) / CcxGetThreadsPerCore ());

//SGEZT#25449:Version C CPUs are mixed, and CPU information displays are inconsistent+>
 Status = AmiCpuInfo->GetCpuInfo(AmiCpuInfo, (UINTN)Socket*CoreDmiInfo->ThreadCount, &GetCpuInfo);
 ...
 CpuId.EAX_Reg=GetCpuInfo->FamModStep;
 CpuId.EDX_Reg=GetCpuInfo->FeatureId;
 }
  CoreDmiInfo->ProcessorID.EaxFamilyId = CpuId.EAX_Reg;
  CoreDmiInfo->ProcessorID.EdxFeatureId = CpuId.EDX_Reg;
//SGEZT#25449:Version C CPUs are mixed, and CPU information displays are inconsistent->
  ...
  NoneZoroIndex = PROC_VERSION_LENGTH - 1;
  while ((NoneZoroIndex >= 0) && (0x0 == CoreDmiInfo->ProcessorVersion[NoneZoroIndex])) { // search the first non-NULL char from string tail
    NoneZoroIndex--;
  }
  while (NoneZoroIndex >= 0) {  // now Index is the first non-NULL char at the tail
    if (CoreDmiInfo->ProcessorVersion[NoneZoroIndex] != 0x20) {
      break;
    }
    CoreDmiInfo->ProcessorVersion[NoneZoroIndex] = 0x0;
    NoneZoroIndex--;
  }
  //<Wangyia002-20170412 Setup info display ->
