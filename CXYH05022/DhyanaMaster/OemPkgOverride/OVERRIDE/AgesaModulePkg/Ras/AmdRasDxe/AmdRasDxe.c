 ...
 *----------------------------------------------------------------------------------------
 */
#define FILECODE RAS_AMDRASDXE_AMDRASDXE_FILECODE


#define RAS_MAX_CORES (CCX_MAX_SOCKETS * CCX_MAX_DIES_PER_SOCKET * CCX_MAX_COMPLEXES_PER_DIE * CCX_MAX_CORES_PER_COMPLEX * CCX_MAX_THREADS_PER_CORE)
 ...
 *
 * Description
 *   Entry point of the AMD RAS DXE driver
...
AmdRasPolicyInit(VOID)
{
  ...
  if ((PcdGet16 (PcdMcaMemErrThreshCount) == 0xFFFF) && (PcdGet16 (PcdMcaProErrThreshCount) == 0xFFFF)){
    //Use default setting
    if (PcdGetBool(PcdAmdCcxCfgPFEHEnable)) {
      PcdSet16(PcdMcaMemErrThreshCount, 0x0FEF);
      PcdSet16(PcdMcaProErrThreshCount, 0x0FEF);
    } else {
      PcdSet16(PcdMcaMemErrThreshCount, 0x0FEF);
      PcdSet16(PcdMcaProErrThreshCount, 0x0FEF);
    }
  } 
  mAmdRasPolicy->McaMemErrThreshCount = PcdGet16(PcdMcaMemErrThreshCount);
  mAmdRasPolicy->McaProErrThreshCount = PcdGet16(PcdMcaProErrThreshCount);
#else
...
CollectDimmMap(VOID)
{
  ...
  for (socketId = 0; socketId < NumberOfInstalledProcessors; socketId++) {
    FabricTopology->GetProcessorInfo (FabricTopology, socketId, &TotalNumberOfDie, NULL, NULL, NULL);
    for (dieId = 0; dieId < TotalNumberOfDie; dieId++) {
      FabricTopology->GetRootBridgeInfo (FabricTopology, socketId, dieId, NumberOfRootBridgesOnDie, NULL, &BusNumberBase, NULL);
    ...
    for (DieLoop = 0; DieLoop < NumberOfDies; DieLoop++) {
      ...
      for (ComplexLoop = 0; ComplexLoop < NumberOfComplexes; ComplexLoop++) {
        ...
        for (CoreLoop = 0; CoreLoop < NumberOfCores; CoreLoop++) {
          ...
          RasCpuMap[Index].DieId = (UINT8)(DieLoop & 0xFF);
