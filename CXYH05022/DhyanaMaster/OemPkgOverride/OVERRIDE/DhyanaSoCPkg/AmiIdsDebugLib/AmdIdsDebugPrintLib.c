...
extern AMD_IDS_DEBUG_PRINT_INSTANCE AmiIdsDebugPrintSerialInstance;
...
CONST AMD_IDS_DEBUG_PRINT_INSTANCE* AmdIdsDebugPrintInstances[] =
{
  ...
  &AmiIdsDebugPrintSerialInstance,//hujf1001-20180906 Oem Debug module support on AMD/Higon project
  NULL,
};
/**
 *
...
{
  *pIdsAmdIdsDebugPrintInstancesPtr = &AmdIdsDebugPrintInstances[0];
}
...
{
  UINT8 i;
  if (AmdIdsDebugPrintCheckSupportAny (AmdIdsDebugPrintInstances)) {
    //init input
    for (i = 0; AmdIdsDebugPrintInstances[i] != NULL; i++) {
      if (AmdIdsDebugPrintInstances[i]->support ()) {
        AmdIdsDebugPrintInstances[i]->constructor ();
      }
    }
  }
  ...
  )
{
  ...
  UINT64 Filter;
  AMD_IDS_DEBUG_PRINT_INSTANCE **IdsDebugPrintInstances;
  UINT8 i;
  ...
  GetAmdIdsDebugPrintInstances (
      (const AMD_IDS_DEBUG_PRINT_INSTANCE ***)&IdsDebugPrintInstances);
  if (AmdIdsDebugPrintCheckSupportAny (
        (const AMD_IDS_DEBUG_PRINT_INSTANCE **)IdsDebugPrintInstances)) {
    //init input
    for (i = 0; IdsDebugPrintInstances[i] != NULL; i++) {
      if (IdsDebugPrintInstances[i]->support ()) {
        Filter = PcdGet64 (PcdAmdIdsDebugPrintFilter);
        //Get Customize filter if implemented(Option)
        IdsDebugPrintInstances[i]->customfilter (&Filter);
        if (FilterFlag & Filter) {
          //Print Physical Layer
            IdsDebugPrintInstances[i]->print (&LocalBuffer[0], OutPutStringLen);
        }
      }
    }
  }
