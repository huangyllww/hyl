  ...
  IN UINT16             Class )
{
    ...
    //Mizl-20210114-Improve Language Tansfer+>
    CHAR16          *String_BMCSelfTest=NULL;
    EFI_STRING_ID   STR_BMC_SELF_TEST_ID;
    //Mizl-20210114-Improve Language Tansfer->
    ...
    if ((!EFI_ERROR (Status)) && (BmcSelfTestProtocol->BmcSelfTestErrorCount == 0)) {
        ...
        STR_BMC_SELF_TEST_ID = STRING_TOKEN(STR_PROTOCOL_LOCATE_STATUS_PASSED);
        //Mizl-20210114-Improve Language Tansfer->
    } else {
        //Mizl-20210114-Improve Language Tansfer+>
        //CharPtrStatus = IpmiHiiGetString (HiiHandle, STRING_TOKEN(STR_PROTOCOL_LOCATE_STATUS_FAILED), CharPtr, &BufferLength, NULL);
        STR_BMC_SELF_TEST_ID = STRING_TOKEN(STR_PROTOCOL_LOCATE_STATUS_FAILED);
        //Mizl-20210114-Improve Language Tansfer->
    }
    //Mizl-20210114-Improve Language Tansfer+>
    ...
    //         CharPtr );
    // }
    String_BMCSelfTest = HiiGetString (HiiHandle, STR_BMC_SELF_TEST_ID,"en-US");
    HiiSetString(HiiHandle, STRING_TOKEN(STR_IPMI_BMC_SELF_TEST_STATUS_VAL), String_BMCSelfTest, "en-US");
    String_BMCSelfTest = HiiGetString (HiiHandle, STR_BMC_SELF_TEST_ID,"zh-chs");
    HiiSetString(HiiHandle, STRING_TOKEN(STR_IPMI_BMC_SELF_TEST_STATUS_VAL), String_BMCSelfTest, "zh-chs");
    //Mizl-20210114-Improve Language Tansfer->
    // 
    ...
    if ((!EFI_ERROR (Status)) && BmcSelfTestProtocol->IsGetDeviceIdCommandPassed) {
        ...
        InitString (
                ...
                //L"%x.%02x.%02x",
                L"%d.%02x.%02x",//SGEZB#28670: Show BMC Major Version By Decimal
                BmcSelfTestProtocol->GetDeviceIdResponse.MajorFirmwareRev, BmcSelfTestProtocol->GetDeviceIdResponse.MinorFirmwareRev, BmcSelfTestProtocol->GetDeviceIdResponse.AuxFirmwareRevInfo);
#else
                //L"%x.%x",
                L"%d.%x", //SGEZB#28670: Show BMC Major Version By Decimal
