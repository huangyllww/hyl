...
//<Lubl-20161219 display bmc build time +>
  ...
  IN  CHAR16  TimeZone[] )
{
    ...
    SERVER_IPMI_DEBUG ((EFI_D_INFO, "%a Entry... TimeZone: %S \n", __FUNCTION__, TimeZone));
    ...
    if ( (TimeZone[5] >= '0') && (TimeZone[5] <= '9') ) {
        ...
        SERVER_IPMI_DEBUG ((EFI_D_INFO, " After Converting char to number - 1's position UtcOffset: %d \n", UtcOffset));
    } else {
        goto ErrorExit;
    }
  ...
  IN UINT16             Class )
{
    ...
    //<Lubl-20161219 display bmc build time ->
    UINT8  index;  //<Yaosr001-20180721 modify the display of bmc build time>
    //
    ...
    if (!EFI_ERROR(Status)) {  
      ...
      //<Yaosr001-20180721 modify the display of bmc build time +>
      //ResponseData[21] = L'\0';
            for (index=sizeof(ResponseData)-1; index>=0; index--){
                if (ResponseData[index] == 0x20){
      ...
      if (Status == EFI_SUCCESS) {      	
        InitString (
	    HiiHandle,
	    STRING_TOKEN(STR_IPMI_BMC_FIRMWARE_BUILDTIME_VAL),
	    L"%s",
	    OemAscii2Unicode(&UnicodeStr[0], &ResponseData[0]));    
      }
    }
    //<Lubl-20161219 display bmc build time ->
        ...
        Status = IpmiTransport->SendIpmiCommand (
                                    ...
                                    (UINT8*)&(BmcUtcOffset),
                                    &DataSize );
        SERVER_IPMI_DEBUG ((EFI_D_INFO, " In InitBmcTimeZoneDisplay : EFI_STORAGE_GET_SEL_TIME_UTC_OFFSET Status: %r\n", Status));
        if (!EFI_ERROR (Status)) {
