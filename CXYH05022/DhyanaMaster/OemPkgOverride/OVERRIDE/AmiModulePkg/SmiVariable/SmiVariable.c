...
EFI_STATUS CheckAddressRangeHiiDb( IN UINT8 *Address, IN UINTN Range )
{
    ...
    if ( (ExportHiiDbBlock.Data == 0) || (ExportHiiDbBlock.DataSize == 0) ||
    	 ...
    	 ((UINT32)Address + Range < (UINT32)Address) || 
    ...
    if (Status == EFI_UNSUPPORTED){        
    	...
    	switch((RegBlockPtr->EAX & 0xff00) >> 8) {
        	...
        	case SET_VARIABLE_SUBFUNCTION:
                ...
                if ( StrCmp((CHAR16*)(UINTN)VariableBlock->VariableName, L"AmdSetup") == 0 ){
                    UINTN                       VariableSize;
                    UINT8                       data8 = 0;
                    ...
                    Status = pRS->SetVariable(L"SceSetBios",&gCbsSystemConfigurationGuid,
                               EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE,VariableSize,&data8);
                }
                //<<duanjun 20210721 sce tool fail to change setting ->				
        	break;
