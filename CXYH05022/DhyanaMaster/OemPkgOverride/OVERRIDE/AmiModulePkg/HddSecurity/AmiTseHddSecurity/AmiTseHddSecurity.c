...
#include "AMIVfr.h"
#include <Protocol/ConsoleControl.h>//<Lvshh001-20190312 Mantis#27328,Solve Legacy optionrom alternate display in the post process>
...
EFI_CONSOLE_CONTROL_PROTOCOL *OemConsoleControl = NULL;//<Lvshh001-20190312 Mantis#27328,Solve Legacy optionrom alternate display in the post process>
//<Lvshh001-20190320 Solve system hang in the post process when raid card,HDD password exist and disabled quiet boot +>
...
EFI_STATUS OemReconnectAllVga()
{
    ...
    for (n = 0 ; n < HandleCount; n++) {
        ...
        if (dData[3]==0x03) { //Find VGA Device
            pBS->DisconnectController(HandleBuffer[n], NULL, NULL);
            pBS->ConnectController(HandleBuffer[n], NULL, NULL, TRUE);
            VgaStatus = EFI_SUCCESS;
            //TRACE((-1,"[OemDbg] OemDbgReconnectAllVga Status %r,VgaHandle %x\n",VgaStatus,HandleBuffer[n]));
        }
    }
...
)
{
    ...
    //<Lvshh001-20190312 Mantis#27328,Solve Legacy optionrom alternate display in the post process +>
    EFI_CONSOLE_CONTROL_SCREEN_MODE  Mode;
    BOOLEAN     OemStdInLocked = FALSE;
    EFI_STATUS  Status;
    //<Lvshh001-20190312 Mantis#27328,Solve Legacy optionrom alternate display in the post process ->
    ...
    for ( i = 0; i < gHddSecurityCount; i++ ) {
        if ( DataPtr->Locked && (!DataPtr->Validated)) {
            ...
            //<Lvshh001-20190312 Mantis#27328,Solve Legacy optionrom alternate display in the post process +>
            Status = pBS->LocateProtocol(&gEfiConsoleControlProtocolGuid,NULL,&OemConsoleControl);
            if (EFI_ERROR(Status)) {
                TRACE((-1,"[OemDbg] LocateProtocol gEfiConsoleControlProtocolGuid Status %r",Status));
                continue ;     
            }
                       
            Status = OemConsoleControl->GetMode(OemConsoleControl,&Mode,NULL,&OemStdInLocked); 
            if (EFI_ERROR(Status)) {
                TRACE((-1,"[OemDbg] OemDbgConsoleControl->GetMode Status %r",Status));
                continue ;   
            }
            //TRUE = locked, FALSE = not locked
            if (OemStdInLocked == TRUE){
                
                //<Lvshh001-20190320 Solve system hang in the post process when raid card,HDD password exist and disabled quiet boot +>
                Status = OemReconnectAllVga();
                if (EFI_ERROR (Status)) {
                    TRACE((-1,"[OemDbg] OemDbgReconnectAllVga Status %r",Status));
                    continue ;  
                }
                OemConnectSerialIO();
                //<Lvshh001-20190320 Solve system hang in the post process when raid card,HDD password exist and disabled quiet boot ->
                
                Status = OemConsoleControl->LockStdIn(OemConsoleControl, L"");
                if (EFI_ERROR(Status)){
                    TRACE((-1,"[OemDbg] OemDbgConsoleControl->LockStdIn Unlock Status %r",Status));
                    continue ;  
                }
                PcdSetBool(OemPcdConsoleLockSupport,TRUE);
            }
            //<Lvshh001-20190312 Mantis#27328,Solve Legacy optionrom alternate display in the post process ->            
            ...
            //<Lvshh001-20190312 Mantis#27328,Solve Legacy optionrom alternate display in the post process +>
            //Revert Lock Status after input HDD password.
            if (OemStdInLocked == TRUE){
                Status = OemConsoleControl->LockStdIn(OemConsoleControl, L"");
                if (EFI_ERROR(Status)){
                    TRACE((-1,"[OemDbg] OemDbgConsoleControl->LockStdIn Lock Status %r",Status));
                    continue ;  
                } 
            }
            //<Lvshh001-20190312 Mantis#27328,Solve Legacy optionrom alternate display in the post process ->
...
)
{
    ...
    *pbCheckUser = FALSE;
    ...
    //<Malj2-20180529 Mantis:20093 HDD Security Support rose from 6 to 24 Hard Disk +>
            ...
            == STRUCT_OFFSET(IDE_SECURITY_CONFIG,IDEMasterPassword_HDD7) )
    {
    *pHardDiskNumber = 6;
    *pbCheckUser = FALSE;
    }
            ...
            == STRUCT_OFFSET(IDE_SECURITY_CONFIG,IDEMasterPassword_HDD8) )
    {
    *pHardDiskNumber = 7;
    *pbCheckUser = FALSE;
    }
    else if( UefiGetQuestionOffset(data) /*data->QuestionId*/
            == STRUCT_OFFSET(IDE_SECURITY_CONFIG,IDEMasterPassword_HDD9) )
    {
     *pHardDiskNumber = 8;
     *pbCheckUser = FALSE;
     }
    else if( UefiGetQuestionOffset(data) /*data->QuestionId*/
            == STRUCT_OFFSET(IDE_SECURITY_CONFIG,IDEMasterPassword_HDD10) )
     {
      *pHardDiskNumber = 9;
      *pbCheckUser = FALSE;
     }
    else if( UefiGetQuestionOffset(data) /*data->QuestionId*/
            == STRUCT_OFFSET(IDE_SECURITY_CONFIG,IDEMasterPassword_HDD11) )
     {
      *pHardDiskNumber = 10;
      *pbCheckUser = FALSE;
     }
    else if( UefiGetQuestionOffset(data) /*data->QuestionId*/
            == STRUCT_OFFSET(IDE_SECURITY_CONFIG,IDEMasterPassword_HDD12) )
     {
      *pHardDiskNumber = 11;
      *pbCheckUser = FALSE;
     }
    else if( UefiGetQuestionOffset(data) /*data->QuestionId*/
            == STRUCT_OFFSET(IDE_SECURITY_CONFIG,IDEMasterPassword_HDD13) )
     {
      *pHardDiskNumber = 12;
      *pbCheckUser = FALSE;
     }
    else if( UefiGetQuestionOffset(data) /*data->QuestionId*/
            == STRUCT_OFFSET(IDE_SECURITY_CONFIG,IDEMasterPassword_HDD14) )
     {
      *pHardDiskNumber = 13;
      *pbCheckUser = FALSE;
     }
    else if( UefiGetQuestionOffset(data) /*data->QuestionId*/
            == STRUCT_OFFSET(IDE_SECURITY_CONFIG,IDEMasterPassword_HDD15) )
     {
      *pHardDiskNumber = 14;
      *pbCheckUser = FALSE;
     }
    else if( UefiGetQuestionOffset(data) /*data->QuestionId*/
            == STRUCT_OFFSET(IDE_SECURITY_CONFIG,IDEMasterPassword_HDD16) )
     {
      *pHardDiskNumber = 15;
      *pbCheckUser = FALSE;
     }
    else if( UefiGetQuestionOffset(data) /*data->QuestionId*/
            == STRUCT_OFFSET(IDE_SECURITY_CONFIG,IDEMasterPassword_HDD17) )
     {
      *pHardDiskNumber = 16;
      *pbCheckUser = FALSE;
     }
    else if( UefiGetQuestionOffset(data) /*data->QuestionId*/
            == STRUCT_OFFSET(IDE_SECURITY_CONFIG,IDEMasterPassword_HDD18) )
     {
      *pHardDiskNumber = 17;
      *pbCheckUser = FALSE;
     }
    else if( UefiGetQuestionOffset(data) /*data->QuestionId*/
            == STRUCT_OFFSET(IDE_SECURITY_CONFIG,IDEMasterPassword_HDD19) )
     {
      *pHardDiskNumber = 18;
      *pbCheckUser = FALSE;
     }
    else if( UefiGetQuestionOffset(data) /*data->QuestionId*/
            == STRUCT_OFFSET(IDE_SECURITY_CONFIG,IDEMasterPassword_HDD20) )
     {
      *pHardDiskNumber = 19;
      *pbCheckUser = FALSE;
     }
    else if( UefiGetQuestionOffset(data) /*data->QuestionId*/
            == STRUCT_OFFSET(IDE_SECURITY_CONFIG,IDEMasterPassword_HDD21) )
     {
      *pHardDiskNumber = 20;
      *pbCheckUser = FALSE;
     }
    else if( UefiGetQuestionOffset(data) /*data->QuestionId*/
            == STRUCT_OFFSET(IDE_SECURITY_CONFIG,IDEMasterPassword_HDD22) )
     {
      *pHardDiskNumber = 21;
      *pbCheckUser = FALSE;
     }
    else if( UefiGetQuestionOffset(data) /*data->QuestionId*/
            == STRUCT_OFFSET(IDE_SECURITY_CONFIG,IDEMasterPassword_HDD23) )
     {
      *pHardDiskNumber = 22;
      *pbCheckUser = FALSE;
     }
    else if( UefiGetQuestionOffset(data) /*data->QuestionId*/
            == STRUCT_OFFSET(IDE_SECURITY_CONFIG,IDEMasterPassword_HDD24) )
     {
      *pHardDiskNumber = 23;
      *pbCheckUser = FALSE;
     }
