...
#endif
#if defined(OemPCIeDev_SUPPORT) && (OemPCIeDev_SUPPORT == 1)  //<lvych001-20170915 Don't create NVME Legacy boot option when device is disable.(mantis:16317)+>
#include <PciBus.h> 
#endif //<lvych001-20170915 Don't create NVME Legacy boot option when device is disable.(mantis:16317)->
    ...
    for (LinkData = NvmeController->ActiveNameSpaceList.ForwardLink; 
        ...
        if(ActiveNameSpace->IdentifyNamespaceData.NSIZE) {
            ...
            if ((NvmeController->IdentifyControllerData->OACS & BIT0)) {
                // Install EFI_STORAGE_SECURITY_COMMAND_PROTOCOL on BlockIo handle itself for each active NameSpace
                Status = gBS->InstallMultipleProtocolInterfaces (
                                     &(ActiveNameSpace->NvmeDeviceHandle),
                                     &gEfiStorageSecurityCommandProtocolGuid, 
                                     &(NvmeController->StorageSecurityInterface),
...
/**
    Check if Security Send/Receive supported by controller.
    If supported, assign interface to the controller structure which will be used by security modules.
    Install EFI_STORAGE_SECUIRTY_COMMAND_PROTOCOL
    Issue Tper Reset if Memory Overrite Request is set.
...
)
{
    ...
    UINT32                      Index;
    //<ChengXW001-20180115 EIP341451 Make NVME name different +>
    UINTN                       SegmentNumber;
    UINTN                       BusNumber;
    UINTN                       DeviceNumber;
    ...
    //<ChengXW001-20180115 EIP341451 Make NVME name different +>
    Status = NvmeController->PciIO->GetLocation (
    ...
    if(!EFI_ERROR(Status)) {
        //L"Bx: "
        DeviceName[0] = 0x42;//L"B"
        //SGMTB#0157685--BootOption Name Error For NVMe+>
        //DeviceName[1] = (UINT16)(0x30 + BusNumber/16);//L"x"
        //DeviceName[2] = (UINT16)(0x30 + BusNumber%16);//L"x"
        DeviceName[1] = (UINT16)(((BusNumber >= 0xA0)?0x37:0x30) + BusNumber/16);//L"x"//(UINT16)(0x30 + BusNumber/16);//L"x"
        DeviceName[2] = (UINT16)((((BusNumber%16)>=0xA)?0x37:0x30)+(BusNumber%16));//(UINT16)(0x30 + BusNumber%16);//L"x"
        //SGMTB#0157685--BootOption Name Error For NVMe->
    ...
    //}
    for (Index = 0; Index < 40 -5; Index ++) {
        DeviceName[Index + 5] = ((UINT8 *)NvmeController->IdentifyControllerData->ModelNumber)[Index];
    }
    //<ChengXW001-20180115 EIP341451 Make NVME name different ->
...
)
{
    ...
    dev = (PCI_DEV_INFO *)NvmeController->PciIO;
    if((dev->Signature == AMI_PCI_SIG)&&(dev->OemPolicy == CSMSETUP_SKIP_OPROMS))
        return;
#endif   //<lvych001-20170915 Don't create NVME Legacy boot option when device is disable.(mantis:16317)->
...
{    
    ...
    for (Index = 0; Index < NvmeHandleCount; Index++) {
        ...
        PreBootLoadCount = CompletionData.DW0;
        ...
        if (PreBootLoadCount < 0xFF) {		            
            ...
            Status = ExecuteNvmeCmd (NvmeController, NvmeCmdWrapper, &CompletionData);
            //ASSERT_EFI_ERROR(Status);  //<wangyia001-20190814 fix Intel SSD 760P series 2TB hang>
        }    
    }
    gBS->FreePool(NvmeControllerHandleBuffer);
    gBS->FreePool(NvmeCmdWrapper);
