...
BOOLEAN CreateTargetEfiOsBootOption(EFI_HANDLE FileSystemHandle, NAME_MAP* NameMap)
{
    ...
    if (EFI_ERROR(Status)) {
        DEBUG((DEBUG_BM | DEBUG_VERBOSE | DEBUG_ERROR, "CreateTargetEfiOsBootOption :: AllocatePool %r\n", Status));
        return FALSE;
    }
    ...
    if (EFI_ERROR(Status)) {
        DEBUG((DEBUG_BM | DEBUG_VERBOSE | DEBUG_ERROR, "CreateTargetEfiOsBootOption :: GetHdNode: %r\n", Status));
    ...
    // CompareHddDevicePath convert pathname to uppercase, it may cause the flash information to be inconsistent.  
    // so convert pathname before saving to the flash.
    Wcsupr(FpDevPath->PathName);
...
#define ValidBootCount(CurrentCount,BootOptionCheck)\
    {\
        UINTN   iBoorOrderCheck=0;\
        for (iBoorOrderCheck = 0; iBoorOrderCheck < BootOrderSize/sizeof(UINT16); iBoorOrderCheck++)\
        {\
            /* code */\
            if(BootOrder[iBoorOrderCheck] == CurrentCount)\
            {\
                DEBUG ((DEBUG_ERROR, "%a %d Boot%04x Match!!! \n", __FUNCTION__, __LINE__,BootOrder[iBoorOrderCheck]));\
                BootOptionCheck=EFI_SUCCESS;\
                break;\
            }\
        }\
        \
...
VOID SyncBootOrderAndOption(SIMPLE_BOOT_OPTION *Options, UINTN OptionCount)
{
    EFI_STATUS                  Status;
    UINTN                       BootOrderSize;
    UINT16                      *BootOrder = NULL,*TempBootOrderBuffer;
    //
    UINT16                      iBootCount,LastBootCount=0;
    UINTN                       sBootOrder,BootOrderCount;
    ...
    UINT32                      MaxBootOptionCount=MaxOptionCountReserved;
    ...
    for(iBootCount=0;iBootCount<BootOrderCount;iBootCount++)
        DEBUG ((DEBUG_ERROR, "%a %d BootOrderSize:%x BootOrder(%x):%04x\n", __FUNCTION__, __LINE__,BootOrderSize,BootOrderCount,BootOrder[iBootCount]));
    //
    sBootOrder=0xFFFF*sizeof(UINT16);
    //todo
    //Loop BootXXXX And Boot Order To Sync Them    
    Status = pBS->AllocatePool (EfiBootServicesData,sBootOrder,&TempBootOrderBuffer);
    MemSet(TempBootOrderBuffer,sBootOrder,0x00);

    MemCpy(TempBootOrderBuffer,BootOrder,BootOrderSize);
    BootOrder=TempBootOrderBuffer;
    DEBUG ((DEBUG_ERROR, "%a %d  TempBootOrderBuffer(%x-%04x):%04x\n", __FUNCTION__, __LINE__,TempBootOrderBuffer,*TempBootOrderBuffer,*BootOrder));

    TempBootOrderBuffer+=BootOrderCount;
    DEBUG ((DEBUG_ERROR, "%a %d  TempBootOrderBuffer(%x)\n", __FUNCTION__, __LINE__,TempBootOrderBuffer));

    for (iBootCount = 0; iBootCount < MaxBootOptionCount; iBootCount++)
    {
        /* code */
        Swprintf(BootStr,L"Boot%04X",iBootCount);
        DEBUG ((DEBUG_ERROR, "%a %d Boot%04X Is Searching!! - ", __FUNCTION__, __LINE__,iBootCount));
        NvramOption=NULL;
        Status = GetEfiVariable(BootStr, &EfiVariableGuid, NULL, &NvramOptionSize, (VOID**)&NvramOption);
        if (EFI_ERROR(Status)) 
        {
            DEBUG ((DEBUG_ERROR, "No Found,Continue Loop!! \n"));
            continue;
        }
        DEBUG ((DEBUG_ERROR, "Has Been Found!! \n"));


        Status=EFI_UNSUPPORTED;
        ValidBootCount(iBootCount,Status);
        DEBUG ((DEBUG_ERROR, "%a %d ValidBootCount %r \n", __FUNCTION__, __LINE__,Status));
        if(EFI_ERROR (Status))
        {
            DEBUG ((DEBUG_ERROR, "%a %d Append Boot To Temp List!!! \n", __FUNCTION__, __LINE__));
            *TempBootOrderBuffer=iBootCount;
            TempBootOrderBuffer++;
            LastBootCount++;
        }
    }
    BootOrderSize+=sizeof(UINT16)*LastBootCount;
    DEBUG ((DEBUG_ERROR, "%a %d BootOrderSize:%x LastBootCount:%x\n", __FUNCTION__, __LINE__,BootOrderSize,LastBootCount));
    Status = pRS->SetVariable (L"BootOrder",&EfiVariableGuid,BOOT_VARIABLE_ATTRIBUTES,BootOrderSize,BootOrder);
    if(EFI_ERROR (Status))
        DEBUG ((DEBUG_ERROR, "%a %d Sync Boot Order:%r \n", __FUNCTION__, __LINE__,Status));

    for(iBootCount=0;iBootCount<(BootOrderSize/sizeof(UINT16));iBootCount++)
        DEBUG ((DEBUG_ERROR, "%a %d BootOrderSize:%x BootOrder(%x):%04x\n", __FUNCTION__, __LINE__,BootOrderSize,BootOrderCount,BootOrder[iBootCount]));

    Status = pBS->FreePool (BootOrder);
