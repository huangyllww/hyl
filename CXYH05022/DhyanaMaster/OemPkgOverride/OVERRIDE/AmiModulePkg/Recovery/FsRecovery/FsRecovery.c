...
//Long name support+
	CHAR8   TmpName[13];
    if (IsUnicode) {
        CHAR16 *Name16 = (CHAR16*)inFileName;
        
        for (i=0; i<13; i++) {
            TmpName[i] = (CHAR8)(Name16[i] & 0xff);
            if (Name16[i] == 0) break;
        }
	} else {
        for (i=0; i<13; i++) {
            TmpName[i] = inFileName[i];
            if (inFileName[i] == 0) break;
        }
    }
    for ( i = 0; TmpName[i] && TmpName[i] != '.'; i++ ) {
        outFileName[i] =     toupper( TmpName[i] );
    }
    j = i;
    ...
    if ( TmpName[j] == '.' ) {
        for ( j++; TmpName[j]; i++, j++ ) {
            outFileName[i] = toupper( TmpName[j] );
        }
    }
...
{
    UINT8   len, i;
    PEI_TRACE((-1, ThisPeiServices, "ThisVolume->FatType is %d\n",ThisVolume->FatType));
    //if (!(ThisVolume->FatType == FAT32 || ThisVolume->FatType == FAT16)) return FALSE;
    if (IsUnicode) {
        CHAR16 *Name16 = (CHAR16*)Name;
        len = StrLen(Name16);
        if ( len > 12 ) return TRUE; // Complete name too long
        for (i=0; i<len; i++) {
            if (Name16[i] > 255) return TRUE; // Non ASCII character
            if (Name16[i] == '.') break;
        }
        if (i > 8) return TRUE; // Name before extension too long
        if ((len - i) > 4) return TRUE; // Extension too long
        return FALSE;
    } else {
        len = Strlen(Name);
...
VOID
FetchLongName( 
    IN DIR_ENTRY_LFN    *DirName,
    IN OUT CHAR8        *LongName
) 
{
    UINT32 i, k;
    UINT8 Order;
    UINT32 NamePos = 0;
    DIR_ENTRY_LFN *DirPtr = DirName;
    CHAR16 *LongName16 = (CHAR16*)LongName;

    Order = (DirPtr->Dir_Order & 0x1F);
    DirPtr += (Order - 1); // Move to first element of name
    i = 0;
    do {
        if (IsUnicode) {
            for (k = 0; k < 5; k++) {
                LongName16[NamePos++] = DirPtr->Dir_Name0_4[k];
            }
            for (k = 0; k < 6; k++) {
                LongName16[NamePos++] = DirPtr->Dir_Name5_10[k];
            }
            for (k = 0; k < 2; k++) {
                LongName16[NamePos++] = DirPtr->Dir_Name11_12[k];
            }
        } else {
            for (k = 0; k < 5; k++) {
                LongName[NamePos++] = (CHAR8)(DirPtr->Dir_Name0_4[k] & 0xFF);
            }
            for (k = 0; k < 6; k++) {
                LongName[NamePos++] = (CHAR8)(DirPtr->Dir_Name5_10[k] & 0xFF);
            }
            for (k = 0; k < 2; k++) {
                LongName[NamePos++] = (CHAR8)(DirPtr->Dir_Name11_12[k] & 0xFF);
            }
        }
        DirPtr--; 
        i++;
...
) 
{
    ...
    if (IsUnicode) {
        CHAR16 *Lname1 = (CHAR16*)Name1;
        CHAR16 *Lname2 = (CHAR16*)Name2;
        
        if (StrLen(Lname1) != StrLen(Lname2))
            return FALSE;
        for (i = 0; i < StrLen(Lname1); i++) {
            if (toupper16(Lname1[i]) != toupper16(Lname2[i]))
                return FALSE;
        }
        return TRUE;
    } else {
...
//Long name support+
    if (IsUnicode) {
        ...
        TempPath = (CHAR16*)*FilePath;
        TempNext = (CHAR16*)*NextName;
        len = StrLen(TempPath);
        GotName = TRUE;
        for ( i = 0; i < len; i++ ) {
            if ( TempPath[i] == 0x002F ) { // "/"
                GotName = FALSE;
                TempPath[i] = 0;
                TempNext = TempPath + i + 1;
                *NextName = (CHAR8*)TempNext;
                return GotName;
            }
        }
        return GotName;
...
)
{
    ...
    *NumberOfFiles = 0;     //no files found yet
    ...
    Status = GetRecoveryFileInfo(ThisPeiServices, &FileName, &FileSize, NULL);
    PEI_TRACE((-1, ThisPeiServices, "FileName is %s\n",FileName)); //Long name support
    if(EFI_ERROR(Status))
        return;
#if SEARCH_PATH
    ...
    while ( IsolateName (&FilePath, &NextName) == FALSE ) {
        // Long name support++
        if (CheckIfLongName(FilePath)) {
            PEI_TRACE((-1, ThisPeiServices, "\nLooking for... %S\n", (CHAR16*)FilePath));
            for( i = 0; i < DirEntries; i++ ) {
                if( ((DirPtr[i].FileName[0] & (CHAR8)0xE0) == (CHAR8)0x40) && (DirPtr[i].DirAttr == 0x0F) ) {
                    CHAR8 TmpPathName[250];
                    FetchLongName((DIR_ENTRY_LFN*)&DirPtr[i].FileName[0], TmpPathName);
                    if (MatchLongName(FilePath, TmpPathName)) {
                        i += (DirPtr[i].FileName[0] & (CHAR8)0x1F); // Point to short name
                        goto GotDirMatch;
                    }
                }
            } 
    ...
    if (CheckIfLongName(FilePath)) {
        for( i = 0; i < DirEntries; i++ ) {
            if( ((DirPtr[i].FileName[0] & (CHAR8)0xE0) == (CHAR8)0x40) && (DirPtr[i].DirAttr == 0x0F) ) {
                CHAR8 TmpPathName[250];
                FetchLongName((DIR_ENTRY_LFN*)&DirPtr[i].FileName[0], TmpPathName);
                PEI_TRACE((-1, ThisPeiServices, "\nabbott long name TmpPathName %s\n", TmpPathName));
                if (MatchLongName(FilePath, TmpPathName)) {
                    i += (DirPtr[i].FileName[0] & (CHAR8)0x1F); // Point to short name
                    Buffer[*NumberOfFiles] = &DirPtr[i];
                    *NumberOfFiles = 1;
                    PEI_TRACE((-1, ThisPeiServices, "\nFound recovery file (long)\n"));
                    return;
                }
            }
        } 
    ...
    for( i = 0; i < DirEntries; i++ ) {
        ...
        if ( ((DirPtr[i].DirAttr & (ATTR_DIRECTORY | ATTR_VOLUME_ID)) == ATTR_VOLUME_ID) && IsMbr ) {
            ...
            for ( j=0; j<11; j++ ) {   //Long name support
                if ( DirPtr[i].FileName[j] ==(CHAR8) 0x20 ) {
                    DirPtr[i].FileName[j] = (CHAR8)0; // Terminate name
                }
            }
