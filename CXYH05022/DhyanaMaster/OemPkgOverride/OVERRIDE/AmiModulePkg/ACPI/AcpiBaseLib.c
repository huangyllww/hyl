...
)
{
	...
	if (EFI_ERROR (Status)) {
		DEBUG((EFI_D_ERROR,"Acpi BuildFPDT: No Cpu Protocol was found.\n"));
	...
	if (!EFI_ERROR (Status)) {
		...
		DEBUG((EFI_D_INFO,"Acpi BuildFPDT:  Cpu Protocol was found.\n"));
...
//-----------------------------
    ...
    *TablPtr = MallocZ (SizeOfFacp);
    ...
    //if ((TablVer == 1) || (ACPI_SPEC_VERSION < 61))
    //{
...
//---Creating IOAPIC or/and IOSAPIC Entries
    ...
    //Alex - Changed to search SDL info, not all APICs at FEC00000..0xFED00000
    Status = pBS->LocateProtocol(
            &gAmiBoardInfo2ProtocolGuid,
            NULL,
            &AmiBoardInfo2Protocol);
    
    for(i=0; i< AmiBoardInfo2Protocol->ApicBrdData->IoApicCount; i++)  
    { //If so read IOAPIC.ID reg and figure out
        //1.How many INTINs it has
        //2.What ID BIOS give to it
        if(i)
            IoapicAddr = (UINT32) AmiBoardInfo2Protocol->ApicBrdData->IoApicInfo[i-1].ApicAddress;
        if(IoapicAddr == 0xDEADBEEF)  {
            TRACE((TRACE_ALWAYS, "\n APIC Num %d, not available \n", i));
            continue;
        }
...
EFI_STATUS PublishTbl (IN UINT8 RsdtBuild, IN UINT8 XsdtBuild)
{
    ...
    {
    	DEBUG((EFI_D_INFO,"In PublishTbl: No Facp and FacpX! Publishing will be delayed until they will be available. \n"));
    	return EFI_SUCCESS;
    }
    if ((Dsdt == NULL) && (DsdtX == NULL))
    {
    	DEBUG((EFI_D_INFO,"In PublishTbl: No Dsdt and DsdtX! Publishing will be delayed until they will be available. \n"));
        return EFI_SUCCESS;
    }
    if (Facp == NULL)
    {
    	if (XsdtBuild == 0)
    	{
    		DEBUG((EFI_D_INFO,"In PublishTbl: No Facp and XsdtBuild = 0! Publishing will be delayed until it will be available. \n"));
    		return EFI_SUCCESS;
    	}
    	DEBUG((DEBUG_INFO,"In PublishTbl: No Facp. RSDT will not be published.\n"));
    	RsdtBuild = 0;
    }
    if (FacpX == NULL)
    {
    	if (RsdtBuild == 0)
    	{
    		DEBUG((EFI_D_INFO,"In PublishTbl: No FacpX and RsdtBuild = 0! Publishing will be delayed until it will be available. \n"));
    	    return EFI_SUCCESS;
    	}
    	DEBUG((EFI_D_INFO,"In PublishTbl: No FacpX. XSDT will not be published.\n"));
        XsdtBuild = 0;
    }
...
//    if (RsdtBuild) Status = pBS->InstallConfigurationTable(&gAcpi11TAbleGuid, NULL); // Delete V1.1 entry - Only one
        ...
        DEBUG((EFI_D_INFO,"Installing ACPI 2.0: %r, %X\n",Status,gRsdtPtrStr));
    }
	...
	if (RsdtBuild) 
	{
    	...
    	DEBUG((EFI_D_INFO,"Installing ACPI 1.1: %r, %X\n",Status,gRsdtPtrStr));
        ASSERT_EFI_ERROR(Status);
...
//    FACP32      *Facp = NULL;
    ...
    static BOOLEAN CollectAmlUpdInfoIsSaved = FALSE;//<Lvshh001-20221108 For MemoryLeak Fix,otherwise when the EfiBootServicesData increases too much, the hang60 will occur>
//-------------------------------------------
    ...
    if (CollectAmlUpdInfoIsSaved){
        return;
    }
    CollectAmlUpdInfoIsSaved = TRUE;
    //<Lvshh001-20221108 For MemoryLeak Fix,otherwise when the EfiBootServicesData increases too much, the hang60 will occur ->
    ...
    ASSERT_EFI_ERROR(Status);
    DEBUG((EFI_D_INFO,"IN Collect AML Info after PT: %x\n",Status));
    ...
    if ( xFacp != NULL)
    {
        ...
        if (xFacp->X_PM1a_CNT_BLK.Address !=0)
        {
        	AcpiResetVar.Addess = xFacp->X_PM1a_CNT_BLK.Address;
        	AcpiResetVar.AccessSize = xFacp->X_PM1a_CNT_BLK.Reserved1;
        	AcpiResetVar.Type = xFacp->X_PM1a_CNT_BLK.AddrSpcID;		
        }
        else
        if (xFacp->PM1a_CNT_BLK !=0)
        {
        	AcpiResetVar.Addess = (UINT64)(xFacp->PM1a_CNT_BLK);
    ...
    if ( Facp != NULL )
    {
        ...
        {
        	if (Facp->PM1a_CNT_BLK !=0)
        	{
        		AcpiResetVar.Addess = (UINT64)(Facp->PM1a_CNT_BLK);
        		AcpiResetVar.AccessSize = 2;
        		AcpiResetVar.Type = 1;		
        	}	
        }
