...
{AGESA_BOOT_LOADER, 0, 0},              //Type 0x30
{PSP_DIR_LV2, 0, 0},                    //Type 0x40
{PSP_SECURE_FW, 0, 0},                  //Type 0x49
{PSP_TPCM_FW, 0, 0},                    //Type 0x5e
};
    ...
    for (i = 0; i < FirmwareVersionCountL1; i++)
    {
        for (j = 0; j < ((PSP_DIRECTORY*)RomPtr)->Header.TotalEntries; j++)
        {
            if (((PSP_DIRECTORY*)RomPtr)->PspEntry[j].Type == FirmwareVersionL1[i].Type)
            {
                ...
                FirmwareVersionL1[i].Size = ((PSP_DIRECTORY*)RomPtr)->PspEntry[j].Size;
                break;
            }
        }
    }
    ...
    for (i = 0; i < FirmwareVersionCountL2; i++)
    {
        for (j = 0; j < ((PSP_DIRECTORY*)RomPtr)->Header.TotalEntries; j++)
        {
            if (((PSP_DIRECTORY*)RomPtr)->PspEntry[j].Type == FirmwareVersionL2[i].Type)
            {
                ...
                FirmwareVersionL2[i].Size = ((PSP_DIRECTORY*)RomPtr)->PspEntry[j].Size;
                break;
            }
        }
    }
    ...
    IN UINT16           Class )
{
    ...
    if (Class == ADVANCED_FORM_SET_CLASS) {
            ...
            if (FirmwareVersionL1[1].Size) {
              p = (UINT8 *)(FirmwareVersionL1[1].Location + 0x60);//SMU FW Version
              InitString(HiiHandle, STRING_TOKEN(STR_SMU_FW_VER_VALUE_L1), L"%d.%d.%d.%d", *(p+3), *(p+2), *(p+1), *p);
            }

            if (FirmwareVersionL1[2].Size) {
            ...
            if (FirmwareVersionL1[4].Size) {
                p = (UINT8 *)(FirmwareVersionL1[4].Location + 0x60);//PSP Secure FW Version
                InitString(HiiHandle, STRING_TOKEN(STR_PSP_SECURE_VER_VALUE), L"%d", *(UINT32 *)p);
            }
            
            if (FirmwareVersionL1[5].Size) {
                p = (UINT8 *)(FirmwareVersionL1[5].Location + 0x60);//TPCM FW Version
                //InitString(HiiHandle, STRING_TOKEN(STR_PSP_TPCM_VER_VALUE), L"%d", *(UINT32 *)p);
                InitString(HiiHandle, STRING_TOKEN(STR_PSP_TPCM_VER_VALUE), L"0x%x", *(UINT32 *)p); //Show TPCM Version In HEX

            }
            
            //PSP directory Level 2 Updatable region
            if (FirmwareVersionL2[0].Size) {
            ...
            if (FirmwareVersionL2[1].Size) {
              p = (UINT8 *)(FirmwareVersionL2[1].Location + 0x60);//SMU FW Version
              InitString(HiiHandle, STRING_TOKEN(STR_SMU_FW_VER_VALUE_L2), L"%d.%d.%d.%d", *(p+3),*(p+2), *(p+1), *p);
            }

            if (FirmwareVersionL2[2].Size) {
