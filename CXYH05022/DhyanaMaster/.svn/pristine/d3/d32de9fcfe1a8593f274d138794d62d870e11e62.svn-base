...
#include <Efi.h>
#include <Token.h>
#include <PciSetup.h>  //<Chendl002-20170521 Add Pcie List Function Which Can Enable/Disable PCIe Device Option by User>
#include <Setup.h>  //<Chendl002-20170521 Add Pcie List Function Which Can Enable/Disable PCIe Device Option by User>
//---------------------------------------------------------------------------
//Protocols used
...
//<Chendl001-20171129 Add The Solution For J11 && J12 Hang Issue +>
#include <Library/PcdLib.h>
#include <Library/PciLib.h>
//<Chendl001-20171129 Add The Solution For J11 && J12 Hang Issue ->

//<licm-20210226 VGA and UART Display 16bit Port80 SUPPORT +>
#if defined(OEM_PORT80_SUPPORT) && (OEM_PORT80_SUPPORT == 1)
...
#ifndef MAKE_BDF
#define MAKE_BDF(Bus, Dev, Fun) ((((UINT32) (Bus)) << 20) | (((UINT32)(Dev)) << 15) | (((UINT32)(Fun)) << 12))
#endif
#if defined(OemPCIeDev_SUPPORT) && (OemPCIeDev_SUPPORT == 1)  //<lvych001-20161128 Sync with PCIe Dev List Module Config +>//<lvych00120160706+>
#include "OemPCIeDev.h"
#endif //<lvych001-20161128 Sync with PCIe Dev List Module Config ->//<lvych00120160706->
...
//<bieyb001-20170406 Revised 82599 do not function properly when it is allocated with an address above 4G. +>
//Intel 82599
{icBad64BitBar, 0x8086,     0x10F8,     0xFFFF,     0x0000,     tBarMaxType,0x0010,     0,          0           }, 
...
//AMD Radeon HD8570
{icBad64BitBar, 0x1002,     0x6611,     0xFFFF,     0x0000,     tBarMaxType,0x0010,     0,          0},
//AMD W4100
//{icBad64BitBar, 0x1002,     0x682C,     0xFFFF,     0x0000,     tBarMaxType, 0x0010,     0,          0},
//Nvidia P400
{icBad64BitBar, 0x10DE,     0x1CBC,     0xFFFF,     0x0000,     tBarMaxType,0x0010,     0,          0},
};
...
//-----------
    //<licm-20210226 VGA and UART Display 16bit Port80 SUPPORT +>
        ...
        PROGRESS_CODE16_EX(DXE_PCI_BUS_BEGIN,(UINT8)PORT81_EX_DXE_PCI_REG_NOTIFY);
    #endif
    //<licm-20210226 VGA and UART Display 16bit Port80 SUPPORT ->
...
//-----------------------------    
    ...
    Status=pRS->GetVariable(L"ROM_CMN",&RomSetupGuid,NULL, &sz, gRomSetupPtr);
    ASSERT_EFI_ERROR(Status);
    return;
}
...
//----------------
    ...
    //-----------------------------------
        if(rp->SdlIdx==Dev->SdlDevIndex){
            ...
            Dev->AmiRomPolicy->CurrentPolicy=rp->Option;
...
//------------------
    ...
    if(Dev->Type<tPci2PciBrg) {
        PCI_TRACE((TRACE_PCI,"Pci2PciBrg \n"));
        return EFI_SUCCESS;
    }
#if EFI_DEBUG || USB_DEBUG_TRANSPORT
    ...
    if(EFI_ERROR(Status)) return Status;
    PCI_TRACE((TRACE_PCI,"  PciCfgreadvalue_b16=%x\n",b16));
    switch (WhatSpace){
        case stOptRomSpace:
    ...
    PCI_TRACE((TRACE_PCI,"  PciCfgreadromvalue_b16=%x\n",b16));
    ...
    //----------------------    
        ...
        b32|=1;
        PCI_TRACE((TRACE_PCI,"  PciCfg_EROMBAR_b32=%x\n",b32));
        Status=PciCfg32(Dev->RbIo,addr,TRUE,&b32);
        if(EFI_ERROR(Status)) return Status;
    }
    Status=PciCfg32(Dev->RbIo,addr,FALSE,&b32);
    PCI_TRACE((TRACE_PCI,"  PciCfg_EROMBAR_b32_out=%x\n",b32));
...
//Mizl-20200704-Skip Bad Device When The Device is Connected To Switch +>
EFI_STATUS
GetLegCapStrucAddr (
    IN UINT64                   PciAddr64,
    IN UINT8                    FindCapNo,
...
PCI_BAD_BAR* CheckPciCompatibility(PCI_DEV_INFO *Device, PCI_BAR *Bar, PCI_BAR_TYPE BarType) //(EIP41687)
{
    ...
    EFI_STATUS          Status;
    UINT8               CapPtr8;
    UINT16              PCIe_PortType;
    UINT8               PCIe_HeaderType;
    //Mizl-20200704-Skip Bad Device When The Device is Connected To Switch ->
//-----------------------
    DEBUG((EFI_D_ERROR, "[%a][%d]BarType:%x\n\n",__FUNCTION__,__LINE__,BarType));
    ...
    for(i=0; i<gBadPciDevList.ItemCount; i++){
        ...
        if(d.DEV_VEN_ID==t.DEV_VEN_ID){
            //Mizl-20200704-Skip Bad Device When The Device is Connected To Switch +>
            DEBUG((EFI_D_ERROR, "[%a][%d]BDF:%x-%x-%x\n\n",__FUNCTION__,__LINE__,Device->ParentBrg->Address.Addr.Bus,Device->ParentBrg->Address.Addr.Device,Device->ParentBrg->Address.Addr.Function));
            Status = GetLegCapStrucAddr(PCI_LIB_ADDRESS(Device->ParentBrg->Address.Addr.Bus,Device->ParentBrg->Address.Addr.Device,Device->ParentBrg->Address.Addr.Function,0x00),0x10,&CapPtr8);
            if(!EFI_ERROR(Status))
            {
                PCIe_PortType = PciRead16(PCI_LIB_ADDRESS(Device->ParentBrg->Address.Addr.Bus,Device->ParentBrg->Address.Addr.Device,Device->ParentBrg->Address.Addr.Function,CapPtr8+0x02));
                PCIe_PortType = (PCIe_PortType >> 4) & 0x0F;

                PCIe_HeaderType = PciRead8(PCI_LIB_ADDRESS(Device->ParentBrg->Address.Addr.Bus,Device->ParentBrg->Address.Addr.Device,Device->ParentBrg->Address.Addr.Function,0x0E));

                if(PCIe_HeaderType && (PCIe_PortType == 0x06))
                {
                    DEBUG((EFI_D_ERROR, "[%a][%d]Device Connect Switch Down Port,Skip 64BIT Setting,Ignore Legacy PXE Function!!!!\n\n",__FUNCTION__,__LINE__));
                    return NULL;
                }
            }
            //Mizl-20200704-Skip Bad Device When The Device is Connected To Switch ->
...
//---------------------------------
    ...
    if(Device->AmiRomPolicy!=NULL) {
        if((rombar->Type!=tBarUnused) ||
            (Device->AmiSdlPciDevData!=NULL && Device->AmiSdlPciDevData->PciDevFlags.Bits.EmbededRom)){
            Device->AmiRomPolicy->RomStatus=TRUE;
        }
    } else {
...
#if HOTPLUG_SUPPORT
EFI_STATUS ForceReservedResourceForHotplugBridge (
    IN     UINTN           ResourceType, 
    IN     PCI_DEV_INFO    *pPciDeviceBridge,
    IN OUT PCI_BAR         *pPciBar, 
    IN OUT BRG_ALIGN_INFO  *pBridgeAlignInfo
)
{
  EFI_STATUS          Status = EFI_SUCCESS;
	UINT8               Index;
	UINT32              ResevePcieMmioLength = 0;
	UINT32              ResevePcieIOLength = 0;
    
    if(pPciDeviceBridge == NULL || 
       pPciBar == NULL || 
       pBridgeAlignInfo == NULL)
    {
        return EFI_INVALID_PARAMETER;
    }

    ResevePcieMmioLength = RSVD_PCI_MEM;
    ResevePcieIOLength = RSVD_PCI_IO;
    
    if(!PcdGetBool(PcdCfgPcieHotplugSupport))
    {
        ResevePcieMmioLength = 0;
        ResevePcieIOLength = 0;
    }
    ...
    case rtIo16:
        pPciBar->Base = 0;
        pPciBar->Length = ResevePcieIOLength * 0x400;
        if(pPciBar->Length!=0){
            Index=FindFirstBit(pPciBar->Length-1,FALSE);
            pPciBar->Gran=Shr64((~0ULL),64-Index);
        } else {
            pPciBar->Gran = P2P_BRG_IO_GRA;
        }
        pBridgeAlignInfo->MinGran = pPciBar->Gran;
        pBridgeAlignInfo->MaxGran = pPciBar->Gran;
        pBridgeAlignInfo->ResGran = 0;
        break;
    ...
    case rtMmio32:
        pPciBar->Base = 0;
        pPciBar->Length = ResevePcieMmioLength * 0x100000; //Multiple 1MB
        if(pPciBar->Length!=0){
            Index=FindFirstBit(pPciBar->Length-1,FALSE);
            pPciBar->Gran=Shr64((~0ULL),64-Index);
        } else {
            pPciBar->Gran = P2P_BRG_MEM_GRA;
        }
        pBridgeAlignInfo->MinGran = pPciBar->Gran;
        pBridgeAlignInfo->MaxGran = pPciBar->Gran;
        pBridgeAlignInfo->ResGran = 0;
    ...
    case rtMmio64p:
        pPciBar->Base = 0;
        pPciBar->Length = ResevePcieMmioLength * 2 * 0x100000;
        if(pPciBar->Length!=0){
            Index=FindFirstBit(pPciBar->Length-1,FALSE);
            pPciBar->Gran=Shr64((~0ULL),64-Index);
        } else {
...
#if 0
        //slot support Hotplug function and slot is empty
        if(ext->ChildCount == 0)
        {
            ForceReservedResourceForHotplugBridge(j, Brg, bbar, balign);
        }
#endif
...
EFI_STATUS EFIAPI EnumerateBus(PCI_DEV_INFO *ParentBrg)
{
    ...
    BOOLEAN         ari_device_is_next_function_zero ; // ARI device with Next Function Number comes as "0". 
...
//----------------------------------    
    ...
    func0ari=NULL;
    ari=FALSE;
    ari_device_is_next_function_zero = FALSE; 

    for(pcidev.Addr.Device=0; pcidev.Addr.Device<=PCI_MAX_DEVICE; pcidev.Addr.Device++){
    if (ari_device_is_next_function_zero) continue; 
#if PCI_DEV_REVERSE_SCAN_ORDER
        ...
        for(pcidev.Addr.Function=0;  pcidev.Addr.Function<=PCI_MAX_FUNC; pcidev.Addr.Function++){
            ...
            PCI_TRACE((TRACE_PCI,"\n==========================================================================\n"));
            PCI_TRACE((TRACE_PCI,"PciBus: Probing PCI Device @ [B%X|D%X|F%X](VIDDID: %X)...", 
                    pcidev.Addr.Bus, pcidev.Addr.Device, pcidev.Addr.Function, id));
            Status=PciCfg32(ParentBrg->RbIo,pcidev,FALSE,&id);
...
#endif
                    {
                        if(dev->SkipDevice){
                            ...
                            if(Pcie2CheckPcie2Compatible(dev) && dev->Address.Addr.Device==0 && dev->Address.Addr.Function==0){
                                ...
                                if (ari)
                                {
                                  if (dev->PciExpress->AriData->AriCaps.NextFuncN == 0)  { 
                                      ari = FALSE;
                                      ari_device_is_next_function_zero = TRUE;
                                  }
                                }
                                if(EFI_ERROR(Status)) goto EEXIT;
                            }
...
//#if HOTPLUG_SUPPORT
                    ...
                    //<licm-20210226 VGA and UART Display 16bit Port80 SUPPORT +>
                    #if defined(VGA_UART_DIS16BIT_PORT80_SUPPORT) && (VGA_UART_DIS16BIT_PORT80_SUPPORT == 1)
                        PROGRESS_CODE16_EX(DXE_PCI_BUS_BEGIN,(UINT8)PORT81_EX_DXE_PCI_INIT_HP);
                    #endif
                    //<licm-20210226 VGA and UART Display 16bit Port80 SUPPORT ->
...
//<licm-20210226 VGA and UART Display 16bit Port80 SUPPORT +>
#if defined(VGA_UART_DIS16BIT_PORT80_SUPPORT) && (VGA_UART_DIS16BIT_PORT80_SUPPORT == 1)
    PROGRESS_CODE16_EX(DXE_PCI_BUS_BEGIN,(UINT8)PORT81_EX_DXE_PCI_INIT_EXPRESS);
#endif
//<licm-20210226 VGA and UART Display 16bit Port80 SUPPORT ->
...
EEXIT:
    if(dev){
        if(dev->PciExpress){
            ...
            if(Pcie4CheckPcie4Compatible(dev)) pBS->FreePool(dev->PciExpress->Pcie4);
...
//----------------------------
    ...
    //<licm-20210226 VGA and UART Display 16bit Port80 SUPPORT +>
#if defined(VGA_UART_DIS16BIT_PORT80_SUPPORT) && (VGA_UART_DIS16BIT_PORT80_SUPPORT == 1)
    PROGRESS_CODE16_EX(DXE_PCI_BUS_BEGIN,(UINT8)PORT81_EX_DXE_PCI_INIT_BAR);
#endif
    //<licm-20210226 VGA and UART Display 16bit Port80 SUPPORT ->
...
// EFI_STATUS GraphicSwitchVgaRoute()
// {
...
//                         SelectedVGA = dev;
//                     #else
//                     if(SelectedVGA->Attrib == EFI_PCI_IO_ATTRIBUTE_EMBEDDED_DEVICE)
//                          SelectedVGA = dev;    
//                     #endif
//                 }
...
//         }
//         DEBUG((EFI_D_ERROR, "[OemDbg][%a][%d]\r\n",__FUNCTION__,__LINE__));
//         if (SelectedVGA != NULL ) Status = VgaRoute(SelectedVGA);
//         break;
//     }
...
//-------------------------------
    ...
    //<licm-20210226 VGA and UART Display 16bit Port80 SUPPORT +>
#if defined(VGA_UART_DIS16BIT_PORT80_SUPPORT) && (VGA_UART_DIS16BIT_PORT80_SUPPORT == 1)
    PROGRESS_CODE16_EX(DXE_PCI_BUS_BEGIN,(UINT8)PORT81_EX_DXE_PCI_SCAN_ROOTBRIDGE);
#endif
    //<licm-20210226 VGA and UART Display 16bit Port80 SUPPORT ->
    for(j=0; j<gHostCnt; j++){
...
//#endif
            ...
            ASSERT_EFI_ERROR(Status);
            DEBUG((EFI_D_ERROR, "[OemDbg][%a][%d]\r\n",__FUNCTION__,__LINE__));
            // Status = GraphicSwitchVgaRoute();
            // DEBUG((EFI_D_ERROR, "[OemDbg][%a][%d]GraphicSwitchVgaRoute:%r\r\n",__FUNCTION__,__LINE__,Status));
...
//<licm-20210226 VGA and UART Display 16bit Port80 SUPPORT +>                
#if defined(VGA_UART_DIS16BIT_PORT80_SUPPORT) && (VGA_UART_DIS16BIT_PORT80_SUPPORT == 1)
        PROGRESS_CODE16_EX(DXE_PCI_BUS_BEGIN,(UINT8)PORT81_EX_DXE_PCI_REQUEST_RES_STARTED);
#endif
//<licm-20210226 VGA and UART Display 16bit Port80 SUPPORT ->
...
//<licm-20210226 VGA and UART Display 16bit Port80 SUPPORT +>
#if defined(VGA_UART_DIS16BIT_PORT80_SUPPORT) && (VGA_UART_DIS16BIT_PORT80_SUPPORT == 1)
        PROGRESS_CODE16_EX(DXE_PCI_BUS_BEGIN,(UINT8)PORT81_EX_DXE_PCI_REQUEST_RES_END);
#endif
//<licm-20210226 VGA and UART Display 16bit Port80 SUPPORT ->            
    ...
    //<licm-20210226 VGA and UART Display 16bit Port80 SUPPORT +>
#if defined(VGA_UART_DIS16BIT_PORT80_SUPPORT) && (VGA_UART_DIS16BIT_PORT80_SUPPORT == 1)
    PROGRESS_CODE16_EX(DXE_PCI_BUS_BEGIN,(UINT8)PORT81_EX_DXE_PCI_ASSIGN_RES_STARTED);
#endif
            Status=AssignBusResources((PCI_BRG_INFO*)rb);
#if defined(VGA_UART_DIS16BIT_PORT80_SUPPORT) && (VGA_UART_DIS16BIT_PORT80_SUPPORT == 1)
    PROGRESS_CODE16_EX(DXE_PCI_BUS_BEGIN,(UINT8)PORT81_EX_DXE_PCI_ASSIGN_RES_END);
#endif
    //<licm-20210226 VGA and UART Display 16bit Port80 SUPPORT ->
            if(EFI_ERROR(Status)) return Status;
...
//#if PCI_EXPRESS_SUPPORT
            ...
            if(gS3SaveState!=NULL){
                PcieRecordBootScript(rb, gS3SaveState, TRUE);
            }
//#endif
...
//---------------------------------
    ...
    else hsz=((LEGACY_OPT_ROM_HEADER*)rhdrptr)->Size512;
...
//lvych001-20170916 Improve device list function +>
#if defined(OemPCIeDev_SUPPORT) && (OemPCIeDev_SUPPORT == 1)
static OEM_PCIE_DEV_DATA_PROTOCOL    *pOemPcieDevProtocol = NULL;
static UINT8                           gCsmDisFlag = 0xFF;
UINT8 GetOpRomPolicyEx(
    IN UINT8 PciClass
)
{
    UINT8 Policy = 0xff;
    switch(PciClass) {
        case PCI_CL_NETWORK:
            Policy = gPciCommonSetup->OpROMEN[PXEINDX];
            break;
        case PCI_CL_MASS_STOR:
            Policy = gPciCommonSetup->OpROMEN[MASSINDX];
            break;
        case PCI_CL_DISPLAY:
            Policy = gPciCommonSetup->OpROMEN[VIDEOINDX];
            break;
        default:
            Policy = gPciCommonSetup->OpROMEN[OLDINDX];
            break;
    }
    return Policy;
}
VOID ParseOpROMInit()
{
  ...
  AMI_OPROM_POLICY_PROTOCOL 		*AmiOpromPolicyProtocol;
    ...
    PCI_TRACE((TRACE_PCI,"\nParseOpROMInit.\n"));
    gCsmDisFlag = 0;
    Status = pBS->LocateProtocol(&gOemPcieDevProtocolGuid, NULL, (VOID**)&pOemPcieDevProtocol); 
    ASSERT_EFI_ERROR (Status);
...
//<licm-20210513 Optimization PCIE Device List module +>
    ...
    pOemPcieDevProtocol->GetTableListInVariable();
#endif
//<licm-20210513 Optimization PCIE Device List module ->
	}
	return;
}
BOOLEAN IsOPROMDisable(PCI_DEV_INFO *Dev)
{
  EFI_STATUS        Status;
  UINTN             DevIdx;
  ParseOpROMInit();
  Status = pOemPcieDevProtocol->GetDeviceIndex(Dev,&DevIdx);
  if(!EFI_ERROR(Status)) {
    if(gPciCommonSetup->PciDevEN[DevIdx] == 0)
      return TRUE;
  }
  if( (Dev != NULL)&&(Dev->Class.BaseClassCode == 0x01)
      &&(Dev->Class.SubClassCode == 0x08)
      &&(Dev->Class.ProgInterface == 0x02))
  {
    if(gPciCommonSetup->OpROMEN[NVMEINDX])
      return TRUE;
  }
  return FALSE;
}
#endif
EFI_STATUS ParseOpROMSelect(PCI_DEV_INFO *Dev)
{
#if defined(OemPCIeDev_SUPPORT) && (OemPCIeDev_SUPPORT == 1)
  EFI_STATUS        Status; 
  UINT8							PortIdx;
  ...
  ParseOpROMInit();
  Dev->OemPolicy = GetOpRomPolicyEx(Dev->Class.BaseClassCode);
  if((Dev->OemPolicy == POLICY_LEGACY) && (gCsmDisFlag == 1)){ 
      Dev->OemPolicy = POLICY_UEFI;
  }
//<licm-20210513 Optimization PCIE Device List module +>
#if defined(Optimization_PCIE_Device_List_SUPPORT) && (Optimization_PCIE_Device_List_SUPPORT == 1)
  Status = pOemPcieDevProtocol->DynamicGenerationRbtable(Dev);
  if(EFI_ERROR(Status)) {
    return Status;
  }
#endif
//<licm-20210513 Optimization PCIE Device List module ->
  ...
  Status = pOemPcieDevProtocol->GetDevicePortVarIndex(Dev,DevIdx,&PortIdx,&SetupDataIndex);
  if(EFI_ERROR(Status)) {
    return Status;
  }
  PCI_TRACE((TRACE_PCI,"ParseOpROMSelect Org SetupDataIndex %d = %d\n",SetupDataIndex,gPciCommonSetup->OpROMEN[SetupDataIndex]));
  if(gPciCommonSetup->PciDevEN[DevIdx] == 0) {
      Dev->OemPolicy = POLICY_DISABLE;
      return EFI_SUCCESS;
  }
  dIdx = SetupDataIndex;
  if(PortIdx > MAX_DEV_PORT-1){
    tIndex = SetupDataIndex - PortIdx + MAX_DEV_PORT -1;
    if(gPciCommonSetup->OpROMEN[tIndex] == PCI_OPROM_DEFAULT_VALUE){
      dIdx = tIndex;
        if((gPciCommonSetup->OpROMEN[SetupDataIndex] & PCI_DEV_EXT)== PCI_DEV_EXT) {            
      dIdx = SetupDataIndex;	
    }
    } else if((gPciCommonSetup->OpROMEN[tIndex] & PCI_DEV_EXT)== PCI_DEV_EXT){//not default value conflict check
       dIdx = tIndex;
    }
  }
  Value8 = gPciCommonSetup->OpROMEN[dIdx];
  Value8 &= ~PCI_DEV_EXT;                     
  PCI_TRACE((TRACE_PCI,"ParseOpROMSelect Port:%d SetupDataIndex %d = %d\n\n",PortIdx,dIdx,gPciCommonSetup->OpROMEN[dIdx] ));
  if(Value8 == OPTION_AUTO_POLICY)
      return EFI_SUCCESS;
  Dev->OemPolicy = Value8;
  return EFI_SUCCESS;
#else
  return EFI_SUCCESS;
#endif        
...
//<licm-20210226 VGA and UART Display 16bit Port80 SUPPORT +>
#if defined(VGA_UART_DIS16BIT_PORT80_SUPPORT) && (VGA_UART_DIS16BIT_PORT80_SUPPORT == 1)
    PROGRESS_CODE16_EX(DXE_PCI_BUS_BEGIN,(UINT8)PORT81_EX_DXE_PCI_GET_OPROM);
#endif
//<licm-20210226 VGA and UART Display 16bit Port80 SUPPORT ->
...
#if defined(OemPCIeDev_SUPPORT) && (OemPCIeDev_SUPPORT == 1) //<lvych001-20170916 Improve device list function+>
	    if(IsOPROMDisable(Dev)) return EFI_NOT_FOUND;    
#endif //<lvych001-20170916 Improve device list function->
...
//<ChengXW-20200424 AMI EIP416827 Update PciBus GetDriver function to support Pci Oproms with two UEFI images +>
                            ...
                            BOOLEAN                 EmbDriver)
{
    ...
    //<licm-20210226 VGA and UART Display 16bit Port80 SUPPORT +>
#if defined(VGA_UART_DIS16BIT_PORT80_SUPPORT) && (VGA_UART_DIS16BIT_PORT80_SUPPORT == 1)
    PROGRESS_CODE16_EX(DXE_PCI_BUS_BEGIN,(UINT8)PORT81_EX_DXE_EFI_OPROM_STARTED);
#endif
    //<licm-20210226 VGA and UART Display 16bit Port80 SUPPORT -
//-------------------------------------------
...
//<licm-20210226 VGA and UART Display 16bit Port80 SUPPORT +>
#if defined(VGA_UART_DIS16BIT_PORT80_SUPPORT) && (VGA_UART_DIS16BIT_PORT80_SUPPORT == 1)
    PROGRESS_CODE16_EX(PORT80_OEM_DEBUG,Dev->Address.Addr.Bus);
    PROGRESS_CODE16_EX(PORT80_OEM_DEBUG,Dev->Address.Addr.Device);
    PROGRESS_CODE16_EX(PORT80_OEM_DEBUG,Dev->Address.Addr.Function);
#endif
//<licm-20210226 VGA and UART Display 16bit Port80 SUPPORT ->
    //Get the space needed
    romdata=MallocZ(sizeof(PCI_ROM_IMAGE_DATA));
    ...
    //<licm-20210226 VGA and UART Display 16bit Port80 SUPPORT +>
#if defined(VGA_UART_DIS16BIT_PORT80_SUPPORT) && (VGA_UART_DIS16BIT_PORT80_SUPPORT == 1)
    PROGRESS_CODE16_EX(DXE_PCI_BUS_BEGIN,(UINT8)PORT81_EX_DXE_EFI_OPROM_END);
#endif
    //<licm-20210226 VGA and UART Display 16bit Port80 SUPPORT 
...
{
//<lvych001-20161128 Sync with PCIe Dev List Module Config +>
#if defined(OemPCIeDev_SUPPORT) && (OemPCIeDev_SUPPORT == 1)
    UINT8	Policy;
	Policy = Dev->OemPolicy;
    if(Policy != POLICY_UEFI && Policy != POLICY_BOTH) 
        return FALSE;
    return TRUE;
#else
    EFI_STATUS Status;
    ...
    return (EFI_ERROR(Status)) ? FALSE : TRUE;
#endif  //<lvych001-20161128 Sync with PCIe Dev List Module Config>
}
...
//------------------------
    ...
    if( Dev->PciIo.RomImage != NULL ){
        ...
        for(romc=0;;romc++){
            ...
            if(pcir->Signature!=PCI_PCIR_SIG) break;
           ...
           //Check CSM Opt OUT Protocol in what order to starty UEFI OPT ROM.
            if( ExecuteUefiRom(Dev) ) {
                ...
                if( efior && pcir->CodeType == PCI_CODE_TYPE_EFI_IMAGE ) 
                {
                    RomMatch=TRUE;
                    DEBUG((DEBUG_INFO,"(Install UEFI OpROM="));
                    if(Embedded){
                        if((Dev->Capab&EFI_PCI_IO_ATTRIBUTE_EMBEDDED_ROM) && (Dev->Attrib&EFI_PCI_IO_ATTRIBUTE_EMBEDDED_ROM)){
                            Status=InstallEfiOpRom(Dev, (VOID*)rh, (VOID*)pcir, FALSE);
                        } //else {
                          //  DEBUG((DEBUG_VERBOSE,"<Delayed>="));
                         //   Status=AppendItemLst(&gThirdPartyRomList, Dev);
                       // }
                    } else {
                        Status=InstallEfiOpRom(Dev, (VOID*)rh, (VOID*)pcir, FALSE);
    ...
    //Some house keeping...
    if(notarom || !RomMatch){
      // If gNoSdlDataMode set to TRUE, assuming onboard Option Rom details will get filled using GetPciRom(). 
      // Don't free Dev->PciIo.RomImage and Dev->PciIo.RomSize
            if (gNoSdlDataMode == FALSE) {
                pBS->FreePool(Dev->PciIo.RomImage);
                Dev->PciIo.RomImage=NULL;
                Dev->PciIo.RomSize=0;
                Status=EFI_NOT_FOUND;
                DEBUG((DEBUG_INFO,"SDL Data Mode \n"));
            }
            else Status=EFI_SUCCESS;
        } else Status=EFI_SUCCESS;
    }
        ...
        if( Dev->EmbRomCnt){
            ...
            for(i=0;i<Dev->EmbRomCnt; i++){
                ...
                if(embrom->UefiDriver && ExecuteUefiRom(Dev)){
                    DEBUG((DEBUG_INFO,"(Install UEFI EmbDrv="));
                    Status=InstallEfiOpRom(Dev,embrom->EmbRom,(VOID*)embrom, TRUE);
                    DEBUG((DEBUG_INFO,"%r) ", Status));
                    if(EFI_ERROR(Status)) { 
                        if(Status!=EFI_ACCESS_DENIED) return EFI_NOT_FOUND;
                        return Status;
                    }
                }
            }
    ...
    //
    //  If gNoSdlDataMode set to TRUE, Onboard OpRom will be getting launched from the below code.
    //
    
    else {
        
        DEBUG((DEBUG_INFO,"(No SDL Data Mode); RomDelayed:%d, RomMatch:%d\n", RomDelayed, RomMatch));
        // If Off board Card Option ROM is Installed once, Skip re-installing again.
        if (RomDelayed == TRUE || RomMatch == TRUE) return Status;
        if(ExecuteUefiRom(Dev)){
            EMB_ROM_INFO    *embrom = NULL;
            //--------------------- 
            
            embrom=MallocZ(sizeof(EMB_ROM_INFO));
            if(embrom == NULL){
                ASSERT_EFI_ERROR(EFI_OUT_OF_RESOURCES);
                return EFI_OUT_OF_RESOURCES;
            }
            embrom->EmbRom=Dev->PciIo.RomImage;
            embrom->EmbRomSize=Dev->PciIo.RomSize;

            DEBUG((DEBUG_INFO,"No SDL Data Mode path : (Install UEFI EmbDrv="));
            Status=InstallEfiOpRom(Dev,embrom->EmbRom,(VOID*)embrom, TRUE);
            DEBUG((DEBUG_INFO,"%r) ", Status));
            pBS->FreePool(embrom);
            if(EFI_ERROR(Status)) { 
...
//<licm-20210226 VGA and UART Display 16bit Port80 SUPPORT +>
#if defined(VGA_UART_DIS16BIT_PORT80_SUPPORT) && (VGA_UART_DIS16BIT_PORT80_SUPPORT == 1)
    PROGRESS_CODE16_EX(DXE_PCI_BUS_BEGIN,(UINT8)PORT81_EX_DXE_PCI_UPDATE_DEVICES_LIST);
#endif
//<licm-20210226 VGA and UART Display 16bit Port80 SUPPORT +->
    Status = ParseOpROMSelect(Dev);
#endif  
//lvych001-20170916 Improve device list function ->    
    offs=0;
                       ...
                       IN EFI_DEVICE_PATH_PROTOCOL     *RemainingDevicePath )
{
    EFI_STATUS          Status;
    BOOLEAN             ft=FALSE;
    //<Chendl002-20170521 Add Pcie List Function Which Can Enable/Disable PCIe Device Option by User +>
#if defined(OemPCIeDev_SUPPORT) && (OemPCIeDev_SUPPORT == 1)
    ...
    //<lvshh001-20200515 Fix hang92 problem when connecting Hikvision switch +>
    UINT8               Command = 0;
    UINT8               BrgCtr = 0;
    //<lvshh001-20200515 Fix hang92 problem when connecting Hikvision switch ->
...
//------------------------------------------------
    //<licm-20210226 VGA and UART Display 16bit Port80 SUPPORT +>
#if defined(VGA_UART_DIS16BIT_PORT80_SUPPORT) && (VGA_UART_DIS16BIT_PORT80_SUPPORT == 1)
    PROGRESS_CODE16_EX(DXE_PCI_BUS_BEGIN,(UINT8)PORT81_EX_DXE_PCI_ENUM_STARTED);
#endif
    //<licm-20210226 VGA and UART Display 16bit Port80 SUPPORT ->
	...
	if (Bootdata.Bits.Reserved == 0) {
	  	PCI_TRACE((TRACE_PCI,"[OemDbg]: Reset Fast Video Resource Begin ... \n"));
	  	Bootdata.Bits.Reserved = 1;
	    PcdSet32 (PcdBootData, Bootdata.Uint32);
        // Clean the Fast Video Now
        VGA_Bus_Num = PEI_PCI_MAX_SUBORDINATE_NUMBER;     // VGA Bus Number = 0x02
		BMC_Bus_Num = PEI_PCI_MAX_SUBORDINATE_NUMBER - 1; // BMC Bridge Number = 0x01
		AMD_Bus_Num = PEI_PCI_MAX_SUBORDINATE_NUMBER - 2; // AMD Root Port Number = 0x00

		// --------------  Release VGA Controller Resource -----------------------//
		// Free MMIO and IO Resource
		PciWrite32(PCI_LIB_ADDRESS(VGA_Bus_Num, 0, 0, 0x10),ResetBuffer); // MMIO
		PciWrite32(PCI_LIB_ADDRESS(VGA_Bus_Num, 0, 0, 0x14),ResetBuffer); // MMIO
		PciWrite32(PCI_LIB_ADDRESS(VGA_Bus_Num, 0, 0, 0x18),ResetBuffer); // IO
		// Not Enable This PCIE Device
		Command = PciRead8(PCI_LIB_ADDRESS(VGA_Bus_Num, 0, 0, 0x04));
		Command &= ~(EFI_PCI_COMMAND_IO_SPACE | EFI_PCI_COMMAND_MEMORY_SPACE | EFI_PCI_COMMAND_BUS_MASTER);
		PciWrite8(PCI_LIB_ADDRESS(VGA_Bus_Num, 0, 0, 0x04), Command);

		// --------------  Release BMC Bridge Resource -----------------------//
		// 01. Primary Bus Number
        PciWrite8(PCI_LIB_ADDRESS(BMC_Bus_Num, 0, 0, PCI_BRIDGE_PRIMARY_BUS_REGISTER_OFFSET), BusNumber);

		// 02. Secondary Bus Number
        PciWrite8(PCI_LIB_ADDRESS(BMC_Bus_Num, 0, 0, PCI_BRIDGE_SECONDARY_BUS_REGISTER_OFFSET), BusNumber);

		// 03. Subornidate Bus Number
        PciWrite8(PCI_LIB_ADDRESS(BMC_Bus_Num, 0, 0, PCI_BRIDGE_SUBORDINATE_BUS_REGISTER_OFFSET), BusNumber);

		// 04. MMIO Base 
        PciWrite16(PCI_LIB_ADDRESS(BMC_Bus_Num, 0, 0, PCI_MEMBASE), (UINT16) ResetBuffer);

		// 05. MMIO Limit
        PciWrite16(PCI_LIB_ADDRESS(BMC_Bus_Num, 0, 0, PCI_MEMLIMIT), (UINT16) ResetBuffer);

		// 06. IO Base 
        PciWrite8(PCI_LIB_ADDRESS(BMC_Bus_Num, 0, 0, PCI_IOBASE), (UINT8) ResetBuffer);

		// 07. IO Limit 
        PciWrite8(PCI_LIB_ADDRESS(BMC_Bus_Num, 0, 0, PCI_IOLIMIT), (UINT8) ResetBuffer);

		// 08. Command Line       
        //<lvshh001-20200515 Fix hang92 problem when connecting Hikvision switch +>
        Command = PciRead8(PCI_LIB_ADDRESS(BMC_Bus_Num, 0, 0, 0x04));
        Command &= ~(EFI_PCI_COMMAND_IO_SPACE | EFI_PCI_COMMAND_MEMORY_SPACE | EFI_PCI_COMMAND_BUS_MASTER );
        //<lvshh001-20200515 Fix hang92 problem when connecting Hikvision switch ->
        PciWrite8(PCI_LIB_ADDRESS(BMC_Bus_Num, 0, 0, 0x04), Command);
        
        //<Lvshh001-20190227 Mantis#27231,#27233,Solve Bridge(BUS:1,DEV:0,FUN:0) resource conflict under WIN10 +>
        // 09. VGA_EN
        //<lvshh001-20200515 Fix hang92 problem when connecting Hikvision switch +>
        BrgCtr = PciRead8(PCI_LIB_ADDRESS(BMC_Bus_Num, 0, 0, 0x3E));
        BrgCtr &= ~BIT3;
        //<lvshh001-20200515 Fix hang92 problem when connecting Hikvision switch ->
        PciWrite8(PCI_LIB_ADDRESS(BMC_Bus_Num, 0, 0, 0x3E), BrgCtr);
        
        //<Lvshh001-20190227 Mantis#27231,#27233,Solve Bridge(BUS:1,DEV:0,FUN:0) resource conflict under WIN10 ->

		// --------------  Release AMD Root Port Resource -----------------------//
        // 01. Primary Bus Number
        PciWrite8(PCI_LIB_ADDRESS(AMD_Bus_Num, AST_SB_DEV_NUM, AST_SB_FUNC_NUM, PCI_BRIDGE_PRIMARY_BUS_REGISTER_OFFSET), BusNumber);

        // 02. Secondary Bus Number
        PciWrite8(PCI_LIB_ADDRESS(AMD_Bus_Num, AST_SB_DEV_NUM, AST_SB_FUNC_NUM, PCI_BRIDGE_SECONDARY_BUS_REGISTER_OFFSET), BusNumber);

        // 03. Subornidate Bus Number
        PciWrite8(PCI_LIB_ADDRESS(AMD_Bus_Num, AST_SB_DEV_NUM, AST_SB_FUNC_NUM, PCI_BRIDGE_SUBORDINATE_BUS_REGISTER_OFFSET), BusNumber);

        // 04. MMIO Base 
        PciWrite16(PCI_LIB_ADDRESS(AMD_Bus_Num, AST_SB_DEV_NUM, AST_SB_FUNC_NUM, PCI_MEMBASE), (UINT16)ResetBuffer);

        // 05. MMIO Limit
        PciWrite16(PCI_LIB_ADDRESS(AMD_Bus_Num, AST_SB_DEV_NUM, AST_SB_FUNC_NUM, PCI_MEMLIMIT), (UINT16)ResetBuffer);

        // 06. IO Base 
        PciWrite8(PCI_LIB_ADDRESS(AMD_Bus_Num, AST_SB_DEV_NUM, AST_SB_FUNC_NUM, PCI_IOBASE), (UINT8)ResetBuffer);

        // 07. IO Limit 
        PciWrite8(PCI_LIB_ADDRESS(AMD_Bus_Num, AST_SB_DEV_NUM, AST_SB_FUNC_NUM, PCI_IOLIMIT), (UINT8)ResetBuffer);

        // 08. Command Line
        Command = PciRead8(PCI_LIB_ADDRESS(AMD_Bus_Num, AST_SB_DEV_NUM, AST_SB_FUNC_NUM, 0x04));
        Command &= ~(EFI_PCI_COMMAND_IO_SPACE | EFI_PCI_COMMAND_MEMORY_SPACE | EFI_PCI_COMMAND_BUS_MASTER);
        PciWrite8(PCI_LIB_ADDRESS(AMD_Bus_Num, AST_SB_DEV_NUM, AST_SB_FUNC_NUM, 0x04), Command);
     
        // 09. VGA_EN
        BrgCtr = PciRead8(PCI_LIB_ADDRESS(AMD_Bus_Num, AST_SB_DEV_NUM, AST_SB_FUNC_NUM, 0x3E));
        BrgCtr &= ~BIT3;
        PciWrite8(PCI_LIB_ADDRESS(AMD_Bus_Num, AST_SB_DEV_NUM, AST_SB_FUNC_NUM, 0x3E), BrgCtr);

        PCI_TRACE((TRACE_PCI,"[OemDbg]: Reset Fast Video Resource End ... \n"));
	}
	//<Chendl001-20171129 Add The Solution For J11 && J12 Hang Issue ->
    ...
    //<licm-20210226 VGA and UART Display 16bit Port80 SUPPORT +>
#if defined(VGA_UART_DIS16BIT_PORT80_SUPPORT) && (VGA_UART_DIS16BIT_PORT80_SUPPORT == 1)
    PROGRESS_CODE16_EX(DXE_PCI_BUS_BEGIN,(UINT8)PORT81_EX_DXE_PCI_ALLOC_SETUPBUF);
#endif
    //<licm-20210226 VGA and UART Display 16bit Port80 SUPPORT ->
    ...
    if(gPciCommonSetup==NULL){
        ...
        if (EFI_ERROR(Status)) {
          gPciDefaultSetup->OpROMEN[PXEINDX]   = 0x03;  // PXE PCIe Devices
          gPciDefaultSetup->OpROMEN[MASSINDX]  = 0x03;  // MASS PCIe Devices
          gPciDefaultSetup->OpROMEN[VIDEOINDX] = 0x03;  // VGA PCIe Devices
          gPciDefaultSetup->OpROMEN[OLDINDX]   = 0x03;  // OTHER PCIe Devices
          pBS->SetMem(gPciDefaultSetup->PciDevEN, PCIDEV_ENTRY_NUMBER * sizeof(gPciDefaultSetup->PciDevEN[0]), 0x01);
        } else {   
          pBS->CopyMem(gPciDefaultSetup->PciDevEN, SetupData->PciDevEN, MAX_DEV_COUNT);
          pBS->CopyMem(gPciDefaultSetup->OpROMEN, SetupData->OpROMEN, PCIDEV_ENTRY_NUMBER);
        }
#endif
        //<Chendl002-20170521 Add Pcie List Function Which Can Enable/Disable PCIe Device Option by User ->
        if(gNoSdlDataMode==FALSE){
