...
#define FILECODE PROC_FCH_TAISHAN_TSSPI_TSSPIENV_FILECODE
  ...
  )
{
  ...
  if (( SpiMode !=  LocalCfgPtr->Spi.SpiMode) || ( SpiFastSpeed !=  (LocalCfgPtr->Spi.SpiFastSpeed -1))) {
    if (FchPlatformSpiQe (FchDataPtr)) {
      if ((LocalCfgPtr->Spi.SpiMode == FCH_SPI_MODE_QUAL_144) || (ACPIMMIO8 (0xFED8060C) != 0x20))  {
        ...
        if (Value16 != 0xffff) {
          FlashWriteAddress = RomSigStartingAddr - RomBase;
          FchSpiTransfer (
            0x06, //IN  UINT8    PrefixCode,
        ...
        if ((LocalCfgPtr->Spi.SpiMode == FCH_SPI_MODE_QUAL_144) && (ACPIMMIO8 (0xFED8060C) == 0x20))  {
          ...
          RwMem (SPI_BASE + FCH_SPI_MMIO_REG32, AccessWidth16, ~((UINT32) (0xF << 8)), ((QuadDummyCycle) << 8));
        }
