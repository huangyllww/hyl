...
)
{
  ...
  DEBUG((EFI_D_ERROR, __FUNCTION__" Base:0x%lX Limit:0x%lX Base + AspeedVgaBufferSize:0x%lX\n", Base,Limit,Base + AspeedVgaBufferSize));
  ...
  */
  ...
  {
    Base = Base+ AspeedVgaBufferSize;
    DEBUG((EFI_D_ERROR, "[OemDbg][%a][%d]Base:%lx  !!!\r\n",__FUNCTION__,__LINE__,Base));
    Tbase  =  Base & mask;
    lsize = Base - Tbase;
    if(lsize > 0)
    {
        //Base = Tbase + (UINT64)(~mask);
        Base = Tbase + 0x1000000;
    }
    DEBUG((EFI_D_ERROR, "[OemDbg][%a][%d]Base:%lx  Tbase:%lx  lsize:%lx!!!\r\n",__FUNCTION__,__LINE__,Base,Tbase,lsize));
  ...
  DEBUG((EFI_D_ERROR, __FUNCTION__" Base:0x%lX Limit:0x%lX\n", Base,Limit));
