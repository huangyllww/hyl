...
VOID ActivateApplication( VOID )
{
	...
	if ( text != NULL )
	{
		...
		if ( newString != NULL )
		{
			...
			//<huangrj-20220307 Remove AMI info when quiet boot disabled +>
			//SPrint( newString, length * sizeof(CHAR16), text, TseMajor, TseMinor, TseBuild,TseBuildYear );
			SPrint( newString, length * sizeof(CHAR16), L"Version %x.%02x.%04x. Copyright (C) %04x.", TseMajor, TseMinor, TseBuild,TseBuildYear );
			//<huangrj-20220307 Remove AMI info when quiet boot disabled ->
