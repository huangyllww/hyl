...
#if (STDLIB_AMI_PORTING)
EFI_STATUS GetSocketBindAddress(EFI_IPv4_ADDRESS **Ipv4Address);
#endif	// STDLIB_AMI_PORTING
//
// AMI PORTING ENDS
//
...
// AMI PORTING STARTS
//
#if !STDLIB_OVERRIDE_GetSocketBindAddress
EFI_STATUS GetSocketBindAddress(EFI_IPv4_ADDRESS **Ipv4Address)
{
        ...
        if ( SockAddrLength >= pSocket->pApi->MinimumAddressLength ) {
          ...
          if ( NULL == pSocket->pApi->pfnRemoteAddrSet ) {
            ...
            if ( NULL == pSocket->pPortList ) {
				...
				//
#if (STDLIB_AMI_PORTING)
                EFI_IPv4_ADDRESS *Ipv4Address = (EFI_IPv4_ADDRESS *)NULL;
                struct sockaddr_in LocalIpV4Address;
                EFI_STATUS RestStatus = EFI_NOT_FOUND;
                
                RestStatus = GetSocketBindAddress(&Ipv4Address);
                if((!EFI_ERROR(RestStatus)) && (Ipv4Address != NULL))
                {
                    //Use the IP Address in Bind, so that only the PORT with that IP Address will be used for communication.
                    int ret = -1;
                    char IpAddress[32] = {0};
                    sprintf(IpAddress, "%d.%d.%d.%d", Ipv4Address->Addr[0], Ipv4Address->Addr[1], Ipv4Address->Addr[2], Ipv4Address->Addr[3]);
                    DEBUG (( DEBUG_ERROR, "Bind IpAddress: %a\r\n", IpAddress));
                    ret = inet_pton(AF_INET, (char *)IpAddress, &(LocalIpV4Address.sin_addr));
                    if(ret == 1)
                    {
                        LocalIpV4Address.sin_len = (uint8_t)pSocket->pApi->MinimumAddressLength;
                        LocalIpV4Address.sin_family = pSocket->pApi->AddressFamily;
                        Status = EslSocketBind ( &pSocket->SocketProtocol,
                                                 (struct sockaddr *)&LocalIpV4Address,
                                                 LocalIpV4Address.sin_len,
                                                 &pSocket->errno );
                        DEBUG (( DEBUG_ERROR, "Bind IpAddress Status: %r\r\n", Status));
                    }
                    else
                    {
                        RestStatus = EFI_NOT_FOUND;
                    }
                }
                
                if(EFI_ERROR(RestStatus))
                {
#endif	// STDLIB_AMI_PORTING				
				//
				// AMI PORTING ENDS
    ...
    if ( NULL != pSocketPrevious ) {
      if ( pSocket == pSocketPrevious ) {
        ...
        if ( NULL != pSocketPrevious ) {
          ...
          //
        	pSocketPrevious->pNext = pSocket->pNext;		// AMI PORTING. Fix for Socket Close function asserts while freeing the specified socket
        }
      }
    }
    else {
...
// AMI PORTING STARTS
#if STDLIB_SOCKETCLOSE_EVENT_SUPPORT
//Trigger the event only CloseEvent present this will be filled in BslSocketCloseWork.
//If this event is not present which means we are not disturbing its path. Only Close will make use of this.
  pSocketProtocol = &(pPort->pSocket->SocketProtocol);
#endif
// AMI PORTING ENDS
...
// AMI PORTING STARTS  
#if STDLIB_SOCKETCLOSE_EVENT_SUPPORT
  if (NULL != pSocketProtocol->CloseEvent)
  {
	  gBS->SignalEvent( pSocketProtocol->CloseEvent );
  }
#endif
// AMI PORTING ENDS
  DBG_EXIT_STATUS ( Status );
