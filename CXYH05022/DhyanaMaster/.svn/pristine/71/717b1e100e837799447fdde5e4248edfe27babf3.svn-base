...
//<Yaosr002-20181207 Import the OemBds module, fixed the boot order changes when load default in the AMD platform +>
#if defined(OemBds_SUPPORT) && (OemBds_SUPPORT == 1)
    // If the Tag values are the same, 
    // Compare by Priority
    if (Option1->Priority < Option2->Priority) 
        return -1;
    else if (Option1->Priority > Option2->Priority) 
        return 1;
