...
EFI_STATUS _BootLaunchDevicePath( EFI_DEVICE_PATH_PROTOCOL *DevicePath, VOID *Options, UINTN OptionSize, BOOLEAN ValidBootOption )
{
	...
	//EfiLibReportStatusCode(EFI_PROGRESS_CODE, DXE_READY_TO_BOOT,0,NULL,NULL);//<Lvshh001-20221108 For MemoryLeak Fix,otherwise when the EfiBootServicesData increases too much, the hang60 will occur>
    ...
    //if(ValidBootOption==TRUE)
        //if (EFI_ERROR (Status)) // Report only if it Boot Option launch - Boot of loading Error!!!
    		//EfiLibReportStatusCode(EFI_ERROR_CODE| EFI_ERROR_MAJOR, DXE_BOOT_OPTION_LOAD_ERROR,0,NULL,NULL);//<Lvshh001-20221108 For MemoryLeak Fix,otherwise when the EfiBootServicesData increases too much, the hang60 will occur>
...
//    UINTN	iIndex = 0;  Unused Variable
   ...
   //<Lvshh001-20221108 For MemoryLeak Fix,otherwise when the EfiBootServicesData increases too much, the hang60 will occur +>
   if(LoadOptions) {
      MemFreePointer((void **) &LoadOptions);//For MemoryLeak Fix
   }
   //<Lvshh001-20221108 For MemoryLeak Fix,otherwise when the EfiBootServicesData increases too much, the hang60 will occur ->
...
VOID SetBootOptionSupportVariable (UINT32 BootManCapabilities)
{
    ...
    //<Lvshh001-20221108 For MemoryLeak Fix,otherwise when the EfiBootServicesData increases too much, the hang60 will occur +>
	if(Data) {
	    MemFreePointer((void **) &Data);//For MemoryLeak Fix
	}
	//<Lvshh001-20221108 For MemoryLeak Fix,otherwise when the EfiBootServicesData increases too much, the hang60 will occur ->
}
