...
static EFI_STATUS BuildMadtByHob(IN UINT32 TableVer, OUT VOID **Entries, OUT UINT32 *NumEntries, OUT UINT32 *TableSize)
{
    ...
    *NumEntries = PopulatedCpus;
    ...
    if (PcdGet8(PcdAmdSmtMode)) {
      ApicId = 0;
      for (Index = 0; Index < PopulatedCpus/2; ++Index) {
        CpuNumApicId[Index].CpuNum = ApicId;
        CpuNumApicId[Index].ApicId = gCpuInfoHob->Cpuinfo[ApicId].ApicId;
        ApicId += 2;
      }
      ApicId = 1;
      for (; Index < PopulatedCpus; ++Index) {
        CpuNumApicId[Index].CpuNum = ApicId;
        CpuNumApicId[Index].ApicId = gCpuInfoHob->Cpuinfo[ApicId].ApicId;
        ApicId += 2;
      }
    ...
    *TableSize = *NumEntries *
    ...
    for (Index = 0; Index < PopulatedCpus; ++Index) {
        ...
        if (LocalXapicSupport) {
            ...
            CreateLxApicEntry(
                    &MadtEntry,
                    gCpuInfoHob->Cpuinfo[CpuNum].Valid ? CpuNum :0xFF,
                    gCpuInfoHob->Cpuinfo[CpuNum].Valid ? CpuNumApicId[Index].ApicId :0xFF,
...
//                    CpuNum + 1,
    ...
    for (; Index < NCPU; ++Index) {
        if (LocalXapicSupport) {
        	//Make the processor IDs 0 based
//            CreateLxApicEntry(&MadtEntry, Index + 1, 0, 0);
            CreateLxApicEntry(&MadtEntry, 0xFF, 0xFF, 0);
        } else  {
//            CreateLSApicEntry(&MadtEntry, Index + 1, 0, 0, 0, 0);
            CreateLSApicEntry(&MadtEntry, Index, 0, 0, 0, 0);
        }
    }
#endif
