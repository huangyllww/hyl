...
)
{
    ...
    else if(gTcg2SpecVersion == TCG2_PROTOCOL_SPEC_TCG_2)
    {
        ...
        {
            sm3_init (&Sm3Ctx);//<Yaosr001-20200908 Added SM3 Software Algorithm Support.>
        }
    }
    ...
    for(i=0; i< FwVolHobCount; i++)
    {
        ...
        else if(gTcg2SpecVersion == TCG2_PROTOCOL_SPEC_TCG_2)
        {
            ...
            if( gPcrBanks & TREE_BOOT_HASH_ALG_SM3)
            {
                sm3_update(&Sm3Ctx, (unsigned char *)(UINTN)TpmFwVolHob[i].baseAddress, (u32)TpmFwVolHob[i].Size);//<Yaosr001-20200908 Added SM3 Software Algorithm Support.>
            }
        }
    }
    ...
    else if(gTcg2SpecVersion == TCG2_PROTOCOL_SPEC_TCG_2)
    {
        ...
        if( gPcrBanks & TREE_BOOT_HASH_ALG_SM3)
        {
            sm3_final((unsigned char *)&TpmDigest.sm3_256, &Sm3Ctx); //<Yaosr001-20200908 Added SM3 Software Algorithm Support.>
...
)
{
    EFI_STATUS Status;
    AMI_TREE_PPI *TrEEPeiPpi = NULL;
#if (FTpmPlatformProfile == 1) && (Hygon_fTPM2 != 1)
    UINTN  Info;
#endif
    TCG_PLATFORM_SETUP_INTERFACE    *TcgPeiPolicy;
    TCG_CONFIGURATION               ConfigFlags;
    ...
    DEBUG((-1, "###Enter AmiTpm20PlatformPeiEntry\n"));
#if (FTpmPlatformProfile == 1) && (Hygon_fTPM2 != 1)
    Status = EFI_SUCCESS;
