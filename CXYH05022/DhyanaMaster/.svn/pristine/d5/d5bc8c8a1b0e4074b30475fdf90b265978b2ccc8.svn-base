...
#define PCI_ENBIT           0x080000000
// Ati SMB ...
#define DEB_SMB_BUS              0x00
#define DEB_SMB_DEV              0x14
#define DEB_SMB_DEV_FUN          ((DEB_SMB_DEV << 3) + 0)
// Ati LPC(ISA) Bridge...
#define DEB_SB_BUS              0x00
#define DEB_SB_DEV              0x14
#define DEB_SB_DEV_FUN          ((DEB_SB_DEV << 3) + 3)
...
//    EnableMmioDecode
    // MMIO BASE
    Address = PCI_LIB_ADDRESS(0, 0x18, 0x0, 0x200);
    PciWrite32(Address, 0xED00);

    // MMIO limit
    Address = PCI_LIB_ADDRESS(0, 0x18, 0x0, 0x204);
    PciWrite32(Address, 0xFFFF);

    // MMIO CTRL
    Address = PCI_LIB_ADDRESS(0, 0x18, 0x0, 0x208); 
    PciWrite32(Address, 0x43); // Fabric ID = 04 ( RB0)

    //
    // The XHCI is behind a P2P bridge. (RB->P2P->xHCI) Let's assign bus resource first.
    //
	Address = PCI_LIB_ADDRESS(0, 0x7, 0x1, 0x18);
	PciWrite32(Address, 0x00010100); //SubBus(Bit23::16) SecBus(Bit15::8)

	// For mmio resource we should consider other device
	Address = PCI_LIB_ADDRESS(0, 0x7, 0x1, 0x20);
	PciWrite16(Address, (XhciBaseAddress >> 16));
    Address = PCI_LIB_ADDRESS(0, 0x7, 0x1, 0x22);
    PciWrite16(Address, ((XhciBaseAddress >> 16) + 0x10));
    
    Address = PCI_LIB_ADDRESS(0, 0x7, 0x1, PCI_CMD);
    PciWrite8(Address, 6);

	Address = PCI_LIB_ADDRESS (Bus, Device, Function, PCI_BAR0);
	PciWrite32(Address, XhciBaseAddress);
    
    Address = PCI_LIB_ADDRESS (Bus, Device, Function, PCI_CMD);
    PciWrite8(Address, 6);
...
)
{
    ...
    // It happened on some CRB board. (Shiner Taishan A12)
...
#define PM1_CNT             0x04
    ...
    ProgramACPIBaseToDisableTCO();
    // Check WAK_STS bit
    if (IoRead16(PM_BASE_ADDRESS + 0) & BIT_WAK_STS)
    {
        // Check the sleep type
        if ((IoRead16(PM_BASE_ADDRESS + PM1_CNT) & BIT_SLP_TYP_MASK) == 0xC00)
            return TRUE;
    }
