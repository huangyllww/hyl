...
#include <Include\Library\PciLib.h> //<liutz001-20190515- Recovery Function >
...
//<liutz001-20190515- Recovery Function+>
GetBootModeHob (
  VOID
  );
//<liutz001-20190515- Recovery Function->
...
//<Lvshh001-20221022 Workaround for hang60 issue +>
#if defined(WORKAROUND_FOR_HANG60)&& (WORKAROUND_FOR_HANG60 == 1) 
...
VOID InstallFwLoadFile(){
    ...
    //OemSize = sizeof (AMITSESETUP);        
    /*Status = pRS->GetVariable(L"AMITSESetup", \
                               &AmiTseSetupguid, \
                               NULL, \
                               &OemSize, \
                               &TSEVar );*/
    DEBUG((DEBUG_INFO, "[OemDbg]SetupData.UEFIShellDisableSelect = %d\n",SetupData.UEFIShellDisableSelect));                           
    if(!EFI_ERROR(Status) && (SetupData.UEFIShellDisableSelect == 0)){
        DEBUG((DEBUG_INFO, "[OemDbg] Disable UEFI Shell!\n"));
        return;
    }            
    //<Lvshh001-20181103 Add UEFI Shell disable token ->
    Status = pRS->GetVariable(EFI_SECURE_BOOT_MODE_NAME, &gEfiGlobalVariableGuid, NULL, &Size, &Data);
    DEBUG((DEBUG_INFO, "[OemDbg]SetupData.UEFIShellDisableSelect Data = %d\n",Data));   
...
#if defined(WORKAROUND_FOR_HANG60)&& (WORKAROUND_FOR_HANG60 == 0) //<Lvshh001-20221024 Workaround for hang60 issue>
typedef BOOLEAN (BDS_UPDATE_MEMORY_USAGE_INFORMATION)(BDS_MEMORY_USAGE_INFO *MemoryInfo, UINTN NumberOfElements);
#else //<Lvshh001-20221024 Workaround for hang60 issue +>
typedef BOOLEAN (BDS_UPDATE_MEMORY_USAGE_INFORMATION)(BDS_MEMORY_USAGE_INFO *MemoryInfo, UINTN NumberOfElements ,EFI_GUID * MemoryInformationVariableGuid);
#endif //<Lvshh001-20221024 Workaround for hang60 issue ->
...
#else //<Lvshh001-20221024 Workaround for hang60 issue +>
BOOLEAN UpdateMemoryUsageInformationDefault(BDS_MEMORY_USAGE_INFO *MemoryInfo, UINTN NumberOfElements , EFI_GUID * MemoryInformationVariableGuid){ 
#endif //<Lvshh001-20221024 Workaround for hang60 issue ->
    UINTN i;
    ...
    //<Lvshh001-20221022 Workaround for hang60 issue +>
#if defined(WORKAROUND_FOR_HANG60)&& (WORKAROUND_FOR_HANG60 == 1) 
    UINTN PreTotalProjectedMemoryUage = 0;
    EFI_STATUS Status;
#endif
    //<Lvshh001-20221022 Workaround for hang60 issue ->
...
#if defined(WORKAROUND_FOR_HANG60)&& (WORKAROUND_FOR_HANG60 == 0) //<Lvshh001-20221022 Workaround for hang60 issue>
    MemoryUsageThreshold = GetTotalMemorySize() >> 1;
    if (MemoryUsageThreshold > 0x80000000) MemoryUsageThreshold = 0x80000000;
#else //<Lvshh001-20221022 Workaround for hang60 issue +>
    MemoryUsageThreshold = MAX_TOLERANCE_LENGTH;//Actual ToleranceLength*2
#endif //<Lvshh001-20221022 Workaround for hang60 issue ->
    ...
    for (i = 0; i<NumberOfElements; i++) {
        ...
        //<Lvshh001-20221022 Workaround for hang60 issue +>
#if defined(WORKAROUND_FOR_HANG60)&& (WORKAROUND_FOR_HANG60 == 1) 
        PreTotalProjectedMemoryUage += MemoryInfo[i].Previous;
        DEBUG((DEBUG_INFO, "[%a][%d] PreTotalProjectedMemoryUage %x\n",__FUNCTION__,__LINE__,PreTotalProjectedMemoryUage));
#endif
        //<Lvshh001-20221022 Workaround for hang60 issue ->
    }
#if defined(WORKAROUND_FOR_HANG60)&& (WORKAROUND_FOR_HANG60 == 0) //<Lvshh001-20221022 Workaround for hang60 issue>
...
#else //<Lvshh001-20221022 Workaround for hang60 issue +>
    if (TotalProjectedMemoryUage > MemoryUsageThreshold) {
        MemoryTypeInformationModified  = FALSE;
        if (PreTotalProjectedMemoryUage > MemoryUsageThreshold) {
            Status = SetVariableWithNewAttributes(
                EFI_MEMORY_TYPE_INFORMATION_VARIABLE_NAME, MemoryInformationVariableGuid,
                EFI_VARIABLE_NON_VOLATILE  | EFI_VARIABLE_BOOTSERVICE_ACCESS,
                sizeof(DefaultMemoryTypeInformation), DefaultMemoryTypeInformation);
            if (EFI_ERROR(Status)) {
                DEBUG((DEBUG_INFO, "SetVariable MemoryTypeInformation Status %r !!! \n",Status));
            }
        }
    }
#endif //<Lvshh001-20221022 Workaround for hang60 issue ->
...
#else //<Lvshh001-20221024 Workaround for hang60 issue +>
    if ( UpdateMemoryUsageInformation(MemoryInfoHistory,NumberOfMemoryInfoElements,MemoryInformationVariableGuid) ){
#endif //<Lvshh001-20221024 Workaround for hang60 issue ->
...
VOID BdsEntry (IN EFI_BDS_ARCH_PROTOCOL *This)
{
	...
	UINTN i;
    UINT32 OemBootMode; //<liutz001-20190515- Recovery Function >
    //Mizl-20220312-SGEZT#22551: USB Recovery Function Work Abnormally+>
    UINT8  DstFabricId;
    UINT8  DstDieBusNumber,iDie,iSocket;
    //Mizl-20220312-SGEZT#22551: USB Recovery Function Work Abnormally->
    ...
    //<liutz001-20190515- Recovery Function +>
    OemBootMode = GetBootModeHob ();
    if (OemBootMode == BOOT_IN_RECOVERY_MODE) {
        
        //Mizl-20220312-SGEZT#22551: USB Recovery Function Work Abnormally+>
        for (iSocket = 0; iSocket < MAX_SOCKETS; iSocket++)
        {
            /* code */
            for (iDie = 0; iDie < MAX_DIES_PER_SOCKET; iDie++)
            {
                /* code */
                DstDieBusNumber = GetCPURbBusNumAndFabricId (iSocket, iDie, &DstFabricId);
                DEBUG((EFI_D_INFO, "[OemDbg][%a][%d] Socket:%x Die:%x Bus:%x\r\n",__FUNCTION__,__LINE__,iSocket,iDie,DstDieBusNumber));
                //Clear USB Bus Range
                PciWrite32(PCI_LIB_ADDRESS(DstDieBusNumber, 7, 1, 0x18), 0x0);
                PciWrite32(PCI_LIB_ADDRESS(DstDieBusNumber, 7, 1, 0x19), 0x0);
                PciWrite32(PCI_LIB_ADDRESS(DstDieBusNumber, 7, 1, 0x1A), 0x0);
                
                //Clear SATA Bus Range
                PciWrite32(PCI_LIB_ADDRESS(DstDieBusNumber, 8, 1, 0x18), 0x0);
                PciWrite32(PCI_LIB_ADDRESS(DstDieBusNumber, 8, 1, 0x19), 0x0);
                PciWrite32(PCI_LIB_ADDRESS(DstDieBusNumber, 8, 1, 0x1A), 0x0);
            }
...
/** @internal
  ...
  We're also not disabling OpROM if legacy OpROM is enabled in the CSM policy
  reported by AmiCsmOpromPolicy protocol.

  @param[in] Handle   EFI handle to disable OpROM.
**/
VOID DisableOpRom(EFI_HANDLE Handle){
    AMI_OPTION_ROM_POLICY_PROTOCOL *OpRomPolicy;
    AMI_CSM_OPROM_POLICY_PROTOCOL *CsmOpRomPolicy;
    EFI_STATUS Status;
    AMI_OPTION_ROM_POLICY_TYPE Policy;
    BOOLEAN RomStatus;

    // We have two OpROM policy protocols:
    // AmiOptionRomPolicy protocol produced by PCI bus driver and
    // AmiCsmOpromPolicy protocol produced by CSM.
    // We're disabling the OpROM only if there are no conflicts with the policy
    // set forth by either of these protocols
    Status = pBS->HandleProtocol(
    ...
    // If UEFI OpROM is not enabled, we shouldn't disable OpROM;
    // otherwise we may mistakenly disable legacy OpROM.
    if (EFI_ERROR(Status) || Policy != AMI_OPTION_ROM_POLICY_UEFI) return;
    
    Status = pBS->HandleProtocol(
        Handle, &gAmiCsmOpromPolicyProtocolGuid, &CsmOpRomPolicy
    );
    if (!EFI_ERROR(Status)){
        Status = CsmOpRomPolicy->CheckOpromPolicy(CsmOpRomPolicy, Handle);
        // If UEFI OpROM is not enabled, we shouldn't disable OpROM;
        // otherwise we may mistakenly disable legacy OpROM.
        if (EFI_ERROR(Status)) return;
    }
