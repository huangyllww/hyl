...
EFI_STATUS GetSioDeviceHandle(CSM_SIO_DEVICE_TYPE, EFI_HANDLE*);
BOOLEAN IsSioDevicePresent(CSM_SIO_DEVICE_TYPE);
...
)
{
...

    {
        ...
        if (IsSioDevicePresent(CsmFddDevice)) {
        ...
        if (IsSioDevicePresent(CsmLptPortDevice)) {
        ...
        if ((bda->machine_config & 4) || IsSioDevicePresent(CsmPS2MouseDevice)) {
            machineconfig |= 0x4;
    
            if (!EFI_ERROR(GetSioDeviceHandle(CsmPS2MouseDevice, &Handle))) {
    ...
    TablePointer = (UINTN)GetEfiConfigurationTable(pST, &gAcpiRsdtPtr);
    ASSERT(TablePointer < 0x100000000);
    Csm16BootTable->AcpiTable = (UINT32)TablePointer;
    if (Csm16BootTable->AcpiTable == 0) {
        TablePointer = (UINTN)GetEfiConfigurationTable(pST, &gAcpiRsdtPtr1_0);
        ASSERT(TablePointer < 0x100000000);
        Csm16BootTable->AcpiTable = (UINT32)TablePointer;
    }

    DEBUG((DEBUG_INFO, "CSM - GetEfiConfigurationTable: ACPI table is at %x\n", Csm16BootTable->AcpiTable));

    TablePointer = (UINTN)GetEfiConfigurationTable(pST, &gSmbiosTable);
    ASSERT(TablePointer < 0x100000000);
    Csm16BootTable->SmbiosTable = (UINT32)TablePointer;
...
FDD_STATUS GetFDDStatus()
{
    ...
    if (EFI_ERROR(GetSioDeviceHandle(CsmFddDevice, &Handle))) return NO_FLOPPY_DRIVE;
        ...
        for (IdeCtl = 0; IdeCtl < MAX_IDE_CONTROLLER; IdeCtl++) {
            ...
            if (AtaAtapiInfo[IdeCtl].CommandBaseAddress) {  // real controller
                ...
                if (AtaAtapiInfo[IdeCtl].Status & (HDD_MASTER_IDE | HDD_MASTER_ATAPI_CDROM)) {
                    ...
                    //DEBUG((DEBUG_INFO, "i=%d, bbsentry=%x, diskinfo=%x\n", Indx, &BbsEntry[Indx], *DiskInfoHandles));
...
)
{
    ...
    *MemMap = NULL;
    ...
    // Let's add space for more descriptors to be sure buffer is big enough
...
)
{
    ...
    // Look for _32_ structure and update offset 16 (0x10) with the MaxBus
    {
        ...
        DEBUG((DEBUG_INFO,"CSM - Attempting to update RT32 binary with the MaxBus (0x%x)\n", MaxBus));
        ...
        for (; Adr < 0x100000; Adr += 0x10) {
            if (*(UINT32*)Adr == 0x5f32335f) {
                ...
                UINT8 i;
                UINT8 CheckSum = 0;
                DEBUG((DEBUG_INFO,"_32_ signature is found at %x,", Adr));
                for (i=0; i<0x10; i++) {
                    CheckSum += *((UINT8*)Adr+i);
                }
                if (CheckSum == 0) {
                    DEBUG((DEBUG_INFO,"checksum is OK\n"));
                    *((UINT8*)Adr+0x10) = MaxBus;
                }
                else {
                    DEBUG((DEBUG_INFO,"checksum is invalid.\n"));
                }
            }
        }
    }
...
)
{
    ...
    // print memory map
    for (mm = MemMap; count < MemEntriesCount; mm=(EFI_MEMORY_DESCRIPTOR*)((UINT8*)mm+MemDescSize), count++) {
        DEBUG((DEBUG_INFO, "%02d: %016lx, %05x, ", count, mm->PhysicalStart, mm->NumberOfPages));
        switch (mm->Type) {
            case EfiReservedMemoryType:     DEBUG((DEBUG_INFO, "EfiReservedMemoryType\n")); break;
            case EfiLoaderCode:             DEBUG((DEBUG_INFO, "EfiLoaderCode\n")); break;
            case EfiLoaderData:             DEBUG((DEBUG_INFO, "EfiLoaderData\n")); break;
            case EfiBootServicesCode:       DEBUG((DEBUG_INFO, "EfiBootServicesCode\n")); break;
            case EfiBootServicesData:       DEBUG((DEBUG_INFO, "EfiBootServicesData\n")); break;
            case EfiRuntimeServicesCode:    DEBUG((DEBUG_INFO, "EfiRuntimeServicesCode\n")); break;
            case EfiRuntimeServicesData:    DEBUG((DEBUG_INFO, "EfiRuntimeServicesData\n")); break;
            case EfiConventionalMemory:     DEBUG((DEBUG_INFO, "EfiConventionalMemory\n")); break;
            case EfiUnusableMemory:         DEBUG((DEBUG_INFO, "EfiUnusableMemory\n")); break;
            case EfiACPIReclaimMemory:      DEBUG((DEBUG_INFO, "EfiACPIReclaimMemory\n")); break;
            case EfiACPIMemoryNVS:          DEBUG((DEBUG_INFO, "EfiACPIMemoryNVS\n")); break;
            case EfiMemoryMappedIO:         DEBUG((DEBUG_INFO, "EfiMemoryMappedIO\n")); break;
            case EfiMemoryMappedIOPortSpace: DEBUG((DEBUG_INFO, "EfiMemoryMappedIOPortSpace\n")); break;
            case EfiPalCode:                DEBUG((DEBUG_INFO, "EfiPalCode\n")); break;
            case EfiPersistentMemory:       DEBUG((DEBUG_INFO, "EfiPersistentMemory\n")); break;
            default: DEBUG((DEBUG_INFO, "%x\n", mm->Type));
        }
    }
...
)
{
    ...
    if (Address != 0)
    {
        ...
        DEBUG((DEBUG_INFO, "Free HI PMM memory: %r\n", Status));
    }
    pBS->CloseEvent(Event);
}
