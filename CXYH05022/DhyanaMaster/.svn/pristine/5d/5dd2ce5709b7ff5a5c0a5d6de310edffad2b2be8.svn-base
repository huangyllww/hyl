//<YinQiang001-20171116 Add Bmc Factory Default Options +>
//**********************************************************************
//**********************************************************************
//**                                                                  **
...
/** @file BmcFactoryDefault.c
    Sends Factory Default command to BMC.
...
/**
    This function sends Factory Default command to BMC
...
EFI_STATUS
SendFactoryDefaultCommandToBMC(
...
/**
    This setup callback function sends Factory Default command to BMC
    as per user choice.
...
EFI_STATUS
BmcFactoryDefaultCallbackFunction(
 ...
 IN  UINT16             Key )
{
    ...
    //
    // Continue only for OEM_BMC_FACTORY_DEFAULT_KEY
    //
    if ( Key != OEM_BMC_FACTORY_DEFAULT_KEY ) {
    ...
    if(EFI_ERROR(Status)) {
        ...
        CharPtrStatus = IpmiHiiGetString (HiiHandle, STRING_TOKEN(STR_OEM_BMC_FACTORY_DEFAULT_ERROR_INFO), CharPtr, &BufferLength, NULL);
    ...
    //
    // Get confirmation from the user before doing factory default
    //
    BufferLength = STRING_BUFFER_LENGTH;
    OutputStrStatus = IpmiHiiGetString (HiiHandle, STRING_TOKEN(STR_OEM_BMC_FACTORY_DEFAULT), OutputString, &BufferLength, NULL);
    BufferLength = STRING_BUFFER_LENGTH;
    CharPtrStatus = IpmiHiiGetString (HiiHandle, STRING_TOKEN(STR_OEM_BMC_FACTORY_DEFAULT_INFO), CharPtr, &BufferLength, NULL);
    if((!EFI_ERROR(OutputStrStatus)) && (!EFI_ERROR(CharPtrStatus))) {
    ...
    //
    Status = SendFactoryDefaultCommandToBMC (IpmiTransport);
    
    BufferLength = STRING_BUFFER_LENGTH;
    OutputStrStatus = IpmiHiiGetString (HiiHandle, STRING_TOKEN(STR_OEM_BMC_FACTORY_DEFAULT_DONE), OutputString, &BufferLength, NULL);
    BufferLength = STRING_BUFFER_LENGTH;
    CharPtrStatus = IpmiHiiGetString (HiiHandle, STRING_TOKEN(STR_OEM_BMC_FACTORY_DEFAULT_INFO), CharPtr, &BufferLength, NULL);
    if((!EFI_ERROR(OutputStrStatus)) && (!EFI_ERROR(CharPtrStatus))) {
        AmiPostMgr->DisplayMsgBox (
