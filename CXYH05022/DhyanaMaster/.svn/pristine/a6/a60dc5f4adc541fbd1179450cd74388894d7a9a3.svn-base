...
)
{
    ...
    for (GetDescIteration = 0; GetDescIteration < 5; GetDescIteration++) { //<Hujf1001-20210412 Fix BMC Virtual USB Keyboard not work with low probability +>
        ...
        Status = UsbSmiControlTransfer(
                        ...
                        Buffer,
                        Length);
//<Hujf1001-20210412 Fix BMC Virtual USB Keyboard not work with low probability +>
/*        if (Status) {
            return Buffer;
        }
...
*/
        if (gUsbData->UsbLastCommandStatusExtended & USB_TRNSFR_TIMEOUT) {
            USB_DEBUG(DEBUG_INFO,3,"UsbSmiControlTransfer Status: %r, gUsbData->UsbLastCommandStatusExtended: 0x%x\n", Status, gUsbData->UsbLastCommandStatusExtended);
            gBS->Stall(100 * 1000); 
            continue;
        }
        if (Status) {
...
)
{
    ...
    USB_DEBUG(DEBUG_INFO, DEBUG_LEVEL_USBBUS, "vendorid:%x deviceid:%x uninstalling children...\n", Dev->dev_info->VendorId, Dev->dev_info->DeviceId);
    if ((Dev->dev_info->VendorId == 0x46b) && (Dev->dev_info->DeviceId == 0xFF20))
    {
        PcdSet8(PcdUsbLanRemoved, 1);
        return EFI_UNSUPPORTED;
    }
    //<Hujf1001-20210617 Add solution for system hang when BMC reset during OOB date transfer ->
