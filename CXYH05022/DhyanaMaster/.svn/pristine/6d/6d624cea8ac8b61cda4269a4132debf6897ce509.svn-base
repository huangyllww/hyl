...
//<zhanghy2012-20200413 change STR_HELP_TITLE +>
//<Lvshh001-20200421 Add token for Setup interface style,0 is normal(AMI default) ,1 is EzportPlus(Byo) +>
#if defined(AMI_BYO_BIOS_SETUP_STYLE_SWITCHING) && (AMI_BYO_BIOS_SETUP_STYLE_SWITCHING==1)
//<Lvshh001-20200421 Add token for Setup interface style,0 is normal(AMI default) ,1 is EzportPlus(Byo) ->
#undef  STR_HELP_TITLE
#define STR_HELP_TITLE STR_HELP_TITLE_AK
#endif
//<zhanghy2012-20200413 change STR_HELP_TITLE ->
...
AMI_IFR_MSGBOX _SaveShutdownMsgBox = { 0, 0, STRING_TOKEN(STR_SAVE_SHUTDOWN), (VOID*)INVALID_HANDLE, STRING_TOKEN(STR_SAVE_SHUTDOWN_MSG) };
AMI_IFR_MSGBOX _ShutdownMsgBox = { 0, 0, STRING_TOKEN(STR_SHUTDOWN), (VOID*)INVALID_HANDLE, STRING_TOKEN(STR_SHUTDOWN_MSG) };
#endif
//<Chendl001-20191221 System can power off under BIOS Setup ->
...
//<Chendl001-20191221 System can power off under BIOS Setup +>
#if defined(POWER_OFF_SYSTEM_UNDER_SETUP_SUPPORT)&& (POWER_OFF_SYSTEM_UNDER_SETUP_SUPPORT == 1)
UINTN gSaveShutdownMsgBox = (UINTN)&_SaveShutdownMsgBox;
UINTN gShutdownMsgBox = (UINTN)&_ShutdownMsgBox;
#endif
//<Chendl001-20191221 System can power off under BIOS Setup ->
...
//	EFI_GUID 	DriverHealthProtocolGuid = EFI_DRIVER_HEALTH_PROTOCOL_GUID;
    ...
    SETUP_DEBUG_UEFI ("[TSE]FindDriverHealthDriverName: DriverEntry = %d\n", DriverEntry );
...
//<lvych001-20190403 Revised Intel OPA device hang in setup when enter to "Intel(R) HFI PCIe Gen3 Driver" item.(mantis:27443)+>
...
EFI_STATUS GetChildHandle(EFI_HANDLE ControllerHandle,EFI_HANDLE **HandleBuffer,UINTN *Count)
{
 // EFI_STATUS                Status;
  IHANDLE                              *Handle;
  LIST_ENTRY                           *Link;
  ...
  if ((NULL == ControllerHandle) || (NULL == HandleBuffer) || (NULL == Count))
    return EFI_INVALID_PARAMETER;
  
  Handle = (IHANDLE *)ControllerHandle;
  //
  ...
  //
  ChildHandleBuffer = EfiLibAllocateZeroPool (ChildHandleCount * sizeof(EFI_HANDLE));
  ...
  *HandleBuffer = ChildHandleBuffer;
  *Count = ChildHandleCount;
...
//<lvych001-20190403 Revised Intel OPA device hang in setup when enter to "Intel(R) HFI PCIe Gen3 Driver" item.(mantis:27443)->
	...
	for (DriverHealthIndex = 0; DriverHealthIndex < NumHandles; DriverHealthIndex++)
	{
		...
		for (ControllerIndex = 0; ControllerIndex < HandleCount; ControllerIndex ++)
		{
			...
			ControllerCnt ++;
//<lvych001-20190403 Revised Intel OPA device hang in setup when enter to "Intel(R) HFI PCIe Gen3 Driver" item.(mantis:27443)+>
      Status = GetChildHandle(Handles [ControllerIndex],&ChildHandles,&ChildCount);
      SETUP_DEBUG_TSE ("[TSE]CtrlsSupportsHealthProtocol: ChildCount=%d\n",ChildCount); 
      if (!EFI_ERROR (Status))
      {
        for (ChildIndex = 0; ChildIndex < ChildCount; ChildIndex++)
        {
          if (ChildHandles [ChildIndex] == NULL)
          {
            continue;
          }
          Status = DriverHealth->GetHealthStatus (DriverHealth, Handles [ControllerIndex], ChildHandles [ChildIndex], &HealthStatus, NULL, NULL);
          if (EFI_ERROR (Status))
          {
            continue;
          }
          SETUP_DEBUG_UEFI ("[TSE]CtrlsSupportsHealthProtocol: GetHealthStatus HealthStatus:%s Status = %r, ChildIndex=%d FormHiiHandle:[0] ControllerHandle:[%X] ChildHandle:[%X]\n",DriverHealthStatus[HealthStatus],Status, ChildIndex,Handles [ControllerIndex],ChildHandles [ChildIndex]); 
          SETUP_DEBUG_UEFI ("[TSE]CtrlsSupportsHealthProtocol: AddHandleIntoList()\n");
          AddHandleIntoList (Handles [ControllerIndex], ChildHandles [ChildIndex], HealthStatus);
          ControllerCnt ++;
        }
        MemFreePointer ((VOID **)&ChildHandles);
      }
//<lvych001-20190403 Revised Intel OPA device hang in setup when enter to "Intel(R) HFI PCIe Gen3 Driver" item.(mantis:27443)->
		}
		break;
...
VOID SetDriverHealthCount (VOID)
{
	...
	for (iIndex = 0; iIndex < NumHandles; iIndex ++)
	{
		...
		if (EFI_ERROR (Status))
		{
			 SETUP_DEBUG_UEFI("\n[TSE] DriverHealthProtocolGuid found, but Status from GetHealthStatus =%r, index = %d\n", Status, iIndex );
		     continue;
		}
		HealthCount ++;
	}
...
VOID DoDriverHealthOperation (VOID *Tempcontrol, UINT16 ControllerEntry)
{
	...
	SETUP_DEBUG_UEFI ("In DoDriverHealthOperation:\n");
	gEnableDrvNotification = TRUE;  
	gBrowserCallbackEnabled = TRUE; 
	...
	if (EfiDriverHealthStatusHealthy == DrvHlthHandles->HealthStatus)
	{
	    ...
	    SETUP_DEBUG_UEFI ("DoDriverHealthOperation: gDrvHealthInstance->GetHealthStatus = %r \n",Status);
		if (NULL != MessageList)
		{
	...
	if (EfiDriverHealthStatusRepairRequired == DrvHlthHandles->HealthStatus)
	{
		...
		SETUP_DEBUG_UEFI ("DoDriverHealthOperation: gDrvHealthInstance->Repair = %r \n",Status);
		if (!EFI_ERROR (Status))
		{
			Status = gDrvHealthInstance->GetHealthStatus (
						gDrvHealthInstance,
			...
			SETUP_DEBUG_UEFI ("DoDriverHealthOperation: gDrvHealthInstance->GetHealthStatus-> LocalHealthStatus \n",LocalHealthStatus);
			if (NULL != MessageList)
			{
				DisplayMsgListMessageBox ((TSE_EFI_DRIVER_HEALTH_HII_MESSAGE *)(MessageList));
			}
		...
		(EfiDriverHealthStatusConfigurationRequired == LocalHealthStatus))
	{
		...
		if (EfiDriverHealthStatusConfigurationRequired != LocalHealthStatus)
		{
			...
			SETUP_DEBUG_UEFI ("DoDriverHealthOperation: gDrvHealthInstance->GetHealthStatus-> LocalHealthStatus = %x \n",LocalHealthStatus);
		}
		else
		{
