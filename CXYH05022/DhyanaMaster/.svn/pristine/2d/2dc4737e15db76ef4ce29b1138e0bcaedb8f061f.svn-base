...
#define MP1_THM_TCON_CUR_TMP      0x59800UL
 ...
 */
 ...
 typedef union {
   struct {                                                             ///<
     UINT32                  PerStepTimeUp :5;                          ///< [0:4]PerStepTimeUpMask
     UINT32                  TmpMaxDiffUp :2;                           ///< [5:6] TmpMaxDiffUpMask
     UINT32                  TmpSlewDnEn :1;                            ///< [7] TmpSlewDnEnMask
     UINT32                  PerStepTimeDn :5;                          ///< [8:12] PerStepTimeDnMask
     UINT32                  Reserved_15_13 :3;                         ///< Unused bits
     UINT32                  CurTempTjSel :2;                           ///< [16:17] CurTempTjSelMask
     UINT32                  CurTempTjSlewSel :1;                       ///< [18] CurTempTjSlewSelMask
     UINT32                  CurTempRangeSel :1;                        ///< [19]CurTempRangeSelMask
     UINT32                  McmEn :1;                                  ///< [20]McmEnMask
     UINT32                  CurTemp :11;                               ///< [21:31]CurTempMask
   } Field;                                                             ///<
   UINT32 Value;                                                        ///<
 } THM_TCON_CUR_TMP;
...
HYGON_PROCESSOR_TEMPERATURE_PROTOCOL mProcessorTMPProtocol ={
  0x01,
  SmuGetProcessorTMP
};
...
/**
 *   Read temperature from specified die
 *
 *
 * @param[in]   Socket             Socket ID.
 * @param[in]   Die                Die ID.
 * @param[out]  ReturnValues       Current temperature for current die
 *
 * @retval      EFI_SUCCESS
 *
 */
...
SmuGetProcessorTMP(
   IN       UINT8                       Socket,
   IN       UINT8                       Die,
   IN OUT   INT32                       *ReturnValues        
)
{
    THM_TCON_CUR_TMP                    ThmTconCurTmp;
    INT32                               Temperature = 0;
    EFI_STATUS                          Status;
    GNB_HANDLE                          *GnbHandle;
    PCIe_PLATFORM_CONFIG                *Pcie;
    ...
    IDS_HDT_CONSOLE (MAIN_FLOW, "SmuGetProcessorTMP Entry\n");
    ...
    if (Status == EFI_SUCCESS) {
      ...
      while (GnbHandle != NULL) {
        //Find GnbHandle for this Socket/Die
        if ((Socket == GnbHandle->SocketId) && (Die == GnbHandle->DieNumber)) {
          NbioRegisterRead (GnbHandle, TYPE_SMN, MP1_THM_TCON_CUR_TMP, &ThmTconCurTmp.Value, 0);
          Temperature = (INT32)(ThmTconCurTmp.Field.CurTemp * 125 - (ThmTconCurTmp.Field.CurTempRangeSel ? 49 : 0) * 1000);
          *ReturnValues = Temperature;
          IDS_HDT_CONSOLE (MAIN_FLOW, "SocketID %d DieID %x: Temperature = %d \n", Socket, Die, *ReturnValues);
          Status = EFI_SUCCESS;
          break;
        }
		GnbHandle = GnbGetNextHandle (GnbHandle);
    ...
    IDS_HDT_CONSOLE (MAIN_FLOW, "SmuGetProcessorTMP Exit\n");
  ...
  )
{
  ...
  //<Yanshp-20220701 Fix can't get 7391CPU TDP +>
  // read the TDP and Save to PCD
  NbioSmuServiceCommonInitArgumentsV9 (SmuArg);
  Status = NbioSmuServiceRequestV9 (NbioGetHostPciAddress (NbioHandle), SMC_MSG_TDP_TJMAX, SmuArg, 0);
  TDP = (SmuArg[0] & 0xFFFF0000) >> 16;
  PcdSet16S (PcdTDP,TDP);
  IDS_HDT_CONSOLE (MAIN_FLOW, "TDP/TJMAX = 0x%x; TDP = %d; PcdGet16 (PcdTDP) = %d\n", SmuArg[0],TDP,PcdGet16 (PcdTDP));
  //<Yanshp-20220701 Fix can't get 7391CPU TDP ->
  ...
  //CBS debug options
  PPTable->DldoPsmMargin = (INT8)(PcdGet8 (PcdDldoPsmMargin) - 9);
  ...
  IDS_HDT_CONSOLE(MAIN_FLOW, "DldoPsmMargin = %d\n", PPTable->DldoPsmMargin);
  ...
  Status = gBS->InstallProtocolInterface(&Handle, &gHygonProcessorTempServicesProtocolGuid, EFI_NATIVE_INTERFACE, &mProcessorTMPProtocol);
