...
#define FILECODE MEM_AMDMEMDDR4PEI_AMDMEMDDR4PEI_FILECODE

STATIC AMD_MEM_TECH_SERVICES_PPI mMemTechServicesPpi = {
  AMD_MEM_TECH_SERVICES_REVISION,
  AmdMemDimmPresenceDdr4
};

STATIC EFI_PEI_PPI_DESCRIPTOR mMemTechPiServicesList =
{
  (EFI_PEI_PPI_DESCRIPTOR_PPI | EFI_PEI_PPI_DESCRIPTOR_TERMINATE_LIST),
  &gAmdMemoryTechServicesPpiGuid,
  &mMemTechServicesPpi
};
  ...
  )
{
  ...
  Status = (**PeiServices).InstallPpi (PeiServices, &mMemTechPiServicesList);
  if (EFI_SUCCESS != Status) {
    return Status;
  }
  return (Status);
