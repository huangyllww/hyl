...
#include "DxeIpl.h"
#include <Token.h>//<Lvshh001-20221024 Workaround for hang60 issue>
...
VOID PublishMemoryInformationHob(EFI_BOOT_MODE BootMode){
	...
	//<Lvshh001-20221024 Workaround for hang60 issue +>
	...
	if (!EFI_ERROR (Status)) {
	    ...
	    if (!EFI_ERROR (Status)){
	    	if (ValidateMemoryTypeInfoVariable(MemoryInformationBuffer, Size)){ //<Lvshh001-20221024 Workaround for hang60 issue>   		
	    	//<Lvshh001-20221024 Workaround for hang60 issue +>
#if defined(WORKAROUND_FOR_HANG60)&& (WORKAROUND_FOR_HANG60 == 1) 
	    	    for (Index = 0; MemoryInformationBuffer[Index].Type != EfiMaxMemoryType; Index++) {
	    		    DEBUG((EFI_D_ERROR, "[OemDbg][%a][%d]MemoryInformationBuffer[%x].NumberOfPages is %x\n\n",__FUNCTION__,__LINE__,Index,MemoryInformationBuffer[Index].NumberOfPages));
	    		    TotalSize += LShiftU64 (MemoryInformationBuffer[Index].NumberOfPages, EFI_PAGE_SHIFT);
	    		}
	    		if (TotalSize > MAX_TOLERANCE_LENGTH) {
	    		    if (BootMode != BOOT_ON_S4_RESUME) {
	    		        Status = VariablePpi->GetVariable ( 
	    		            VariablePpi, 
	    		            L"PreviousMemoryTypeInformation", &gAmiGlobalVariableGuid,
	    		            NULL, &Size, MemoryInformationBuffer
	    		        );
	    		        if ( Status == EFI_NOT_FOUND ) {
	    		            Status = VariablePpi->GetVariable ( 
	                    ...
	                    if (TotalSize > MAX_TOLERANCE_LENGTH) {
	                        Status = EFI_UNSUPPORTED;//MemoryInformationPtr point to DefaultMemoryTypeInformationPtr
	                    }   
	    		    }
	    		}
#endif
