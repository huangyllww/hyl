  ...
  )
{
  ...
  case KEY_FIX_PSTATE:
    FixPstate(This, Action, QuestionId, Type, Value, ActionRequest, pSetup_Config, HiiHandle);
    break;
  //<hujf1001-20200512 Add Fix P state feature ->
  ...
  )
{
  ...
  if (PackageType == ZP_SOCKET_SP4) {
      if ((LogicalId.Family == HYGON_FAMILY_18) && (LogicalId.Revision & HYGON_REV_F18_DN_Dx)) {
          NbioSmuServices->AmdSmuRegisterRead (NbioSmuServices, 0, HYGON_DX_OPNFUSEPKG_SP3R2, &FuseValue);
          if ((FuseValue & HYGON_DX_OPEN_FUSE_BIT) != 0) {
              PackageType = ZP_SOCKET_SP3r2;
          }
      } else {
          NbioSmuServices->AmdSmuRegisterRead (NbioSmuServices, 0, OPNFUSEPKG_SP3R2, &FuseValue);
          if ((FuseValue & OPEN_FUSE_BIT) != 0) {
              PackageType = ZP_SOCKET_SP3r2;
          }
      }
  }
  ...
  if ((LogicalId.Family == HYGON_FAMILY_18) && (LogicalId.Revision & HYGON_REV_F18_DN_Dx))
      NbioSmuServices->AmdSmuRegisterRead (NbioSmuServices, 0, HYGON_DX_MP0_CORE_DISABLE, &CoreDisFusePerDie);
  else
      NbioSmuServices->AmdSmuRegisterRead (NbioSmuServices, 0, MP0_CORE_DISABLE, &CoreDisFusePerDie);
  ...
  // Get OPN TDP
  if ((LogicalId.Family == HYGON_FAMILY_18) && (LogicalId.Revision & HYGON_REV_F18_DN_Dx)) {
      NbioSmuServices->AmdSmuRegisterRead (NbioSmuServices, 0, HYGON_DX_MP1_MAX_TDP, &FuseValue);
      OpnMaxTdp = (FuseValue >> HYGON_DX_MP1_MAX_TDP_START_BITS) & 0xFF;
  } else {
      NbioSmuServices->AmdSmuRegisterRead (NbioSmuServices, 0, MP1_MAX_TDP, &FuseValue);
      OpnMaxTdp = (FuseValue >> MP1_MAX_TDP_START_BITS) & 0xFF;
  }
