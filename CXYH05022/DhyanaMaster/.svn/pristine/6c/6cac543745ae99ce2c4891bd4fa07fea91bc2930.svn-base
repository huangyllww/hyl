  ...
  )
{
  ...
  for(Index=0; Index<DiskNum; Index++) {
    ...
    //Check HardDisk / SSD
    OemHDDInfo.HDDMediaType = DISK_UNKNOWN;
    if(DriveInfo->HddMode == TypeAhci) OemHDDInfo.HDDMediaType = (DriveInfo->MediaRotationRate == 0x1)?DISK_SSD:DISK_HDD;
    if(DriveInfo->HddMode == TypeNvme) OemHDDInfo.HDDMediaType = DISK_SSD;
    //SGPZT23830#SyncDiskType(HDD/SSD)WithBMC->
