...
{		
    EFI_STATUS          	Status=EFI_SUCCESS;
    UINTN               	vsz;
    //<Machong-20170909-For APTIOV Porting+>//Machong 2014.11.21 Support for OpROM Selecting   {  
    #if defined(OemPCIeDev_SUPPORT) && (OemPCIeDev_SUPPORT == 1)
...
//-------------------------------------------------------------
	...
	if(PciCommon!=NULL){
		...
		//<Machong-20170909-For APTIOV Porting+>//Machong 2014.11.21 Support for OpROM Selecting   { 
        #if defined(OemPCIeDev_SUPPORT) && (OemPCIeDev_SUPPORT == 1)
        Status = pBS->LocateProtocol(&gOemPcieDevProtocolGuid, NULL, (VOID**)&OemPcieDevProtocol); 
        if(EFI_ERROR(Status)){
            pBS->SetMem(PciCommon->PciDevEN, MAX_DEV_COUNT,1);
            pBS->SetMem(PciCommon->OpROMEN, PCIDEV_ENTRY_NUMBER,CSMSETUP_LEGACY_ONLY_OPROMS);
        } else {   
//<Yuanshai-20190710 For :Move PciDevEN OpROMEN SriovEN from "Setup" to "OEMPCIE_DEV" --BEGIN>  
            Status = OemPcieDevProtocol->GetSetPcieDevPolicy(PciCommon->PciDevEN,PCIDEVEN,TRUE);
            ASSERT_EFI_ERROR (Status);
            Status = OemPcieDevProtocol->GetSetPcieDevPolicy(PciCommon->OpROMEN,OPROMEN,TRUE);
            ASSERT_EFI_ERROR (Status);
//<Yuanshai-20190710 For :Move PciDevEN OpROMEN SriovEN from "Setup" to "OEMPCIE_DEV" --END> 
        }
...
//-------------------------------------
	...
	if(EFI_ERROR(Status)){
		...
		if(PciCommon!=NULL){
			...
			PciCommon->Above4gDecode=ABOVE_4G_PCI_DECODE;
			PciCommon->SriovSupport=SRIOV_SUPPORT;       
			//Machong 2014.11.21 Support for OpROM Selecting+>
            #if defined(OemPCIeDev_SUPPORT) && (OemPCIeDev_SUPPORT == 1)
			pBS->SetMem(PciCommon->PciDevEN, MAX_DEV_COUNT,1);
			pBS->SetMem(PciCommon->OpROMEN, PCIDEV_ENTRY_NUMBER,CSMSETUP_LEGACY_ONLY_OPROMS);
			#endif
...
//#endif
		...
		if (PciCommon!=NULL){
			...
			//<lvych00120160706+>//Machong 2014.11.21 Support for OpROM Selecting {
            #if defined(OemPCIeDev_SUPPORT) && (OemPCIeDev_SUPPORT == 1)

            //<Yuanshai-20190710 For :Move PciDevEN OpROMEN SriovEN from "Setup" to "OEMPCIE_DEV" --BEGIN>
            //pBS->CopyMem(PciCommon->PciDevEN, SetupData->PciDevEN, MAX_DEV_COUNT);
            //pBS->CopyMem(PciCommon->OpROMEN, SetupData->OpROMEN, PCIDEV_ENTRY_NUMBER);
            Status = pBS->LocateProtocol(&gOemPcieDevProtocolGuid, NULL, (VOID**)&OemPcieDevProtocol); 
            ASSERT_EFI_ERROR (Status);
            Status = OemPcieDevProtocol->GetSetPcieDevPolicy(PciCommon->PciDevEN,PCIDEVEN,TRUE);
            ASSERT_EFI_ERROR (Status);
			...
			//PciSetupData->OpROMEN[4] = 1; //Always Enable M.2 OpROM
			#endif
			//<lvych00120160706->//Machong 2014.11.21 Support for OpROM Selecting }
		}
