...
//---------------------------------
	...
	if(Get){
		...
		vname=GetSioLdVarName(Dev,FALSE);
		vs=sizeof(SIO_DEV_NV_DATA);
        Status = pRS->GetVariable(vname, &ssg, 
                &Attributes, 
                &vs, &tmpvar);
...
//-------------------
	for(i=0; i<Spio->LdCount; i++){
        ...
        //If any resource shared device disabled by setup, adjust resource shared relationship for other devices.
        if (dev->DeviceInfo->HasSetup && !dev->NvData.DevEnable) {
            if(dev->ResOwner==NULL && dev->ResConsumer!=NULL){ // The first resource shared device
                dev->ResConsumer->ResOwner = NULL;
                dev->ResConsumer->DeviceInfo->Flags = SIO_SHR_NONE;
                
                dev->ResConsumer = NULL;
            }
            
            if(dev->ResOwner!=NULL && dev->ResConsumer!=NULL) {
                dev->ResOwner->ResConsumer = dev->ResConsumer;
                dev->ResConsumer->ResOwner = dev->ResOwner;
                
                dev->ResOwner = NULL;
                dev->ResConsumer = NULL;
                dev->DeviceInfo->Flags = SIO_SHR_NONE;
            }

            if(dev->ResOwner!=NULL && dev->ResConsumer==NULL) { // The last resource shared device
                dev->ResOwner->ResConsumer = NULL;                
                dev->ResOwner = NULL;
                dev->DeviceInfo->Flags = SIO_SHR_NONE;
            }                          
        }
