...
#include "Include/Setup.h" //<Yaosr001-20190731 Support BIOS HotKey select.>
#endif
#ifdef TSE_FOR_APTIO_4_50
...
HII_FORM_ADDRESS RootPageList[] = {
		AMITSE_SUBPAGE_AS_ROOT_PAGE_LIST{NULL_GUID, 0, NULL},
};
HII_FORM_ADDRESS HiddenPageList[] = {
		AMITSE_HIDDEN_PAGE_LIST{NULL_GUID, 0, NULL},
};
...
#if MSFT_HLK_TEST_SUPPORT
EFI_STATUS pSetGraphicsResolution(UINT32 HorResolution, UINT32 VerResolution)
{
	...
	for (iIndex = 0; iIndex < pGOP->Mode->MaxMode; iIndex++)
	{
		Status = pGOP->QueryMode(pGOP, iIndex, &SizeOfInfo, &pModeInfo);
	}
	for (iIndex = 0; iIndex < pGOP->Mode->MaxMode; iIndex++)
	{
		Status = pGOP->QueryMode(pGOP, iIndex, &SizeOfInfo, &pModeInfo);
		if (!EFI_ERROR(Status))
		{
			...
			{
				if (pGOP->Mode->Mode != iIndex) //If Current mode not equals the found mode then call SetMode otherwise return success
				{
					Status = pGOP->SetMode(pGOP, iIndex);
...
EFI_STATUS pSetNativeResFromEdid(VOID)
{
	EFI_STATUS Status;
	UINT32 HorRes;
	UINT32 VerRes;
	...
	{
		Status = pSetGraphicsResolution(1024, 768); //If EDID not found set 1024*768 mode
		return Status;
	}
	...
	Status = pSetGraphicsResolution(HorRes, VerRes);
	if (EFI_ERROR(Status))
	{
		Status = pSetGraphicsResolution(1024, 768); //If EDID not found set 1024*768 mode
		return Status;
										...
										EFI_KEY EfiKeySDL, UINT32 ShiftStateSDL, EFI_KEY_TOGGLE_STATE ToggleStateSDL)
{
	...
	if ((((AmiKey->Key.UnicodeChar == UnicodeCharSDL) && (UnicodeCharSDL != 0)) || ((AmiKey->Key.ScanCode == ScanCodeSDL) && (ScanCodeSDL != 0))) ^ ((AmiKey->EfiKey == EfiKeySDL) && (EfiKeySDL != 0)))
		bKeyMatch = TRUE;
	...
	{
		if (((AmiKey->KeyState.KeyToggleState & (~TOGGLE_STATE_VALID)) & ToggleStateSDL) || ((AmiKey->KeyState.KeyToggleState == TOGGLE_STATE_VALID) && (ToggleStateSDL == TOGGLE_STATE_VALID)))
			bToggleStateMatch = TRUE;
	}
...
BOOLEAN CheckOEMKey(AMI_EFI_KEY_DATA *AmiKey, UINT16 UnicodeCharSDL, UINT16 ScanCodeSDL)
{
	if ((AmiKey->Key.UnicodeChar == UnicodeCharSDL) && (AmiKey->Key.ScanCode == ScanCodeSDL))
		return TRUE;
	else
		return FALSE;
}
...
VOID CheckForKey(EFI_EVENT Event, VOID *Context)
{
	...
	do
	{
		...
		if (!EFI_ERROR(Status))
		{
#if defined(OEM_TSE_SUPPORT) && (OEM_TSE_SUPPORT == 1) //<lvych001-20170502 Improve code+>
			OemCheckForKey(AmiKey);													 //<lvych001-20161129 Add press hot key tips>
#endif																								 //<lvych001-20170502 Improve code->

			//<Yaosr001-20190731 Support BIOS HotKey select. +>
#if defined(BIOS_HOTKEY_SUPPORT) && (BIOS_HOTKEY_SUPPORT == 1)
			//DEBUG((DEBUG_INFO, "CheckForKey\n"));
			...
			//<Yaosr001-20190731 Support BIOS HotKey select. ->
			if (
					((SETUP_ENTRY_UNICODE == AmiKey.Key.UnicodeChar) && (SETUP_ENTRY_SCAN == AmiKey.Key.ScanCode)
#if TSE_USE_AMI_EFI_KEYCODE_PROTOCOL
					 && (TSE_CHECK_SHIFTSTATE(AmiKey.KeyState.KeyShiftState, SETUP_ENTRY_SHIFT_STATE))
#endif
							 )
#if SETUP_ALT_ENTRY_ENABLE
					|| ((SETUP_ALT_ENTRY_UNICODE == AmiKey.Key.UnicodeChar) && (SETUP_ALT_ENTRY_SCAN == AmiKey.Key.ScanCode)
#if TSE_USE_AMI_EFI_KEYCODE_PROTOCOL
...
#if SETUP_BBS_POPUP_ENABLE
			else if ((AmiKey.Key.UnicodeChar == POPUP_MENU_ENTRY_UNICODE) && (AmiKey.Key.ScanCode == POPUP_MENU_ENTRY_SCAN)
#if TSE_USE_AMI_EFI_KEYCODE_PROTOCOL
			...
			else if ((AmiKey.Key.UnicodeChar == SETUP_TOGGLE_KEY_UNICODE) && (AmiKey.Key.ScanCode == SETUP_TOGGLE_KEY_SCANCODE)
//<hujf1001-20200522 Avoid gibberish cause quiet mode changed, replace TAB key with Ctrl+TAB +>
#if TSE_USE_AMI_EFI_KEYCODE_PROTOCOL
							 && (TSE_CHECK_SHIFTSTATE(AmiKey.KeyState.KeyShiftState, RIGHT_LEFT_CONTROL_PRESSED))
#endif
							 //<hujf1001-20200522 Avoid gibberish cause quiet mode changed, replace TAB key with Ctrl+TAB ->
			)
			{
														...
														UINT8 *PopupFGColor, UINT8 *PopupBGColor)
{
	...
	if (SecFGColor != NULL)
		*SecFGColor = Colors.SecFGColor;
	if (SecBGColor != NULL)
		*SecBGColor = Colors.SecBGColor;

	if (SelFGColor != NULL)
		*SelFGColor = Colors.SelFGColor;
	if (SelBGColor != NULL)
		*SelBGColor = Colors.SelBGColor;

	if (NSelFGColor != NULL)
		*NSelFGColor = Colors.NSelFGColor;
	if (NSelBGColor != NULL)
		*NSelBGColor = Colors.NSelBGColor;

	if (LabelFGColor != NULL)
		*LabelFGColor = Colors.LabelFGColor;
	if (LabelBGColor != NULL)
		*LabelBGColor = Colors.LabelBGColor;

	if (NSelLabelFGColor != NULL)
		*NSelLabelFGColor = Colors.NSelLabelFGColor;

	if (EditFGColor != NULL)
		*EditFGColor = Colors.EditFGColor;
	if (EditBGColor != NULL)
		*EditBGColor = Colors.EditBGColor;

	if (PopupFGColor != NULL)
		*PopupFGColor = Colors.PopupFGColor;
	if (PopupBGColor != NULL)
		*PopupBGColor = Colors.PopupBGColor;
