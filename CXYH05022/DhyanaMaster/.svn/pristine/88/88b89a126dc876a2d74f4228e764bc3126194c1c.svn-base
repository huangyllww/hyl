...
//mizl-sync XWH Rome Solution for Mantis 0053723:BIOS-Driver Health intel abnormal +>
#if 0 
...
#if defined(OemPCIeDev_SUPPORT) && (OemPCIeDev_SUPPORT == 1) //<lvych001-20161128 Sync with PCIe Dev List Module Config +>
#include <PciBus.h>
#endif //<lvych001-20161128 Sync with PCIe Dev List Module Config ->
#define DEFAULT_HANDLE_BUFFER_SIZE 0x40
#define CMOS_PXE_BOOTTIME_OFFSET      0x50   //<lubl-20170428 PXE boot retry>
#define CMOS_PXE_BOOTTIME_OFFSET_FLAG      0x51   //<lubl-20170428 PXE boot retry>
typedef struct {
    UINTN TotalSize;
    UINTN CurrentSize;
    EFI_HANDLE *Array;
...
//VOID *ProcessOpRomRegistration; //mizl-sync XWH Rome Solution for Mantis 0053723:BIOS-Driver Health intel abnormal +>
VOID *DriverBindingRegistration;
//static UINTN CurrentCount = 0; //mizl-sync XWH Rome Solution for Mantis 0053723:BIOS-Driver Health intel abnormal +>
//static BOOLEAN ThunkFirst;
SETUP_DATA SetupData;

//EFI_BUS_SPECIFIC_DRIVER_OVERRIDE_PROTOCOL *BusSpecific = NULL; //mizl-sync XWH Rome Solution for Mantis 0053723:BIOS-Driver Health intel abnormal +>
...
EFI_STATUS  OemCheckOpRomExecution(
        IN EFI_PCI_IO_PROTOCOL *PciIo,        
        IN UINT8 PciClass,
        IN BOOLEAN UefiOprom
        )
{
    ...
    Policy = OemGetOpRomPolicy(PciIo,PciClass);
    ...
    switch(Policy) {
        ...
        case 2:     //only Legacy OpROMs are enabled
            Result = (UefiOprom) ? FALSE : TRUE;
            break;
//<lvych001-20161215 Add both uefi and legacy policy support +>
        case 3:     //add both uefi and legacy
            Result = TRUE;
            break;
//<lvych001-20161215 Add both uefi and legacy policy support ->
        default:    //all OpROMs are enabled
...
//check if someone already disabled execution of this OpROM
    ...
    if (EFI_ERROR(Status)) 
        return;
#if defined(OemPCIeDev_SUPPORT) && (OemPCIeDev_SUPPORT == 1) //<lvych001-20161128 Sync with PCIe Dev List Module Config +>
    Status = OemCheckOpRomExecution(PciIo,PciClassCode, FALSE);
#else
...
*/
    ...
    Status = pBS->InstallMultipleProtocolInterfaces(&Handle,
                    ...
                    //&gEfiPlatformDriverOverrideProtocolGuid, //mizl-sync XWH Rome Solution for Mantis 0053723:BIOS-Driver Health intel abnormal +>
                    //&CsmPolicyPlatformDriverOverride,//mizl-sync XWH Rome Solution for Mantis 0053723:BIOS-Driver Health intel abnormal +>
                    NULL);
    ASSERT_EFI_ERROR(Status);
...
)
{
    ...
    VOID *Registration;
    UINT8  ConfigPXEBootRetry;  //<lubl-20170428 PXE boot retry>
    ...
    //<lubl-20170428 PXE boot retry +>
    ConfigPXEBootRetry = SetupData.ConfigPXEBootRetry;
		
    IoWrite8(0x70, CMOS_PXE_BOOTTIME_OFFSET);
    IoWrite8(0x71, ConfigPXEBootRetry);
    
    IoWrite8(0x70, CMOS_PXE_BOOTTIME_OFFSET_FLAG);
    IoWrite8(0x71, 0x01);
    //<lubl-20170428 PXE boot retry -> 
...
{
#if defined(OemPCIeDev_SUPPORT) && (OemPCIeDev_SUPPORT == 1) //<lvych001-20161128 Sync with PCIe Dev List Module Config +>
    return OemCheckOpRomExecution(NULL,PciClass, TRUE);
#else
...
#if defined(OemPCIeDev_SUPPORT) && (OemPCIeDev_SUPPORT == 1) //<lvych001-20161128 Sync with PCIe Dev List Module Config +>
    return (EFI_ERROR(Status))? Status : OemCheckOpRomExecution(PciIo,PciClass, TRUE);
#else
...
)
{
    ...
    */
    ...
    switch (Policy) {
        case CSMSETUP_LEGACY_ONLY_OPROMS:
        ...
        case CSMSETUP_UEFI_ONLY_OPROMS:
    ...
    /* now get other Driver binding handles except those with thunk protocol */
        ...
        case CSMSETUP_SKIP_OPROMS: return EFI_NOT_FOUND;
    }
