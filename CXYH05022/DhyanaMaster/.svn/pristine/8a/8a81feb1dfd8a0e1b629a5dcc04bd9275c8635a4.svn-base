  ...
  IN  EFI_IPMI_BMC_INSTANCE_DATA    *gIpmiInstance )
{
    ...
    UINT8                 RequSize;
    UINT8                 RespSize;
    UINT8                 BMCTimeSyncMode;
    UINT8                 flag = 0;
    UINTN                 Size;
//<Malj-20190823 Fixed 'bmc time sync mode as utc' every cycle will update time +>
    UINT32                BmcToUtcDiffTime;
//<Malj-20190823 Fixed 'bmc time sync mode as utc' every cycle will update time ->
#endif
    //<Yaosr001-20190730  Add "BMC Time Sync Mode" options. ->
    ...
    gSetupStatus = gRT->GetVariable (
                      ...
                      &Size,
                      &gServerMgmtConfiguration );
    SERVER_IPMI_DEBUG ((EFI_D_INFO, "[OemDbg]gRT->GetVariable status %r, ServerMgmtConfiguration.TimeZoneMechanism: %d \n", gSetupStatus, gServerMgmtConfiguration.BMCTimeSyncMode));

    //Get BMCTimeSyncMode value through IPMI command.
    TempBuf[0] = 5; //Parameter Selector: OemIpmiSetupManagementOptions=5
    TempBuf[1] = 0; //Data Type: get MOD_DATA from BMC
    RequSize   = 2;
    RespSize   = 20;
    Status = gIpmiInstance->IpmiTransport.SendIpmiCommand (
                 &gIpmiInstance->IpmiTransport,
                 ...
                 &IpmiBuf[1],
                 &RespSize );
    if ((!EFI_ERROR(Status)) && (IpmiBuf[0] == 0 /*CompletionCode*/) && (IpmiBuf[1] == 1/*Flag*/) && ((IpmiBuf[10]) & (0x80))) { //<Lvshh001-20191114 Fix Data transmission error when BMC does not have BMCTimeSyncMode function >
      BMCTimeSyncMode = IpmiBuf[10] & 0x01;
    }else{
      BMCTimeSyncMode = gServerMgmtConfiguration.BMCTimeSyncMode;
    }
    SERVER_IPMI_DEBUG ((EFI_D_INFO, "[OemDbg]Get BMCTimeSyncMode: %x\n", BMCTimeSyncMode));
    
    //<Yaosr001-20190821 Optimize the function of BMCTimeSyncMode. +>
    //judge whether the BMCTimeSyncMode option is modified? (flag)  
    SERVER_IPMI_DEBUG ((EFI_D_INFO, "[OemDbg]Get recorded BMCTimeSyncMode: %x\n", gServerMgmtConfiguration.RecordBMCTimeMode));
    if(gServerMgmtConfiguration.RecordBMCTimeMode != BMCTimeSyncMode) {
      flag = 1;
      gServerMgmtConfiguration.RecordBMCTimeMode = BMCTimeSyncMode;
      gSetupStatus = gRT->SetVariable (
                  L"ServerSetup",
                  &gEfiServerMgmtSetupVariableGuid,
                  gAttributes,
                  Size,
                  &gServerMgmtConfiguration);
      SERVER_IPMI_DEBUG ((EFI_D_INFO, "[OemDbg]Get modified recorded BMCTimeSyncMode: %x\n", gServerMgmtConfiguration.RecordBMCTimeMode));
    }
    //<Yaosr001-20190821 Optimize the function of BMCTimeSyncMode. ->
    //<Yaosr001-20190730  Add "BMC Time Sync Mode" options. ->
    //<Malj-20190823 Fixed 'bmc time sync mode as utc' every cycle will update time +>
    if (BMCTimeSyncMode == 1) {//UTC 
        BmcToUtcDiffTime = CurrentBmcTime - 60 * CurrentBmcUtcOffset;
    } else if (BMCTimeSyncMode == 0) {//Local Time
        BmcToUtcDiffTime = CurrentBmcTime;
    }
    SERVER_IPMI_DEBUG ((EFI_D_INFO, "BmcToUtcDiffTime %x NumOfSeconds: %x \n", BmcToUtcDiffTime, NumOfSeconds));

    if(BmcToUtcDiffTime < NumOfSeconds - 2 || BmcToUtcDiffTime > NumOfSeconds + 2 || flag ) { //<Yaosr001-20190821 Optimize the function of BMCTimeSyncMode.> 
#else //<Malj-20190823 Fixed 'bmc time sync mode as utc' every cycle will update time>
    if(CurrentBmcTime < NumOfSeconds - 2 || CurrentBmcTime > NumOfSeconds + 2) {
#endif //<Malj-20190823 Fixed 'bmc time sync mode as utc' every cycle will update time>
        //
...
#if defined(BMC_TIME_SYNC_MODE_SUPPORT) && (BMC_TIME_SYNC_MODE_SUPPORT == 1) //<Yaosr001-20190730  Add "BMC Time Sync Mode" options +>
        if(BMCTimeSyncMode == 0){
#endif //<Yaosr001-20190730  Add "BMC Time Sync Mode" options ->
          //<yuxla-20170406:Sync up time with BMC+>
          // Currently BMC always treats BIOS time as UTC time
          // to sync up time with BMC, before send BIOS time via SEL, adjust the time with BMC UTC timezone offset
          if (CurrentBmcUtcOffset != EFI_UNSPECIFIED_TIMEZONE) {
            NumOfSeconds -= 60 * CurrentBmcUtcOffset;
          }
          //<yuxla-20170406:Sync up time with BMC->
#if defined(BMC_TIME_SYNC_MODE_SUPPORT) && (BMC_TIME_SYNC_MODE_SUPPORT == 1) //<Yaosr001-20190730  Add "BMC Time Sync Mode" options +>
        }
  ...
  IN EFI_SYSTEM_TABLE       *SystemTable )
{
    ...
    //
    IpmiSmbiosTypesSupport ();  //<SGEZT24089 BMC Type38 create always>
    if (gIpmiInstance->BmcStatus != EFI_BMC_HARDFAIL) {
        ...
        //
        //<SGEZT24089 BMC Type38 create always +>
        //FreePool (gIpmiInstance);
        return EFI_SUCCESS;
        //<SGEZT24089 BMC Type38 create always ->
