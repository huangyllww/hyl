...
#include <SataControllerInfoLib.h> //<hujf1001-20210107 Add more Hard Disk asset info for OEMIPMICmdV1.8  +>
...
//<hujf1001-20211011 Fix system hang when wrong PciIo protocol used for external AHCI controller +>
...
//<hujf1001-20211011 Fix system hang when wrong PciIo protocol used for external AHCI controller ->
{
  ...
  //SATA_DEVICE_INTERFACE     *SataDevInterface = NULL; //<hujf1001-20211011 Fix system hang when wrong PciIo protocol used for external AHCI controller +>
  EFI_PCI_IO_PROTOCOL       *PciIO;
  //PCI_DEV_INFO              *Prt; //<hujf1001-20210107 Add more Hard Disk asset info for OEMIPMICmdV1.8  +>
  UINT8                     Index;
  UINT32                    SectorSize;
  ...
  #endif
  //<hujf1001-20211011 Fix incorrect slot number for external AHCI controller +>
  UINT32                    IdeChannel;
  UINT32                    IdeDevice;
  //<hujf1001-20211011 Fix incorrect slot number for external AHCI controller ->
  EFI_DEVICE_PATH_PROTOCOL  *TempDevicePath = NULL; //<hujf1001-20211011 Fix system hang when wrong PciIo protocol used for external AHCI controller +>
  ...
  //<hujf1001-20211011 Fix system hang when wrong PciIo protocol used for external AHCI controller +>
  Status = pBS->HandleProtocol(Handle, &gEfiDevicePathProtocolGuid, (VOID**)&TempDevicePath);
  ...
  DEBUG((EFI_D_INFO, "LocateDevicePath gEfiPciIoProtocolGuid Status is %r\n",Status));
  ...
  DiskInfo->WhichIde ( DiskInfo, &IdeChannel, &IdeDevice ); //<hujf1001-20211011 Fix incorrect slot number for external AHCI controller +>
  //PciIO = SataDevInterface->AhciBusInterface->PciIO; //<hujf1001-20211011 Fix system hang when wrong PciIo protocol used for external AHCI controller +>
  ...
  DriveInfo->HddSlot = (UINT8)IdeChannel; //<hujf1001-20211011 Fix incorrect slot number for external AHCI controller +>
  ...
  DriveInfo->MediaRotationRate = IdentifyDriveInfo->AtaData.nominal_media_rotation_rate;
  DEBUG((EFI_D_ERROR, "MediaRotationRate: %x\n", DriveInfo->MediaRotationRate));
  //SGPZT23830#SyncDiskType(HDD/SSD)WithBMC->
  ...
  for (Index = 0; Index < HandleCount; Index++) {
    ...
    if (!EFI_ERROR(Status)) {
      ...
      if ( CompareGuid(&DiskInfo->Interface, &gEfiDiskInfoAhciInterfaceGuid) ) {
        Status = AhciDriveInfoConstruction(DiskInfo, &HddInfo,HandleBuffer[Index]); //<hujf1001-20211011 Fix system hang when wrong PciIo protocol used for external AHCI controller +>
