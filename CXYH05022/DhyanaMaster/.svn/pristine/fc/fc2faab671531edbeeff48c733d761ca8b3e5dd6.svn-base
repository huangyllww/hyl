...
//#if BUILD_OS != BUILD_OS_LINUX    TODO: Linux
#if 1	
...
VOID HiiRemoveString(EFI_HII_HANDLE Handle, UINT16 Token)
{
  ...
  {
  	Uefi21HiiChangeStringLanguage( Handle, Token, gLanguages[i].Unicode, nullString );
  }
}
		...
		for (ControllerIndex = 0; ControllerIndex < HandleCount; ControllerIndex ++)
		{
			...
			for (ChildIndex = 0; ChildIndex < HandleCount; ChildIndex++)
			{
              ...
              if (EFI_ERROR (Status))
                ...
                SETUP_DEBUG_UEFI ("[TSE]CtrlsSupportsHealthProtocol: GetHealthStatus Status = %r, ChildIndex=%d\n",Status, ChildIndex); 
                SETUP_DEBUG_UEFI ("[TSE]CtrlsSupportsHealthProtocol: AddHandleIntoList()\n"); 
...
EFI_STATUS DisplayParseFormSetData(PAGE_INFO *PageInfo)
{
	...
	while(i < pkgHdr->Length)
	{
		...
		switch(opHeader->OpCode)
		{
			case EFI_IFR_FORM_OP: //0x01 - Form
                ...
                if(((EFI_IFR_FORM*)(ifrData + i))->FormId == pageInfo->PageFormID)
                {
                   ...
                   SETUP_DEBUG_TSE (  "FormTitle: %s\n", HiiGetString(pageInfo->PageHandle, pageInfo->PageSubTitle  ));
        ...
        if(CurrentForm && questionId) SETUP_DEBUG_TSE (  "StringId: 0x%x  %s \n", questionId, HiiGetString(pageInfo->PageHandle , questionId  ));    
...
EFI_STATUS _GetDefaultNvramVariableIDList(UINT32 PageID, BOOLEAN DefaultsFlag, BOOLEAN *ResetVal, BOOLEAN *ValChanged)
{
  ...
  for(index = 0; index < count; index++)
  {
    ...
    if(ctrlInfo->ControlFlags.ControlEvaluateDefault && DEFAULT_CONDITIONAL_EXPRESSION_SUPPORT)
    {
      UINT64 defaults = 0;
