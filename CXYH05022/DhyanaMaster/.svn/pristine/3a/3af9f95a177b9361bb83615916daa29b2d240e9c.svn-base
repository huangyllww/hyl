...
GetNvmeDeviceDetails(VOID) {
    ...
    LIST_ENTRY                            *LinkData;
    UINT32                                Nsid = 1;
    //<xuyj1-20220422 Add Chinese language of NVME options +>
    EFI_STRING              Str = NULL;
    //<xuyj1-20220422 Add Chinese language of NVME options ->
    ...
    for (Index = 0; Index < HandleCount; Index++) {
        ...
        NvmeTextItem = (NVME_TEXT_ITEM *)NvmeControllerInfo->ActiveNameSpace;
         ...
         for (LinkData = NvmeController->ActiveNameSpaceList.ForwardLink; 
            ...
            Str = HiiGetString( gNvmeHiiHandle, STRING_TOKEN(STR_NVME_NAME_SPACE),NULL);
            Swprintf(String, L"%s: %d",Str, ActiveNameSpaceData->ActiveNameSpaceID);
            NvmeHiiAddString(gNvmeHiiHandle, String, &NvmeTextItem->Prompt);
            Str = HiiGetString( gNvmeHiiHandle, STRING_TOKEN(STR_NVME_NAME_SPACE),"en-US");
            Swprintf(String, L"%s: %d",Str, ActiveNameSpaceData->ActiveNameSpaceID);
            HiiSetString(gNvmeHiiHandle, NvmeTextItem->Prompt, String, "en-US");
            Str = HiiGetString( gNvmeHiiHandle, STRING_TOKEN(STR_NVME_NAME_SPACE),"zh-chs");
            Swprintf(String, L"%s: %d",Str, ActiveNameSpaceData->ActiveNameSpaceID);
            HiiSetString(gNvmeHiiHandle, NvmeTextItem->Prompt, String, "zh-chs");
            //<xuyj1-20220422 Add Chinese language of NVME options ->
            pBS->SetMem(String, sizeof(String), 0);       
            //<xuyj1-20220422 Add Chinese language of NVME options +>
            ...
            //Swprintf(String, L"Size: %d GB", NameSpaceSizeInGB);
            //<LiuYB036-20180118 Remove NVMe Tenth of GB info. to sync with BMC web asset info.->
            Str = HiiGetString( gNvmeHiiHandle, STRING_TOKEN(STR_NVME_DEVICE_SIZE),NULL);
            Swprintf(String, L"%s: %d GB",Str, NameSpaceSizeInGB);
            NvmeHiiAddString(gNvmeHiiHandle, String, &NvmeTextItem->TextTwo);
            Str = HiiGetString( gNvmeHiiHandle, STRING_TOKEN(STR_NVME_DEVICE_SIZE),"en-US");
            Swprintf(String, L"%s: %d GB",Str, NameSpaceSizeInGB);
            HiiSetString(gNvmeHiiHandle, NvmeTextItem->TextTwo, String, "en-US");
            Str = HiiGetString( gNvmeHiiHandle, STRING_TOKEN(STR_NVME_DEVICE_SIZE),"zh-chs");
            Swprintf(String, L"%s: %d GB",Str, NameSpaceSizeInGB);
            HiiSetString(gNvmeHiiHandle, NvmeTextItem->TextTwo, String, "zh-chs");
            //<xuyj1-20220422 Add Chinese language of NVME options ->
            NvmeTextItem++;
            Nsid++;
         }
        ...
        Swprintf(String, L"%d GB", TotalNameSpaceSizeInGB);
        //<LiuYB036-20180118 Remove NVMe Tenth of GB info. to sync with BMC web asset info.->
    ...
    if(!EFI_ERROR(Status)) {
       for (Link = gNvmeControllerList.ForwardLink; Link != &gNvmeControllerList; Link = Link->ForwardLink, Count++) {
        ...
        HiiCreateSubTitleOpCode(StartOpCodeHandle,
                                ...
                                0,0);*/
        HiiCreateSubTitleOpCode(StartOpCodeHandle,
                                STRING_TOKEN(STR_NON_NVME_DEVICE),
                                STRING_TOKEN(STR_EMPTY),
                                0,0);
        //<xuyj1-20220422 Add Chinese language of NVME options ->
        StringId = 0;
