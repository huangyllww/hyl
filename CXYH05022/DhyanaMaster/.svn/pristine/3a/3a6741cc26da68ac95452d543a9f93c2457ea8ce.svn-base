            ...
            if (TempPtr != NULL) {
                ...
                if (!EFI_ERROR (Status)) {
                    ...
                    if ( CalculateCheckSum8 ( TempPtr, Length ) == 0 ) {
                        ...
                        Status = GetFRUData (&TempPtr[Offset], &StrPtr);
                        if (!EFI_ERROR (Status)) {
						    AliMocExist = identifyMoc(StrPtr);
							if (AliMocExist == TRUE)
								PcdSet8(PcdAliMocExist, 1);
							else
								PcdSet8(PcdAliMocExist, 0);
						    Size = sizeof(LastFruStr);
                            Status = gRT->GetVariable (L"LastFruChassisExtra", &gEfiRedirFruProtocolGuid, &Attribute, &Size, LastFruStr);
                            if(EFI_ERROR(Status))
                            {
                                SERVER_IPMI_DEBUG ((EFI_D_INFO, "get LastFruBaseBoardExtra1 error\n"));
								if (AliMocExist == TRUE)
									PcdSet8(PcdAliConfigChange, 1); //Ali common platform by default
                            }
							else
							{
								if (AliMocExist != identifyMoc(LastFruStr)) //Ali FRU updated
									PcdSet8(PcdAliConfigChange, 1);
							}
                            SizeofBuffer = StrSize (StrPtr);
                            Attribute = EFI_VARIABLE_NON_VOLATILE | EFI_VARIABLE_BOOTSERVICE_ACCESS;
                            gRT->SetVariable (L"LastFruChassisExtra", &gEfiRedirFruProtocolGuid, Attribute, SizeofBuffer, StrPtr);

                            SERVER_IPMI_DEBUG ((EFI_D_INFO, "PcdChassisExtra: StrPtr: %S\n", StrPtr));
                            PcdSetPtr (PcdChassisExtra, &SizeofBuffer, (VOID*)StrPtr);
                            SERVER_IPMI_DEBUG ((EFI_D_INFO, "PcdChassisExtra: %S\n", (CHAR16 *) PcdGetPtr (PcdChassisExtra)));
                            FreePool (StrPtr);
                        }
...
//<<Kangmm001-20190828 fix Mantis 0032076 DMI fru information lost during AC Power cycle test ->    
    ...
    if (GenerateDummySmbiosRecords) {  // Use saved FRU strings to fill smbios tables, which saved at last boot
        ...
        if (!EFI_ERROR(Status)) {
            SizeofBuffer = StrSize (LastFruStr);
            PcdSetPtr (PcdChassisExtra, &SizeofBuffer, (VOID*)LastFruStr);
			AliMocExist = identifyMoc(LastFruStr);
			if (AliMocExist == TRUE) //Get FRU Chasis Extra failed, default use common platform
			{
				PcdSet8(PcdAliConfigChange, 1);
				PcdSet8(PcdAliMocExist, 1);
			}
