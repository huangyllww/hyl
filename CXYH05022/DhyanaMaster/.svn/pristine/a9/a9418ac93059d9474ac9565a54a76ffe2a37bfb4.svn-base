...
UINT8                       gChannelArray[] = { 1, 2, 8 };
//<liuyb-20170801 add bmc user in multi-channel ->

// <xuyj1-20190115 adjust user privilege +>
BMC_GROUP_INFORMATION       gBmcUserGroupList[MAX_BMC_GROUP_COUNT];
// <xuyj1-20190115 adjust user privilege ->
//----------------------------------------------------------------------
  ...
  IN  UINT16            Key )
{
    ...
    ServerMgmtConfiguration->AddBmcChannel[0] = L'A';
    ServerMgmtConfiguration->AddBmcChannel[1] = L'l';
    ServerMgmtConfiguration->AddBmcChannel[2] = L'l';
    ServerMgmtConfiguration->AddBmcChannel[3] = L'\0';
    //<liuyb-20170801 add bmc user in multi-channel ->
  ...
  IN  UINT16            Key )
{
    ...
    //<AMI-20170502-debug for BMC Set User Password Failed!!+>
...
//<liuyb-20170801 add bmc user in multi-channel>    SERVER_IPMI_DEBUG ((EFI_D_INFO, "IPMI: User entered channel no = %x\n", ServerMgmtConfiguration->AddBmcChannelNo));

    //<liuyb-20170801 add bmc user in multi-channel +>
    //GetChannelAccessCommand.ChannelNo = ServerMgmtConfiguration->AddBmcChannelNo;
    GetChannelAccessCommand.ChannelNo = gChannelArray[0];
    //<liuyb-20170801 add bmc user in multi-channel ->
  ...
  IN  UINT16            Key )
{
    ...
    UINT8                               DataSize;
    UINT8                               ChannelIndex;  //<liuyb-20170801 add bmc user in multi-channel>

    // <xuyj1-20190115 adjust user privilege +>
    BMC_ADD_DELETE_USER_FROM_GROUP      AddDeleteUserGroup={0};
    BMC_GET_GROUP_OF_USER_COMMAND       GetUserGroupIdCommand={0};
    BMC_GET_GROUP_OF_USER_RESPONSE      GetUserGroupIdRespond={0};
//    UINT8                               NewGroup;
    BMC_SET_USER_SNMP_COMMAND           SetUserSNMPCommand={0,0,2,1,2}; 
    BMC_SET_USER_PRIVILEGE_COMMAND      SetUserPrivilegeCommand={0};
    // <xuyj1-20190115 adjust user privilege ->
    ...
    if(ServerMgmtConfiguration->AddBmcUserPrivilegeLimit >0) 
    {
        //Get user group Id Value
        
        //Add the user to new group
        AddDeleteUserGroup.UserId=gBmcAddUserDetails.UserId;
        AddDeleteUserGroup.GroupId=gBmcUserGroupList[ServerMgmtConfiguration->AddBmcUserPrivilegeLimit].GroupIndex;
        AddDeleteUserGroup.Flags=0x1;
        DataSize = 0;
        Status = gIpmiTransport->SendIpmiCommand (
                    gIpmiTransport,
                    OEM_IPMI_NET_FUNCTION,
                    BMC_LUN,
                    ADD_DELETE_USER_FROM_GROUP,
                    (UINT8 *)&AddDeleteUserGroup,
                    sizeof(BMC_ADD_DELETE_USER_FROM_GROUP),
                    NULL,
                    &DataSize);
        SERVER_IPMI_DEBUG ((EFI_D_INFO, "Add User to New Group Status: %r \n", Status));
        if (EFI_ERROR(Status)) {
            DisplayErrorMessagePopUp ( HiiHandle, STRING_TOKEN(STR_BMC_ADD_USER_INTO_NEWGROUP) );
            return EFI_SUCCESS;
        }
       
        // Set user Extended Privilege
        DataSize=0;
        SetUserPrivilegeCommand.UserId=gBmcAddUserDetails.UserId;
        SetUserPrivilegeCommand.ExtendedPrivilege=gBmcUserGroupList[ServerMgmtConfiguration->AddBmcUserPrivilegeLimit].GroupExtendedPrivilege & 0x03;
        Status = gIpmiTransport->SendIpmiCommand (
                        gIpmiTransport,
                        AMI_IMPI_NET_FUNCTION,
                        BMC_LUN,
                        SET_USER_PRIVILEGE_COMMAND,
                        (UINT8 *)&SetUserPrivilegeCommand,
                        sizeof(BMC_SET_USER_PRIVILEGE_COMMAND),
                        NULL,
                        &DataSize);
        SERVER_IPMI_DEBUG ((EFI_D_INFO, "Set User Extended Privilege Command Status: %r \n", Status));
        if (EFI_ERROR(Status)) {
            DisplayErrorMessagePopUp ( HiiHandle, STRING_TOKEN(STR_BMC_SET_USER_PRIVILEGE_COMMAND) );
            return EFI_SUCCESS;
        }
        
        // Set user SNMP
        DataSize=0;
        SetUserSNMPCommand.UserId=gBmcAddUserDetails.UserId;
        SetUserSNMPCommand.SnmpByte=((gBmcUserGroupList[ServerMgmtConfiguration->AddBmcUserPrivilegeLimit].GroupExtendedPrivilege)>>2) & 0x01;
        Status = gIpmiTransport->SendIpmiCommand (
                        gIpmiTransport,
                        AMI_IMPI_NET_FUNCTION,
                        BMC_LUN,
                        SET_USER_SNMP_COMMAND,
                        (UINT8 *)&SetUserSNMPCommand,
                        sizeof(BMC_SET_USER_SNMP_COMMAND),
                        NULL,
                        &DataSize);
        SERVER_IPMI_DEBUG ((EFI_D_INFO, "Set User SNMPC ommand Status: %r \n", Status));
        if (EFI_ERROR(Status)) {
            DisplayErrorMessagePopUp ( HiiHandle, STRING_TOKEN(STR_BMC_SET_USER_SNMP_COMMAND) );
            return EFI_SUCCESS;
        }   
    }
    ...
    // Byte1
    //
//<liuyb-20170801 add bmc user in multi-channel>    SetUserAccess.ChannelNo = ServerMgmtConfiguration->AddBmcChannelNo;
   ...
   // SetUserAccess.UserPrivilegeLimit = ServerMgmtConfiguration->ChangeUserSettingsBmcUserPrivilegeLimit;
    switch(ServerMgmtConfiguration->AddBmcUserPrivilegeLimit){
        case 0: //Reserved
        ...
        case 3: // User
        case 4: // None
            SetUserAccess.UserPrivilegeLimit=gBmcUserGroupList[ServerMgmtConfiguration->AddBmcUserPrivilegeLimit].GroupPrivilege;
            break;
        default://OEM
            SetUserAccess.UserPrivilegeLimit=0x5; 
            break;
    }    
    // <xuyj1-20190115 adjust user privilege ->
    ...
    //<liuyb-20170801 add bmc user in multi-channel +>
    for (ChannelIndex = 0; ChannelIndex < sizeof(gChannelArray) / sizeof(gChannelArray[0]); ChannelIndex++) {
        SetUserAccess.ChannelNo = gChannelArray[ChannelIndex];
    //<liuyb-20170801 add bmc user in multi-channel ->
    ...
    //<liuyb-20170801 add bmc user in multi-channel +>
        SERVER_IPMI_DEBUG ((EFI_D_INFO, "IPMI: Channel: %d \n", SetUserAccess.ChannelNo));
        if(EFI_ERROR (Status)) {
            break;
        }
    }
  ...
  IN  UINT16            Key )
{
    ...
    ASSERT_EFI_ERROR(Status);
        ...
        ServerMgmtConfiguration->ChangeUserSettingsBmcChannel[0] = L'A';
        ServerMgmtConfiguration->ChangeUserSettingsBmcChannel[1] = L'l';
        ServerMgmtConfiguration->ChangeUserSettingsBmcChannel[2] = L'l';
        ServerMgmtConfiguration->ChangeUserSettingsBmcChannel[3] = L'\0';
        //<liuyb-20170801 add bmc user in multi-channel ->
  ...
  IN  UINT16            Key )
{
    ...
    //<liuyb-20170801 add bmc user in multi-channel +>
    EFI_GET_USER_ACCESS_COMMAND	        GetUserAccessCmd = { 0 };
    EFI_GET_USER_ACCESS_RESPONSE        GetUserAccessResp;
    UINT8                               DataSize;
    //<liuyb-20170801 add bmc user in multi-channel ->
    ...
    //show current user enable status and privilege limit status
    GetUserAccessCmd.ChannelNo = gChannelArray[0];
    GetUserAccessCmd.UserId = gBmcChangeUserDetails.UserId;
    DataSize = sizeof(GetUserAccessResp);
    ...
    Status = gIpmiTransport->SendIpmiCommand (
                ...
                BMC_LUN,
                IPMI_APP_GET_USER_ACCESS,
                (UINT8 *)&GetUserAccessCmd,
                sizeof(GetUserAccessCmd),
                (UINT8 *)&GetUserAccessResp,
                &DataSize);

    SERVER_IPMI_DEBUG ((EFI_D_INFO, "IPMI: CommandCompletionCode: %x \n", gIpmiTransport->CommandCompletionCode));
    SERVER_IPMI_DEBUG ((EFI_D_INFO, "IPMI: Status of IPMI_APP_GET_USER_ACCESS Command: %r \n", Status));

    if (!EFI_ERROR(Status)) {
        ServerMgmtConfiguration->ChangeUserSettingsBmcUser = GetUserAccessResp.Reserve2 % 2;  //ipmi spec 2.0: 01b=enable, 10b=disable
        // <xuyj1-20190115 adjust user privilege +>
        // ServerMgmtConfiguration->ChangeUserSettingsBmcUserPrivilegeLimit = GetUserAccessResp.UserPrivilegeLimit;
        // <xuyj1-20190115 adjust user privilege ->
    } else {
        ServerMgmtConfiguration->ChangeUserSettingsBmcUser = 2;  //disabled
        ServerMgmtConfiguration->ChangeUserSettingsBmcUserPrivilegeLimit = 0;  //reserved
    }

    // <xuyj1-20190115 adjust user privilege +>
    DataSize=sizeof(BMC_GET_GROUP_OF_USER_RESPONSE);
    GetUserGroupIdCommand.UserId=gBmcChangeUserDetails.UserId;
    Status = gIpmiTransport->SendIpmiCommand (
                gIpmiTransport,
                OEM_IPMI_NET_FUNCTION,
                BMC_LUN,
                GET_GROUP_OF_USER,
                (UINT8 *)&GetUserGroupIdCommand,
                sizeof(BMC_GET_GROUP_OF_USER_COMMAND),
                (UINT8 *)&GetUserGroupIdRespond,
                &DataSize);
    if (!EFI_ERROR(Status)) {
        for(GroupIndex=0;GroupIndex< ServerMgmtConfiguration->GroupCount;GroupIndex++)
        {
            if(GetUserGroupIdRespond.GroupId==gBmcUserGroupList[GroupIndex].GroupIndex)
                { ServerMgmtConfiguration->ChangeUserSettingsBmcUserPrivilegeLimit=GroupIndex;
                    break;
                }
        }
    }
  ...
  IN  UINT16            Key )
{
    ...
    if (ServerMgmtConfiguration->ChangeUserSettingsBmcUserPassword[0] == '\0') {
        DEBUG ((DEBUG_INFO, "IPMI: Change User Password is NULL\n"));
        FreePool (ServerMgmtConfiguration);
        return EFI_SUCCESS;
    }
    //<AMI-20170502-debug for BMC Set User Password Failed!!->
...
//    UINT8                               NewGroup;
    ...
    if(ServerMgmtConfiguration->ChangeUserSettingsBmcUserPrivilegeLimit >0) 
    {
        //Get user group Id Value
        DataSize=sizeof(BMC_GET_GROUP_OF_USER_RESPONSE);
        GetUserGroupIdCommand.UserId=gBmcChangeUserDetails.UserId;
        Status = gIpmiTransport->SendIpmiCommand (
                    ...
                    (UINT8 *)&GetUserGroupIdRespond,
                    &DataSize);
        SERVER_IPMI_DEBUG ((EFI_D_INFO, "Get Group ID From UserId Status: %r \n", Status));
        if (EFI_ERROR(Status)) {
            DisplayErrorMessagePopUp ( HiiHandle, STRING_TOKEN(STR_SET_USER_GET_Group_ID_FAIL) );
            return EFI_SUCCESS;
        } 

        // Delete user from current group
        AddDeleteUserGroup.UserId=gBmcChangeUserDetails.UserId;
        AddDeleteUserGroup.GroupId=GetUserGroupIdRespond.GroupId;
        AddDeleteUserGroup.Flags=0x2;//Delete flag
        DataSize = 0;
        Status = gIpmiTransport->SendIpmiCommand (
                    gIpmiTransport,
                    OEM_IPMI_NET_FUNCTION,
                    ...
                    NULL,
                    &DataSize);
        SERVER_IPMI_DEBUG ((EFI_D_INFO, "Delete user from current group Status: %r \n", Status));
        if (EFI_ERROR(Status)) {
            DisplayErrorMessagePopUp ( HiiHandle, STRING_TOKEN(STR_SET_USER_ACCESS_CMD_FAIL) );
            return EFI_SUCCESS;
        }
        ...
        //Add the user to new group
        AddDeleteUserGroup.UserId=gBmcChangeUserDetails.UserId;
        AddDeleteUserGroup.GroupId=gBmcUserGroupList[ServerMgmtConfiguration->ChangeUserSettingsBmcUserPrivilegeLimit].GroupIndex;
        AddDeleteUserGroup.Flags=0x1;
        DataSize = 0;
        Status = gIpmiTransport->SendIpmiCommand (
                    gIpmiTransport,
        ...
        // Set user Extended Privilege
        DataSize=0;
        SetUserPrivilegeCommand.UserId=gBmcChangeUserDetails.UserId;
        SetUserPrivilegeCommand.ExtendedPrivilege=gBmcUserGroupList[ServerMgmtConfiguration->ChangeUserSettingsBmcUserPrivilegeLimit].GroupExtendedPrivilege & 0x03;
        Status = gIpmiTransport->SendIpmiCommand (
                        gIpmiTransport,
                        AMI_IMPI_NET_FUNCTION,
                        BMC_LUN,
        ...
        // Set user SNMP
        DataSize=0;
        SetUserSNMPCommand.UserId=gBmcChangeUserDetails.UserId;
        SetUserSNMPCommand.SnmpByte=((gBmcUserGroupList[ServerMgmtConfiguration->ChangeUserSettingsBmcUserPrivilegeLimit].GroupExtendedPrivilege)>>2) & 0x01;
        Status = gIpmiTransport->SendIpmiCommand (
                        gIpmiTransport,
                        AMI_IMPI_NET_FUNCTION,
                        BMC_LUN,
    ...
    // Byte1
    //
//<liuyb-20170801 add bmc user in multi-channel>    SetUserAccess.ChannelNo = ServerMgmtConfiguration->ChangeUserSettingsBmcChannelNo;
    SetUserAccess.EnableIpmiMessaging = 1;
    SetUserAccess.EnableUserLinkAuthetication = 1;
    SetUserAccess.UserRestrictedToCallback = 0;
    SetUserAccess.EnableChangeBitsInByte1 = 1;
    ...
    // <xuyj1-20190115 adjust user privilege +>
   // SetUserAccess.UserPrivilegeLimit = ServerMgmtConfiguration->ChangeUserSettingsBmcUserPrivilegeLimit;
    switch(ServerMgmtConfiguration->ChangeUserSettingsBmcUserPrivilegeLimit){
        case 0: //Reserved
            SetUserAccess.UserPrivilegeLimit=ServerMgmtConfiguration->ChangeUserSettingsBmcUserPrivilegeLimit;
            break;  
        case 1: // Admin
        ...
        case 3: // User
        case 4: // None
            SetUserAccess.UserPrivilegeLimit=gBmcUserGroupList[ServerMgmtConfiguration->ChangeUserSettingsBmcUserPrivilegeLimit].GroupPrivilege;
            break;
        default://OEM
            SetUserAccess.UserPrivilegeLimit=0x5;            
    }    
...
/*
 * Get all user group information and update the user privilege limit string information in setup 
 */
VOID GetUserGroupInforamtion(
        IN EFI_HII_HANDLE     HiiHandle,
        IN UINT16             Class )
{
    BMC_GET_USER_GROUP_COMMAND          GetUserGroupCommand={0};
    BMC_GROUP_INFORMATION               GetUserGroupInformation[MAX_BMC_GROUP_COUNT]={0};
    UINT8                               GroupIndex=0;
    UINT8                               GroupListIndex=1;
    UINT8                               GroupCount=0;
    EFI_IPMI_TRANSPORT                  *GetUserIpmiTransport;
    EFI_STATUS                          Status = EFI_SUCCESS;
    UINT8                               DataSize=0;
    ...
    UINT16                              GroupName[]=
    {
      STRING_TOKEN(STR_RESERVED),
      STRING_TOKEN(STR_BMC_USER_GROUP_1),
      STRING_TOKEN(STR_BMC_USER_GROUP_2),
      STRING_TOKEN(STR_BMC_USER_GROUP_3),
      STRING_TOKEN(STR_BMC_USER_GROUP_4),
      STRING_TOKEN(STR_BMC_USER_GROUP_5),
      STRING_TOKEN(STR_BMC_USER_GROUP_6),
      STRING_TOKEN(STR_BMC_USER_GROUP_7),
      STRING_TOKEN(STR_BMC_USER_GROUP_8),
      STRING_TOKEN(STR_BMC_USER_GROUP_9),
      STRING_TOKEN(STR_BMC_USER_GROUP_A),
    };
    SERVER_IPMI_DEBUG ((EFI_D_INFO,"GetUserGroupInforamtion Entry...\n"));
    Status = gBS->LocateProtocol (
                    &gEfiDxeIpmiTransportProtocolGuid,
                    NULL,
                    (VOID **)&GetUserIpmiTransport );
    SERVER_IPMI_DEBUG ((EFI_D_INFO, " IPMI: gEfiDxeIpmiTransportProtocolGuid Status: %r \n", Status));
    if(EFI_ERROR (Status)) {
        SERVER_IPMI_DEBUG ((EFI_D_INFO,"IPMI: LocateProtocol gEfiDxeIpmiTransportProtocolGuid Error\n"));
        return ;
    }
    ...
    Status = gRT->GetVariable(L"ServerSetup", &gEfiServerMgmtSetupVariableGuid, &Attributes, &SelectionBufferSize, 
                              ServerMgmtConfiguration);
    SERVER_IPMI_DEBUG ((EFI_D_INFO, "IPMI: Status of GetVariable() = %r\n", Status));
    if(EFI_ERROR (Status)) {
        SERVER_IPMI_DEBUG ((EFI_D_INFO,"IPMI: GetVariable Error\n"));
        return ;
    }

    // Get all User Group Information
    for(GroupIndex=1;GroupIndex<MAX_BMC_GROUP_COUNT;GroupIndex++){
    DataSize = sizeof(BMC_GROUP_INFORMATION);
    GetUserGroupCommand.UserId=GroupIndex;
    Status = GetUserIpmiTransport->SendIpmiCommand (
                    GetUserIpmiTransport,
                    OEM_IPMI_NET_FUNCTION,
                    BMC_LUN,
                    GET_USER_GROUP_COMMAND,
                    (UINT8 *)&GetUserGroupCommand,
                    sizeof(BMC_GET_USER_GROUP_COMMAND),
                    (UINT8 *)&GetUserGroupInformation[GroupIndex],
                    (UINT8 *)&DataSize);
    
    SERVER_IPMI_DEBUG ((EFI_D_INFO, "Get User Group Command Status: %r \n", Status));
    SERVER_IPMI_DEBUG ((EFI_D_INFO, "Get User Group Command Loop Number =%x \n", GroupIndex));
    
    //Add the User Group Information into Global User Group Information variable
    if((GetUserGroupInformation[GroupIndex].GroupIndex > 0)&&(GetUserGroupInformation[GroupIndex].GroupIndex < MAX_BMC_GROUP_COUNT)){
        CopyMem(&gBmcUserGroupList[GroupListIndex],&GetUserGroupInformation[GroupIndex],sizeof(BMC_GROUP_INFORMATION));
        //Initial the Setup Group Name information
        GroupCount++;
        GroupListIndex++;
        }

    }
    SERVER_IPMI_DEBUG ((EFI_D_INFO, "Get User Group Command GroupCount Number =%x \n", GroupCount));
    SERVER_IPMI_DEBUG ((EFI_D_INFO, "Get User Group Command Loop Number =%s \n", gBmcUserGroupList[GroupIndex].GroupName));
    for(GroupIndex=1;GroupIndex<=GroupCount;GroupIndex++)
    {
        AsciiToUnicodeSize ( UnicodeString,sizeof(gBmcUserGroupList[GroupIndex].GroupName),gBmcUserGroupList[GroupIndex].GroupName); 
        SERVER_IPMI_DEBUG ((EFI_D_INFO, "Get User Group GroupName =%S \n", UnicodeString));
        InitString(HiiHandle,GroupName[GroupIndex],L"%s",UnicodeString);
    }
    
    //Set the value of Suppress variable
    ServerMgmtConfiguration->GroupCount = GroupCount;
    Status = gRT->SetVariable (L"ServerSetup", &gEfiServerMgmtSetupVariableGuid, Attributes, SelectionBufferSize, ServerMgmtConfiguration);
    SERVER_IPMI_DEBUG ((EFI_D_INFO, "IPMI: Status of SetVariable() = %r\n", Status));
    if(EFI_ERROR (Status)) {
        SERVER_IPMI_DEBUG ((EFI_D_INFO,"IPMI: SetVariable Error\n"));
        return ;
    }
