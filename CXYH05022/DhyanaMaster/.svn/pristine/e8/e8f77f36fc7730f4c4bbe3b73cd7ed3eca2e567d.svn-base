...
//-----------------------------------------------------------------------------------
    ...
    DEBUG((EFI_D_INFO, "ACPI: SetAcpiTable() Table=0x%X; Handle=0x%X; *Handle=0x%X\n", Table, Handle, *Handle));
    if (Table != NULL)
    {
        Hdr1 = (ACPI_HDR*) Table;
        DEBUG((EFI_D_INFO, "ACPI: SetAcpiTable() Table Signature %x %c%c%c%c  \n", Hdr1->Signature, (Hdr1->Signature & 0xFF), ((Hdr1->Signature >> 8) & 0xFF), ((Hdr1->Signature >> 16) & 0xFF), ((Hdr1->Signature >> 24) & 0xFF)));
        ...
        if (Hdr1->Signature == FACP_SIG)
        {
            ...
            if (Hdr1->Revision <= ACPI_REV2)
            {
                ...
                DEBUG((EFI_D_INFO, "Setting FACP Version to 1_B\n"));
            }
        }
        ...
        if (Hdr1->Signature == DSDT_SIG)
        {
            ...
            {
                Version = EFI_ACPI_TABLE_VERSION_1_0B;
                DEBUG((EFI_D_INFO, "Setting DSDT Version to 1_B\n"));
            }
        }
    ...
    if (*Handle)
    {
        ...
        DEBUG((EFI_D_INFO, "ACPI: SetAcpiTable() Hnd!=0, Removing Tbl. Ver=0x%X, ",Version));
        ...
        DEBUG((EFI_D_INFO, "TblNum=0x%X ", TblNum));
        ...
        if (EFI_ERROR(Status))
        {
        	DEBUG((EFI_D_ERROR,"Error removing ACPI table! Status = %r\n", Status));
            return Status;
        }
        ...
        DEBUG((EFI_D_INFO,"ACPI:  Deleting Table From Storage: Status = %r\n", Status));
        ASSERT_EFI_ERROR(Status);
        ...
        {
        	DEBUG((EFI_D_INFO,"ACPI: !!! Deleting DSDT !!! \n"));
    ...
    *Handle=(UINTN)Ptr;
    ...
    if (CorrectFacp)
    {
        ...
        DEBUG((EFI_D_INFO,"UpdateFacp Status = %r\n", Status));
    }
    
    DEBUG((EFI_D_INFO,"ACPI: SetAcpiTable() Exiting... Status = %r\n", Status));

    if (gPublishedOnReadyToBoot)
    {
        if (!gForceAcpi1) Status = PublishTbl (1, 1);
        else Status = PublishTbl (1, 0);
        DEBUG((EFI_D_INFO,"ACPI: PublishTables in SetAcpiTable() Status = %r\n", Status));
    }
...
VOID SubmitAcpiTablesFromFile ()
{
	...
	while (Status == EFI_SUCCESS) 
	{
		...
		if (!EFI_ERROR(Status)) 
		{
			...
			Instance++;
			DEBUG((EFI_D_INFO, "SubmitAcpiTablesFromFile Acpi Tbl submitted Instance = 0x%X\n ", Instance));
			CurrentTable = NULL;
		}
	}
...
EFI_STATUS FindAcpiTable(UINT32 Signature, EFI_ACPI_SDT_HEADER **Table, EFI_ACPI_TABLE_VERSION *Version, UINTN *Key)
{
    ...
    Idx = 0;
    do {
        Status = GetAcpiTable2( Idx, Table, Version, Key);
        ...
        VOID *Context)
{
    ...
    static BOOLEAN PublishWsmtTableIsSaved = FALSE;//<Lvshh001-20221108 For MemoryLeak Fix,otherwise when the EfiBootServicesData increases too much, the hang60 will occur>
	
    //<Lvshh001-20221108 For MemoryLeak Fix,otherwise when the EfiBootServicesData increases too much, the hang60 will occur +>
    DEBUG(( DEBUG_INFO, "PublishWsmtTable Entry\n"));
    if (PublishWsmtTableIsSaved){
        return EFI_SUCCESS;
    }
    PublishWsmtTableIsSaved = TRUE;
    //<Lvshh001-20221108 For MemoryLeak Fix,otherwise when the EfiBootServicesData increases too much, the hang60 will occur ->
    ...
    if (!EFI_ERROR(Status))
    {
        ...
        Status = AcpiUninstallAcpiTable(&gAcpiData.AcpiTableProtocol, Key);
        if (EFI_ERROR(Status))
            DEBUG(( EFI_D_ERROR, "Can not uninstall WSMT table.  Status = %r\n", Status));
    }
	
    PrepareHdr20(Signature, (ACPI_HDR*)&Table, 4);
	...
	// Publish Table
	Status = AcpiInstallAcpiTable(&gAcpiData.AcpiTableProtocol, &Table, sizeof(WSMT_ACPI_TABLE), &Key);
	if (EFI_ERROR(Status))
	{
...
//------------------------
    ...
    DEBUG((EFI_D_INFO,"IN ACPI Start: %r\n", Status));
