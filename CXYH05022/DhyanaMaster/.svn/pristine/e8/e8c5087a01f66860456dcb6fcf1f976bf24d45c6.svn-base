...
Routine Description:
  ...
  I2cMaster Driver Entry. Publish I2cMaster Ppi
  ...
  )
{
  ...
  //Init I2cCotroller capabilities data structure
  ...
  for (Index = 0; Index < I2C_CONTROLLER_COUNT; Index++) {
    ...
    I2cSetSdaHold(I2cPrivate->I2cMasterPpi[Index].I2cSdpAddress, Index);
  }
...
/**
 *
 * This function use for change I2C IC_SDA_HOLD register default
 * value from I2cSdaHoldTimeList.
 *
 *
 * @param[in]   I2cSdpAddress
 * @param[in]   I2cBus
...
{
  if(I2cSdaHoldTimeList[I2cBus].OverrideEn) {
    MmioWrite32 (I2cSdpAddress + DW_IC_SDA_HOLD, I2cSdaHoldTimeList[I2cBus].SdaHold);
  }
  return EFI_SUCCESS;
}
...
//     *BusClockHertz = HS_SPEED;
  ...
  DEBUG ((EFI_D_INFO, "Standard-mode HCNT:LCNT = %d:%d\n", hcnt, lcnt));
  ...
  DEBUG ((EFI_D_INFO, "Fast-mode HCNT:LCNT = %d:%d\n", hcnt, lcnt));
  ...
  )
{
  ...
  if (MmioRead32(Base + DW_IC_RAW_INTR_STAT) & DW_IC_INTR_TX_ABRT) {
      DEBUG ((EFI_D_ERROR, "I2cTx Abort\n"));
      return EFI_ABORTED;
  }
  DEBUG ((EFI_D_INFO, "I2C_STATUS : %x, Tx timeout remaining : %d of %d\n", MmioRead32 (Base + DW_IC_STATUS), Timeout, DW_TX_DATA_RETRY));
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  DEBUG ((EFI_D_INFO, "I2C_STATUS : %x, Rx timeout remaining : %d of %d\n", MmioRead32 (Base + DW_IC_STATUS), Timeout, DW_TX_DATA_RETRY));
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  if (MmioRead32 (Base + DW_IC_TAR) != Chip) {
    ...
    do {
      MicroSecondDelay (I2cPollTime);
      if (I2cStatusCnt == 0) {
        DEBUG ((EFI_D_INFO, "I2cSet Target: Timeout during disable\n"));
        MmioWrite32 (Base + DW_IC_TAR, 0); // Clear Target Address
    ...
    DEBUG ((EFI_D_INFO, "I2cTarget Set - Chip Address:%x\n", Chip));
    I2cStatusCnt = DW_STATUS_WAIT_RETRY;
  ...
  )
{
  ...
  DEBUG ((EFI_D_INFO, "1 : Enable I2cInterface\n"));
  ...
  if (0 < Private->TransferCount) {
    while (0 < Private->WriteCount) {
      ...
      DEBUG ((EFI_D_INFO, "2 : Load up the TX FIFO, TxFifofDepth: %d\n", TxFifoCount));
      for (Index = 0; Index < TxFifoCount ; Index++) {
        if ((Private->TransferCount == 1) && (Private->ReadCount == 0)) {
          //DEBUG((EFI_D_INFO, "Write Data: 0x%x\n", *(Private->WriteData)));
      ...
      DEBUG ((EFI_D_INFO, "3 : Wait for TxFifo empty\n"));
  ...
  DEBUG ((EFI_D_INFO, "Write Count : %d, Transfer Count : %d\n", Private->WriteCount, Private->TransferCount));

  if (0 < Private->TransferCount) {
    DEBUG ((EFI_D_INFO, "4 : Start Read trasaction\n"));
    // Start READ trasaction if any
    ...
    while (0 < Private->ReadCount) {
      ...
      DEBUG ((EFI_D_INFO, "5 : fille up Rx Fifo, RxFifoDepth : %d\n", RxFifoCount));
      // Fill up Rx Fifo
      ...
      DEBUG ((EFI_D_INFO, "6 : Wait Rx data ready \n"));
      ...
      DEBUG ((EFI_D_INFO, "7 : Read data from Rxfifo\n"));
      for (Index = 0; Index < RxFifoCount; Index++) {
      ...
      DEBUG ((EFI_D_INFO, "Read Count : %d, Transfer Count : %d\n", Private->ReadCount, Private->TransferCount));
    }
  }

  // Transaction complete
  DEBUG ((EFI_D_INFO, "8 : transaction complete\n"));
  ...
  )
{
  ...
  // Disable the interface
  DEBUG ((EFI_D_INFO, "9 : disable I2cInterface\n"));
  MmioWrite32 (Base + DW_IC_ENABLE, 0);
  I2cDwWaitI2cEnable (Base, CHECK_IC_EN_HIGH);  //Wait controller status change
