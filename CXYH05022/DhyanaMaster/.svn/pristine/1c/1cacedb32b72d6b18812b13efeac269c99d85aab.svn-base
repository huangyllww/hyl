...
#define OemBootOptionTag(BaseTag,Priority) ((BaseTag)|((Priority)<<24))
#define OEM_UNASSIGNED_HIGHEST_TAG 0xFF

extern FIXED_BOOT_OPTION_TAG FixedLegacyBootOptionTags[];
#if BDS_VERSION >= 17
...
UINT32 FBO_FindUefiTagPriority(UINT16 Tag);
...
typedef enum{
  ...
  BoTagNetwork,
  BoTagOther
} OEM_BOOT_OPTION_DEVICE_TAG;

typedef struct _SATA_CONTROLLER
{
  UINT8   RbNum;
  UINT8   RbDevNum;
  UINT8   RbFunNum;
...
OEM_BOOT_OPTION_DEVICE_TAG BootDeviceTagPriorities[] = {
    BOOT_OPTION_DEVICE_TAG_PRIORITIES,
    UNASSIGNED_HIGHEST_TAG
};

SATA_CONTROLLER gSATAControllerList[] = {SATA_CONTROLLER_PRIORITIES };

#if BOOT_OPTION_DEVICE_TAG_NVME_M2_SUPPORT == 1
PCI_BRIDGE_LIST gM2RootPortList[] = {NVME_M2_SLOT_LIST };
#endif

//ASM1061 M.2 won't be used now
#if BOOT_OPTION_DEVICE_TAG_ASM1061_M2_SUPPORT == 1
PCI_BRIDGE_LIST gAsm1061RootPortList[] = {ARM1061_M2_SLOT_LIST };
#endif


UINT32 BootSataPortPrioritiesSataController[8][9] = {
    {BOOT_SATA_PORT_PRIORITIES_CONTROLLER0,UNASSIGNED_HIGHEST_TAG},
    {BOOT_SATA_PORT_PRIORITIES_CONTROLLER1,UNASSIGNED_HIGHEST_TAG},
    {BOOT_SATA_PORT_PRIORITIES_CONTROLLER2,UNASSIGNED_HIGHEST_TAG},
    {BOOT_SATA_PORT_PRIORITIES_CONTROLLER3,UNASSIGNED_HIGHEST_TAG},
    {BOOT_SATA_PORT_PRIORITIES_CONTROLLER4,UNASSIGNED_HIGHEST_TAG},
    {BOOT_SATA_PORT_PRIORITIES_CONTROLLER5,UNASSIGNED_HIGHEST_TAG},
    {BOOT_SATA_PORT_PRIORITIES_CONTROLLER6,UNASSIGNED_HIGHEST_TAG},
    {BOOT_SATA_PORT_PRIORITIES_CONTROLLER7,UNASSIGNED_HIGHEST_TAG}
};

BOOLEAN IsSpecificM2(
        PCI_BRIDGE_LIST *DevRootPortList,
        UINT8           Count,
        UINTN           Bus)
{
    UINT8       i;
    ...
    BusBase = FabricTopologyGetHostBridgeBusBase(0, 0, 0);
    BusLimit = FabricTopologyGetHostBridgeBusLimit(0, 0, 0);
    BusDelta = (UINT8)(BusLimit - BusBase + 1);
    
    for(i = 0; i < Count; i++, DevRootPortList++)
    {
        RbBusNum = DevRootPortList->RbNum * BusDelta;
        SecBus = PciRead8(PCI_LIB_ADDRESS(RbBusNum, DevRootPortList->RbDevNum, DevRootPortList->RbFunNum, 0x19));
        if ((UINT8)Bus == SecBus)
            return TRUE;
    }
    return FALSE;
...
{
  UINT8 i;
  UINT8 SataPortPriority;
  UINT8 ControllerPriority;
  UINT32 *ptr = NULL;
  UINT8  BusDelta;
  UINT8  Bus;
  UINT8  SecBus;
  ...
  Count = sizeof(gSATAControllerList) / sizeof(SATA_CONTROLLER);
  for(i=0;i<Count;i++)
  {
    Bus = gSATAControllerList[i].RbNum * BusDelta;
    SecBus = PciRead8(PCI_LIB_ADDRESS(Bus,gSATAControllerList[i].RbDevNum,gSATAControllerList[i].RbFunNum,0x19));
    if((BusNum == SecBus) && (DevNum == gSATAControllerList[i].Dev) && (FuncNum == gSATAControllerList[i].Func))
    {
        ptr = BootSataPortPrioritiesSataController[i];
        ControllerPriority = i;
        break;
    }
  }
  if (i == Count)
      return OEM_UNASSIGNED_HIGHEST_TAG;
  for(i=0; ptr[i] != UNASSIGNED_HIGHEST_TAG; i++)
  {
      if(PortNumber == ptr[i]){
        SataPortPriority = (UINT8)(8*ControllerPriority+i);
...
UINT8 FindDeviceTagPriority(UINT16 Tag){
    UINT8 i;
...
UINT16 FindNicPriority(UINT8 Seg,UINT8 Bus,UINT8 Dev,UINT8 Func)
{
  ...
  if(dfound){
    if(ptrdninfo->Speed)
      return (~NICSPEEDMAP[ptrdninfo->Speed]);//min low priority
  }

  return OEM_UNASSIGNED_HIGHEST_TAG;
}

EFI_STATUS GetOptionHighPriority(
        BOOT_OPTION *Option, 
        UINT16 *HighPriority, 
        UINT16 *DevTag,
        UINT8 *BusNum,
        ...
        UINT8 *FuncNum)
{
    ...
    UINT8                   Class;
    UINT8                   SubClass;
#if (BOOT_OPTION_DEVICE_TAG_ASM1061_M2_SUPPORT == 1) || (BOOT_OPTION_DEVICE_TAG_NVME_M2_SUPPORT == 1)
    UINT8                   Count;
#endif
    UINTN                   Seg;
    UINTN                   Bus = 0xFF;
  ...
  if (Option->BbsEntry != NULL)
  {
    ...
    DEBUG((DEBUG_INFO,"Class:%x,SubClass:%x ",Class,SubClass));
    switch(BbsTable->DeviceType){
        ...
        if(Class == PCI_CLASS_MASS_STORAGE){
          if(SubClass == PCI_CLASS_MASS_STORAGE_SATADPA){//AHCI
            *DevTag = BoTagSata;
            ...
            if (IsSpecificM2(gAsm1061RootPortList, Count, Bus))
            {
              *DevTag = BoTagASM1061M2;
            }
            #endif
          }else if(SubClass == PCI_CLASS_MASS_STORAGE_SOLID_STATE){//NVME
            *DevTag = BoTagNvme;
            #if BOOT_OPTION_DEVICE_TAG_NVME_M2_SUPPORT == 1
            Count = sizeof(gM2RootPortList) / sizeof(PCI_BRIDGE_LIST);
            if (IsSpecificM2(gM2RootPortList, Count, Bus))
            {
              *DevTag = BoTagNvmeM2;
            }
            #endif
      ...
      case BBS_EMBED_NETWORK:
        ...
        *DevTag = BoTagNetwork;
        *HighPriority = FindNicPriority(0,(UINT8)Bus,(UINT8)Device,(UINT8)Function);
        return EFI_SUCCESS;
#endif
        break;
        default:
  ...
  *HighPriority = (UINT16)FindDeviceTagPriority(*DevTag);
  *BusNum = (UINT8)Bus;
  *DevNum = (UINT8)Device;
  *FuncNum = (UINT8)Function;
  return EFI_SUCCESS;
}

EFI_STATUS GetOptionMidPriority(
        BOOT_OPTION *Option, 
        UINT8 *MidPriority, 
        UINT16 DevTag,
        UINT8 BusNum,
        ...
        UINT8 FuncNum)
{
  ...
  switch (DevTag) {
    ...
    case BoTagsSata:
      ...
      {
          DeviceHandle = Option->DeviceHandle;
      }
      Handle = GetPhysicalBlockIoHandle(DeviceHandle);
      PortNumber = (UINT8)GetHddPort(Handle);
      *MidPriority = FindSataPortPriority(BusNum,DevNum,FuncNum,PortNumber);
      break;
    case BoTagSAS:
    case BoTagNvme:
    case BoTagRAID:
    ...
    case BoTagASM1061M2:
      ...
      Handle = GetPhysicalBlockIoHandle(DeviceHandle);
      PortNumber = (UINT8)GetHddPort(Handle);
      *MidPriority = PortNumber;
      DEBUG((DEBUG_ERROR, "PortNumber %x \n",PortNumber));
      break;
#endif
    default:
      break;
...
UINT32 GetOemPriority(BOOT_OPTION *Option)
{
    EFI_STATUS          Status;
    UINT16              HighPriority = OEM_UNASSIGNED_HIGHEST_TAG;
    UINT8               MidPriority = OEM_UNASSIGNED_HIGHEST_TAG;
    UINT8               BusNum = 0xFF;
    UINT8               DevNum = 0xFF;
    UINT8               FuncNum = 0xFF;
    UINT16              DevTag = BoTagOther;
    UINT32              OemPriority = 0xFFFF00;
    
    Status = GetOptionHighPriority(Option, &HighPriority, &DevTag, &BusNum, &DevNum, &FuncNum);
    DEBUG((DEBUG_ERROR, "HighPriority %x DevTag %x BusNum %x DevNum %x FuncNum %x\n",HighPriority, DevTag, BusNum, DevNum, FuncNum));
    if (Status != EFI_SUCCESS) //Build In Shell
    {
        OemPriority = 0xFFFF00;
        return OemPriority;
    }
    if (DevTag == BoTagNetwork)
        OemPriority = HighPriority << 8;
    if (DevTag != BoTagOther) //Storage device need mid priority, network only need high priority
    {
        GetOptionMidPriority(Option, &MidPriority, DevTag, BusNum, DevNum, FuncNum);
        DEBUG((DEBUG_ERROR, "MidPriority %x \n",MidPriority));
        OemPriority = (MidPriority << 8) | (((UINT8)HighPriority) << 16);
    }
    DEBUG((DEBUG_ERROR, "OemPriority %x \n",OemPriority));
...
{
    UINT32              OemTagPriority1;
    UINT32              OemTagPriority2;

  if (((Option1->Priority & 0xFF) == 0) && ((Option2->Priority & 0xFF) == 0))
    return 0;

  OemTagPriority1 = GetOemPriority(Option1);
  OemTagPriority2 = GetOemPriority(Option2);
  DEBUG((DEBUG_INFO, "OemTagPriority1 %x OemTagPriority2 %x \n", OemTagPriority1, OemTagPriority2));

  if (OemTagPriority1 < OemTagPriority2) return -1;
  else if (OemTagPriority1 > OemTagPriority2) return 1;
...
INT32 OemCompareTagThenPriority(DLINK *Link1, DLINK *Link2){
    ...
    DEBUG((DEBUG_INFO, "Option1->Tag %x Option1->Priority %x Option1->GroupHeader %x\n", Option1->Tag, Option1->Priority, Option1->GroupHeader));
    DEBUG((DEBUG_INFO, "Option2->Tag %x Option2->Priority %x Option2->GroupHeader %x\n", Option2->Tag, Option2->Priority, Option2->GroupHeader));
...
)
{
    ...
    UINT32              UefiBootOptionsInc = 0;
    
    DUMP_BOOT_OPTION_LIST(BootOptionList,"Before OemSortBootOptionList");
    ...
    FOR_EACH_BOOT_OPTION(BootOptionList,Link,Option) {
        ...
        if (IsLegacyBootOption(Option)) {
            ...
            if (TagPriority == UNASSIGNED_HIGHEST_TAG)
                TagPriority = FBO_FindTagPriority(BoTagLegacyOther);
        } else {
#endif
#if BDS_VERSION >= 17
            if (IsShellDevicePath(Option->FilePathList)) BaseTag = BoTagUefiOther;
#else
            if (IsShellBootOption(Option)) BaseTag = BoTagUefiOther;
#endif
            else BaseTag = GetUefiBootOptionTag(Option);
            
            TagPriority = FBO_FindUefiTagPriority((UINT16)BaseTag);
#if CSM_SUPPORT
        }
#endif
        DEBUG((DEBUG_INFO, "BaseTag %x\n", BaseTag ));
        DEBUG((DEBUG_INFO, "TagPriority %x\n", TagPriority ));

        if (Option->GroupHeader){
            Option->Tag = ((Option->Tag & 0xFFFF0000) << 8) + (Option->Tag & 0xFFFF);
        }
        else
        {
        Option->Tag = BaseTag;
        BaseTag |= GetOemPriority(Option);
        ...
        Option->Tag = OemBootOptionTag(BaseTag, TagPriority);
        }
        DEBUG((DEBUG_INFO, "Option->Tag %x\n", Option->Tag ));
    }
    
    DUMP_BOOT_OPTION_LIST(BootOptionList,"After OemSortBootOptionList");
...
UINT32 FBO_FindUefiTagPriority(UINT16 Tag)
{
    ...
    for (i = 0; i < DeviceMapCount; i++)
        if (Tag == pFBOUefiDevMap[i].DevType) return i;
...
UINT16 FBO_GetBbsEntryDeviceType(BBS_TABLE *BbsEntry)
{
    ...
    if (DeviceType == BBS_HARDDISK)
    {
        ...
        {
        //<Hujf1001-20220531 Override SD Group to Other if SD_GROUP_SUPPORT is disable +>
        #if SD_GROUP_SUPPORT
            FboLegacyType = BoTagLegacySd;
        #else
            FboLegacyType = BoTagLegacyOther;
        #endif
        //<Hujf1001-20220531 Override SD Group to Other if SD_GROUP_SUPPORT is disable ->
        }
...
)
{
    ...
    UINT16 OptionIndex;
    BOOT_OPTION **OptionList = NULL; //AR-BIOS-0002-0011 Boot order function support
    ...
    OptionList = MallocZ((BootOptionList->Size + 1) * sizeof(BOOT_OPTION *)); //AR-BIOS-0002-0011 Boot order function support
    ...
    for (i = 0; i < DeviceMapCount; i++)    // scan throgh all types in pFBOUefiDevMap
    {
            ...
            for (k = 0; k < DEVORDER_COUNT(OldDevOrder2); k++)
            {
                ...
                FOR_EACH_BOOT_OPTION(BootOptionList, Link, Option)
                {
                    ...
                    if (Option->BootOptionNumber == (OldDevOrder2->Device[k] & FBO_UEFI_ORDER_MASK)) // highest byte is for disabled flag
                    {
                        ...
                        OptionList[j] = Option; //AR-BIOS-0002-0011 Boot order function support
        ...
        FOR_EACH_BOOT_OPTION(BootOptionList, Link, Option)
        {
            ...
            {
                //AR-BIOS-0002-0011 Boot order function support +
                UINTN DevIndex, reIndex;
                DevOrder->Length += sizeof(DevOrder->Device);
                //DevOrder->Device[j] = Option->BootOptionNumber;
                for (DevIndex = 0; DevIndex < j; DevIndex++)
                {
                    if(OptionList[DevIndex]->Tag > Option->Tag)
                    {
                        for(reIndex = j; reIndex > DevIndex; reIndex--)
                        {
                            DevOrder->Device[reIndex] = DevOrder->Device[reIndex-1];
                            OptionList[reIndex] = OptionList[reIndex-1];
                        }
                        DevOrder->Device[DevIndex] = Option->BootOptionNumber;
                        OptionList[DevIndex] = Option;
                        DEBUG((FBO_TRACE_LEVEL | DEBUG_VERBOSE, "INSERT DevOrder->Device[%d] = %x TAG %x\n", DevIndex, DevOrder->Device[DevIndex], Option->Tag));
                        break;
                    }
                }
                if (DevIndex == j)
                {
                    DevOrder->Device[j] = Option->BootOptionNumber;
                    OptionList[j] = Option;
                    DEBUG((FBO_TRACE_LEVEL | DEBUG_VERBOSE, "ADD DevOrder->Device[%d] = %x TAG %x  %x\n", DevIndex, DevOrder->Device[DevIndex], OptionList[j]->Tag, Option->Tag));
                }
                //AR-BIOS-0002-0011 Boot order function support -
                BootOrderFlag[OptionIndex] = -1;    // boot order assigned
                j++;
...
//<<(EIP99368+)
    ...
    OemSortBootOptionList();  //AR-BIOS-0002-0011 Boot order function support
    BuildUEFIDevOrder();
    ...
    FBO_SetBootOptionTags(); //AR-BIOS-0002-0011 Boot order function support
