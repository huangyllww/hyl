...
#include "minisetup.h"
#include "ServerAutoId.h"//<zhaoyf1-20180907 remove BMC user password Complexity Policy in Server Mgmt -> BMC User Settings >
...
AMI_IFR_MSGBOX _gInvalidComplexityFailMsgBox     = { 0, 0, STRING_TOKEN(STR_ERROR), INVALID_HANDLE, STRING_TOKEN(STR_ERROR_COMPLEXITY_INPUT) };
AMI_IFR_MSGBOX _gInvalidRepeatFailMsgBox     = { 0, 0, STRING_TOKEN(STR_ERROR), INVALID_HANDLE, STRING_TOKEN(STR_ERROR_REPEAT_INPUT) };
//zanggy-20201217-Fix user can set complex password in simple password mode+>
AMI_IFR_MSGBOX _gInvalidComplexModeSimpleInputFailMsgBox     = { 0, 0, STRING_TOKEN(STR_ERROR), INVALID_HANDLE, STRING_TOKEN(STR_ERROR_COMPLEX_MODE_SIMPLE_INPUT) };
AMI_IFR_MSGBOX _gInvalidSimpleModeComplexInputFailMsgBox     = { 0, 0, STRING_TOKEN(STR_ERROR), INVALID_HANDLE, STRING_TOKEN(STR_ERROR_SIMPLE_MODE_COMPLEX_INPUT) };
AMI_IFR_MSGBOX _gInvalidSimpleModeSimpleInputFailMsgBox      = { 0, 0, STRING_TOKEN(STR_ERROR), INVALID_HANDLE, STRING_TOKEN(STR_ERROR_SIMPLE_MODE_SIMPLE_INPUT) };

UINTN gInvalidComplexModeSimpleInputFailMsgBox    =   (UINTN)&_gInvalidComplexModeSimpleInputFailMsgBox;
UINTN gInvalidSimpleModeComplexInputFailMsgBox    =   (UINTN)&_gInvalidSimpleModeComplexInputFailMsgBox;
UINTN gInvalidSimpleModeSimpleInputFailMsgBox     =   (UINTN)&_gInvalidSimpleModeSimpleInputFailMsgBox;
//zanggy-20201217-Fix user can set complex password in simple password mode->
UINTN gInvalidComplexityFailMsgBox    =   (UINTN)&_gInvalidComplexityFailMsgBox;
UINTN gInvalidRepeatFailMsgBox    =   (UINTN)&_gInvalidRepeatFailMsgBox;
extern BOOLEAN CheckPasswordComplexityLocal(CHAR16*Password,UINTN PwdLength);
extern BOOLEAN CheckPasswordRepeatLocal(CHAR16*Password,UINTN PwdLength);
//<xuyj1-20180829-add password complex and simple select+>
GUID    gEfiSetupGuid  = SETUP_GUID;
AMI_IFR_MSGBOX _gInvalidSimpleFailMsgBox     = { 0, 0, STRING_TOKEN(STR_ERROR), INVALID_HANDLE, STRING_TOKEN(STR_ERROR_SIMPLE_INPUT) };
UINTN gInvalidSimpleFailMsgBox    =   (UINTN)&_gInvalidSimpleFailMsgBox;
//<xuyj1-20180829-add password complex and simple select->
//<Wangbn003-20191030 Confirm simple password+>
#if defined(OEM_SETUP_PASSWORD_COMPLEXITY_MODE_SUPPORT)&& (OEM_SETUP_PASSWORD_COMPLEXITY_MODE_SUPPORT == 1)
AMI_IFR_MSGBOX _gConfirmSimplePswMsgBox     = { 0, 0, STRING_TOKEN(STR_WARNING), INVALID_HANDLE, STRING_TOKEN(STR_OEM_CONFIRM_SIMPLE_PASSWORD) };
UINTN gConfirmSimplePswMsgBox    =   (UINTN)&_gConfirmSimplePswMsgBox;

UINT8 GetPasswordComplexityMode();
#endif
#endif

//<Wangbn001-20191029 BIOS password policy, password complexity function, password repeat function->
//<Wangbn001-20191029 BIOS password policy, Screen lock function+>
...
extern CHAR16 gTmpAdminPlaintextPsw[PASSWORD_MAX_SIZE+1];
extern CHAR16 gTmpUserPlaintextPsw[PASSWORD_MAX_SIZE+1];
#endif
#endif
//<Wangbn001-20191102-password four Consecutive Chars check->
//<lvych006-20160310 Administrator and User Password Policy ->
...
EFI_STATUS HandleESCOnInteractivePassword(POPUP_PASSWORD_DATA *popuppassword,CHAR16 *Text )
{
	...
	UINTN OldPwLen = 0;
	 ...
	 EFI_STATUS Status = EFI_SUCCESS;
   //zanggy-20201217-Fix Hdd master password is clear when press ESC in "set password dialog"+>
    EFI_GUID AmitseEscOnPasswordPromptGuid = AMITSE_ESC_ON_PWD_PROMPT_GUID; 
    //zanggy-20201217-Fix Hdd master password is clear when press ESC in "set password dialog"->
	if((Text != NULL) ){
        ...
        //zanggy-20201217-Fix Hdd master password is clear when press ESC in "set password dialog"+>
        EfiLibNamedEventSignal (&AmitseEscOnPasswordPromptGuid);
        //zanggy-20201217-Fix Hdd master password is clear when press ESC in "set password dialog"->
...
EFI_STATUS _PopupPasswordActivate(POPUP_PASSWORD_DATA *popuppassword)
{
	...
	UINT16 						Key = UefiGetControlKey(&(popuppassword->ControlData));
	//<xuyj1-20180829-add password complex and simple select+>
	SETUP_DATA     				*gSetupData = 0;
	UINTN          				DataSize = sizeof(SETUP_DATA);
	EFI_GUID 					gTseTransferGuid = TSE_TRANSFER_GUID;
	Tse_Setup_transfer 			*gTseSetuptransferInterface = NULL;
	//<xuyj1-20180829-add password complex and simple select->
	UINT32                      PasswordMin = 0;      //zhouyj1-20220128 Password mini Length is 3 with simple password,and 8 with complex password +
...
Password:
		...
		if(!Status) //If status is success
		{
			...
			//<zhouyj1-20220128 Password mini Length is 3 with simple password,and 8 with complex password. +>          
			            ...
			            if ( VARIABLE_ID_AMITSESETUP == popuppassword->ControlData.ControlVariable ) {
			                UINT8 PasswordMode = GetPasswordComplexityMode();
			                if ( PasswordMode == 0 )
			                    PasswordMin = 3;
			                else PasswordMin = 8;
			                if(
			                    ( (NewPwLen < (UINT8)PasswordMin) ||
...
//<Wangbn001-20191029 BIOS password policy, password complexity function, password repeat function+>
#if defined(OEM_SETUP_PASSWORD_COMPLEXITY_MODE_SUPPORT)&& (OEM_SETUP_PASSWORD_COMPLEXITY_MODE_SUPPORT == 1)
    if(!Status){
        if(Key!=ADD_BMC_USER_PASSWORD_KEY&&Key!=CHANGE_USER_SETTINGS_BMC_USER_PASSWORD_KEY){//<zhaoyf1-20180907 remove BMC user password Complexity Policy in Server Mgmt -> BMC User Settings >
            UINT8   PasswordComplexityMode = PASSWORD_SENSITIVE_LEVEL;
            BOOLEAN PswComplexityStatus = CheckPasswordComplexityLocal(NewPswd,NewPwLen);
      //zanggy-20201217-Fix user can set complex password in simple password mode+>
			//<libj-20201119 fix password complexity function+>
            PasswordComplexityMode = GetPasswordComplexityMode();
			
                    if(PasswordComplexityMode && !PswComplexityStatus){
                        //Complex Mode, Simple Input
                        CallbackShowMessageBox( (UINTN)gInvalidComplexModeSimpleInputFailMsgBox, MSGBOX_TYPE_OK );
                        Status = EFI_UNSUPPORTED;   
                    }
                    //zanggy-20210628-Change Password Complexity Check Rule Following new rule+>
                    /*
                    else if (PasswordComplexityMode == 0 && PswComplexityStatus && NewPwLen != 0) {
                        //Check NewPwLen, because when set Hdd master password, user can input null string
                        //Simple Mode, Complex Input
                        CallbackShowMessageBox( (UINTN)gInvalidSimpleModeComplexInputFailMsgBox, MSGBOX_TYPE_OK );
                        Status = EFI_UNSUPPORTED;   
                    }	                    	                
                        //Simple Mode, Simple Input
                    else if ((PasswordComplexityMode == 0 && !PswComplexityStatus)
                        && (CallbackShowMessageBox( (UINTN)gInvalidSimpleModeSimpleInputFailMsgBox, MSGBOX_TYPE_YESNO ) != MSGBOX_YES)
                    )
                    {
                        Status = EFI_UNSUPPORTED;
                    }
                                        
                    else if(CheckPasswordRepeatLocal(NewPswd,NewPwLen)){ 
                        CallbackShowMessageBox( (UINTN)gInvalidRepeatFailMsgBox, MSGBOX_TYPE_OK );
                        Status = EFI_UNSUPPORTED;
                    }
                    */
                    if (!Status) {
                        if(CheckPasswordRepeatLocal(NewPswd,NewPwLen)){ 
                            CallbackShowMessageBox( (UINTN)gInvalidRepeatFailMsgBox, MSGBOX_TYPE_OK );
                            Status = EFI_UNSUPPORTED;
                           }
                    }
    ...
    #if defined(OEM_PSW_CONSECUTIVE_CHARS_CHECK_WITH_PREVIOUS) && (OEM_PSW_CONSECUTIVE_CHARS_CHECK_WITH_PREVIOUS)
                        if( !EFI_ERROR(Status)
                           &&  (VARIABLE_ID_AMITSESETUP == popuppassword->ControlData.ControlVariable)
                           && Oem_CheckNewTsePasswordWithPrevious(UefiGetQuestionOffset(popuppassword->ControlData.ControlPtr) == STRUCT_OFFSET(AMITSESETUP, AdminPassword)?AMI_PASSWORD_ADMIN:AMI_PASSWORD_USER, NewPswd, NewPwLen)
                          )
                        {
                            AMI_IFR_MSGBOX mPswConseutiveCharsSameMsgBox     = { 0, 0, STRING_TOKEN(STR_ERROR), INVALID_HANDLE, STRING_TOKEN(STR_OEM_PSW_CONSECUTIVE_CHARS_SAME_WITH_PREVIOUS) };
                            //DEBUG((-1, "CP 8000, new password has 4 chars same with previous password\r\n"));
                            CallbackShowMessageBox( (UINTN)&mPswConseutiveCharsSameMsgBox, MSGBOX_TYPE_OK );
                            Status = EFI_UNSUPPORTED;
                        }
    #endif
    //<Wangbn001-20191102-password four Conseutive Chars check->      
    ...
    if(!Status){
        if(Key!=ADD_BMC_USER_PASSWORD_KEY&&Key!=CHANGE_USER_SETTINGS_BMC_USER_PASSWORD_KEY){//<zhaoyf1-20180907 remove BMC user password Complexity Policy in Server Mgmt -> BMC User Settings >
            ...
            gTseSetuptransferInterface->VarGetBuffer(L"Setup",gEfiSetupGuid,(UINT8**)&gSetupData,&DataSize);
            if ((gSetupData != NULL) && (gSetupData->Pawdtype == 0)) {
                if (CheckPasswordComplexityLocal(NewPswd,NewPwLen)) {
                    CallbackShowMessageBox( (UINTN)gInvalidSimpleFailMsgBox, MSGBOX_TYPE_OK );
                    Status = EFI_UNSUPPORTED; 
                } else if (CheckPasswordRepeatLocal(NewPswd,NewPwLen)) { 
                    CallbackShowMessageBox( (UINTN)gInvalidRepeatFailMsgBox, MSGBOX_TYPE_OK );
                    Status = EFI_UNSUPPORTED;
                }
            } else if ((gSetupData != NULL) && (gSetupData->Pawdtype == 1)) {
                if (!CheckPasswordComplexityLocal(NewPswd,NewPwLen)) {
                    CallbackShowMessageBox( (UINTN)gInvalidComplexityFailMsgBox, MSGBOX_TYPE_OK );
                    Status = EFI_UNSUPPORTED; 
                } else if (CheckPasswordRepeatLocal(NewPswd,NewPwLen)) { 
                    CallbackShowMessageBox( (UINTN)gInvalidRepeatFailMsgBox, MSGBOX_TYPE_OK );
                    Status = EFI_UNSUPPORTED;
...
//<Wangbn001-20191102-password four Consecutive Chars check+>
                    ...
                    if(VARIABLE_ID_AMITSESETUP == popuppassword->ControlData.ControlVariable)
                    {
                        EfiStrCpy(UefiGetQuestionOffset(popuppassword->ControlData.ControlPtr) == STRUCT_OFFSET(AMITSESETUP, AdminPassword)?gTmpAdminPlaintextPsw:gTmpUserPlaintextPsw, NewPswd);
                    }
#endif
#endif
//<Wangbn001-20191102-password four Conseutive Chars check->
