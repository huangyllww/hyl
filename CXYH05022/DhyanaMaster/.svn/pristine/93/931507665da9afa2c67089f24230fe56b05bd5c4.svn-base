...
//    BoTagXxx
    ...
    BoTagUefi,
    BoTagEmbeddedShell,
//<Wangbn001-20200205-Support adjust UEFI boot option order+>
    BoTagUefiHardDisk,
    BoTagUefiCdrom,
...
//<Yanshp-20220610 Fix function conflict when enable FixedBootOrder. +>
#if (FIXED_BOOT_ORDER_SUPPORT == 0)
//<Wangbn001-20200205-Support adjust UEFI boot option order+>
...
)
{
    ...
    EFI_DEVICE_PATH_PROTOCOL *DpPtr;
    BOOLEAN IsUSB = FALSE;
    
    if( Option == NULL || Option->FilePathList == NULL)
    {
	    return UNASSIGNED_HIGHEST_TAG;
    }
	Dp = Option->FilePathList;

    for(DpPtr = Dp ; !(isEndNode(DpPtr)); DpPtr = NEXT_NODE(DpPtr))
    {
        if(DpPtr->Type == MESSAGING_DEVICE_PATH)
        {
			...
			else if(DpPtr->SubType == MSG_MAC_ADDR_DP)
            {
			    BOOLEAN IsIP4 = FALSE, IsIP6 = FALSE;
                DpPtr=NEXT_NODE(DpPtr);
                for(; !(isEndNode(DpPtr)); DpPtr = NEXT_NODE(DpPtr))
                {
                    if(DpPtr->Type == MESSAGING_DEVICE_PATH)
                    {
                        if(DpPtr->SubType == MSG_IPv4_DP)
                        {
                            IsIP4 = TRUE;
                        }
                        else if(DpPtr->SubType == MSG_IPv6_DP)
                        {
                            IsIP6 = TRUE;
                        }
                        else if(DpPtr->SubType == MSG_URI_DP)
                        {
                            if(IsIP4)
                            {
                                return BoTagUefiIP4UriNetWork;
                            }
                            else if(IsIP6)
                            {
                                return BoTagUefiIP6UriNetWork;
                            }
                            return BoTagUefiUriNetWork;
                        }
                    }
                }
                if(IsIP4)
                {
                    return BoTagUefiIP4NetWork;
                }
                else if(IsIP6)
                {
                    return BoTagUefiIP6NetWork;
                }
...
//liutz-20200714-Add Token To Control The Slotion+>
#if defined(SEPARATED_UEFI_DEV) && SEPARATED_UEFI_DEV==1 && (!defined(FIXED_BOOT_ORDER_SUPPORT) || (FIXED_BOOT_ORDER_SUPPORT == 0))
//<Wangbn001-20200205-Support adjust UEFI boot option order+>
//        
