    ...
    IN EFI_PEI_SERVICES    **PeiServices )
{
    ...
    // Rome SOC porting START
    // Assuming PEI_XHCI_CONTROLLER_PCI_ADDRESS list is unchanged
    // {0x1, 0x0, 0x3},{0x13, 0x0, 0x3},{0x41, 0x0, 0x3},{0x53, 0x0, 0x3}
    if(gXhciControllerCount == 4){
        UINT8	RootBus;
        UINT32  RootBridgeCount;

        (*PeiServices)->PciCfg->Read(PeiServices,(*PeiServices)->PciCfg,
                                    EfiPeiPciCfgWidthUint32, EFI_PEI_PCI_CFG_ADDRESS(0, 0x18, 1, 0x204),&RootBridgeCount);
        //
        //Refer SYS_COMP_COUNT_REGISTER definition in FabricRegistersDf3.h.
        //
        RootBridgeCount = (RootBridgeCount & 0xFF0000) >> 16;
        (*PeiServices)->PciCfg->Read(PeiServices,(*PeiServices)->PciCfg,
            EfiPeiPciCfgWidthUint8, EFI_PEI_PCI_CFG_ADDRESS(0, 0x18, 0, 0xA6),&RootBus);
        
        if(gXhciControllerPciTable[1].Bus != (RootBus + 5)){
            gXhciControllerPciTable[1].Bus = (RootBus + 5);
            if(RootBridgeCount > 4){
                (*PeiServices)->PciCfg->Read(PeiServices,(*PeiServices)->PciCfg,
                            EfiPeiPciCfgWidthUint8, EFI_PEI_PCI_CFG_ADDRESS(0, 0x18, 0, 0xB2),&RootBus);
                gXhciControllerPciTable[2].Bus = (RootBus + 3);
                (*PeiServices)->PciCfg->Read(PeiServices,(*PeiServices)->PciCfg,
                                            EfiPeiPciCfgWidthUint8, EFI_PEI_PCI_CFG_ADDRESS(0, 0x18, 0, 0xB6),&RootBus);
                gXhciControllerPciTable[3].Bus = (RootBus + 5);
            }else{
                // Assign last bus, so no device there, to skip enumeration 
                gXhciControllerPciTable[2].Bus = 0xFF;
                gXhciControllerPciTable[3].Bus = 0xFF;
            }
        }
    }
    // Rome SOC porting END
