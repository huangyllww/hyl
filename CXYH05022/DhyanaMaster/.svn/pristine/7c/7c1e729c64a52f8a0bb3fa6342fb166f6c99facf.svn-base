  ...
  )
{
  ...
  //WorkAround for HYGON_DX stepping 0 Chips
  if ((LogicalId.Family == HYGON_FAMILY_18) && (LogicalId.Revision & HYGON_REV_F18_DN_D0)){
  ...
  )
{
  ...
  FchSmnRW (DieBusNum, FCH_TS_USB_SMU_ASSIST_POWER_STATE_CTL, 0x00, 0x00, LocalCfgPtr->StdHeader);
...
// c.   To enable SMI/NMI (0x16D8_0118)
  ...
  //Workaround for HYGON_DX Stepping-0 chips
  if ((LogicalId.Family == HYGON_FAMILY_18) && (LogicalId.Revision & HYGON_REV_F18_DN_D0)){
  //d.  If need SMU FW to handle D3hot entry/D3hot exit flow:      
    FchSmnRW (DieBusNum, RCC_BIF_STRAP1_ADDRESS, 0xFFFFFFEF, 0x10, LocalCfgPtr->StdHeader);
    FchSmnRW (DieBusNum, NBIF_SMN_VWR_VCHG_DIS_CTRL_1, 0xFFFFFFEF, 0x10, LocalCfgPtr->StdHeader);
    FchSmnRW (DieBusNum, FCH_TS_USB_SMU_ASSIST_POWER_STATE_CTL, 0xFFFFFEFF, 0x100, LocalCfgPtr->StdHeader); 
  }
  ...
  )
{
  ...
  // Re-init XHC of slave die 
  if(0 != DieBusNum){
    FchTSXhciInitBootProgram(DieBusNum, FchDataPtr);  
  }
 
  //Reset die0 USB PHY
  if(0 == DieBusNum){ 
    FchSmnRW (DieBusNum, FCH_TS_USB_RESET, 0xFFFFFFFF, BIT2, LocalCfgPtr->StdHeader);
    FchStall (200, LocalCfgPtr->StdHeader);
    FchSmnRW (DieBusNum, FCH_TS_USB_RESET, 0xFFFFFFFB, 0x0, LocalCfgPtr->StdHeader);
  } 
  ...
  //WorkAround for HYGON_DX stepping-0 chips
  if ((LogicalId.Family == HYGON_FAMILY_18) && (LogicalId.Revision & HYGON_REV_F18_DN_D0)){
    //Set SHADOWED_POWERSTATE as 0 to close SMU
    FchSmnRW (DieBusNum, FCH_TS_USB_SMU_ASSIST_POWER_STATE_CTL, 0, 0x0, LocalCfgPtr->StdHeader);
  }
  ...
  //WorkAround for HYGON_DX stepping-0 chips
  if ((LogicalId.Family == HYGON_FAMILY_18) && (LogicalId.Revision & HYGON_REV_F18_DN_D0)){
    //Set SHADOWED_POWERSTATE as 1 for SMU work
    FchSmnRW (DieBusNum, FCH_TS_USB_SMU_ASSIST_POWER_STATE_CTL, 0xFFFFFEFF, 0x100, LocalCfgPtr->StdHeader);
  }
