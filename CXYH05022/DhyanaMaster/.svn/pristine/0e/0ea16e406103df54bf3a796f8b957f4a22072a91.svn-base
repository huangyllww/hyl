...
#include <Library/PcdLib.h> //<Chendl001-20181103 Optimized Memory Leaky Bucket_2>
  ...
  )
{
  ...
  //PciExpressWrite32 ((UINTN) pciAddress, smnIndex);

  pciAddress = (BusNumberBase << 20) + NB_SMN_DATA_2;
  value = PciRead32 (pciAddress);
  //value = PciExpressRead32 ((UINTN) pciAddress);
  return value;
}
  ...
  )
{
  ...
  normalizedAddress.normalizedSocketId = (csFabricId & socketIdMask) >> socketIdShift;
  normalizedAddress.normalizedDieId = (csFabricId & dieIdMask) >> dieIdShift;
  normalizedAddress.normalizedChannelId = csFabricId & ~(socketIdMask | dieIdMask);
