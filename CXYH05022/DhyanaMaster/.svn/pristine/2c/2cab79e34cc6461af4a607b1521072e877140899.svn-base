 ...
 * @file
 *
 * AmdSmuServicesV9Ppi Implementation
 *
 *
...
#define FILECODE        NBIO_SMU_AMDNBIOSMUV9PEI_AMDSMUSERVICESV9PPI_FILECODE
  ...
  )
{
  ...
  if (Status == EFI_SUCCESS) {
    ...
    SmuResult = NbioSmuServiceRequestV9 (NbioGetHostPciAddress (GnbHandle), ServiceRequest, SmuArg, 0);
    if (SmuResult == 1) {
  ...
  )
{
  ...
  if (Status == EFI_SUCCESS) {
    ...
    LibAmdMemCopy (SmuArg, InputParameters, 24, (AMD_CONFIG_PARAMS *) NULL);
    SmuResult = NbioDxioServiceRequestV9 (GnbHandle, ServiceRequest, SmuArg, 0);
    if (SmuResult == 1) {
      LibAmdMemCopy (ReturnValues, SmuArg, 24, (AMD_CONFIG_PARAMS *) NULL);
    } else {
      Status = EFI_INVALID_PARAMETER;
  ...
  )
{
  ...
  if (Status == EFI_SUCCESS) {
    ...
    while (GnbHandle != NULL) {
      ...
      NbioSmuServiceCommonInitArgumentsV9 (SmuArg);
      SmuArg[0] = 0xAA55AA55;
      Status = NbioSmuServiceRequestV9 (NbioGetHostPciAddress (GnbHandle), SMC_MSG_TestMessage, SmuArg, 0);
      GnbHandle = GnbGetNextHandle (GnbHandle);
    }
  }
