  ...
  )
{
  ...
  //for Boundary Scan
  FchSmnRW (0, 0x16D8014C, ~ (UINT32) BIT31, BIT31, LocalCfgPtr->StdHeader);

  if (LocalCfgPtr->FchReset.Xhci0Enable == FALSE) {
    FchSmnRW (0, FCH_TS_USB_NBIF_STRAP0, ~(UINT32) BIT28, 0, LocalCfgPtr->StdHeader);
    return;
  }

  if ((ReadFchSleepType (LocalCfgPtr->StdHeader) == ACPI_SLPTYP_S3) || (AmdCapsuleGetStatus ()) ) {
    FchTSXhciInitS3ExitProgram (0, FchDataPtr);
  } else {
    FchTSXhciInitBootProgram (0, FchDataPtr);
  }
}
