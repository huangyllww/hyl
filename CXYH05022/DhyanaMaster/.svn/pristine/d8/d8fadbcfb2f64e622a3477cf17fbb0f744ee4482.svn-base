...
UINTN                        gHddSecCommunicateBufferSize;
...
)
{
    ...
    gHddSecCommunicateBufferSize  = (OFFSET_OF (EFI_SMM_COMMUNICATE_HEADER, Data)) + DataSize; // Header size (without data) + data size

    pBS->SetMem( gHddSecCommunicateBuffer, gHddSecCommunicateBufferSize, 0 );
    ...
    Status = gSmmCommunication->Communicate (gSmmCommunication, gHddSecCommunicateBuffer, &gHddSecCommunicateBufferSize);
    if (EFI_ERROR(Status)) {
        ASSERT_EFI_ERROR(Status);
        return Status;  
    }
...
)
{
    ...
    // Calculate Size of Communication buffer
    gHddSecCommunicateBufferSize  = SMM_COMMUNICATE_HEADER_SIZE + sizeof (UINT8 *);
...
)
{
    ...
    gHddSecCommunicateBufferSize  = SMM_COMMUNICATE_HEADER_SIZE + HDD_PASSWORD_SIZE + sizeof (UINT8 *);
#else		    
    gHddSecCommunicateBufferSize  = SMM_COMMUNICATE_HEADER_SIZE + HDD_PASSWORD_SIZE + sizeof (UINT8 *) + \
                                    sizeof(BOOT_SCRIPT_NVME_SECURITY_SAVE_ENTRY)*(MAX_NVME_DEVICES) + (2 * sizeof(UINT32));
#endif
...
)
{
    ...
    gHddSecCommunicateBufferSize  = SMM_COMMUNICATE_HEADER_SIZE + HDD_PASSWORD_SIZE;
