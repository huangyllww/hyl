...
#include "AMITSEOem.h"
UINT8 PasswordEventFlag = 0; //<bieyb-20190112  Fix watchdog will be reopened after back from lock screen +>
//<Kangmm001-20221015-SGPZT#7413 Add warnning message when enter default password +>
AMI_IFR_MSGBOX _gConfirmDefaultPswMsgBox     = { 0, 0, STRING_TOKEN(STR_WARNING), INVALID_HANDLE, STRING_TOKEN(STR_PASSWORD_DEFAULT_WARNNING) };
//<Kangmm001-20221015-SGPZT#7413 Add warnning message when enter default password ->
...
//<Kangmm001-20221015-SGPZT#7413 Add warnning message when enter default password +>
	CHAR16 DefaultPassword[6] = L"admin";
	UINTN DefaultPasswordCheck = 0;
	UINTN gConfirmDefaultPswMsgBox    =   (UINTN)&_gConfirmDefaultPswMsgBox;
//<Kangmm001-20221015-SGPZT#7413 Add warnning message when enter default password ->
	...
	if (BoxLength < 40)
		BoxLength = 40;
	//<Wangbn001-20191029 BIOS password policy, password windows freeze function->
	//<bieyb-20190112  Fix watchdog will be reopened after back from lock screen+>
	if (!PasswordEventFlag)
		//<bieyb-20190112  Fix watchdog will be reopened after back from lock screen->
	...
	// GopStatus is not important enough to break pwd window support
	GopStatus = SaveCurrentTextGOP(&currenttextModeCols, &currenttextModeRows, &currentGOPMode);
	...
	//Get PLockTime which is saved by last boot. If reboot occur when system locked, lock time will be set as PLockTime * 2
	GetPasswordLockTime(&LockTime);
	if (LockTime == 0) //PLockTime not set or cleared
	{
		//SGPZT23195#PasswdPolicy(Timeout-Count)+>
		//LockTime = PASSWORD_LOCK_TIME;
		GetFixedPasswordLockTime(&LockTime);
		//SGPZT23195#PasswdPolicy(Timeout-Count)->
	}
	else
	{
		if (LockTime * 2 < 0xFFFF)
			LockTime = LockTime * 2;
	}

	if (CheckPasswordLock(&LeftLockTime))
	{
		*NoOfRetries = 1;
		ApplyOemPasswordPolicy(TotalRetries, NoOfRetries, LeftLockTime, BoxLength, CurrXPos, CurrYPos);
	}
	//<Wangbn001-20191029 BIOS password policy, password windows freeze function->
	...
	while (*NoOfRetries)
	{
		...
		//<Wangbn001-20191029 BIOS password policy, password windows freeze function+>
		Status = _GetPassword(PasswordEntered, TsePasswordLength, CurrXPos + (BoxLength - TsePasswordLength) / 2, CurrYPos, TimeOut);
		//<Wangbn001-20191029 BIOS password policy, password windows freeze function->
...
//<Kangmm001-20221015-SGPZT#7413 Add warnning message when enter default password +>
		...
		{
	        DefaultPasswordCheck = EfiCompareMem( PasswordEntered, DefaultPassword, sizeof(DefaultPassword) ) ;
	        if(DefaultPasswordCheck == 0){
	            CallbackShowMessageBox((UINTN)gConfirmDefaultPswMsgBox,MSGBOX_TYPE_OK);
	        }
			break;
		}
//<Kangmm001-20221015-SGPZT#7413 Add warnning message when enter default password ->
		...
		{
			ApplyOemPasswordPolicy(TotalRetries, NoOfRetries, LockTime, BoxLength, CurrXPos, CurrYPos);
			if (1 == *NoOfRetries)
			{
				if (LockTime * 2 < 0xFFFF)
					LockTime = LockTime * 2;
