...
//<ChengXW-20200923 Update Onbrd/Ext VGA function +>
EFI_STATUS  GetPcieDevListBuffer(
        IN  PCI_DEV_SETUP_FORM_DATA **PcieData
);
//<ChengXW-20200923 Update Onbrd/Ext VGA function ->

//<huangrj-20220627 exclude devices unable to be retrained +>
PCI_RETRAIN_ERROR_DEV PciRetrainErrorDevList[] = {
//-----------------------------------------------------------------------------------------------------------------------
//   UINT16      UINT16      UINT16      
//   VendorId;   DeviceId;   SubsystemId;  
//-----------------------------------------------------------------------------------------------------------------------
        //NV A2 GPU
        {0x10DE, 0x25B6, 0x157E }
};

UINTN PciRetrainErrorDevCount=sizeof(PciRetrainErrorDevList)/sizeof(PCI_RETRAIN_ERROR_DEV);
//<huangrj-20220627 exclude devices unable to be retrained ->
...
//<Kangmm001-20200619 Fix mantis 0039408 X710 link width change from x8 to x4 during reboot +>
void CheckLinkWidth(PCI_DEV_INFO *dev)
{
    UINT8                         CapIdOffset;
    UINT16                        Data16 = 0; 
    PCI_DEV_INFO                  *ParentBrg;
    UINT8                         MaxWidth = 0 ;
    ...
    for(i = 0; i < PciRetrainErrorDevCount; i++)
    {
        PCI_RETRAIN_ERROR_DEV *RetrainErrorDev;
        RetrainErrorDev = &PciRetrainErrorDevList[i];
        if((VenId == RetrainErrorDev->VendorId)&&(DevId == RetrainErrorDev->DeviceId)&&(SubSID == RetrainErrorDev->SubsystemId))
        {
            TRACE((-1, "Current device can not be retrained.\n"));
            return;
        }
    }

    //<huangrj-20220622 exclude devices unable to be retrained ->
    ...
    if(CapIdOffset != 0){
      ...
      do{
        ...
        MaxWidth = (Data16 >> 4) & 0x3f;// Max width

        Data16 = 0;
        dev->RbIo->Pci.Read(dev->RbIo,EfiPciIoWidthUint16,(dev->Address.ADDR + CapIdOffset + PCIE_LINK_STATUS_OFFSET),1,&Data16);
        CurrentWidth = (Data16 >> 4) & 0x3f;// Current width
        TRACE((-1, "BUS=%02x, dev=%02x FUN=%02x\n", dev->Address.Addr.Bus,dev->Address.Addr.Device,dev->Address.Addr.Function));
        TRACE((-1, "MaxWidth=%02x, CurrentWidth=%02x\n", MaxWidth,CurrentWidth));
      
        if(CurrentWidth < MaxWidth)
        {
          ParentBrg = dev->ParentBrg;
          CapIdOffset = PcieFindCapId(&ParentBrg->PciIo, 0x10);
          if(CapIdOffset != 0){
            Data16 = 0;
            ParentBrg->RbIo->Pci.Read(ParentBrg->RbIo,EfiPciIoWidthUint16,(ParentBrg->Address.ADDR + CapIdOffset + PCIE_LINK_CAPABILITY_OFFSET),1,&Data16);
            MaxWidth = (Data16 >> 4) & 0x3f;// Max width

            Data16 = 0;
            ParentBrg->RbIo->Pci.Read(ParentBrg->RbIo,EfiPciIoWidthUint16,(ParentBrg->Address.ADDR + CapIdOffset + PCIE_LINK_STATUS_OFFSET),1,&Data16);
            CurrentWidth = (Data16 >> 4) & 0x3f;// Current width
            TRACE((-1, "BUS=%02x, dev=%02x FUN=%02x\n", ParentBrg->Address.Addr.Bus,ParentBrg->Address.Addr.Device,ParentBrg->Address.Addr.Function));
            TRACE((-1, "Root bridge CurrentWidth = %x\n", CurrentWidth));
            TRACE((-1, "Root bridge MaxWidth = %x\n", MaxWidth));
            if(CurrentWidth < MaxWidth)
            {
                TRACE((-1, "Need retrain.\n"));
                Data16 = 0;
                ParentBrg->RbIo->Pci.Read (ParentBrg->RbIo,
                            EfiPciIoWidthUint16,
                            (ParentBrg->Address.ADDR + CapIdOffset + PCIE_LINK_CONTROL_OFFSET),
                            1,
                            &Data16
                            );
                Data16 |= BIT4; 
                ParentBrg->RbIo->Pci.Write (ParentBrg->RbIo,
                                            EfiPciIoWidthUint16,
                                            (ParentBrg->Address.ADDR + CapIdOffset + PCIE_LINK_CONTROL_OFFSET),
                                            1,
                                            &Data16
                                            );
                IoRead8(0x80); //IoDelay
                IoRead8(0x80); //IoDelay
                Data16 = 0;
                ParentBrg->RbIo->Pci.Read (ParentBrg->RbIo,
                                            EfiPciIoWidthUint16,
                ...
                Data16 &= (~BIT4);  //<Kangmm-20201016 Fix X710 link width change from x8 to x4 during reboot>
                Data16 |= BIT5; 
                ParentBrg->RbIo->Pci.Write (ParentBrg->RbIo,
                                            EfiPciIoWidthUint16,
                                            (ParentBrg->Address.ADDR + CapIdOffset + PCIE_LINK_CONTROL_OFFSET),
...
PCI_ATTRIBUTE_PARAM* GetPciAttribute(PCI_DEV_INFO *dev, PCI_DEV_SETUP_DATA  *DevItem)
{
    UINT8                         CapIdOffset;
    UINT16                        Data16 = 0; 
...
EFI_STATUS InitPCIeDevSetting(EFI_HII_HANDLE HiiHandle,PCI_DEV_SETUP_FORM_DATA  *PciDevFormData)
{
  ...
  EFI_STRING                EnableStr = NULL; 
  EFI_STRING                LocationStr = NULL;
  EFI_STRING                BusdevfuncStr = NULL;
  EFI_STRING                VenIdStr = NULL;
  EFI_STRING                DevIdStr = NULL;
  EFI_STRING                MaxSpeedStr = NULL;
  EFI_STRING                MaxWidthStr = NULL;
  EFI_STRING                CurrentSpeedStr = NULL;
  EFI_STRING                CurrentWidthStr = NULL;
  //<licm-20200921 Added Simplified Chinese to PCIE device list help message in Setup ->                   
...
//<licm-20210531 Optimization PCIE Device List module ->
      //Last Update
      //<Chendl001-20190903 Pcie device list option can show device link Status +>
      DevAttribute = GetPciAttribute(dev, DevItem);
/*
...
*/
      if (DevAttribute != NULL){
        if(DevItem->SlotNum !=0xFF)
          Swprintf(tString, L"Enable/Disable PCIe Device\nLocation:\nSeg:Bus:Dev:Func %02X:%02X:%02X:%02X\n%sVender ID:%04X\nDevice ID:%04X\nMax Speed:%s\nMax Width:%s\nCurrent Speed:%s\nCurrent Width:%s\nSlot Number: %d\n",dev->RbIo->SegmentNumber,dev->Address.Addr.Bus,dev->Address.Addr.Device,dev->Address.Addr.Function,DevItem->Prefix,dev->DevVenId.VenId,dev->DevVenId.DevId,DevAttribute->MaxSpeed,DevAttribute->MaxWidth,DevAttribute->CurrentSpeed,DevAttribute->CurrentWidth, DevItem->SlotNum);
        else
          Swprintf(tString, L"Enable/Disable PCIe Device\nLocation:\nSeg:Bus:Dev:Func %02X:%02X:%02X:%02X\n%sVender ID:%04X\nDevice ID:%04X\nMax Speed:%s\nMax Width:%s\nCurrent Speed:%s\nCurrent Width:%s\n",dev->RbIo->SegmentNumber,dev->Address.Addr.Bus,dev->Address.Addr.Device,dev->Address.Addr.Function,DevItem->Prefix,dev->DevVenId.VenId,dev->DevVenId.DevId,DevAttribute->MaxSpeed,DevAttribute->MaxWidth,DevAttribute->CurrentSpeed,DevAttribute->CurrentWidth);

        //<licm-20200921 Added Simplified Chinese to PCIE device list help message in Setup +>
                  EnableStr = HiiGetString(HiiHandle, STRING_TOKEN(STR_PCIE_DEV_ENABLE_DISABLE_HELP),NULL );
                  LocationStr = HiiGetString(HiiHandle, STRING_TOKEN(STR_PCIE_DEV_LOCATION_HELP),NULL );
                  BusdevfuncStr = HiiGetString(HiiHandle, STRING_TOKEN(STR_PCIE_DEV_BUSDEVFUNC_HELP),NULL );
                  VenIdStr = HiiGetString(HiiHandle, STRING_TOKEN(STR_PCIE_DEV_VENID_HELP),NULL );
                  DevIdStr = HiiGetString(HiiHandle, STRING_TOKEN(STR_PCIE_DEV_DEVID_HELP),NULL );
                  MaxSpeedStr = HiiGetString(HiiHandle, STRING_TOKEN(STR_PCIE_DEV_MAX_SPEED_HELP),NULL );
                  MaxWidthStr = HiiGetString(HiiHandle, STRING_TOKEN(STR_PCIE_DEV_MAX_WIDTH_HELP),NULL );
                  CurrentSpeedStr = HiiGetString(HiiHandle, STRING_TOKEN(STR_PCIE_DEV_CURRENT_SPEED_HELP),NULL );
                  CurrentWidthStr = HiiGetString(HiiHandle, STRING_TOKEN(STR_PCIE_DEV_CURRENT_WIDTH_HELP),NULL );
                  //Swprintf(tString, L"Enable/Disable PCIe Device\nSeg/Bus/Dev/Func: %02X/%02X/%02X/%02X\nVender ID      : %02X\nDevice ID    : %04X\nMaximum Speed: %s\nMaximum Width: %s\nCurrent Speed: %s\nCurrent Width: %s\n",dev->RbIo->SegmentNumber,dev->Address.Addr.Bus,dev->Address.Addr.Device,dev->Address.Addr.Function,dev->DevVenId.VenId,dev->DevVenId.DevId,DevAttribute->MaxSpeed,DevAttribute->MaxWidth,DevAttribute->CurrentSpeed,DevAttribute->CurrentWidth);
                  Swprintf(tString, L"%s%s: %02X/%02X/%02X/%02X\n%s%02X\n%s%04X\n%s%s\n%s%s\n%s%s\n%s%s\n",EnableStr,BusdevfuncStr,dev->RbIo->SegmentNumber,dev->Address.Addr.Bus,dev->Address.Addr.Device,dev->Address.Addr.Function,VenIdStr,dev->DevVenId.VenId,DevIdStr,dev->DevVenId.DevId,MaxSpeedStr,DevAttribute->MaxSpeed,MaxWidthStr,DevAttribute->MaxWidth,CurrentSpeedStr,DevAttribute->CurrentSpeed,CurrentWidthStr,DevAttribute->CurrentWidth);
        //<licm-20200921 Added Simplified Chinese to PCIE device list help message in Setup ->
	    pBS->FreePool(DevAttribute);
	  } else {
        if(DevItem->SlotNum !=0xFF)
        //<xuyj1-20210802 delete slot information from pcie device list help information +> 
        // Swprintf(tString, L"Enable/Disable PCIe Device\nLocation:\nSeg:Bus:Dev:Func %02X:%02X:%02X:%02X\n%s\n",dev->RbIo->SegmentNumber,dev->Address.Addr.Bus,dev->Address.Addr.Device,dev->Address.Addr.Function,DevItem->Prefix);
        Swprintf(tString, L"Enable/Disable PCIe Device\nLocation:\nSeg:Bus:Dev:Func %02X:%02X:%02X:%02X\n",dev->RbIo->SegmentNumber,dev->Address.Addr.Bus,dev->Address.Addr.Device,dev->Address.Addr.Function);
        //<xuyj1-20210802 delete slot information from pcie device list help information -> 
        else
...
VOID CreatePciOpRomItems(
  ...
  BOOLEAN                     Tcap;
  //<licm 20200921 Added Simplified Chinese to PCIE device list help message in Setup +>
     EFI_STRING              OpROMHelpStr = NULL;
     EFI_STRING              SegDevHelpStr = NULL;
   //<licm 20200921 Added Simplified Chinese to PCIE device list help message in Setup -> 
  ...
  {   
      ...
      if(PcieNicInfo->NicMacGuidSelect == 1)  //MAC
      {
        ...
        //<licm 20200921 Added Simplified Chinese to PCIE device list help message in Setup +>
                ...
                OpROMHelpStr = HiiGetString(HiiHandle, STRING_TOKEN(STR_PCIE_DEV_OPTION_ROM_HELP),NULL );
                SegDevHelpStr = HiiGetString(HiiHandle, STRING_TOKEN(STR_PCIE_DEV_BUSDEVFUNC_HELP),NULL );
                Swprintf(tString, L"%s %02X:%02X:%02X:%02X:%02X:%02X%s %02X:%02X:%02X:%02X\n",OpROMHelpStr,NicMac[0],NicMac[1],NicMac[2],NicMac[3],NicMac[4],NicMac[5],SegDevHelpStr,dev->RbIo->SegmentNumber,dev->Address.Addr.Bus,dev->Address.Addr.Device,dev->Address.Addr.Function);//<Lvshh001-20190605 Add per port PCIe BDF of device >
        //<licm 20200921 Added Simplified Chinese to PCIE device list help message in Setup ->
...
VOID CreatePciDeviceItems(IN EFI_EVENT Event, IN VOID *Context)
{
  ...
  UINTN                                 i,j;
  CHAR16                        *DevicehelpString=NULL,*PciLoaction;
  PCIE_DEV_INFO             PcieDevInfo;
  EFI_STATUS                        Status;
  ...
  if(NULL == PciDevFormData || NULL == HiiHandle )
  { 
      return;
  
  }

  if(PciDevFormData->PciDevCount == 0) {
     TRACE ((-1, "PciDevFormData->PciDevCount.\n"));
      return;
  }
  StartOpCodeHandle = PciDevFormData->StartOpCodeHandle;
  ...
  for (i=0; i<PciDevFormData->PciDevCount; i++) {
    ...
    if(IdxDev->PortData[0].ShowItem != TRUE)
      continue;
    PciLoaction=MallocZ(20);
    DevicehelpString = HiiGetString (HiiHandle,IdxDev->PortData->DevHelpStringId, NULL);
    Status = gpOemPcieDevProtocol->GetPcieDevInfo(PciDevFormData,IdxDev->PortData->PciDevIdx,IdxDev->PortData->PortIdx,&PcieDevInfo);
    TRACE((-1, "PcieDevInfo.PcieLocation = %s\n", PcieDevInfo.PcieLocation));
    TRACE((-1, "PcieDevInfo.PcieLocation = %a\n", PcieDevInfo.PcieLocation));
    AsciiToUnicodeSize(PciLoaction,16,PcieDevInfo.PcieLocation);
    StrCat(DevicehelpString, L"Location: ");
    StrCat(DevicehelpString, PciLoaction);
    TRACE((-1, "PciLoaction = %s\n", PciLoaction));
    TRACE((-1, "PciLoaction = %a\n", PciLoaction));
    StrCat(DevicehelpString, L"\n");
    IdxDev->PortData->DevHelpStringId = HiiSetString(HiiHandle, 0,DevicehelpString, NULL ); 
    //1.Create Pci Setup OneOf items
...
//<licm-20210531 Optimization PCIE Device List module ->
  ...
  Status = gpOemPcieDevProtocol->GetPcieDevList(&DevListData);
   TRACE ((-1, "gpOemPcieDevProtocol->GetPcieDevList status = %r \n", Status));  
    ...
    )
{
  ...
  TRACE((-1, "PcieDevData->PciDevCount:%X /n",PcieDevData->PciDevCount));
