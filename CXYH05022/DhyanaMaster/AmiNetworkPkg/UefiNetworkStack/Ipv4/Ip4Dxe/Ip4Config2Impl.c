...
// AMI PORTING START
#ifndef EFI_IP4_CONFIG2_INTERFACE_INFO_NAME_SIZE
   #define EFI_IP4_CONFIG2_INTERFACE_INFO_NAME_SIZE 32
#endif
// AMI PORTING END

//AMI PORTING START
// Global variables to store IP and Gateway.
IP4_ADDR	gStationAddress = 0x0;
IP4_ADDR	gSubnetMask = 0x0;
//AMI PORTING END
...
//AMI PORTING START
/**
  Check whether the given The given IpConfig Details are Valid or Not
  @param[in]     Data           The data buffer to set. The type of the data buffer is
                                associated with the DataType.
  @param[in]     DataType       The type of data to set.
 

  @retval EFI_SUCCESS           The given IpConfig Details are Valid Pair.
  @retval EFI_INVALID_PARAMETER One or more of the following are TRUE:
                                - Data is NULL.
                                - StationAddress and SubnetMask pair is Invalid
                                - Gateway is Invalid

**/
EFI_STATUS
EFIAPI
ValidateIpConfigDetails(IN VOID *Data,IN EFI_IP4_CONFIG2_DATA_TYPE  DataType)
{
	...
	if(DataType==Ip4Config2DataTypeManualAddress) {
		ManualAddress = (EFI_IP4_CONFIG2_MANUAL_ADDRESS *) Data;
		// Check the first octet of the IP address and Subnet mask. Return if ZERO.
		if((ManualAddress->Address.Addr[0]== 0) || (ManualAddress->SubnetMask.Addr[0]== 0)) {
			Status = EFI_INVALID_PARAMETER;
			goto ON_EXIT;
		}
		
		gStationAddress = *((IP4_ADDR*)(ManualAddress->Address.Addr));
		gSubnetMask =*((IP4_ADDR*)(ManualAddress->SubnetMask.Addr));
		
		NetworkBytes1 = NTOHL (gStationAddress);
		NetworkBytes2 = NTOHL (gSubnetMask);
		
		// Validate NetMask. It should be one present in gIp4AllMasks[32].
		if ( NetGetMaskLength (NetworkBytes2) >= IP4_MASK_NUM ||
				!NetIp4IsUnicast (NetworkBytes1, NetworkBytes2)) {
			Status = EFI_INVALID_PARAMETER;
			goto ON_EXIT;
		}
	}
	else if(DataType==Ip4Config2DataTypeGateway) {
		// Check the first octet of the Gateway. Return if ZERO.
		if(((UINT8*)Data)[0]==0) {
			Status = EFI_INVALID_PARAMETER;
			goto ON_EXIT;
		}
		
		// If gStationAddress and gSubnetMask are ZERO then call is coming from HttpBootRegisterIp4Gateway(),
		// Then No need to validate Gateway with StationAddress and SubnetMask so return EFI_SUCCESS
		if( (gStationAddress!= 0) && (gSubnetMask != 0)) {
			// If gStationAddress and gSubnetMask are Non ZERO then call is coming from IPConfig Module or Shell
			// So gStationAddress and gSubnetMask are already updated in first call i.e setting IP and SubnetMask
			// Using gStationAddress and gSubnetMask pair Validate Gateway
			GateWay= *((IP4_ADDR *)Data);  
			NetworkBytes1 = NTOHL (GateWay); 
			NetworkBytes2 = NTOHL (gSubnetMask);
			if (!IP4_NET_EQUAL (gStationAddress, GateWay, gSubnetMask) ||
					!NetIp4IsUnicast (NetworkBytes1, NetworkBytes2)) {
				Status = EFI_INVALID_PARAMETER;
				goto ON_EXIT;
			}
...
ON_EXIT:
	if(DataType==Ip4Config2DataTypeGateway) {
		gStationAddress = 0;
		gSubnetMask = 0; 
	}
	return Status;
...
//AMI PORTING START    	
#if (NET_VALIDATE_IPCONFIG_DETAILS)
    	Status = ValidateIpConfigDetails(Data,DataType);
    	if (EFI_ERROR (Status)) {
    		 // Restore TPL if any of the parameter is invalid.
