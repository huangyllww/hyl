...
//SGEZB#27631-Legacy-Mode-Graphic-SW +>
...
)
{
    ...
    //Mizl-20211026-Improve GraphicSw Function For Offborad Legacy Option+>
	UINT64                  	TotalMMIO32; 
	UINT32                  	id;
  	UINTN                   	i;
	...
	BufferSize = sizeof(SETUP_DATA);
  	...
  	if (OemOpROMEN[VIDEOINDX] == 2)
  	{
    	...
    	for(i = 0; i < PCI_MAX_BAR_NO; i++)
		{
            ...
            //if(ParentBrg->Bar[i].Type==tBarMmio64pf) ParentBrg->Bar[i].Type=tBarMmio32pf;
      		//if(ParentBrg->Bar[i].Type==tBarMmio64) ParentBrg->Bar[i].Type=tBarMmio32;
    	}
    ...
    switch (Phase) {
      ...
      case EfiPciHostBridgeEndBusAllocation:
        ...
        while(!IsNull (&gVgaList, NodeEntry)) {
          ...
          DEBUG((EFI_D_INFO, "[OemDbg][%a][%d]\r\n",__FUNCTION__,__LINE__));
          if (SelectedVGA != NULL ) OemVgaRoute(SelectedVGA); //SGEZB#27631-Legacy-Mode-Graphic-SW +>
            break;
      default:
