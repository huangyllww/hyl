  ...
  IN    EFI_FFS_FILE_HEADER         *pFile)
{
    ...
    Status = (*PeiServices)->LocatePpi(PeiServices, &gAmiDigitalSignaturePPIGuid, 0, NULL, &pAmiSigPPI);
    ...
    if( 0 != CmpStatus) return  EFI_VOLUME_CORRUPTED;

    (UINT8*)AptioFwCapsHdrPtr = CurrPosition + sizeof(EFI_GUID);

    if(AptioFwCapsHdrPtr->RomImageOffset > FWCAPSULE_MAX_HDR_SIZE) return EFI_SECURITY_VIOLATION;
    
    // for now Aptio FW Capsule only supports WIN_CERT_TYPE_EFI_GUID 
    if(AptioFwCapsHdrPtr->FWCert.SignCert.Hdr.Hdr.wCertificateType != WIN_CERT_TYPE_EFI_GUID) return EFI_SECURITY_VIOLATION;
    ...
    Status = pAmiSigPPI->VerifyKey(pAmiSigPPI, &gFWkeyGuid, &PubKeyHndl); 
    ...
    // 2.2  Compute FWCert.SignCert Hash
        ...
        if (!Sha256Final (HashCtx, HashDB)) return EFI_SECURITY_VIOLATION;
    
        pSig = (void*)AptioFwCapsHdrPtr->FWCert.RootCert.Signature; 
        Status = pAmiSigPPI->VerifySig(pAmiSigPPI, 
            &PubKeyHndl, &HashHndl, pSig, DEFAULT_RSA_SIG_LEN, Flags ); 
...
//####    CopyMem_LpcDma(FvInRamStart, pFv, (UINTN)pFv->FvLength);
    ...
    RomAreaTbl = (ROM_AREA *)(UINTN)((UINT32)AptioFwCapsHdrPtr+AptioFwCapsHdrPtr->RomLayoutOffset);
    ...
    pSig = (void*)AptioFwCapsHdrPtr->FWCert.SignCert.CertData.Signature; 
    PubKeyHndl.Blob = (UINT8*)AptioFwCapsHdrPtr->FWCert.SignCert.CertData.PublicKey;
    Status = pAmiSigPPI->VerifySig(pAmiSigPPI, &PubKeyHndl, &HashHndl, pSig, DEFAULT_RSA_SIG_LEN, Flags); 
  ...
  )
{
    ...
    if(BootMode == BOOT_IN_RECOVERY_MODE) return Status;

    Status = GetRomLayout(PeiServices, &Area);

    for(Area; Area->Size != 0; Area++) {
        if (!(Area->Attributes & ROM_AREA_FV_SIGNED)) continue;
        if ((ROM_AREA_FV_PEI+ROM_AREA_FV_DXE) != (Area->Attributes & (ROM_AREA_FV_PEI+ROM_AREA_FV_DXE))) continue;
        // Find DXE FV
        if(((Area->Address - 1)+Area->Size) != 0xFFFFFFFF) break; // not FV_BB
    }
    
    // if not found - stop
    if((ROM_AREA_FV_PEI+ROM_AREA_FV_DXE) != (Area->Attributes & (ROM_AREA_FV_PEI+ROM_AREA_FV_DXE))) goto ErrorLoop;
    
    pFv = (EFI_FIRMWARE_VOLUME_HEADER*)Area->Address;
    
    Status = (*PeiServices)->FfsFindNextFile (PeiServices, \
                            EFI_FV_FILETYPE_FREEFORM, pFv, &pFile);

    if(EFI_ERROR(Status)) goto ErrorLoop;    
    
    // Compare GUID - it can be capsule FFS file or other files
    CmpStatus = guidcmp(&((EFI_FFS_FILE_HEADER*)pFile)->Name,&gDxeSigFfsGuid);
    if(0 != CmpStatus){
        ...
        if(0 != CmpStatus) goto ErrorLoop;
    }

    Status = CheckDxeSign(PeiServices, pFv, pFile);
