...
EFI_GUID    gBootScriptSaveGuid           = EFI_BOOT_SCRIPT_SAVE_GUID;
EFI_BOOT_SCRIPT_SAVE_PROTOCOL             *gBootScriptSave=NULL;
#else
EFI_GUID    gBootScriptSaveGuid           = EFI_S3_SAVE_STATE_PROTOCOL_GUID;
...
// CSP definition
//-------------------------------------------------------
HYGONSB_POLICY_PROTOCOL           *gHygonSbPolicy = NULL;
  ...
  )
{
    ...
    TRACE((-1,"IdeControllerEntryPoint Status = %r\n", Status));
  ...
  )
{
    ...
    if (!CspInit) {
        ...
        GetSbSetupData(pRS, &gSetupData, FALSE);

        Status = pBS->LocateProtocol(&gEfiPciRootBridgeIoProtocolGuid, NULL, \
                    &gPciRootBridgeIo);
        ASSERT_EFI_ERROR(Status);
        if (EFI_ERROR(Status)) {
            TRACE ((TRACE_ALWAYS, "SB Ide Driver locate PciRootBridgeIoProtocol = %r\n",Status));
            return Status;
        }

        Status = pBS->LocateProtocol(&gBootScriptSaveGuid,NULL, \
                    &gBootScriptSave);
        ASSERT_EFI_ERROR(Status);
        if (EFI_ERROR(Status)) {
            TRACE ((TRACE_ALWAYS, "SB Ide Driver locate BootScriptSaveProtocol = %r\n",Status));
            return Status;
        }

        Status = pBS->LocateProtocol(&gHygonSbPolicyProtocolGuid, NULL, \
                    &gHygonSbPolicy);
        ASSERT_EFI_ERROR(Status);
        if (EFI_ERROR(Status)) {
            TRACE ((TRACE_ALWAYS, "SB Ide Driver locate Policy = %r\n",Status));
            return Status;
        }
