...
)
{
  ...
  */
  ...
  {
    PcdSet64(AmiPcdPeiPciMmioTopAddress,Base+0x2000000); //SGEZB#28384:Screen Blurred In Fast Video Phase
    return EFI_SUCCESS;
  }      

  if(Base < PcdGet64(PcdPciExpressBaseAddress))
    if(Base + AspeedVgaBufferSize + 0x2000000 <= PcdGet64(PcdPciExpressBaseAddress)) //SGEZB#28384:Screen Blurred In Fast Video Phase
    {
      PcdSet64(AmiPcdPeiPciMmioTopAddress,Base + AspeedVgaBufferSize +0x2000000); //SGEZB#28384:Screen Blurred In Fast Video Phase
      return EFI_SUCCESS;
    }
    else if(((Limit&0xFE000000) - AspeedVgaBufferSize) > PcdGet64(PcdPciExpressBaseAddress)) //SGEZB#28384:Screen Blurred In Fast Video Phase
    {
      PcdSet64(AmiPcdPeiPciMmioTopAddress,Limit);
      return EFI_SUCCESS;
    }else
  ...
  )    
{
  ...
  UINT8 idx;
  for (idx = *Index; idx < 0xF; idx++)
  {
    MMIOAddCtl.AllBits=PciRead32(DataFabricAddress|0x208|(idx*0x10));
    *Limit=PciRead32(DataFabricAddress|0x204|(idx*0x10));
    *Base=PciRead32(DataFabricAddress|0x200|(idx*0x10));
    DEBUG((EFI_D_ERROR, "[OemDbg][%a][%d]Offset:%x-%x-%x,DstFabricID:%x Base:%x Limit:%x RE:%x WE:%x\r\n",__FUNCTION__,__LINE__,0x200|(idx*0x10),0x204|(idx*0x10),0x208|(idx*0x10),\
                              MMIOAddCtl.Field.DstFabricID,*Base,*Limit,MMIOAddCtl.Field.RE,MMIOAddCtl.Field.WE));
    *Index = idx;
    if(((MMIOAddCtl.Field.DstFabricID == FabricId) && (*Base!= 0) && (*Base < 0xFFFF))) 
      return TRUE;                          
  }
  return FALSE;      
}
...
{ 
  ...
  UINT8 idxIo;
  for (idxIo = *IndexIo; idxIo < 0x8; idxIo++)
  {
    PCIeBusConfig.AllBits=PciRead32(DataFabricAddress|0xA0|(idxIo*0x4));
    DEBUG((EFI_D_ERROR, "[OemDbg][%a][%d]Offset:%x,DstFabricID:%x BusNumBase:%x BusNumLimit:%x RE:%x WE:%x\r\n",__FUNCTION__,__LINE__,0xA0|(idxIo*0x4),\
    ...
    IoSpaceAddress.AllBits=PciRead32(DataFabricAddress|0xC0|(idxIo*0x8));
    IoLimitAddress.AllBits=PciRead32(DataFabricAddress|0xC4|(idxIo*0x8));
    DEBUG((EFI_D_ERROR, "[OemDbg][%a][%d]Offset:%x-%x DstFabricID:%x IOBase:%x IOLimit:%x RE:%x WE:%x\r\n",__FUNCTION__,__LINE__,0xC0|(idxIo*0x8),0xC4|(idxIo*0x8),\
    ...
    *IndexIo = idxIo;
    if((IoLimitAddress.Field.DstFabricID == FabricId)) 
         return TRUE;      
  }
  return FALSE;
}
...
//<Kangmm-20210121 Code improve for Fast Video ->  
  ...
  if((!IsSOCSSP()) && (!IsSOCGn()))
  {             
      if((GetMMIOFabricId(DataFabricAddress,FabricId,&Index,&MMIOBase,&MMIOLimit)) && (GetIoFabricId(DataFabricAddress,FabricId,&IndexIo)))
      {
        Status = AdjustMMIOResource(PeiServices,MMIOBase<<16,MMIOLimit<<16);
        if(EFI_ERROR (Status))
          return Status;
        DEBUG((EFI_D_ERROR, "[OemDbg][%a][%d]IoBaseAddress:%x Index:%x  IndexIo:%x \r\n",__FUNCTION__,__LINE__,DataFabricAddress|0xc0|(IndexIo*0x08),Index,IndexIo));
        IoBaseReg.AllBits = PciRead32(DataFabricAddress|0xc0|(IndexIo*0x08));
        IoBaseReg.Field.ReadEnable = 1;
        IoBaseReg.Field.WriteEnable = 1;
        IoBaseReg.Field.VgaEnable = 1;
        PciWrite32(DataFabricAddress|0xc0|(IndexIo*0x08),IoBaseReg.AllBits);
        
        IoLimitReg.AllBits = PciRead32(DataFabricAddress|0xc4|(IndexIo*0x08));
        IoLimitReg.Field.DstFabricID = FabricId;
        PciWrite32(DataFabricAddress|0xc4|(IndexIo*0x08),IoLimitReg.AllBits);
        DEBUG((EFI_D_ERROR, "[OemDbg][%a][%d]Offset:%x,DstFabricID:%x  Index =%x IndexIo=%x\r\n",__FUNCTION__,__LINE__,0xC4|((IndexIo)*0x8),\
                                 IoLimitReg.Field.DstFabricID,Index,IndexIo));
        
        MMIOAddCtl.AllBits=PciRead32(DataFabricAddress|0x208|(Index*0x10));
        MMIOAddCtl.Field.RE = 1;
        MMIOAddCtl.Field.WE = 1;

       PciWrite32(DataFabricAddress|0x208|(Index*0x10), MMIOAddCtl.AllBits);
