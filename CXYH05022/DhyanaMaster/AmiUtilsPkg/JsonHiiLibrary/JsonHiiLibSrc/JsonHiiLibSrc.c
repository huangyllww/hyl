...
EFI_STATUS SetMapStringControlDatabase (IN UINT8 *HiiDatabase, UINTN  HiiSize,IN UINT8 *MapControlInfo, IN UINT32 BufferSize, OUT void **FailMapIdList, OUT UINT32 *FailMapListSize )
{
   ...
   // Obtain boot option MapId saved
    ...
    if( NULL != ImportValue )
    {
        ...
        //libj-20220719 fix restore hang 92 + 
        if(NULL != BootOptMapId)
        {
          free(BootOptMapId);
		  BootOptMapId = NULL;
		}
        if(NULL != gBootOptions)
        {
          free(gBootOptions);
		  gBootOptions = NULL;
		}
...
//<kuaibai001-20180813 Fix hang when importing bootorder changes from OOB ->
    ...
    for( ; BuffIndex < BufferSize; Index+= 2)
    {
        ...
        //libj-20220719 fix restore hang 92 +
        DEBUG((DEBUG_ERROR, "\n[SK]Index:%d gBootCount:%d",Index,(gBootCount * sizeof( UINT16 ))));
        if(Index > (gBootCount * sizeof( UINT16 )))
            goto DONE ;
        //libj-20220719 fix restore hang 92 -
...
//kuaibai add update >>>
    ...
    //libj-20220719 fix restore hang 92 +
DONE :
    DEBUG((DEBUG_ERROR, "\n[SK]Index:%d BootIndex:%d",Index,BootIndex));
