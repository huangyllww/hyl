...
static EFI_GUID gEfiRegularExpressionProtocolGuid = EFI_REGULAR_EXPRESSION_PROTOCOL_GUID;
/**
  ...
  )
{
  ...
  NewStack = EfiLibAllocateZeroPool (Size * sizeof (EFI_HII_VALUE));
  if (NewStack == NULL) {
  ...
  )
{
  ...
  StringPtr= EfiLibAllocateZeroPool (EfiStrSize (String[1]) + EfiStrSize (String[0]));
