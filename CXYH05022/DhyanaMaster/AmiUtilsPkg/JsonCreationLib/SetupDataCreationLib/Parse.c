...
EFI_STATUS _AddVariable(UINT8 *IFRData, EFI_HII_HANDLE Handle)
{
	...
	case EFI_IFR_VARSTORE_EFI_OP:
        ...
        if ((gST->Hdr.Revision) > 0x2001E) //If UEFI spec version is greater than 2.3 then store the Name and size from the opcode      
        {
      ...
      break;
		...
		if(addControl)
		{
	      defaults_size =0;
            ...
            //Allocating the control Size depending on the default count of the control
  ...
  while(i < pkgHdr->Length)
	{
		...
		case EFI_IFR_FORM_SET_OP:
			...
			if ( 0 ) //Suppressing formsets mentioned in elink AMITSE_SUPPRESS_DYNAMIC_FORMSET_LIST
			{
				FormSetTitle = 0;
...
*/
        ...
        case EFI_IFR_FORM_MAP_OP: 
			...
			//Check for Dynamic Page Flag
				if (tmpPgInfo->PageParentID == 0 && SETUP_GROUP_DYNAMIC_PAGES)			//PageParentID is 0 && SDL token is true
				{
