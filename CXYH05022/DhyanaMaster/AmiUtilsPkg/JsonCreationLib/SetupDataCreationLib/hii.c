...
EFI_STATUS HiiFixupData()
{
   ...
   /*
	...
	gSetupData[i].FormSet = EfiLibAllocateZeroPool(sizeof(EFI_HII_PACKAGE_HEADER) +
			sizeof(EFI_IFR_FORM_SET) + /*i*/ gToolPages->PageCount * sizeof(EFI_IFR_REF)); // No. of root pages can be more then hii handle because of AMITSE_SUBPAGE_AS_ROOT_PAGE_LIST_SUPPORT
	if(gSetupData[i].FormSet == NULL)
	{
		status = EFI_OUT_OF_RESOURCES;
		goto DONE;
...
//		PAGE_INFO *NewPageInfo;
		...
		{
//#if SETUP_RUNTIME_IFR_PROCESSING
			if(RTIfrProcessFormIfUpdated(link) == TRUE)
				continue;
//#endif //SETUP_RUNTIME_IFR_PROCESSING
		...
		for ( j = 0; j < NewPageInfo->PageControls.ControlCount; j++ )
		{
			...
			//Updated the contol size to copy the Default information for the control
...
//	status = HiiInitializeProtocol();
	...
	gSetupHandles = (EFI_HII_HANDLE*)EfiLibAllocateZeroPool((count + 1) * sizeof(EFI_HII_HANDLE));
...
//    				gFirstPageRef = (VOID*)(*ref);
    ...
    VarUpdateVariable(VARIABLE_ID_DYNAMIC_PAGE_COUNT);  //Update nvram cache for DynamicPageCount
...
**/
/*
UINTN HiiMyGetStringLength(VOID* handle, UINT16 token )
{
	CHAR16 *string;
	UINTN length = 0;
...
/*  return HiiChangeStringLanguage( handle, token, L"   ", string );*/
  ...
  for ( i = 0; i < gLangCount; i++ )
    {
      returntoken = Uefi21HiiChangeStringLanguage( handle, token, gLanguages[i].Unicode, string );
...
//    PageRemoveHook (ii, (VOID *)pageInfo, pageInfo->PageHandle, pageInfo->PageFormID);        //Used for ESA UI not needed so commented
  ...
  else
  {
	...
	for(ii = 0; ii < gSetupCount; ii++)
	{
		...
		if(!IsDriverHealthFormset)
		{
			if(count && (jIndex == gGuidDumpCount)) //Decrement the gDynamicPageCount if current handle/formset is present in gSetupData else dont decrement
			{
				count-- ;
				gDynamicPageCount = count ;   
				VarSetNvramName( L"DynamicPageCount", &DynamicPageGuid, EFI_VARIABLE_BOOTSERVICE_ACCESS, &count, sizeof(count) );
...
//	FixMergePagesExtraHook ((VOID *)gPages, (VOID *)tempPages ); //needed for ESA so commented out
   ...
   //Getting the current page info	
	...
	for ( control= 0; control < pageInfo->PageControls.ControlCount; control++ )
	{
		...
		//Checking for the ControlEvaluateDefault Flag and the Default Id equal tp Manufacturing Id 
		 if(controlInfo->ControlFlags.ControlEvaluateDefault && DEFAULT_CONDITIONAL_EXPRESSION_SUPPORT)
		 {
			 UINT64 Defaults = 0 ;
			 UINT16 size = 0;
			 offset = UefiGetQuestionOffset(controlInfo->ControlPtr);
...
EFI_STATUS GetPageIdFromGuid(EFI_GUID *FormGuid, UINT16 PageFormID, UINT16 *PageID)
{
  ...
  if(gPages && gApplicationData && gPageIdList)
  {
    for(pgIndex = 0; pgIndex < gPages->PageCount; pgIndex++)
    {
      ...
      if ((NULL != pgInfo) && (NULL == pgInfo->PageHandle))		//if relash module formsetguid passed then it will return the pageid
...
{
	EFI_STATUS status = EFI_UNSUPPORTED;
	BOOLEAN isDefaultStringSupported = TSE_SUPPORT_DEFAULT_FOR_STRING_CONTROL;
	UINT32	page = 0, control = 0;
	VOID *defaults4FirstBootVar = NULL;
	BOOLEAN FirstEvalExprFixup = FALSE;
	UINTN varSize = 0;

	defaults4FirstBootVar = (VOID *)VarGetNvramName (L"RemoteEvaluateDefaults4FirstBoot", &gEvaluateDefaults4FirstBootGuid, NULL, &varSize);
	if (NULL == defaults4FirstBootVar)
	{
		VarSetNvramName( L"RemoteEvaluateDefaults4FirstBoot", &gEvaluateDefaults4FirstBootGuid, EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_NON_VOLATILE, &varSize, sizeof (varSize) );
	...
	for ( page = 0; page < gPages->PageCount; page++ )
	{
		...
		for ( control= 0; control < pageInfo->PageControls.ControlCount; control++ )
		{
			...
			else if (DEFAULT_CONDITIONAL_EXPRESSION_SUPPORT && (controlInfo->ControlFlags.ControlEvaluateDefault) && (TRUE == FirstEvalExprFixup))
...
VOID CheckRefreshIDForPage(PAGE_INFO *PageInfo,UINT16 Link)
{
	...
	if(pkgHdr->Type == EFI_HII_PACKAGE_FORMS)
	{
		...
		while(i < pkgHdr->Length)
		{
			...
			PageIdInfo = (PAGE_ID_INFO*)((UINT8*)gToolPageIdList + gToolPageIdList->PageIdList[NewPageInfo->PageIdIndex]);
					...
					if(opHeader->OpCode == EFI_IFR_FORM_OP)	
					{
						if(((EFI_IFR_FORM *)opHeader)->FormId == PageInfo->PageFormID) // to get current form header
						{
							while(1)//Check for EFI_IFR_REFRESH_ID_OP until endform reaches,if available then create event
							{
								...
								{
//									CreateEventForPageRefresh (NewPageInfo,((AMI_EFI_IFR_REFRESH_ID *)(opHeader))->RefreshEventGroupId);
									return;
								}
...
VOID DefaultsEvaluateExpression (EFI_EVENT Event, VOID *Context)
{
	...
	if (NULL == defaults4FirstBootVar)
	{
	 	if(gApplicationData == NULL )
	 	{
	   		Status = InitApplicationData(); // To initialize gApplication
