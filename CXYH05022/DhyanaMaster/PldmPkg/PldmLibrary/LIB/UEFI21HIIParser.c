...
UINT64 _GetUefiMinimumValue(UINT8 *HiiData)
{
	...
	{
		case EFI_IFR_NUMERIC_OP:
			MinVal = ( UINT16 )TSE_GET_NUMERIC_MINVALUE( HiiData );
			break;
		case EFI_IFR_STRING_OP:
  ...
  *FormID = *FormTitle = 0;
  ...
  do
  {
    ...
    switch( OpHeader->OpCode )
    {
		...
		//EFI_IFR_VARSTORE_EFI_OP and EFI_IFR_VARSTORE_NAME_VALUE_OP not needed for test version
	...
	do
	{
		...
		switch( OpHeader->OpCode )
		{
            ...
            case EFI_IFR_TIME_OP:
                ...
                Flags = UefiGetFlagsField(OpHeader) ;
