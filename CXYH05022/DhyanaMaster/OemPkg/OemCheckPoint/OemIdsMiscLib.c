...
extern VOID IdsVGACheckPoint(UINT16 checkpoint);
  ...
  )
{
  ...
  if(DataValue==1) {
    IdsVGACheckPoint((UINT16)TestPoint);
  }
  ...
  if (PcdGetBool (PcdAgesaTestPointEnable)) {
    ...
    LibAmdIoWrite_IDS (PcdGet8 (PcdAgesaTestPointWidth), PcdGet16 (PcdIdsDebugPort), &SizedTp, StdHeader);
#ifndef IDSOPT_NDEBUG
