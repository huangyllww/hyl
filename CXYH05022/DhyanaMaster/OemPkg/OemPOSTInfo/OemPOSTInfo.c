...
GetMemoryInfo(VOID)
{
  ...
  UINT8                                     EccCapbility;
  UINT8                                     StrIndex;
  UINT32                                    MemSingleSize; //<hety-20230531 SGMTB#163448: Adjust variable size. >
  CHAR16                                    *MemLocator;
  UINT8                                     MemNumbers = 0;
  //<Yaosr001-20200506 Display Memory manufacturer/count/EccCapbility/Locator during the post. ->
    ...
    if ((Str != NULL) && (Text != NULL) && (TmpStr != NULL)) {
      ...
      for (Index = 0; Index < NSOCKETS; Index++) {
        if (gPostInfo.CpuPostInfo[Index].Present != 0){
          ...
          if (TmpStr != NULL) {
            //SGMTB#163189:Add Cpu Frequency& TDP Info Support+>
            Swprintf(Text, TmpStr, gPostInfo.CpuPostInfo[Index].CurrentCore,(UINTN) (gPostInfo.CpuPostInfo[Index].Freq / 1000),\
                                                        (UINTN) (gPostInfo.CpuPostInfo[Index].Freq % 1000),PcdGet16 (PcdTDP));
            //SGMTB#163189:Add Cpu Frequency& TDP Info Support->
            PostManagerDisplayPostMessage (Text);
          }
          //<Yaosr001-20200511 Display CPU cores during the post. ->
...
POSTShowBMCInfoOnScreen()
{
  ...
  if ((!EFI_ERROR(Status)) && BmcSelfTestProtocol->IsGetDeviceIdCommandPassed) {
    ...
    Swprintf(uniText, L"%d.%02x.%02x", BmcSelfTestProtocol->GetDeviceIdResponse.MajorFirmwareRev, BmcSelfTestProtocol->GetDeviceIdResponse.MinorFirmwareRev, BmcSelfTestProtocol->GetDeviceIdResponse.AuxFirmwareRevInfo);
#else
