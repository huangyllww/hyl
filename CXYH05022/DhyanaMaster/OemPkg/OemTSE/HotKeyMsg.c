...
//<lvych003-20170408 Improve show clear cmos message behavior+>
  ...
  //SGEZB#30114:SMT Load Default Cause PassWord Restore Fail +>
  //Clear CmosClear flag
  PcdSetBool(PcdCmosClear, FALSE);
  
  //Clear CmosClear register
  IoWrite8(0x72, 0xFF);
  if (IoRead8(0x73) != 0xAA) {
    IoWrite8(0x73, 0xAA);
    DEBUG((EFI_D_ERROR, "[OemDbg] CmosReg:0x%x\n", IoRead8(0x73)));
  }
  //SGEZB#30114:SMT Load Default Cause PassWord Restore Fail ->
