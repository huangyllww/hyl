...
//<zhangyan-20200912 Add a token to control the presence of the SD boot group +>
#if SD_GROUP_SUPPORT
...
EFI_STATUS AliFboBootModeFormCallback(
    IN EFI_HII_HANDLE HiiHandle,
    IN UINT16 Class,
    IN UINT16 SubClass,
    IN UINT16 KeyValue
)
{
    ...
    //<SGETB24078: Load default for 'Boot mode select' item failed +>
    ...
    Status = HiiLibGetBrowserData(&Size, &Setup, &SetupGuid, L"Setup");
    if (EFI_ERROR(Status)) {
      DEBUG((EFI_D_INFO, "%a HiiLibGetBrowserData Setup (%r)\n", __FUNCTION__, Status));
      return Status;
    }
    
    PCIeBufferSize = sizeof(OEMPCIE_SETUP_DATA);
    Status = HiiLibGetBrowserData(&PCIeBufferSize, PcieNvData, &gEfiSetupGuid, L"OEMPCIE_DEV");
    if (EFI_ERROR(Status)) {
      DEBUG((EFI_D_INFO, "%a HiiLibGetBrowserData OEMPCIE_DEV (%r)\n", __FUNCTION__, Status));
      return Status;
    }
    
    if(Setup.BootMode == 0) //Legacy Mode
    {
        Setup.BootOptionFilter = 1;
        PcieNvData->OpROMEN[PXEINDX] = 2;
        PcieNvData->OpROMEN[MASSINDX] = 2;
        PcieNvData->OpROMEN[VIDEOINDX] = 2;
        PcieNvData->OpROMEN[OLDINDX] = 2;
    }
    if(Setup.BootMode == 1) //Uefi Mode
    {
        Setup.BootOptionFilter = 2;
        PcieNvData->OpROMEN[PXEINDX] = 1;
        PcieNvData->OpROMEN[MASSINDX] = 1;
        PcieNvData->OpROMEN[VIDEOINDX] = 1;
        PcieNvData->OpROMEN[OLDINDX] = 1;
    }
    if(Setup.BootMode == 2) //Dual Mode, compatible for all project
    {
        Setup.BootOptionFilter = 0;
        PcieNvData->OpROMEN[PXEINDX] = 3;
        PcieNvData->OpROMEN[MASSINDX] = 3;
        PcieNvData->OpROMEN[VIDEOINDX] = 3;
        PcieNvData->OpROMEN[OLDINDX] = 1;
    }
    
    Status = HiiLibSetBrowserData(Size, &Setup, &SetupGuid, L"Setup");
    if (EFI_ERROR(Status)) {
      DEBUG((EFI_D_INFO, "%a HiiLibSetBrowserData Setup (%r)\n", __FUNCTION__, Status));
    }
    Status = HiiLibSetBrowserData(PCIeBufferSize, PcieNvData, &gEfiSetupGuid, L"OEMPCIE_DEV");
    if (EFI_ERROR(Status)) {
      DEBUG((EFI_D_INFO, "%a HiiLibSetBrowserData OEMPCIE_DEV (%r)\n", __FUNCTION__, Status));
    }
