  ...
  VOID )
{
    ...
    if (DeviceOrderBuffer != NULL) {
        Status = gRT->SetVariable (
                          ...
                          DeviceOrderBuffer);
        if (EFI_ERROR (Status)) {
            SERVER_IPMI_DEBUG ((EFI_D_ERROR, "IPMI Boot: IpmiLegacyDevOrder SetVariable fails.\n"));
        }
    ...
    if (DeviceOrderBuffer != NULL) {
        Status = gRT->SetVariable (
                          ...
                          DeviceOrderBuffer);
        if (EFI_ERROR (Status)) {
            SERVER_IPMI_DEBUG ((EFI_D_ERROR, "IPMI Boot: IpmiOldLegacyDevOrder SetVariable fails.\n"));
        }
        gBS->FreePool (DeviceOrderBuffer); // Freeing the memory allocated in GetVariableAndSize().
    } // if (DeviceOrderBuffer != NULL)
}
    ...
    if (Buffer != NULL) {
        ...
        if (MatchedBootOptionDetails != NULL) {
            *(UINT16 *)MatchedBootOptionDetails = BootOptionNumber;
            ...
            if (EFI_ERROR (Status)) {
                SERVER_IPMI_DEBUG ((EFI_D_ERROR, "IPMI Boot: IpmiMatchBootOptionData SetVariable fails.\n"));
            }
  ...
  VOID )
{
    ...
    SERVER_IPMI_DEBUG ((EFI_D_INFO, "IPMI Boot: Entered %a\n", __FUNCTION__));
    //
    ...
    Buffer = GetVariableAndSize (
                 ...
                 &BufferSize);
    if (Buffer != NULL) {
        SERVER_IPMI_DEBUG ((EFI_D_INFO, "IPMI Boot: Saving IPMI Set BootOrder.\n"));
        //
    ...
    SERVER_IPMI_DEBUG ((EFI_D_INFO, "IPMI Boot: Exiting SaveIPMISetBootOrder.\n"));
    return;
}
  ...
  VOID )
{
    ...
    if (IPMIBootOrder != NULL) {
        ...
        SERVER_IPMI_DEBUG ((EFI_D_INFO, "IPMI Boot: Exiting as IPMI Set Boot Order not available.\n"));
        return;
    }
    //
    ...
    if (CurrBootOrder != NULL) {
        while (Index < (CurrBootOrderSize / sizeof (UINT16))) {
            if (IPMIBootOrder[Index] != CurrBootOrder[Index]) {
                ...
                SERVER_IPMI_DEBUG ((EFI_D_INFO, "IPMI Boot: Boot Order Changed by user.\n"));
            }
            Index = Index + 1;
        }
    }
    ...
    if(!BootOrderChange) {
        if (CheckDevicePriorityChange ()) {
            BootOrderChange = TRUE;
            SERVER_IPMI_DEBUG ((EFI_D_INFO, "IPMI Boot: Legacy device priority Changed by user.\n"));
        }
    }
#endif //#if CSM_SUPPORT
    if (BootOrderChange) {
    ...
    SERVER_IPMI_DEBUG ((EFI_D_INFO, "IPMI Boot: Exiting %a\n", __FUNCTION__));
  ...
  VOID )
{
    ...
    Buffer = GetVariableAndSize (
                 ...
                 &BootOptionSize);
    if (Buffer != NULL) {
        SERVER_IPMI_DEBUG ((EFI_D_INFO, "IPMI Boot: IpmiMatchBootOptionData Available.\n"));
        ...
        //
        if (!EFI_ERROR(Status)) {
            SERVER_IPMI_DEBUG ((EFI_D_INFO, "IPMI Boot: Clearing IpmiMatchBootOptionData.\n"));
            Status = gRT->SetVariable (
                              L"IpmiMatchBootOptionData",
                              &gEfiIpmiBootGuid,
                              VAR_FLAG,
        ...
        if (Buffer != NULL) {
            ...
            if (!EFI_ERROR(Status)) {
                Status = gRT->SetVariable (
                                  ...
                                  NULL);
                if (!EFI_ERROR(Status)) {
                    SERVER_IPMI_DEBUG ((EFI_D_INFO, "IPMI Boot: Restored LegacyDevOrder and Cleared IpmiLegacyDevOrder.\n"));
                }
            }
            gBS->FreePool (Buffer); // Freeing the memory allocated in GetVariableAndSize().
            Buffer = NULL;
        ...
        if (Buffer != NULL) {
            ...
            if (!EFI_ERROR(Status)) {
                Status = gRT->SetVariable (
                                  ...
                                  NULL);
                if (!EFI_ERROR(Status)) {
                    SERVER_IPMI_DEBUG ((EFI_D_INFO, "IPMI Boot: Restored OldLegacyDevOrder and Cleared IpmiOldLegacyDevOrder.\n"));
                }
            }
            gBS->FreePool (Buffer); // Freeing the memory allocated in GetVariableAndSize().
        } // if (Buffer != NULL)
    ...
    if (BootOrder != NULL) {
        SERVER_IPMI_DEBUG ((EFI_D_INFO, "IPMI Boot: Restoring Original boot order.\n"));
        //
  ...
  VOID )
{
    ...
    Status = gBS->LocateProtocol (
                 ...
                 NULL,
                 (VOID **)&gIpmiTransport );
    SERVER_IPMI_DEBUG((EFI_D_INFO, "IPMI Boot: LocateProtocol gEfiDxeIpmiTransportProtocolGuid. Status: %r\n", Status));
    if ( EFI_ERROR (Status) ) {
        SERVER_IPMI_DEBUG ((EFI_D_ERROR, "SetIpmiBootOptionPriorities: Error in locating \
    ...
    if ( EFI_ERROR(Status) ) {
        //AR-BIOS-0002-0010 Add warning information
        SERVER_IPMI_DEBUG ((DEBUG_ALI_MIN, "BIOS Get BootDevice info from BMC fail!!\n"));
        return;
    }
  ...
  VOID )
{
    ...
    if (BootOrderBuffer == NULL) {
        SERVER_IPMI_DEBUG((EFI_D_ERROR, "SaveCurrentBootorder: AllocateZeroPool Fails!!! \n"));
        return;
    }
    //
    ...
    if (EFI_ERROR(Status)) {
        SERVER_IPMI_DEBUG((EFI_D_ERROR, "SaveCurrentBootorder: BootOrderBeforeIpmiBoot SetVariable Fails!!! \n"));
    }
  ...
  )
{
    ...
    SERVER_IPMI_DEBUG ((EFI_D_INFO, "IPMI Boot: gIpmiForceBootDevice = %d gPersistentBoot = %d gUefiBoot = %d gDeviceInstanceSelector = %d\n", \
