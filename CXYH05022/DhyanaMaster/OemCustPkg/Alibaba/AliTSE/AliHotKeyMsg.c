...
#define EXPERT_ENTRY_KEY_BIT                          0x00000008
  ...
  if(HotKeys){
    ...
    if ( gQuietBoot )
    {
        ...
        if (!EFI_ERROR(Status)){
            ...
            else if(HotKeys & EXPERT_ENTRY_KEY_BIT){
              HotKeyMsgText = HiiGetString( gHiiHandle, STRING_TOKEN(STR_ENTER_EXPERT) );
            }
...
VOID OemCheckForKey (AMI_EFI_KEY_DATA AmiKey)
{
    ...
    if((gPressHotKeys & EXPERT_ENTRY_KEY_BIT) != 0)
        return;
//<lvych001-20171214 Revised system maybe hang postcode B2 when press hotkey.+>
    Status = InitUgaStr();
    if (EFI_ERROR(Status))
...
//<lvych001-20171214 Revised system maybe hang postcode B2 when press hotkey.->
    ...
    if (gPressHotKeys == 0)
    {
        if( OemCheckKeyStateCond(AmiKey,SETUP_ENTRY_SCAN,SETUP_ENTRY_UNICODE,SETUP_ENTRY_SHIFT_STATE)
           ...
           || OemCheckKeyStateCond(AmiKey,SETUP_ALT_ENTRY_SCAN,SETUP_ALT_ENTRY_UNICODE,SETUP_ALT_ENTRY_SHIFT_STATE)
#endif
          )
        {
            //<Yaosr001-20190731 Support BIOS HotKey select. +>
        ...
        else if ((OemCheckKeyStateCond(AmiKey,SETUP_OEM_KEY2_SCAN,SETUP_OEM_KEY2_UNICODE,SETUP_OEM_KEY2_SHIFT))
            || (OemCheckKeyStateCond(AmiKey,SETUP_OEM_KEY2_SCAN,SETUP_OEM_KEY2_UNICODE_CASE,SETUP_OEM_KEY2_SHIFT)))
        {
            gBootFlow = BOOT_FLOW_CONDITION_OEM_KEY2;
            gEnterSetup = TRUE;
            gPressHotKeys = EXPERT_ENTRY_KEY_BIT;
        }
    #endif
    }
    else
    {
        if ((OemCheckKeyStateCond(AmiKey,SETUP_OEM_KEY2_SCAN,SETUP_OEM_KEY2_UNICODE,SETUP_OEM_KEY2_SHIFT))
                || (OemCheckKeyStateCond(AmiKey,SETUP_OEM_KEY2_SCAN,SETUP_OEM_KEY2_UNICODE_CASE,SETUP_OEM_KEY2_SHIFT)))
        {
            gBootFlow = BOOT_FLOW_CONDITION_OEM_KEY2;
            gEnterSetup = TRUE;
