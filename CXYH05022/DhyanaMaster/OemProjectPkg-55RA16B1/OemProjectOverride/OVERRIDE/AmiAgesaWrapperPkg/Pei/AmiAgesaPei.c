  ...
  )
{
    ...
    if (SpdData->SocketId == 0) {
      if ((SpdData->MemChannelId >= 0) && (SpdData->MemChannelId < 4)) {
        SmbusI2CMuxSelection (PeiServices, SPD_I2C_MUX_ADDRESS, 0x01, 0x01);//Mizl-20210825- Porting For CRB18 >
      }
      if ((SpdData->MemChannelId >= 4) && (SpdData->MemChannelId < 8)) {
        SmbusI2CMuxSelection (PeiServices, SPD_I2C_MUX_ADDRESS, 0x02, 0x02);//Mizl-20210825- Porting For CRB18 >
      }
    }
    
   if (SpdData->SocketId == 1) {
      if ((SpdData->MemChannelId >= 0) && (SpdData->MemChannelId < 4)) {
        SmbusI2CMuxSelection (PeiServices, SPD_I2C_MUX_ADDRESS, 0x04, 0x04);//Mizl-20210825- Porting For CRB18 >
      }
      if ((SpdData->MemChannelId >= 4) && (SpdData->MemChannelId < 8)) {
        SmbusI2CMuxSelection (PeiServices, SPD_I2C_MUX_ADDRESS, 0x08, 0x08);//Mizl-20210825- Porting For CRB18 >
      }
  }
#else
    if (SpdData->SocketId == 0) {
