...
//<bieyb-20180727 add function enable SRIOV by port->
      ...
      //<Kangmm-20210810 Modify brandname for front board X710+>
      if((VenId == 0x8086)&&(DevId == 0x1572)&&((SubSID == 0x0000)||(SubSID == 0x0305)))
          ptrdninfo->BrandName = L"X710 10G SFP+ 2-Ports NIC";
      //<Kangmm-20210810 Modify brandname for front board X710->
      //<huangrj-20220617 Modify brandname for front board I350+>
      if((VenId == 0x8086)&&(DevId == 0x1523)&&(SubSID == 0x0000))
          ptrdninfo->BrandName = L"Intel I350 1Gb RJ45 NIC";
      //<huangrj-20220617 Modify brandname for front board I350->
...
*/
      if (DevAttribute != NULL){
        if(DevItem->SlotNum !=0xFF)
        {
            ...
            if(DevItem->RearSlotID!=0xff)
            {
                Swprintf(tString, L"Enable/Disable PCIe Device\nLocation:\nSeg:Bus:Dev:Func %02X:%02X:%02X:%02X\n%sVender ID:%04X\nDevice ID:%04X\nMax Speed:%s\nMax Width:%s\nCurrent Speed:%s\nCurrent Width:%s\nSlot Number: %d\n",dev->RbIo->SegmentNumber,dev->Address.Addr.Bus,dev->Address.Addr.Device,dev->Address.Addr.Function,DevItem->Prefix,dev->DevVenId.VenId,dev->DevVenId.DevId,DevAttribute->MaxSpeed,DevAttribute->MaxWidth,DevAttribute->CurrentSpeed,DevAttribute->CurrentWidth, DevItem->RearSlotID);
            }
            else
            {
...
#else
            Swprintf(tString, L"Enable/Disable PCIe Device\nSeg/Bus/Dev/Func: %02X/%02X/%02X/%02X\nVender ID    : %02X\nDevice ID    : %04X\nMaximum Speed: %s\nMaximum Width: %s\nCurrent Speed: %s\nCurrent Width: %s\nSlot Number: %d\n",dev->RbIo->SegmentNumber,dev->Address.Addr.Bus,dev->Address.Addr.Device,dev->Address.Addr.Function,dev->DevVenId.VenId,dev->DevVenId.DevId,DevAttribute->MaxSpeed,DevAttribute->MaxWidth,DevAttribute->CurrentSpeed,DevAttribute->CurrentWidth, DevItem->SmBiosT9SlotID);
            //Swprintf(tString, L"Enable/Disable PCIe Device\nLocation:\nSeg:Bus:Dev:Func %02X:%02X:%02X:%02X\n%sVender ID:%04X\nDevice ID:%04X\nMax Speed:%s\nMax Width:%s\nCurrent Speed:%s\nCurrent Width:%s\nSlot Number: %d\n",dev->RbIo->SegmentNumber,dev->Address.Addr.Bus,dev->Address.Addr.Device,dev->Address.Addr.Function,DevItem->Prefix,dev->DevVenId.VenId,dev->DevVenId.DevId,DevAttribute->MaxSpeed,DevAttribute->MaxWidth,DevAttribute->CurrentSpeed,DevAttribute->CurrentWidth, DevItem->SmBiosT9SlotID);
#endif       
            //dengyh-20220117 change the physical slot ID to the slot ID at rear -
        }
        else
