...
#include <HygonFuse.h>
#include <PiPei.h>//<huangjin001-20190813 Fix no display clear CMOS information issue >
  ...
  )
{
  ...
  EFI_BOOT_MODE                         BootMode;//<huangjin001-20190813 Fix no display clear CMOS information issue >
    ...
    for (i = 0; (i < SocketCount) && Attainable; i++) {
      ...
      for (j = 0; (j < DieCount) && Attainable; j++) {
        ...
        if ((LogicalId.Family == HYGON_FAMILY_18) && (LogicalId.Revision & HYGON_REV_F18_DN_Dx))  
            NbioSmuServices->SmuRegisterRead (NbioSmuServices, SystemDieLoop, HYGON_DX_MP0_CORE_DISABLE, &CoreDisFuse);
        else
            NbioSmuServices->SmuRegisterRead (NbioSmuServices, SystemDieLoop, MP0_CORE_DISABLE, &CoreDisFuse);
...
//<huangjin001-20190813 Fix no display clear CMOS information issue +>      
