  ...
  )
{
  ...
  UINT32                        Index;
  //*** AMI PORTING BEGIN ***//
  //Workaround to have each capsule aligned on 8 bytes boundary
...
//  UINT64                        Length;
  ...
  //*** AMI PORTING BEGIN ***//
  Base = (UINTN) ((UINT8 *)PrivateData + sizeof(EFI_CAPSULE_PEIM_PRIVATE_DATA) + (CapsuleNumber - 1) * sizeof(UINT64));
  Size = 0;
  for (Index = 0; Index < CapsuleNumber; Index++) {
      Cap = (EFI_CAPSULE_HEADER *)(VOID *)(Base + (UINTN)PrivateData->CapsuleOffset[Index]);
      Size += Cap->CapsuleImageSize;
      Size += (8 - (Cap->CapsuleImageSize % 8));    //add 8-bytes padding
  }
  //*** AMI PORTING END *****//
...
//    Length      = ((EFI_CAPSULE_HEADER *)((UINTN) BaseAddress))->CapsuleImageSize;
    ...
    Cap = (EFI_CAPSULE_HEADER *)(VOID *)(Base + (UINTN)PrivateData->CapsuleOffset[Index]);
    Size = Cap->CapsuleImageSize;
    CopyMem ((VOID *) (UINTN) NewBuffer, Cap, Size);

    //BuildCvHob (BaseAddress, Length);
	BuildCvHob (NewBuffer, Size);
    // Perform additional processing of the capsule.
	ProcessCapsuleImage(Cap);
	NewBuffer += Size;
	NewBuffer += (8 - (Size % 8));
	//*** AMI PORTING END *****//
  }
