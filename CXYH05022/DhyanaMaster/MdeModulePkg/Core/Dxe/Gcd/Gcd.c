...
//*** AMI PORTING BEGIN ***//
// Bug fix: Make sure memory buffer used for early DXE Core initialization has not been allocated in PEI 
EFI_STATUS GetLargestBuffer(
  IN OUT EFI_PHYSICAL_ADDRESS  *MemoryBaseAddress, IN OUT UINT64 *MemoryLength, 
  IN UINT64 MinimalMemorySizeNeeded, IN VOID *HobStart
){
    ...
    EFI_PHYSICAL_ADDRESS AllocationEnd;
    UINT64 BufferLength;
    UINT64 AllocatedLength;
    EFI_STATUS Status;
    
    if (*MemoryLength < MinimalMemorySizeNeeded) return EFI_NOT_FOUND;
        
    BufferAddress = *MemoryBaseAddress;
    BufferLength = *MemoryLength;
    BufferEnd = BufferAddress + BufferLength;
    
    for (Hob.Raw = HobStart; !END_OF_HOB_LIST(Hob); Hob.Raw = GET_NEXT_HOB(Hob)) {
      if (GET_HOB_TYPE (Hob) != EFI_HOB_TYPE_MEMORY_ALLOCATION) continue;
    
      MemoryHob = Hob.MemoryAllocation;
      AllocatedAddress = MemoryHob->AllocDescriptor.MemoryBaseAddress;
      AllocatedLength = MemoryHob->AllocDescriptor.MemoryLength;
      AllocationEnd = AllocatedAddress + AllocatedLength;
      
      // if allocated memory range does not overlap with the passed in memory buffer, continue
      if ( AllocationEnd <= BufferAddress || AllocatedAddress >= BufferEnd) continue;
      
      // try portion of the buffer after allocated memory range
      if (AllocationEnd < BufferEnd){
        BufferAddress = AllocationEnd;
        BufferLength = BufferEnd - AllocationEnd;
        Status = GetLargestBuffer(&BufferAddress, &BufferLength, MinimalMemorySizeNeeded, HobStart);
        if (!EFI_ERROR(Status)){
            *MemoryBaseAddress = BufferAddress;
            *MemoryLength = BufferLength;
            return EFI_SUCCESS;
        }
      }

      // try portion of the buffer before allocated memory range
      BufferAddress = *MemoryBaseAddress;
      if (AllocatedAddress > BufferAddress){
        BufferLength = AllocatedAddress - BufferAddress;
        Status = GetLargestBuffer(&BufferAddress, &BufferLength, MinimalMemorySizeNeeded, HobStart);
        if (!EFI_ERROR(Status)){
            *MemoryBaseAddress = BufferAddress;
            *MemoryLength = BufferLength;
      ...
      // if we've reached this far, no buffer has been found.
      return EFI_NOT_FOUND;
    }
    
    // If we're here, no memory allocation HOBs overlapping with the buffer are found.
    // The buffer can be used as is.
    return EFI_SUCCESS;
}
//*** AMI PORTING END *****//
  ...
  )
{
  ...
  for (Hob.Raw = *HobStart; !END_OF_HOB_LIST(Hob); Hob.Raw = GET_NEXT_HOB(Hob)) {
    ...
    if (EFI_ERROR( GetLargestBuffer(&BaseAddress, &Length, MinimalMemorySizeNeeded, *HobStart) ) ) {
    //*** AMI PORTING END *****//
      //
  ...
  //*** AMI PORTING END *****//
    ...
    for (Hob.Raw = *HobStart; !END_OF_HOB_LIST(Hob); Hob.Raw = GET_NEXT_HOB(Hob)) {
      ...
      if (EFI_ERROR( GetLargestBuffer(&TestedMemoryBaseAddress, &TestedMemoryLength, MinimalMemorySizeNeeded, *HobStart) ) ) {
      //*** AMI PORTING END *****//
        continue;
      }
