...
)
{
    ...
    DEBUG((DEBUG_INFO,"..........IRQ MASK %x....", Mask));
    //
    ...
    for (IrqIndex = 0; IrqIndex < sizeof(irq_priority_map); IrqIndex++) {
        DEBUG((DEBUG_INFO,"IrqIndex %d....Allocated %d....\n", IrqIndex, irq_allocated_count[IrqIndex]));
  ...
  )
{
    ...
    if (pirq > 3) {
        DEBUG((DEBUG_ERROR, "Invalid PIRQ value (%d, %d, %d) %d\n", PciBus, PciDevice, PciFunction, pirq));
...
)
{
    ...
    DEBUG((DEBUG_INFO, "AddonPciBusTable entry [%d] created: Bus %x, PIRQs: %x %x %x %x\n",
        gAddonPciBusIndx,
...
)
{
    ...
    if(EFI_ERROR(Status)){
        *IrqMask = ISA_IRQ_MASK;    // allow IRQ 0..8, 12..15 for ISA
        ...
        DEBUG((DEBUG_INFO, "PciInterrupts: Set ISA_IRQ_MASK, Status=%r\n", Status));
    }
    return Status;
}
    ...
    VOID *Context)
{
    ...
    DEBUG((DEBUG_INFO,"PCI device: %x %x %x ... ", Bus, Dev, Func));
    ...
    if (!Int) {
        DEBUG((DEBUG_INFO,"does not require IRQ.\n"));
    ...
    DEBUG((DEBUG_INFO,"Intx2Pirq returns %r\n", Status));

    if (Status == EFI_NOT_FOUND) {
        // Device is not in the table
        DEBUG((DEBUG_INFO,"Device is not found in the PCI IRQ routing table.\n"));
    }
...
/* replace a non-standard EfiGetPlatformPciIrqMask with the direct custom call
    ...
    DEBUG((DEBUG_INFO,"RoutePciIrq status: %r,  Intx2Pirq: Int %x IRQ %x mask %x\n", Status, Int, Irq, IrqMask));
...
//-----------------------------------------
    ...
    PrtAddress = CopyLegacyTable(
                ...
                F0000_BIT);
    ASSERT(PrtAddress);
    DEBUG((DEBUG_INFO, "$PIR table, %d Bytes long, is copied to %x\n", (UINT16)gMsPrt->TableSize, PrtAddress));
...
//------------------------------------------
    if (!gIsValidPrt) {
		Status = pBS->LocateProtocol (&gEfiLegacyInterruptProtocolGuid, NULL, &iInterrupt);
		DEBUG((DEBUG_INFO,"CSM PciIrq: Locating LegacyInterrupts Protocol...Status=%r\n",Status));
		if (EFI_ERROR(Status)) return;
        ...
        DEBUG((DEBUG_INFO, "PciInterrupts: Init PRT Get ISA_IRQ_MASK, Status=%r\n", Status));
        ASSERT_EFI_ERROR(Status);
