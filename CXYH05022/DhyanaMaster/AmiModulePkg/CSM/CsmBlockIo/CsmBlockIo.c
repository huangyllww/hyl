...
#if CSM16_VERSION_MINOR > 80
#define EFI_CSM_IPLDT_EBDA_OFFSET 0x460
#else
#define EFI_CSM_IPLDT_EBDA_OFFSET 0x440
#endif
...
)
{
    ...
    Ipldt = Ebda + EFI_CSM_IPLDT_EBDA_OFFSET;

    DEBUG((DEBUG_ERROR, "CsmBlockIo L2E: BCV at %x:%x\n", Device->BcvSegment, Device->BcvOffset));
    ...
    {
        DEBUG((DEBUG_ERROR, "Entry is not found in IPLDT\n"));
        return;
    }
    ...
    DEBUG((DEBUG_ERROR, "IPLDT[%x], index %x, handle %x\n", i, VectorIndex, Handle));
    ASSERT(Handle > 0x7f);
