...
//<Yaosr001-20200908 Added SM3 Software Algorithm Support. +>
EFI_STATUS
EFIAPI SM3HashAll(
    IN  VOID            *HashData,
    IN  UINTN           HashDataLen,
                              ...
                              UINTN           *Digestsize)
{
    ...
    {
        //SM3
        SM3HashAll(HashData, HashDataLen, (UINT8 *)&HashValues->digests[Count].digest.sha256);//<Yaosr001-20200908 Added SM3 Software Algorithm Support.>
...
#if Hygon_fTPM2 == 1
    Status=TcgUpdateAslNameObject(dsdt, "HYGT", (IsHygonFtpmEnabled() ? 1 : 0));
#else
    Status=TcgUpdateAslNameObject(dsdt, "HYGT", 0);
#endif
    ...
    OUT UINTN                     *HashCount)
{
    ...
    SM3_CTX                           Sm3Ctx;  //<Yaosr001-20200908 Added SM3 Software Algorithm Support.>
    UINTN                             count=0;
    UINTN                             TempSize=0;
    ...
    if(Tcg2SpecVersion == TCG2_PROTOCOL_SPEC_TCG_2)
    {
        ...
        {
            sm3_update(&Sm3Ctx, HashBase, HashSize); //<Yaosr001-20200908 Added SM3 Software Algorithm Support.>
        }
    }
...
//      printbuffer(HashBase, 106);
    ...
    if(Tcg2SpecVersion == TCG2_PROTOCOL_SPEC_TCG_2)
    {
        ...
        if( mPcrBanks & TREE_BOOT_HASH_ALG_SM3)
        {
            sm3_final(TpmDigest->sm3256, &Sm3Ctx); //<Yaosr001-20200908 Added SM3 Software Algorithm Support.>
        }
    }
...
)
{
    ...
    {
        sm3_update((SM3_CTX *)Context, Data, Datasize); //<Yaosr001-20200908 Added SM3 Software Algorithm Support.>
    }
    ...
    IN EFI_SYSTEM_TABLE *SystemTable )
{
    ...
    UINT32              TpmError=0;
#if Hygon_fTPM2 == 1
    //SGPZT#23733-fTPM-Works-Abnormal +>
    //SETUP_DATA          SetupDataBuffer;
    //UINTN               SetupVariableSize = sizeof(SETUP_DATA);
...
static VOID             *VarLockreg;
    ...
    Tpm2GetCapabilityCapPCRS();
    // When PCR banks enabled by default are not supported by the underlying TPM device, PCR extend operation
    // in BIOS especially PEI stage will fail. Hygon_fTPM2 enables SM3 PCR bank by default, which may be not
    // supported by dTPM.
    if ((TcgSupportedBankBitMap | Config.PcrBanks) != TcgSupportedBankBitMap) {
        Status = gRT->GetVariable (L"Setup",
                                   &gSetupGuid,
                                   &SetupVariableAttributes,
                                   &SetupVariableSize,
                                   &SetupDataBuffer);
        if(EFI_ERROR(Status)) return Status;
        if (!(TcgSupportedBankBitMap & 0x1)) {
            SetupDataBuffer.Sha1 = 0;
        }
        if (!(TcgSupportedBankBitMap & 0x2)) {
            SetupDataBuffer.Sha256 = 0;
        }
        if (!(TcgSupportedBankBitMap & 0x4)) {
            SetupDataBuffer.Sha384 = 0;
        }
        if (!(TcgSupportedBankBitMap & 0x8)) {
            SetupDataBuffer.Sha512 = 0;
        }
        if (!(TcgSupportedBankBitMap & 0x10)) {
        ...
        Status = gRT->SetVariable (
                     ...
                     &SetupDataBuffer);
        if(EFI_ERROR(Status)) return Status;
        // PEI stage PCR extend failed and need reset.
        gRT->ResetSystem (EfiResetCold, EFI_SUCCESS, 0, NULL);
