...
//-------------------------------------
    if(IgnoreOptionRom) {
        PCI_TRACE((TRACE_PCI,"IgnoreOptionRom \n"));
        return EFI_NOT_FOUND;
    }
    ...
    PCI_TRACE((TRACE_PCI,"CheckPlatformRom status %r \n", Status));
    if(!EFI_ERROR(Status)) return Status;       
    
    Status=EFI_NOT_FOUND;
    //we got Opt ROM to check...
    PCI_TRACE((TRACE_PCI,"Device ROM BAR Type %d, Base 0x%x, Length 0x%x\n", rombar->Type, rombar->Base, rombar->Length));
    
    if(Dev->Bar[PCI_MAX_BAR_NO].Type!=tBarUnused){
        UINT32      trs; //temporary rom size
    //------------------------
        //If policy tells to skip ROM ... 
        if(Dev->AmiRomPolicy!=NULL && Dev->AmiRomPolicy->CurrentPolicy==0) {
            PCI_TRACE((TRACE_PCI,"CurrentPolicy is 0, skip ROM \n"));
            return EFI_NOT_FOUND;
        }
        ...
        if(EFI_ERROR(Status)){
            PCI_TRACE((TRACE_PCI,"EnableDeviceDecoding error status %r \n", Status));
            return Status;
        }
        ...
        for(trs=0;;){
            ...
            // Get the first 0x1A bytes of the ROM header
            Status=Dev->RbIo->Mem.Read(Dev->RbIo, EfiPciWidthUint8, base+trs,
            sizeof(PCI_STD_OPT_ROM_HEADER), &PciRomHdr);
            if(EFI_ERROR(Status)){
                PCI_TRACE((TRACE_PCI,"Read ROM HEADER error status %r \n", Status));
                goto ExitLbl;  
            }
            for(i=0;i<16;i++)
                PCI_TRACE((TRACE_PCI,"PciRomHdr.Reserved 0x%x \n", PciRomHdr.Reserved[i]));
            // Check the validity of the ROM
            PCI_TRACE((TRACE_PCI,"PciRomHdr.Signature 0x%x \n", PciRomHdr.Signature));
            PCI_TRACE((TRACE_PCI,"PciRomHdr.PcirOffset 0x%x \n", PciRomHdr.PcirOffset));
            ...
            if(EFI_ERROR(Status)){
                PCI_TRACE((TRACE_PCI,"Read PCI_DATA_STRUCTURE error status %r  \n", Status));
                goto ExitLbl;  
            }
        
            PCI_TRACE((TRACE_PCI,"Pcir.Signature 0x%x \n", Pcir.Signature));
        ...
        if(UsePciIoForOptRom){
            ...
            if(EFI_ERROR(Status)) {
                PCI_TRACE((TRACE_PCI,"Copy Rom image into memory error status %r \n", Status));
                goto ExitLbl;
            }
        } else {
...
ExitLbl:
        ...
        PCI_TRACE((TRACE_PCI,"Disable decoding option rom status %r \n", ExitStatus));
    } //if(Dev->Bar[PCI_MAX_BAR_NO].Type!=tBarUnused) 
    ...
    //------------  
        ...
        PCI_TRACE((TRACE_PCI,"GetEmbeddedRom status %r \n", Status));
        if(!EFI_ERROR(Status)){
            EMB_ROM_INFO    *embrom;
        //---------------   
...
//(EIP42750+)
    ...
    PCI_TRACE((TRACE_PCI,"AmiPciLibLaunchInitRoutine status %r \n", ExitStatus));
...
//------------------------------------------------------------------  
    ...
    //AMI EIP416827 Update PciBus GetDriver function to support PCI oproms with two UEFI images
...
//-----------------------------------
    ...
    if(cr){
        PCI_TRACE((TRACE_PCI,"Call GetOptRom \n"));
        OpRomStatus=GetOptRom(Dev);
    } else {
        //OpROM was initialized earlier
        PCI_TRACE((TRACE_PCI,"OpROM was initialized earlier \n"));
