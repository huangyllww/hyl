...
//
// Input:       UINT8 Dec - Decimal value
//
// Output:      UINT8 - BCD value
//
// Notes:       Only for 2 digit decimal.
//----------------------------------------------------------------------------
//<AMI_PHDR_END>
UINT8 DecToBCD (
  IN UINT8 Dec )
...
//
// Output:      UINT8 - decimal value
//
// Notes:       Only for 2 digit BCD.
//----------------------------------------------------------------------------
//<AMI_PHDR_END>
UINT8 BCDToDec (
  IN UINT8 BCD )
...
//
// Description: This function sets the Wake time for RTC
//
  ...
  IN OUT UINTN   *CommBufferSize  OPTIONAL )
{
    ...
    // D register - RTC date alarm
       //   7th bit is read-only, 
       //      1 - denotes RTC Date, Time and CMOS RAM are valid.
       //      0 - denotes RTC Date, Time and CMOS RAM are not valid.
       //   6th bit is Scratch bit. Both Read & Write bit.
       //   0-5 bits are Read-write bits.
       //      Not zero - date alarm is considered for alarm generation.
       //      0        - date alarm is not considered for alarm generation.
       // Clear the bits 0 to 6 to remove the junk data in the Date Alarm field.
     
    Value = ReadRtcIndex(RTC_STATUS_REG_D);
    Value &= 1 << 7;
    WriteRtcIndex(RTC_STATUS_REG_D,Value);
  ...
  IN EFI_SYSTEM_TABLE  *SystemTable )
{
...
#if RTCWAKEUP_POWERBUTTON_SUPPORT
    // Power Button SMI
    EFI_SMM_POWER_BUTTON_DISPATCH2_PROTOCOL  *PBtnDispatch;
    EFI_SMM_POWER_BUTTON_REGISTER_CONTEXT    PBtnContext = {EfiPowerButtonEntry};
#endif
    ...
    if (PBtnDispatch != NULL) {
        Status = PBtnDispatch->Register( PBtnDispatch,
                                         RTCPBtnSmiCallback,
                                         &PBtnContext,
                                         &SxDispatchHandle );
        if (EFI_ERROR(Status)) {
