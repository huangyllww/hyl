...
HOTPLUG_USERCONFIG_DESCRIPTOR HotplugUserDescriptor_J27[] = {
        {
                //
                //StartLane, EndLane, SocketNumber
                //
...
EFI_STATUS SetNbioHotplugDescriptorProtocol(VOID)
{
    ...
    HOTPLUG_USERCONFIG_DESCRIPTOR    *mHotplugUserDescriptor = NULL;
    UINT8                            RetimerPresent = 0;
    UINT8                            CPU0NVMePresent = 0;

    RetimerPresent   = PcdGet8(PcdRetimerPresent);
    CPU0NVMePresent = PcdGet8(PcdCPU0NVMePresent);
    mHotplugUserDescriptor = pHotplugUserDescriptor;
    if(CPU0NVMePresent == 1)
    {
      HotplugUserDescriptorCount += sizeof(HotplugUserDescriptor_J27) / sizeof(HOTPLUG_USERCONFIG_DESCRIPTOR);
      pHotplugUserDescriptor = HotplugUserDescriptor_J27;
      mHotplugUserDescriptor = pHotplugUserDescriptor + HotplugUserDescriptorCount;
    }
    ...
    if(RetimerPresent == 1 )
    {
        if (Is2diecpu()){      //2Die CPU
            ...
            mHotplugUserDescriptor = HotplugUserDescriptor_2DIE_J34;
        }
        else{   
            HotplugUserDescriptorCount += sizeof(HotplugUserDescriptor_2DIE_J34) / sizeof(HOTPLUG_USERCONFIG_DESCRIPTOR);
            mHotplugUserDescriptor = HotplugUserDescriptor_2DIE_J34;
