...
)
{
  ...
  Status = EqTablePpi->GetCfgTable (PeiServices,Die,&EqCfgTable);//SGEZT#25974-Improve-Code-For-Preset
  if (EFI_ERROR (Status)) {
     return;
  }
      ...
      switch (EqCfgTable->EqCfgList[i].Type) {
        ...
        case usb3_tx_vboost:
          if (CpuGen > CPU_HYGON_CX) {
            DEBUG ((DEBUG_INFO, "Die%x Port%x: HYGON_DX Usb3 Tx vboost to %x\n", Die, EqCfgTable->EqCfgList[i].UsbPort, EqCfgTable->EqCfgList[i].UsbCfgValue));
            Usb3TxVboost_V2 (DieBusNum, EqCfgTable->EqCfgList[i].UsbPort, EqCfgTable->EqCfgList[i].UsbCfgValue);
          } else {
            DEBUG ((DEBUG_INFO, "Die%x Port%x: HYGON_AX,HYGON_BX,HYGON_CX Usb3 Tx vboost to %x\n", Die, EqCfgTable->EqCfgList[i].UsbPort, EqCfgTable->EqCfgList[i].UsbCfgValue));
        ...
        case usb3_gen1_tx_eq_level:
          ...
          if (CpuGen > CPU_HYGON_CX) {
            DEBUG ((DEBUG_INFO, "Die%x Port%x: HYGON_DX Usb3 Gen1 Tx eq level to %x\n", Die, EqCfgTable->EqCfgList[i].UsbPort, EqCfgTable->EqCfgList[i].UsbCfgValue));
            Usb3Gen1Txeq (
                DieBusNum, 
            ...
            DEBUG ((DEBUG_INFO, "Die%x Port%x: HYGON_AX,HYGON_BX,HYGON_CX Usb3 Gen1 Tx eq level to %x\n", Die, EqCfgTable->EqCfgList[i].UsbPort, EqCfgTable->EqCfgList[i].UsbCfgValue));
  ...
  )
{
  ...
  UINT8                     MaxDieSupport=MAX_DIE_SUPPORT; //SGMTB#-0147294_USBPortError
  ...
  // Check Die#
  //
  //SGMTB#-0147294_USBPortError +>
  if(GetPkgType () == PKG_SL1R2)
    MaxDieSupport = 4;
  //for (Die = 1; Die < MAX_DIE_SUPPORT; Die++) {
  for (Die = 1; Die < MaxDieSupport; Die++) {
  //SGMTB#-0147294_USBPortError ->
    //
  ...
  )
{
  ...
  //
  //SGMTB#-0147294_USBPortError +>
  if ((Die & BIT1) && ( GetPkgType ()!= PKG_SL1R2)){
  //if (Die & BIT1){
  //SGMTB#-0147294_USBPortError ->
