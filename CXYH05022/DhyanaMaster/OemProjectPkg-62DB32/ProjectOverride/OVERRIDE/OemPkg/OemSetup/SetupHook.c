...
InitFruStrings(EFI_HII_HANDLE HiiHandle)
{
    ...
    //Hugc_20220121_Modify Hotplug address by FRU Setting>>>
        // STR_BRD_EXTRA1_VAL
        UpdateStr = (CHAR16 *) PcdGetPtr (PcdBaseBoardExtra1);
        DEBUG((EFI_D_ERROR, "BaseBoardExtra1: UpdateStr: %S\n", UpdateStr));
        if ( StrCmp (UpdateStr, PcdDefaultString) != 0x00 ) {
            InitString (
                HiiHandle,
                STRING_TOKEN(STR_BRD_EXTRA1_VAL),
                L"%s",
                UpdateStr );
        }    
...
)
{
    ...
    if ( (!EFI_ERROR (Status)) && BmcSelfTestProtocol->IsGetDeviceIdCommandPassed ) {
        ...
        InitString (
                ...
                L"%d.%02x.%02x", //liutz001-20191216 for ipmispec major need 10decode//<Kangmm001-20200428 Fix BMC minor version show incorrectly>
