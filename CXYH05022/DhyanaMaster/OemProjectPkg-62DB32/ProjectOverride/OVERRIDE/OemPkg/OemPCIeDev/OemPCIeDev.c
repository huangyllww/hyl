...
PCI_ATTRIBUTE_PARAM* GetPciAttribute(PCI_DEV_INFO *dev)
{
    ...
    if(CapIdOffset != 0){
      ...
      } else if(ReturnAttribute[0] == 0x04){
        Swprintf(tDevAttribute->MaxSpeed, L"16.0 GT/s");
      }else {
        Swprintf(tDevAttribute->MaxSpeed, L"32.0 GT/s");//<Liuby- for Cambricon X2000 box>
      }
      ...
      } else if(ReturnAttribute[2] == 0x04){
        Swprintf(tDevAttribute->CurrentSpeed, L"16.0 GT/s");
      }else {
        Swprintf(tDevAttribute->CurrentSpeed, L"32.0 GT/s");//<Liuby- for Cambricon X2000 box>
      }
...
//<bieyb-20180727 add function enable SRIOV by port->
      ...
      if(dfound == 0){
            Swprintf(tString, L"%s V:%04X D:%04X S:%04X ",DevItem->BrandName,VenId,DevId,SubSID);
      }else{
            Swprintf(tString, L"%s ",DevItem->BrandName);
      }
