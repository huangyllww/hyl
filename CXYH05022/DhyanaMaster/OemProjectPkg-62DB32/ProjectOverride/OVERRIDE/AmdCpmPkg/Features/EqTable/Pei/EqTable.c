...
#include "EqTable.h"
//SGEZT#25974-Improve-Code-For-Preset-S
#include "HygonPresetTable.c"
//SGEZT#25974-Improve-Code-For-Preset-E
...
)
{
  ...
  for (i=0; (EqCfgTable->EqCfgList[i].Flag & EQ_TABLE_END) != EQ_TABLE_END; i++) {
    if ((EqCfgTable->EqCfgList[i].Type & EQ_TYPE_MASK) == pcie_eq_type) {
        DEBUG((EFI_D_INFO, "PCIE_EQ { 0x%02x, ", EqCfgTable->EqCfgList[i].Flag));
        DEBUG((EFI_D_INFO, "0x%02x, ", EqCfgTable->EqCfgList[i].Type));
        DEBUG((EFI_D_INFO, "0x%02x, ", EqCfgTable->EqCfgList[i].CpuGen)); 
        DEBUG((EFI_D_INFO, "0x%02x, ", EqCfgTable->EqCfgList[i].Die)); 
        DEBUG((EFI_D_INFO, "0x%08x, ", EqCfgTable->EqCfgList[i].VidDid)); 
        DEBUG((EFI_D_INFO, "0x%08x, ", EqCfgTable->EqCfgList[i].SubSysVidDid)); 
        DEBUG((EFI_D_INFO, "0x%02x, ", EqCfgTable->EqCfgList[i].StartLane)); 
        DEBUG((EFI_D_INFO, "0x%02x, ", EqCfgTable->EqCfgList[i].EndLane)); 
        DEBUG((EFI_D_INFO, "0x%02x, ", EqCfgTable->EqCfgList[i].PerPort));
      DEBUG((EFI_D_INFO, "{ "));
      for (j=0; j < 16; j++) {
        DEBUG((EFI_D_INFO, "0x%08x, ", EqCfgTable->EqCfgList[i].CfgValue[j]));     
      }
      DEBUG((EFI_D_INFO, "}}\n"));
    }
    if ((EqCfgTable->EqCfgList[i].Type & EQ_TYPE_MASK) == usb_eq_type) {
        ...
        DEBUG((EFI_D_INFO, "0x%02x, ", EqCfgTable->EqCfgList[i].CpuGen)); 
        DEBUG((EFI_D_INFO, "0x%02x, ", EqCfgTable->EqCfgList[i].Die)); 
        DEBUG((EFI_D_INFO, "0x%02x, ", EqCfgTable->EqCfgList[i].UsbPort)); 
        DEBUG((EFI_D_INFO, "0x%02x, ", EqCfgTable->EqCfgList[i].UsbCfgValue)); 
...
CoreGetCfgTable (
    IN  CONST EFI_PEI_SERVICES     **PeiServices,
    IN  UINT8                      DieNumber,//SGEZT#25974-Improve-Code-For-Preset
...
)
{
  ...
  pEqCfgTable=HygonPresetTableList[DieNumber];
  //SGEZT#25974-Improve-Code-For-Preset-E
  if (pEqCfgTable == NULL) {
