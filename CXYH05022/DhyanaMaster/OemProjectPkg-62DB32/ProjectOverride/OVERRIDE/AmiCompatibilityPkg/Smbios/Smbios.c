    ...
    if (Status != EFI_NOT_FOUND) {
        ...
        for (i = 0; i < gSmbiosBoardProtocol->NumberOfSystemSlots; i++) {
            if (FindStructureType(&SrcBuffer, &StructureFoundPtr, 9, 1)) {
                ...
                ((SMBIOS_SYSTEM_SLOTS_INFO*)StructureFoundPtr)->SlotDataBusWidth = ((SYSTEM_SLOT_DYNAMIC_DATA*)SmbiosVariableBuffer)->SlotDataBusWidth[i];
            }
        }
    }
    pBS->FreePool(SmbiosVariableBuffer);
    ...
    if (Status != EFI_NOT_FOUND) {
        ...
        for (i = 0; i < gSmbiosBoardProtocol->NumberOfOnboardDevExt; i++) {
            if (FindStructureType(&SrcBuffer, &StructureFoundPtr, 41, 1)) {
                ...
                ((SMBIOS_ONBOARD_DEV_EXT_INFO*)StructureFoundPtr)->SegGroupNumber = 0;
                ((SMBIOS_ONBOARD_DEV_EXT_INFO*)StructureFoundPtr)->BusNumber = ((ONBOARD_DEV_EXT_DYNAMIC_DATA*)SmbiosVariableBuffer)->BusNumber[i];
                ((SMBIOS_ONBOARD_DEV_EXT_INFO*)StructureFoundPtr)->DevFuncNumber = ((ONBOARD_DEV_EXT_DYNAMIC_DATA*)SmbiosVariableBuffer)->DevFuncNumber[i];
            }
        }
    }
    pBS->FreePool(SmbiosVariableBuffer);
...
//----------------------------------------------------------------------------
    ...
    CHAR8           SmbiosDefaultLang[] = SMBIOS_DEFAULT_LANGUAGE;
    CHAR8           SmbiosSupportedLang[] = SMBIOS_SUPPORTED_LANGUAGES;
    BOOLEAN         DefaultLanguage = FALSE;
    BOOLEAN         DefaultLanguageCodes = FALSE;
...
//----------------------------------------------------------------------------
    ...
    Status = GetEfiVariable(L"PlatformLang",
                            ...
                            (void**)&Lang);
    if (EFI_ERROR(Status)) {
        DEBUG((DEBUG_ERROR | DEBUG_WARN, "Failed to get PlatformLang variable. Set DefaultLanguage to SDL token value\n"));
        Lang = SmbiosDefaultLang;
    ...
    Status = GetEfiVariable(L"PlatformLangCodes",
                            ...
                            (void**)&LangCodes);
    if (EFI_ERROR(Status)) {
        DEBUG((DEBUG_ERROR | DEBUG_WARN, "Failed to get PlatformLangCodes variable. Set DefaultLanguageCodes to SDL token value\n"));
        LangCodes = SmbiosSupportedLang;
        Size = sizeof(SmbiosSupportedLang);
        DefaultLanguageCodes = TRUE;
    }
...
#if ( defined(CSM_SUPPORT) && (CSM_SUPPORT != 0) )
{
    ...
    if (Status == EFI_SUCCESS) {
        ...
        //<xiaozhch-20210421 Add more space for SMBIOS to avoid system hang when SMBIOS is added after EndofDxe +>
                MaximumTableSize = (UINT16)BufferSize + EXTRA_RESERVED_BYTES;
        //<hujf1-20211110 Avoid smbios total size is larger then UINT16 +>
                if (BufferSize + EXTRA_RESERVED_BYTES > 0xFFFF)
                    MaximumTableSize = 0xFFFF; //smbios table can't be larger then 0xFFFF
        //<hujf1-20211110 Avoid smbios total size is larger then UINT16 ->
        //<xiaozhch-20210421 Add more space for SMBIOS to avoid system hang when SMBIOS is added after EndofDxe +>
